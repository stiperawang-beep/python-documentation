...
To avoid the precision loss caused by the [`float`](../library/functions.md#float) type,
...
This example shows how [`epoll()`](../library/select.md#select.epoll) can be used with timer file
...
This example shows how [`select()`](../library/select.md#select.select) can be used with timer file
descriptors to wait until the file descriptor is ready for reading:
