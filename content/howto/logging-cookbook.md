...
[Other resources](#cookbook-ref-links).
...
Loggers are plain Python objects.  The [`addHandler()`](../library/logging.md#logging.Logger.addHandler) method has no
...
[`SMTPHandler`](../library/logging.handlers.md#logging.handlers.SMTPHandler): sending emails can take a long time, for a
number of reasons outside the developer’s control (for example, a poorly
performing mail or network infrastructure). But almost any network-based
handler can block: Even a [`SocketHandler`](../library/logging.handlers.md#logging.handlers.SocketHandler) operation may do a
...
[`QueueHandler`](../library/logging.handlers.md#logging.handlers.QueueHandler) to those loggers which are accessed from
performance-critical threads. They simply write to their queue, which can be
sized to a large enough capacity or initialized with no upper bound to their
size. The write to the queue will typically be accepted quickly, though you
will probably need to catch the [`queue.Full`](../library/queue.md#queue.Full) exception as a precaution
...
The second part of the solution is [`QueueListener`](../library/logging.handlers.md#logging.handlers.QueueListener), which has been
designed as the counterpart to [`QueueHandler`](../library/logging.handlers.md#logging.handlers.QueueHandler).  A
[`QueueListener`](../library/logging.handlers.md#logging.handlers.QueueListener) is very simple: it’s passed a queue and some handlers,
and it fires up an internal thread which listens to its queue for LogRecords
sent from `QueueHandlers` (or any other source of `LogRecords`, for that
matter). The `LogRecords` are removed from the queue and passed to the
handlers for processing.

The advantage of having a separate [`QueueListener`](../library/logging.handlers.md#logging.handlers.QueueListener) class is that you
...
[`asyncio`](../library/asyncio.md#module-asyncio) internals. It might be best, if any async code is used in an
application, to use the above approach for logging, so that any blocking code
runs only in the `QueueListener` thread.

#### Versionchanged
Changed in version 3.5: Prior to Python 3.5, the [`QueueListener`](../library/logging.handlers.md#logging.handlers.QueueListener) always passed every message
...
#### Versionchanged
Changed in version 3.14: The [`QueueListener`](../library/logging.handlers.md#logging.handlers.QueueListener) can be started (and stopped) via the
[`with`](../reference/compound_stmts.md#with) statement. For example:
...
<a id="network-logging"></a>

## Sending and receiving logging events across a network

Let’s say you want to send logging events across a network, and handle them at
the receiving end. A simple way of doing this is attaching a
[`SocketHandler`](../library/logging.handlers.md#logging.handlers.SocketHandler) instance to the root logger at the sending end:
...
At the receiving end, you can set up a receiver using the [`socketserver`](../library/socketserver.md#module-socketserver)
...
the [`makePickle()`](../library/logging.handlers.md#logging.handlers.SocketHandler.makePickle) method and implementing your
...
[Here is a Gist](https://gist.github.com/vsajip/4b227eeec43817465ca835ca66f75e2b)
which provides the bare-bones files to run the above functionality using
Supervisor. It consists of the following files:

| File              | Purpose                                                                                                           |
|-------------------|-------------------------------------------------------------------------------------------------------------------|
| `prepare.sh`      | A Bash script to prepare the environment for<br/>testing                                                          |
| `supervisor.conf` | The Supervisor configuration file, which has<br/>entries for the listener and a multi-process web<br/>application |
| `ensure_app.sh`   | A Bash script to ensure that Supervisor is running<br/>with the above configuration                               |
| `log_listener.py` | The socket listener program which receives log<br/>events and records them to a file                              |
| `main.py`         | A simple web application which performs logging<br/>via a socket connected to the listener                        |
| `webapp.json`     | A JSON configuration file for the web application                                                                 |
| `client.py`       | A Python script to exercise the web application                                                                   |

The web application uses [Gunicorn](https://gunicorn.org/), which is a
...
1. Download [the Gist](https://gist.github.com/vsajip/4b227eeec43817465ca835ca66f75e2b)
   as a ZIP archive using the Download ZIP button.
...
3. In the scratch directory, run `bash prepare.sh` to get things ready.
   This creates a `run` subdirectory to contain Supervisor-related and
   log files, and a `venv` subdirectory to contain a virtual environment
...
[`Logger`](../library/logging.md#logging.Logger) instances on a per-connection basis, this is not a good idea
because these instances are not garbage collected. While this is not a problem
in practice, when the number of [`Logger`](../library/logging.md#logging.Logger) instances is dependent on the
level of granularity you want to use in logging an application, it could
be hard to manage if the number of [`Logger`](../library/logging.md#logging.Logger) instances becomes
effectively unbounded.

### Using LoggerAdapters to impart contextual information

An easy way in which you can pass contextual information to be output along
with logging event information is to use the [`LoggerAdapter`](../library/logging.md#logging.LoggerAdapter) class.
This class is designed to look like a [`Logger`](../library/logging.md#logging.Logger), so that you can call
[`debug()`](../library/logging.md#logging.debug), [`info()`](../library/logging.md#logging.info), [`warning()`](../library/logging.md#logging.warning), [`error()`](../library/logging.md#logging.error),
[`exception()`](../library/logging.md#logging.exception), [`critical()`](../library/logging.md#logging.critical) and [`log()`](../library/logging.md#logging.log). These methods have the
same signatures as their counterparts in [`Logger`](../library/logging.md#logging.Logger), so you can use the
two types of instances interchangeably.

When you create an instance of [`LoggerAdapter`](../library/logging.md#logging.LoggerAdapter), you pass it a
[`Logger`](../library/logging.md#logging.Logger) instance and a dict-like object which contains your contextual
information. When you call one of the logging methods on an instance of
[`LoggerAdapter`](../library/logging.md#logging.LoggerAdapter), it delegates the call to the underlying instance of
[`Logger`](../library/logging.md#logging.Logger) passed to its constructor, and arranges to pass the contextual
information in the delegated call. Here’s a snippet from the code of
[`LoggerAdapter`](../library/logging.md#logging.LoggerAdapter):
...
The [`process()`](../library/logging.md#logging.LoggerAdapter.process) method of [`LoggerAdapter`](../library/logging.md#logging.LoggerAdapter) is where the
...
merged into the [`LogRecord`](../library/logging.md#logging.LogRecord) instance’s \_\_dict_\_, allowing you to use
customized strings with your [`Formatter`](../library/logging.md#logging.Formatter) instances which know about
the keys of the dict-like object. If you need a different method, e.g. if you
want to prepend or append the contextual information to the message string,
you just need to subclass [`LoggerAdapter`](../library/logging.md#logging.LoggerAdapter) and override
[`process()`](../library/logging.md#logging.LoggerAdapter.process) to do what you need. Here is a simple example:
...
You don’t need to pass an actual dict to a [`LoggerAdapter`](../library/logging.md#logging.LoggerAdapter) - you could
...
<a id="filters-contextual"></a>

### Using Filters to impart contextual information

You can also add contextual information to log output using a user-defined
[`Filter`](../library/logging.md#logging.Filter). `Filter` instances are allowed to modify the `LogRecords`
passed to them, including adding additional attributes which can then be output
using a suitable format string, or if needed a custom [`Formatter`](../library/logging.md#logging.Formatter).

For example in a web application, the request being processed (or at least,
the interesting parts of it) can be stored in a threadlocal
([`threading.local`](../library/threading.md#threading.local)) variable, and then accessed from a `Filter` to
...
Since Python 3.7, the [`contextvars`](../library/contextvars.md#module-contextvars) module has provided context-local storage
which works for both [`threading`](../library/threading.md#module-threading) and [`asyncio`](../library/asyncio.md#module-asyncio) processing needs. This type
...
logged to `app.log`. Each webapp-specific log will contain only log entries for
...
Each [`Handler`](../library/logging.md#logging.Handler) has its own chain of filters.
If you want to add contextual information to a [`LogRecord`](../library/logging.md#logging.LogRecord) without leaking
it to other handlers, you can use a filter that returns
a new [`LogRecord`](../library/logging.md#logging.LogRecord) instead of modifying it in-place, as shown in the following script:
...
to have all the processes log to a [`SocketHandler`](../library/logging.handlers.md#logging.handlers.SocketHandler), and have a
...
You could also write your own handler which uses the [`Lock`](../library/multiprocessing.md#multiprocessing.Lock)
class from the [`multiprocessing`](../library/multiprocessing.md#module-multiprocessing) module to serialize access to the
file from your processes. The stdlib [`FileHandler`](../library/logging.handlers.md#logging.FileHandler) and subclasses do
not make use of [`multiprocessing`](../library/multiprocessing.md#module-multiprocessing).

Alternatively, you can use a `Queue` and a [`QueueHandler`](../library/logging.handlers.md#logging.handlers.QueueHandler) to send
...
If you want to use [`concurrent.futures.ProcessPoolExecutor`](../library/concurrent.futures.md#concurrent.futures.ProcessPoolExecutor) to start
...
to this (remembering to first import [`concurrent.futures`](../library/concurrent.futures.md#module-concurrent.futures)):
...
When deploying web applications using [Gunicorn](https://gunicorn.org/) or [uWSGI](https://uwsgi-docs.readthedocs.io/en/latest/) (or similar), multiple worker
...
[`SocketHandler`](../library/logging.handlers.md#logging.handlers.SocketHandler) to log from the web application to a listener in a separate
...
logging package provides a [`RotatingFileHandler`](../library/logging.handlers.md#logging.handlers.RotatingFileHandler):
...
[`string.Template`](../library/string.md#string.Template) (added in Python 2.4) and [`str.format()`](../library/stdtypes.md#str.format)
(added in Python 2.6).

Logging (as of 3.2) provides improved support for these two additional
formatting styles. The [`Formatter`](../library/logging.md#logging.Formatter) class been enhanced to take an
...
[`str.format()`](../library/stdtypes.md#str.format) or [`string.Template`](../library/string.md#string.Template). Here’s an example console
...
you cannot directly make logging calls using [`str.format()`](../library/stdtypes.md#str.format) or
[`string.Template`](../library/string.md#string.Template) syntax, because internally the logging package
...
something, but it’s quite palatable if you use an alias such as \_\_ (double
underscore — not to be confused with \_, the single underscore used as a
synonym/alias for [`gettext.gettext()`](../library/gettext.md#gettext.gettext) or its brethren).
...
string. That’s because the \_\_ notation is just syntax sugar for a constructor
call to one of the `*XXX*Message` classes.

If you prefer, you can use a [`LoggerAdapter`](../library/logging.md#logging.LoggerAdapter) to achieve a similar effect
...
<a id="custom-logrecord"></a>

## Customizing `LogRecord`

Every logging event is represented by a [`LogRecord`](../library/logging.md#logging.LogRecord) instance.
When an event is logged and not filtered out by a logger’s level, a
[`LogRecord`](../library/logging.md#logging.LogRecord) is created, populated with information about the event and
...
* [`Logger.makeRecord()`](../library/logging.md#logging.Logger.makeRecord), which is called in the normal process of
  logging an event. This invoked [`LogRecord`](../library/logging.md#logging.LogRecord) directly to create an
  instance.
* [`makeLogRecord()`](../library/logging.md#logging.makeLogRecord), which is called with a dictionary containing
  attributes to be added to the LogRecord. This is typically invoked when a
  suitable dictionary has been received over the network (e.g. in pickle form
  via a [`SocketHandler`](../library/logging.handlers.md#logging.handlers.SocketHandler), or in JSON form via an
  [`HTTPHandler`](../library/logging.handlers.md#logging.handlers.HTTPHandler)).

This has usually meant that if you need to do anything special with a
[`LogRecord`](../library/logging.md#logging.LogRecord), you’ve had to do one of the following.

* Create your own [`Logger`](../library/logging.md#logging.Logger) subclass, which overrides
  [`Logger.makeRecord()`](../library/logging.md#logging.Logger.makeRecord), and set it using [`setLoggerClass()`](../library/logging.md#logging.setLoggerClass)
  before any loggers that you care about are instantiated.
* Add a [`Filter`](../library/logging.md#logging.Filter) to a logger or handler, which does the
  necessary special manipulation you need when its
  [`filter()`](../library/logging.md#logging.Filter.filter) method is called.

The first approach would be a little unwieldy in the scenario where (say)
several different libraries wanted to do different things. Each would attempt
to set its own [`Logger`](../library/logging.md#logging.Logger) subclass, and the one which did this last would
win.

The second approach works reasonably well for many cases, but does not allow
you to e.g. use a specialized subclass of [`LogRecord`](../library/logging.md#logging.LogRecord). Library
...
could also add the filter to a [`NullHandler`](../library/logging.handlers.md#logging.NullHandler) attached to their
...
In Python 3.2 and later, [`LogRecord`](../library/logging.md#logging.LogRecord) creation is done through a
factory, which you can specify. The factory is just a callable you can set with
[`setLogRecordFactory()`](../library/logging.md#logging.setLogRecordFactory), and interrogate with
[`getLogRecordFactory()`](../library/logging.md#logging.getLogRecordFactory). The factory is invoked with the same
signature as the [`LogRecord`](../library/logging.md#logging.LogRecord) constructor, as [`LogRecord`](../library/logging.md#logging.LogRecord)
...
the use of a [`Filter`](../library/logging.md#logging.Filter) does not provide the desired result.

<a id="zeromq-handlers"></a>

## Subclassing QueueHandler and QueueListener- a ZeroMQ example

### Subclass `QueueHandler`

You can use a [`QueueHandler`](../library/logging.handlers.md#logging.handlers.QueueHandler) subclass to send messages to other kinds
...
You can also subclass [`QueueListener`](../library/logging.handlers.md#logging.handlers.QueueListener) to get messages from other kinds
...
using [pynng](https://pypi.org/project/pynng/), which is a Python binding to
[NNG](https://nng.nanomsg.org/), billed as a spiritual successor to ZeroMQ.
...
the [documentation on the Django project](https://docs.djangoproject.com/en/stable/topics/logging/#configuring-logging).
This dictionary is passed to [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig) to put the configuration into effect:
...
[**RFC 5424**](https://datatracker.ietf.org/doc/html/rfc5424.html) requires that a
Unicode message be sent to a syslog daemon as a set of bytes which have the
following structure: an optional pure-ASCII component, followed by a UTF-8 Byte
Order Mark (BOM), followed by Unicode encoded using UTF-8. (See the
[**relevant section of the specification**](https://datatracker.ietf.org/doc/html/rfc5424.html#section-6).)

In Python 3.1, code was added to
[`SysLogHandler`](../library/logging.handlers.md#logging.handlers.SysLogHandler) to insert a BOM into the message, but
...
want to produce [**RFC 5424**](https://datatracker.ietf.org/doc/html/rfc5424.html)-compliant messages which include a BOM, an optional
pure-ASCII sequence before it and arbitrary Unicode after it, encoded using
UTF-8, then you need to do the following:

1. Attach a [`Formatter`](../library/logging.md#logging.Formatter) instance to your
   [`SysLogHandler`](../library/logging.handlers.md#logging.handlers.SysLogHandler) instance, with a format string
   such as:
   ```python3
...
2. Replace the ASCII section with whatever placeholders you like, but make sure
   that the data that appears in there after substitution is always ASCII (that
   way, it will remain unchanged after UTF-8 encoding).
3. Replace the Unicode section with whatever placeholders you like; if the data
   which appears there after substitution contains characters outside the ASCII
   range, that’s fine – it will be encoded using UTF-8.

The formatted message *will* be encoded using UTF-8 encoding by
`SysLogHandler`. If you follow the above rules, you should be able to produce
[**RFC 5424**](https://datatracker.ietf.org/doc/html/rfc5424.html)-compliant messages. If you don’t, logging may not complain, but your
...
<a id="custom-handlers"></a>

## Customizing handlers with [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig)

There are times when you want to customize logging handlers in particular ways,
and if you use [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig) you may be able to do this without
subclassing. As an example, consider that you may want to set the ownership of a
log file. On POSIX, this is easily done using [`shutil.chown()`](../library/shutil.md#shutil.chown), but the file
...
You can then specify, in a logging configuration passed to [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig),
...
Note that this example uses Python 3.3 because that’s where [`shutil.chown()`](../library/shutil.md#shutil.chown)
makes an appearance. This approach should work with any Python version that
supports [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig) - namely, Python 2.7, 3.2 or later. With pre-3.3
versions, you would need to implement the actual ownership change using e.g.
[`os.chown()`](../library/os.md#os.chown).
...
is resolved by [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig) from the `ext://` specification.

This example hopefully also points the way to how you could implement other
types of file change - e.g. setting specific POSIX permission bits - in the
same way, using [`os.chmod()`](../library/os.md#os.chmod).

Of course, the approach could also be extended to types of handler other than a
[`FileHandler`](../library/logging.handlers.md#logging.FileHandler) - for example, one of the rotating file handlers,
or a different type of handler altogether.

<a id="formatting-styles"></a>

## Using particular formatting styles throughout your application

In Python 3.2, the [`Formatter`](../library/logging.md#logging.Formatter) gained a `style` keyword
...
supported by [`str.format()`](../library/stdtypes.md#str.format) and [`string.Template`](../library/string.md#string.Template). Note that this
...
Logging calls ([`debug()`](../library/logging.md#logging.Logger.debug), [`info()`](../library/logging.md#logging.Logger.info) etc.) only take
positional parameters for the actual logging message itself, with keyword
...
logging calls using [`str.format()`](../library/stdtypes.md#str.format) or [`string.Template`](../library/string.md#string.Template) syntax,
...
In Python 3.2, along with the [`Formatter`](../library/logging.md#logging.Formatter) changes mentioned
above, the logging package gained the ability to allow users to set their own
[`LogRecord`](../library/logging.md#logging.LogRecord) subclasses, using the [`setLogRecordFactory()`](../library/logging.md#logging.setLogRecordFactory) function.
You can use this to set your own subclass of [`LogRecord`](../library/logging.md#logging.LogRecord), which does the
Right Thing by overriding the [`getMessage()`](../library/logging.md#logging.LogRecord.getMessage) method. The base
...
Refer to the reference documentation on [`setLogRecordFactory()`](../library/logging.md#logging.setLogRecordFactory) and
[`LogRecord`](../library/logging.md#logging.LogRecord) for more information.
...
[Using arbitrary objects as messages](logging.md#arbitrary-object-messages)) that when logging you can use an arbitrary
object as a message format string, and that the logging package will call
[`str()`](../library/stdtypes.md#str) on that object to get the actual format string. Consider the
following two classes:
...
[`str.format()`](../library/stdtypes.md#str.format):
...
Secondly, formatting with [`string.Template`](../library/string.md#string.Template):
...
parentheses go around the format string and the arguments, not just the format
string. That’s because the \_\_ notation is just syntax sugar for a constructor
call to one of the `*XXX*Message` classes shown above.

<a id="filters-dictconfig"></a>

## Configuring filters with [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig)

You *can* configure filters using [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig), though it
might not be obvious at first glance how to do it (hence this recipe). Since
[`Filter`](../library/logging.md#logging.Filter) is the only filter class included in the standard
library, and it is unlikely to cater to many requirements (it’s only there as a
base class), you will typically need to define your own [`Filter`](../library/logging.md#logging.Filter)
subclass with an overridden [`filter()`](../library/logging.md#logging.Filter.filter) method. To do this,
...
[`Filter`](../library/logging.md#logging.Filter) instance). Here is a complete example:
...
* If you can’t refer to the callable directly in the configuration (e.g. if it
  ...
  in [Access to external objects](../library/logging.config.md#logging-config-dict-externalobj). For example, you could have used
  the text `'ext://__main__.MyFilter'` instead of `MyFilter` in the above
  example.
* As well as for filters, this technique can also be used to configure custom
  handlers and formatters. See [User-defined objects](../library/logging.config.md#logging-config-dict-userdef) for more
  information on how logging supports using user-defined objects in its
  configuration, and see the other cookbook recipe [Customizing handlers with dictConfig()](#custom-handlers) above.

<a id="custom-format-exception"></a>
...
information can be formatted to your liking. The [`traceback`](../library/traceback.md#module-traceback) module may be
helpful for more specialized needs.

<a id="spoken-messages"></a>
...
this can be invoked from a handler using [`subprocess`](../library/subprocess.md#module-subprocess). It’s assumed here
...
[`logging.handlers.MemoryHandler`](../library/logging.handlers.md#logging.handlers.MemoryHandler), which allows buffering of logged events
...
records buffered). These default to a [`StreamHandler`](../library/logging.handlers.md#logging.StreamHandler) which
...
[`BufferingHandler`](../library/logging.handlers.md#logging.handlers.BufferingHandler). In the following  example, which you can
...
[`Formatter`](../library/logging.md#logging.Formatter). If you want to do that via configuration, you can
use the [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig) API with an approach illustrated by
...
application. The [Qt](https://www.qt.io/) framework is a popular
cross-platform UI framework with Python bindings using [PySide2](https://pypi.org/project/PySide2/)
or [PyQt5](https://pypi.org/project/PyQt5/) libraries.
...
[`threading`](../library/threading.md#module-threading) module, as there are circumstances where one has to use
...
Although [**RFC 5424**](https://datatracker.ietf.org/doc/html/rfc5424.html) dates from 2009, most syslog servers are configured by default to
use the older [**RFC 3164**](https://datatracker.ietf.org/doc/html/rfc3164.html), which hails from 2001. When `logging` was added to Python
...
servers, the [`SysLogHandler`](../library/logging.handlers.md#logging.handlers.SysLogHandler) functionality has not been
updated.
...
example, the [`basicConfig()`](../library/logging.md#logging.basicConfig) call does this (using the
...
[`basicConfig()`](../library/logging.md#logging.basicConfig), but you can use a different formatter when you
configure logging.
...
If, on the other hand, you are concerned about [log injection](https://owasp.org/www-community/attacks/Log_Injection), you can use a
...
* Forking a process, following which both parent and child have a reference to
  the same file. This might be through use of the [`multiprocessing`](../library/multiprocessing.md#module-multiprocessing) module,
  for example.
...
Use the techniques outlined in [Logging to a single file from multiple processes](#multiple-processes) to circumvent such
issues.
...
### Adding handlers other than [`NullHandler`](../library/logging.handlers.md#logging.NullHandler) to a logger in a library
...
loggers other than a [`NullHandler`](../library/logging.handlers.md#logging.NullHandler) instance.
...
the [existing mechanisms](#context-info) for passing contextual
...
<a id="cookbook-ref-links"></a>

## Other resources

#### SEE ALSO
Module [`logging`](../library/logging.md#module-logging)
: API reference for the logging module.

Module [`logging.config`](../library/logging.config.md#module-logging.config)
: Configuration API for the logging module.

Module [`logging.handlers`](../library/logging.handlers.md#module-logging.handlers)
: Useful handlers included with the logging module.

[Basic Tutorial](logging.md#logging-basic-tutorial)

[Advanced Tutorial](logging.md#logging-advanced-tutorial)
