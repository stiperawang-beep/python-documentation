<a id="isolating-extensions-howto"></a>
...
This guide is written for maintainers of [C-API](../c-api/index.md#c-api-index) extensions
...
- in sequence, with several [`Py_InitializeEx()`](../c-api/interp-lifecycle.md#c.Py_InitializeEx)/[`Py_FinalizeEx()`](../c-api/interp-lifecycle.md#c.Py_FinalizeEx)
  cycles, and
- in parallel, managing “sub-interpreters” using
  [`Py_NewInterpreter()`](../c-api/subinterpreters.md#c.Py_NewInterpreter)/[`Py_EndInterpreter()`](../c-api/subinterpreters.md#c.Py_EndInterpreter).
...
a module is just like any other ; there are no “on
...
(see [Managing Global State]()), but they will need more
...
[Opt-Out: Limiting to One Module Object per Process](#isolating-extensions-optout).

### Managing Per-Module State

To use per-module state, use
[multi-phase extension module initialization](../c-api/extension-modules.md#multi-phase-initialization).
...
and settings (e.g. `csv`’s [`field_size_limit`](../library/csv.md#csv.field_size_limit))
...
<a id="isolating-extensions-optout"></a>
...
If your module’s [`PyModuleDef.m_clear`](../c-api/module.md#c.PyModuleDef.m_clear) function is able to prepare
...
Python runtime shutdown ([`Py_FinalizeEx()`](../c-api/interp-lifecycle.md#c.Py_FinalizeEx)) and re-initialization
([`Py_Initialize()`](../c-api/interp-lifecycle.md#c.Py_Initialize)).
...
**CPython implementation detail:** Sharing truly immutable objects between interpreters is fine,
...
* Unlike static types, heap type objects are mutable by default.
  Use the [`Py_TPFLAGS_IMMUTABLETYPE`](../c-api/typeobj.md#c.Py_TPFLAGS_IMMUTABLETYPE) flag to prevent mutability.
* Heap types inherit [`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new) by default,
  so it may become possible to instantiate them from Python code.
  You can prevent this with the [`Py_TPFLAGS_DISALLOW_INSTANTIATION`](../c-api/typeobj.md#c.Py_TPFLAGS_DISALLOW_INSTANTIATION) flag.

### Defining Heap Types

Heap types can be created by filling a [`PyType_Spec`](../c-api/type.md#c.PyType_Spec) structure, a
description or “blueprint” of a class, and calling
[`PyType_FromModuleAndSpec()`](../c-api/type.md#c.PyType_FromModuleAndSpec) to construct a new class object.

#### NOTE
Other functions, like [`PyType_FromSpec()`](../c-api/type.md#c.PyType_FromSpec), can also create
heap types, but [`PyType_FromModuleAndSpec()`](../c-api/type.md#c.PyType_FromModuleAndSpec) associates the module
...
- Have the [`Py_TPFLAGS_HAVE_GC`](../c-api/typeobj.md#c.Py_TPFLAGS_HAVE_GC) flag.
- Define a traverse function using [`Py_tp_traverse`](../c-api/typeobj.md#c.Py_tp_traverse), which
  visits the type (e.g. using `Py_VISIT(Py_TYPE(self))`).

Please refer to the documentation of
[`Py_TPFLAGS_HAVE_GC`](../c-api/typeobj.md#c.Py_TPFLAGS_HAVE_GC) and [`tp_traverse`](../c-api/typeobj.md#c.PyTypeObject.tp_traverse)
...
Unfortunately, [`Py_Version`](../c-api/apiabiversion.md#c.Py_Version) was only added in Python 3.11.
As a replacement, use:

* [`PY_VERSION_HEX`](../c-api/apiabiversion.md#c.PY_VERSION_HEX), if not using the stable ABI, or
* [`sys.version_info`](../library/sys.md#sys.version_info) (via [`PySys_GetObject()`](../c-api/sys.md#c.PySys_GetObject) and
  [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple)).

#### Delegating `tp_traverse`

If your traverse function delegates to the [`tp_traverse`](../c-api/typeobj.md#c.PyTypeObject.tp_traverse)
...
[`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new) and [`tp_clear`](../c-api/typeobj.md#c.PyTypeObject.tp_clear).

#### Defining `tp_dealloc`

If your type has a custom [`tp_dealloc`](../c-api/typeobj.md#c.PyTypeObject.tp_dealloc) function,
it needs to:

- call [`PyObject_GC_UnTrack()`](../c-api/gcsupport.md#c.PyObject_GC_UnTrack) before any fields are invalidated, and
...
The [`tp_free`](../c-api/typeobj.md#c.PyTypeObject.tp_free) slot of a heap type must be set to
[`PyObject_GC_Del()`](../c-api/gcsupport.md#c.PyObject_GC_Del).
...
If you use [`PyObject_New()`](../c-api/allocation.md#c.PyObject_New) or [`PyObject_NewVar()`](../c-api/allocation.md#c.PyObject_NewVar):

- Get and call type’s [`tp_alloc`](../c-api/typeobj.md#c.PyTypeObject.tp_alloc) slot, if possible.
...
- If the above is not possible (e.g. inside a custom `tp_alloc`),
  call [`PyObject_GC_New()`](../c-api/gcsupport.md#c.PyObject_GC_New) or [`PyObject_GC_NewVar()`](../c-api/gcsupport.md#c.PyObject_GC_NewVar):
  ```c
...
If you have a type object defined with [`PyType_FromModuleAndSpec()`](../c-api/type.md#c.PyType_FromModuleAndSpec),
you can call [`PyType_GetModule()`](../c-api/type.md#c.PyType_GetModule) to get the associated module, and then
[`PyModule_GetState()`](../c-api/module.md#c.PyModule_GetState) to get the module’s state.

To save a some tedious error-handling boilerplate code, you can combine
these two steps with [`PyType_GetModuleState()`](../c-api/type.md#c.PyType_GetModuleState), resulting in:
...
[METH_METHOD | METH_FASTCALL | METH_KEYWORDS](../c-api/structures.md#meth-method-meth-fastcall-meth-keywords)
[`calling convention`](../c-api/structures.md#c.PyMethodDef)
and the corresponding [`PyCMethod`](../c-api/structures.md#c.PyCMethod) signature:
...
Once you have the defining class, call [`PyType_GetModuleState()`](../c-api/type.md#c.PyType_GetModuleState) to get
...
[`nb_add`](../c-api/typeobj.md#c.PyNumberMethods.nb_add) for [`__add__`](../reference/datamodel.md#object.__add__) or
[`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new) for initialization—have a very simple API that
doesn’t allow passing in the defining class, unlike with [`PyCMethod`](../c-api/structures.md#c.PyCMethod).
The same goes for getters and setters defined with
[`PyGetSetDef`](../c-api/structures.md#c.PyGetSetDef).

To access the module state in these cases, use the
[`PyType_GetModuleByDef()`](../c-api/type.md#c.PyType_GetModuleByDef) function, and pass in the module definition.
Once you have the module, call [`PyModule_GetState()`](../c-api/module.md#c.PyModule_GetState)
to get the state:
...
[method resolution order](../glossary.md#term-method-resolution-order) (i.e. all superclasses) for the first
...
[`PyType_FromModuleAndSpec()`](../c-api/type.md#c.PyType_FromModuleAndSpec), and their instances, hold a reference
to the module.
...
Discussions about improving the situation are best held on the [discuss forum under c-api tag](https://discuss.python.org/c/core-dev/c-api/30).
