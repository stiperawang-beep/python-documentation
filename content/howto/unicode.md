...
Unicode ([https://www.unicode.org/](https://www.unicode.org/)) is a specification that aims to
...
Since Python 3.0, the language’s [`str`](../library/stdtypes.md#str) type contains Unicode
...
In addition, one can create a string using the [`decode()`](../library/stdtypes.md#bytes.decode) method of
[`bytes`](../library/stdtypes.md#bytes).  This method takes an *encoding* argument, such as `UTF-8`,
and optionally an *errors* argument.

The *errors* argument specifies the response when the input string can’t be
converted according to the encoding’s rules.  Legal values for this argument are
`'strict'` (raise a [`UnicodeDecodeError`](../library/exceptions.md#UnicodeDecodeError) exception), `'replace'` (use
...
[Standard Encodings](../library/codecs.md#standard-encodings) for a list.  Some encodings have multiple names; for
example, `'latin-1'`, `'iso_8859_1'` and `'8859`’ are all synonyms for
the same encoding.

One-character Unicode strings can also be created with the [`chr()`](../library/functions.md#chr)
built-in function, which takes integers and returns a Unicode string of length 1
that contains the corresponding code point.  The reverse operation is the
built-in [`ord()`](../library/functions.md#ord) function that takes a one-character Unicode string and
...
The opposite method of [`bytes.decode()`](../library/stdtypes.md#bytes.decode) is [`str.encode()`](../library/stdtypes.md#str.encode),
which returns a [`bytes`](../library/stdtypes.md#bytes) representation of the Unicode string, encoded in the
requested *encoding*.

The *errors* parameter is the same as the parameter of the
[`decode()`](../library/stdtypes.md#bytes.decode) method but supports a few more possible handlers. As well as
...
encodings are found in the [`codecs`](../library/codecs.md#module-codecs) module.  Implementing new
encodings also requires understanding the [`codecs`](../library/codecs.md#module-codecs) module.
...
can also assemble strings using the [`chr()`](../library/functions.md#chr) built-in function, but this is
even more tedious.
...
[`casefold()`](../library/stdtypes.md#str.casefold) string method that converts a string to a
...
A second tool is the [`unicodedata`](../library/unicodedata.md#module-unicodedata) module’s
[`normalize()`](../library/unicodedata.md#unicodedata.normalize) function that converts strings to one
of several normal forms, where letters followed by a combining character are
replaced with single characters.  [`normalize()`](../library/unicodedata.md#unicodedata.normalize) can
...
The first argument to the [`normalize()`](../library/unicodedata.md#unicodedata.normalize) function is a
...
there are a few characters that make [`casefold()`](../library/stdtypes.md#str.casefold) return a
...
The regular expressions supported by the [`re`](../library/re.md#module-re) module can be provided
...
out.  If you supply the [`re.ASCII`](../library/re.md#re.ASCII) flag to
[`compile()`](../library/re.md#re.compile), `\d+` will match the substring “57” instead.

Similarly, `\w` matches a wide variety of Unicode characters but
only `[a-zA-Z0-9_]` in bytes or if [`re.ASCII`](../library/re.md#re.ASCII) is supplied,
...
The [`str`](../library/stdtypes.md#str) type is described in the Python library reference at
[Text Sequence Type — str](../library/stdtypes.md#textseq).

The documentation for the [`unicodedata`](../library/unicodedata.md#module-unicodedata) module.

The documentation for the [`codecs`](../library/codecs.md#module-codecs) module.
...
done for you: the built-in [`open()`](../library/functions.md#open) function can return a file-like object
that assumes the file’s contents are in a specified encoding and accepts Unicode
parameters for methods such as [`read()`](../library/io.md#io.TextIOBase.read) and
[`write()`](../library/io.md#io.TextIOBase.write).  This works through [`open()`](../library/functions.md#open)'s *encoding* and
*errors* parameters which are interpreted just like those in [`str.encode()`](../library/stdtypes.md#str.encode)
and [`bytes.decode()`](../library/stdtypes.md#bytes.decode).
...
there will only be a [filesystem encoding](../glossary.md#term-filesystem-encoding-and-error-handler). if you’ve set the `LANG` or `LC_CTYPE` environment variables; if
you haven’t, the default encoding is again UTF-8.

The [`sys.getfilesystemencoding()`](../library/sys.md#sys.getfilesystemencoding) function returns the encoding to use on
...
Functions in the [`os`](../library/os.md#module-os) module such as [`os.stat()`](../library/os.md#os.stat) will also accept Unicode
filenames.

The [`os.listdir()`](../library/os.md#os.listdir) function returns filenames, which raises an issue: should it return
the Unicode version of filenames, or should it return bytes containing
the encoded versions?  [`os.listdir()`](../library/os.md#os.listdir) can do both, depending on whether you
...
assuming the default [filesystem encoding](../glossary.md#term-filesystem-encoding-and-error-handler) is UTF-8, running the following program:
...
you do e.g. `str + bytes`, a [`TypeError`](../library/exceptions.md#TypeError) will be raised.
...
The [`StreamRecoder`](../library/codecs.md#codecs.StreamRecoder) class can transparently convert between
...
can wrap it with a [`StreamRecoder`](../library/codecs.md#codecs.StreamRecoder) to return bytes encoded in
UTF-8:
...
One section of [Mastering Python 3 Input/Output](https://pyvideo.org/video/289/pycon-2010--mastering-python-3-i-o),
a PyCon 2010 talk by David Beazley, discusses text processing and binary data handling.

The [PDF slides for Marc-André Lemburg’s presentation “Writing Unicode-aware
Applications in Python”](https://downloads.egenix.com/python/LSM2005-Developing-Unicode-aware-applications-in-Python.pdf)
discuss questions of character encodings as well as how to internationalize
and localize an application.  These slides cover Python 2.x only.

[The Guts of Unicode in Python](https://pyvideo.org/video/1768/the-guts-of-unicode-in-python)
