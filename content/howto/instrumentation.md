...
CPython must then be [`configured with the --with-dtrace option`](../using/configure.md#cmdoption-with-dtrace):
...
(with the [`--enable-shared`](../using/configure.md#cmdoption-enable-shared) configure option), you
...
For a [`--enable-shared`](../using/configure.md#cmdoption-enable-shared) build of CPython, the markers are contained within the
...
(assuming a [debug build](../using/configure.md#debug-build) of CPython 3.6)

<a id="static-markers"></a>

## Available static markers

### function_\_entry(str filename, str funcname, int lineno)
...
### function_\_return(str filename, str funcname, int lineno)
...
### gc_\_start(int generation)

Fires when the Python interpreter starts a garbage collection cycle.
`arg0` is the generation to scan, like [`gc.collect()`](../library/gc.md#gc.collect).

### gc_\_done(long collected)

Fires when the Python interpreter finishes a garbage collection
cycle. `arg0` is the number of collected objects.

### import_\_find_\_load_\_start(str modulename)

Fires before [`importlib`](../library/importlib.md#module-importlib) attempts to find and load the module.
`arg0` is the module name.

#### Versionadded
Added in version 3.7.

### import_\_find_\_load_\_done(str modulename, int found)

Fires after [`importlib`](../library/importlib.md#module-importlib)’s find_and_load function is called.
...
Fires when [`sys.audit()`](../library/sys.md#sys.audit) or [`PySys_Audit()`](../c-api/sys.md#c.PySys_Audit) is called.
`arg0` is the event name as C string, `arg1` is a [`PyObject`](../c-api/structures.md#c.PyObject)
...
| ### void PyDTrace_GC_DONE([Py_ssize_t](../c-api/intro.md#c.Py_ssize_t) arg0)      | `gc__done()`                  |                    |
