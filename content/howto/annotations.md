...
> in your code, please see the [`typing`](../library/typing.md#module-typing) module.

## Accessing The Annotations Dict Of An Object In Python 3.10 And Newer

Python 3.10 adds a new function to the standard library:
[`inspect.get_annotations()`](../library/inspect.md#inspect.get_annotations).  In Python versions 3.10
...
In Python 3.14, there is a new [`annotationlib`](../library/annotationlib.md#module-annotationlib) module
with functionality for working with annotations. This
includes a [`annotationlib.get_annotations()`](../library/annotationlib.md#annotationlib.get_annotations) function,
which supersedes [`inspect.get_annotations()`](../library/inspect.md#inspect.get_annotations).

If for some reason [`inspect.get_annotations()`](../library/inspect.md#inspect.get_annotations) isn’t
...
callables created by [`functools.partial()`](../library/functools.md#functools.partial)–may
...
newer is to call [`getattr()`](../library/functions.md#getattr) with three arguments,
...
[`inspect.get_annotations()`](../library/inspect.md#inspect.get_annotations): you should use three-argument
[`getattr()`](../library/functions.md#getattr) to access the object’s `__annotations__`
attribute.
...
they are stored in the class’s [`__dict__`](../reference/datamodel.md#type.__dict__) dictionary.  Since
the class may or may not have annotations defined, best practice
is to call the [`get()`](../library/stdtypes.md#dict.get) method on the class dict.
...
the type of `ann` using [`isinstance()`](../library/functions.md#isinstance) before further
examination.

Note that some exotic or malformed type objects may not have
a [`__dict__`](../reference/datamodel.md#type.__dict__) attribute, so for extra safety you may also wish
to use [`getattr()`](../library/functions.md#getattr) to access `__dict__`.
...
call [`inspect.get_annotations()`](../library/inspect.md#inspect.get_annotations) to do this work
for you.

If you’re using Python 3.9 or older, or if for some reason
you can’t use [`inspect.get_annotations()`](../library/inspect.md#inspect.get_annotations), you’ll need
to duplicate its logic.  You’re encouraged to examine the
implementation of [`inspect.get_annotations()`](../library/inspect.md#inspect.get_annotations) in the
...
* If `o` is a module, use `o.__dict__` as the
  `globals` when calling [`eval()`](../library/functions.md#eval).
* If `o` is a class, use `sys.modules[o.__module__].__dict__`
  as the `globals`, and `dict(vars(o))` as the `locals`,
  when calling [`eval()`](../library/functions.md#eval).
* If `o` is a wrapped callable using [`functools.update_wrapper()`](../library/functools.md#functools.update_wrapper),
  [`functools.wraps()`](../library/functools.md#functools.wraps), or [`functools.partial()`](../library/functools.md#functools.partial), iteratively
...
* If `o` is a callable (but not a class), use
  [`o.__globals__`](../reference/datamodel.md#function.__globals__) as the globals when calling
  [`eval()`](../library/functions.md#eval).

However, not all string values used as annotations can
be successfully turned into Python values by [`eval()`](../library/functions.md#eval).
...
* Definitions that aren’t needed at runtime, only imported
  when [`typing.TYPE_CHECKING`](../library/typing.md#typing.TYPE_CHECKING) is true.

If [`eval()`](../library/functions.md#eval) attempts to evaluate such values, it will
...
* You should avoid accessing `__annotations__` directly on any object.
  Instead, use [`annotationlib.get_annotations()`](../library/annotationlib.md#annotationlib.get_annotations) (Python 3.14+)
  or [`inspect.get_annotations()`](../library/inspect.md#inspect.get_annotations) (Python 3.10+).
...
[**749**](https://peps.python.org/pep-0749/#pep749-metaclasses) for some examples. You can avoid these
quirks by using [`annotationlib.get_annotations()`](../library/annotationlib.md#annotationlib.get_annotations) on Python 3.14+ or
[`inspect.get_annotations()`](../library/inspect.md#inspect.get_annotations) on Python 3.10+. On earlier versions of
Python, you can avoid these bugs by accessing the annotations from the
class’s [`__dict__`](../reference/datamodel.md#type.__dict__)
...
annotations of an instance, you can use [`type()`](../library/functions.md#type) to access its class
