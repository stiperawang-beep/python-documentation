...
> with the [`re`](../library/re.md#module-re) module.  It provides a gentler introduction than the
...
available through the [`re`](../library/re.md#module-re) module. Using this little language, you specify
...
provided by the [`unicodedata`](../library/unicodedata.md#module-unicodedata) module.  You can use the more
restricted definition of `\w` in a string pattern by supplying the
[`re.ASCII`](../library/re.md#re.ASCII) flag when compiling the regular expression.
...
patterns, see the last part of [Regular Expression Syntax](../library/re.md#re-syntax) in the Standard Library reference.  In general, the
...
newline character, and there’s an alternate mode ([`re.DOTALL`](../library/re.md#re.DOTALL)) where it will
...
|      2 | `abcbd`   | The engine matches `[bcd]*`,<br/>going as far as it can, which<br/>is to the end of the string.               |
|      3 | *Failure* | The engine tries to match<br/>`b`, but the current position<br/>is at the end of the string, so<br/>it fails. |
|      4 | `abcb`    | Back up, so that  `[bcd]*`<br/>matches one less character.                                                    |
|      5 | *Failure* | Try `b` again, but the<br/>current position is at the last<br/>character, which is a `'d'`.                   |
|      6 | `abc`     | Back up again, so that<br/>`[bcd]*` is only matching<br/>`bc`.                                                |
|      7 | `abcb`    | Try `b` again.  This time<br/>the character at the<br/>current position is `'b'`, so<br/>it succeeds.         |
...
them in Python?  The [`re`](../library/re.md#module-re) module provides an interface to the regular
...
[`re.compile()`](../library/re.md#re.compile) also accepts an optional *flags* argument, used to enable
...
The RE is passed to [`re.compile()`](../library/re.md#re.compile) as a string.  REs are handled as strings
...
including them.) Instead, the [`re`](../library/re.md#module-re) module is simply a C extension module
included with Python, just like the [`socket`](../library/socket.md#module-socket) or [`zlib`](../library/zlib.md#module-zlib) modules.

Putting REs in strings keeps the Python language simpler, but has one
disadvantage which is the topic of the next section.

<a id="the-backslash-plague"></a>
...
to [`re.compile()`](../library/re.md#re.compile) must be `\\section`.  However, to express this as a
Python string literal, both backslashes must be escaped *again*.

| Characters      | Stage                                                               |
|-----------------|---------------------------------------------------------------------|
| `\section`      | Text string to be matched                                           |
| `\\section`     | Escaped backslash for [`re.compile()`](../library/re.md#re.compile) |
...
[`SyntaxWarning`](../library/exceptions.md#SyntaxWarning) and will eventually become a [`SyntaxError`](../library/exceptions.md#SyntaxError),
...
Only the most significant ones will be covered here; consult the [`re`](../library/re.md#module-re) docs
for a complete listing.

| Method/Attribute   | Purpose                                                                                                                            |
|--------------------|------------------------------------------------------------------------------------------------------------------------------------|
| `search()`         | Scan through a string, looking for any<br/>location where this RE matches.                                                         |
| `prefixmatch()`    | Determine if the RE matches at the beginning<br/>of the string. Previously named [match()](../library/re.md#prefixmatch-vs-match). |
| `findall()`        | Find all substrings where the RE matches, and<br/>return them as a list.                                                           |
| `finditer()`       | Find all substrings where the RE matches, and<br/>return them as an [iterator](../glossary.md#term-iterator).                      |

[`search()`](../library/re.md#re.Pattern.search) and [`prefixmatch()`](../library/re.md#re.Pattern.prefixmatch) return `None` if no match can be found.  If
they’re successful, a [match object](../library/re.md#match-objects) instance is returned,
containing information about the match: where it starts and ends, the substring
it matched, and more.

You can learn about this by interactively experimenting with the [`re`](../library/re.md#module-re)
module.

This HOWTO uses the standard Python interpreter for its examples. First, run the
Python interpreter, import the [`re`](../library/re.md#module-re) module, and compile a RE:
...
[`search()`](../library/re.md#re.Pattern.search) should return `None` in this case, which will cause the
interpreter to print no output.  You can explicitly print the result of
`search()` to make this clear.
...
case, [`search()`](../library/re.md#re.Pattern.search) will return a [match object](../library/re.md#match-objects), so you
...
Now you can query the [match object](../library/re.md#match-objects) for information
...
| `span()`           | Return a tuple containing the (start, end)<br/>positions  of the match |
...
[`group()`](../library/re.md#re.Match.group) returns the substring that was matched by the RE.  [`start()`](../library/re.md#re.Match.start)
and [`end()`](../library/re.md#re.Match.end) return the starting and ending index of the match. [`span()`](../library/re.md#re.Match.span)
returns both start and end indexes in a single tuple.
The [`search()`](../library/re.md#re.Pattern.search) method of patterns
scans through the string, so the match may not start at zero.
However, the [`prefixmatch()`](../library/re.md#re.Pattern.prefixmatch)
method only checks if the RE matches at the start of a string, so `start()`
...
[match object](../library/re.md#match-objects) in a variable, and then check if it was
...
[`findall()`](../library/re.md#re.Pattern.findall) returns a list of matching strings:
...
[`SyntaxWarning`](../library/exceptions.md#SyntaxWarning) and will eventually become a [`SyntaxError`](../library/exceptions.md#SyntaxError).  See
[The backslash plague](#the-backslash-plague).

[`findall()`](../library/re.md#re.Pattern.findall) has to create the entire list before it can be returned as the
result.  The [`finditer()`](../library/re.md#re.Pattern.finditer) method returns a sequence of
[match object](../library/re.md#match-objects) instances as an [iterator](../glossary.md#term-iterator):
...
[`re`](../library/re.md#module-re) module also provides top-level functions called [`search()`](../library/re.md#re.search),
[`prefixmatch()`](../library/re.md#re.prefixmatch), [`findall()`](../library/re.md#re.findall), [`sub()`](../library/re.md#re.sub), and so forth.  These functions
take the same arguments as the corresponding pattern method with
the RE string added as the first argument, and still return either `None` or a
[match object](../library/re.md#match-objects) instance.
...
Flags are available in the [`re`](../library/re.md#module-re) module under two names, a long name such as
[`IGNORECASE`](../library/re.md#re.IGNORECASE) and a short, one-letter form such as [`I`](../library/re.md#re.I).  (If you’re
familiar with Perl’s pattern modifiers, the one-letter forms use the same
letters; the short form of [`re.VERBOSE`](../library/re.md#re.VERBOSE) is [`re.X`](../library/re.md#re.X), for example.)
Multiple flags can be specified by bitwise OR-ing them; `re.I | re.M` sets
both the [`I`](../library/re.md#re.I) and [`M`](../library/re.md#re.M) flags, for example.
...
| Flag                                                                                        | Meaning                                                                                                                   |
|---------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------|
| [`ASCII`](../library/re.md#re.ASCII), [`A`](../library/re.md#re.A)                          | Makes several escapes like `\w`, `\b`,<br/>`\s` and `\d` match only on ASCII<br/>characters with the respective property. |
| [`DOTALL`](../library/re.md#re.DOTALL), [`S`](../library/re.md#re.S)                        | Make `.` match any character, including<br/>newlines.                                                                     |
| [`IGNORECASE`](../library/re.md#re.IGNORECASE), [`I`](../library/re.md#re.I)                | Do case-insensitive matches.                                                                                              |
| [`LOCALE`](../library/re.md#re.LOCALE), [`L`](../library/re.md#re.L)                        | Do a locale-aware match.                                                                                                  |
| [`MULTILINE`](../library/re.md#re.MULTILINE), [`M`](../library/re.md#re.M)                  | Multi-line matching, affecting `^` and<br/>`$`.                                                                           |
| [`VERBOSE`](../library/re.md#re.VERBOSE), [`X`](../library/re.md#re.X)<br/>(for ‘extended’) | Enable verbose REs, which can be organized<br/>more cleanly and understandably.                                           |
...
letters, too. Full Unicode matching also works unless the [`ASCII`](../library/re.md#re.ASCII)
flag is used to disable non-ASCII matches.  When the Unicode patterns
`[a-z]` or `[A-Z]` are used in combination with the [`IGNORECASE`](../library/re.md#re.IGNORECASE)
...
it will if you also set the [`LOCALE`](../library/re.md#re.LOCALE) flag.
...
Setting the [`LOCALE`](../library/re.md#re.LOCALE) flag when compiling a regular expression will cause
...
[More metacharacters](#more-metacharacters).)
...
For example, here’s a RE that uses [`re.VERBOSE`](../library/re.md#re.VERBOSE); see how much easier it
is to read?
...
to understand than the version using [`re.VERBOSE`](../library/re.md#re.VERBOSE).
...
`^`
: Matches at the beginning of lines.  Unless the [`MULTILINE`](../library/re.md#re.MULTILINE) flag has been
  set, this will only match at the beginning of the string.  In [`MULTILINE`](../library/re.md#re.MULTILINE)
...
`\A`
: Matches only at the start of the string.  When not in [`MULTILINE`](../library/re.md#re.MULTILINE) mode,
  `\A` and `^` are effectively the same.  In [`MULTILINE`](../library/re.md#re.MULTILINE) mode, they’re
...
to [`group()`](../library/re.md#re.Match.group), [`start()`](../library/re.md#re.Match.start), [`end()`](../library/re.md#re.Match.end), and
[`span()`](../library/re.md#re.Match.span).  Groups are
numbered starting with 0.  Group 0 is always present; it’s the whole RE, so
[match object](../library/re.md#match-objects) methods all have group 0 as their default
...
[`group()`](../library/re.md#re.Match.group) can be passed multiple group numbers at a time, in which case it
...
The [`groups()`](../library/re.md#re.Match.groups) method returns a tuple containing the strings for all the
...
with a group.  The [match object](../library/re.md#match-objects) methods that deal with
...
[`groupdict()`](../library/re.md#re.Match.groupdict):
...
of having to remember numbers.  Here’s an example RE from the [`imaplib`](../library/imaplib.md#module-imaplib)
module:
...
| Method/Attribute   | Purpose                                                                                            |
|--------------------|----------------------------------------------------------------------------------------------------|
| `split()`          | Split the string into a list, splitting it<br/>wherever the RE matches                             |
| `sub()`            | Find all substrings where the RE matches, and<br/>replace them with a different string             |
| `subn()`           | Does the same thing as `sub()`,  but<br/>returns the new string and the number of<br/>replacements |

### Splitting strings

The [`split()`](../library/re.md#re.Pattern.split) method of a pattern splits a string apart
wherever the RE matches, returning a list of the pieces. It’s similar to the
[`split()`](../library/stdtypes.md#str.split) method of strings but provides much more generality in the
...
[`re.split()`](../library/re.md#re.split) function, too.
...
The module-level function [`re.split()`](../library/re.md#re.split) adds the RE to be used as the first
...
with a different string.  The [`sub()`](../library/re.md#re.Pattern.sub) method takes a replacement value,
...
Here’s a simple example of using the [`sub()`](../library/re.md#re.Pattern.sub) method.  It replaces colour
...
The [`subn()`](../library/re.md#re.Pattern.subn) method does the same work, but returns a 2-tuple containing the
...
[match object](../library/re.md#match-objects) argument for the match and can use this
...
When using the module-level [`re.sub()`](../library/re.md#re.sub) function, the pattern is passed as
...
Sometimes using the [`re`](../library/re.md#module-re) module is a mistake.  If you’re matching a fixed
string, or a single character class, and you’re not using any [`re`](../library/re.md#module-re) features
such as the [`IGNORECASE`](../library/re.md#re.IGNORECASE) flag, then the full power of regular expressions
...
example, you might replace `word` with `deed`.  [`re.sub()`](../library/re.md#re.sub) seems like the
function to use for this, but consider the [`replace()`](../library/stdtypes.md#str.replace) method.  Note that
...
something like `re.sub('\n', ' ', S)`, but [`translate()`](../library/stdtypes.md#str.translate) is capable of
doing both tasks and will be faster than any regular expression operation can
be.

In short, before turning to the [`re`](../library/re.md#module-re) module, consider whether your problem
can be solved with a faster and simpler string method.

<a id="match-versus-search"></a>

### prefixmatch() (aka match) versus search()

[`prefixmatch()`](../library/re.md#re.prefixmatch) was added in Python 3.15 as the [preferred name](../library/re.md#prefixmatch-vs-match) for [`match()`](../library/re.md#re.match). Before this, it was only known
as `match()` and the distinction with [`search()`](../library/re.md#re.search) was often
misunderstood.

`prefixmatch()` aka `match()` only checks if the RE matches at the
beginning of the string while `search()` scans forward through the
string for a match.
...
On the other hand, [`search()`](../library/re.md#re.search) scans forward through the string,
...
This distinction is important to remember when using the old [`match()`](../library/re.md#re.match)
...
For such REs, specifying the [`re.VERBOSE`](../library/re.md#re.VERBOSE) flag when compiling the regular
...
improvements to the [issue tracker](../bugs.md#using-the-tracker).
