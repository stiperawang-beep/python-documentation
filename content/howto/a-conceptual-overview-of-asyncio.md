<a id="a-conceptual-overview-of-asyncio"></a>

# A conceptual overview of `asyncio`

This [HOWTO](index.md#how-tos) article seeks to help you build a sturdy mental
model of how [`asyncio`](../library/asyncio.md#module-asyncio) fundamentally works, helping you understand the
...
* This in-depth [YouTube tutorial series](https://www.youtube.com/watch?v=Xbl7XjFYsN4&list=PLhNSoGM2ik6SIkVGXWBwerucXjgP1rHmB) on
  `asyncio` created by Python core team member, Łukasz Langa.
* [500 Lines or Less: A Web Crawler With asyncio Coroutines](https://aosabook.org/en/500L/a-web-crawler-with-asyncio-coroutines.html) by A.
...
The [async def](../reference/compound_stmts.md#async-def), as opposed to just a plain `def`, makes
this an asynchronous function (or “coroutine function”).
Calling it creates and returns a [coroutine](../library/asyncio-task.md#coroutine) object.
...
precisely, an instance of [`types.CoroutineType`](../library/types.md#types.CoroutineType) (native coroutine).
Note that coroutines can also exist as instances of
[`collections.abc.Coroutine`](../library/collections.abc.md#collections.abc.Coroutine) – a distinction that matters for type
checking.
...
of [generators](../glossary.md#term-generator-iterator) and
[generator functions](../glossary.md#term-generator).
Recall, a generator function is a function that [`yield`](../reference/simple_stmts.md#yield)s, like this
one:
...
built-in function [`next()`](../library/functions.md#next).
...
Roughly speaking, [tasks](../library/asyncio-task.md#asyncio-task-obj) are coroutines (not coroutine
functions) tied to an event loop.
A task also maintains a list of callback functions whose importance will become
clear in a moment when we discuss [`await`](../reference/expressions.md#await).
...
The recommended way to create tasks is via [`asyncio.create_task()`](../library/asyncio-task.md#asyncio.create_task).

`asyncio` automatically associates tasks with the event loop for you.
This automatic association was purposely designed into `asyncio` for
...
In practice, it’s recommended to use (and common to see) [`asyncio.run()`](../library/asyncio-runner.md#asyncio.run),
...
[`await`](../reference/expressions.md#await) is a Python keyword that’s commonly used in one of two
different ways:
...
loop.
[`asyncio.run()`](../library/asyncio-runner.md#asyncio.run) can help you detect such occurrences via the
`debug=True` flag, which enables
[debug mode](../library/asyncio-dev.md#asyncio-debug-mode).
...
`asyncio` leverages four components of Python to pass
around control.

[`coroutine.send(arg)`](../reference/expressions.md#generator.send) is the method used to start or
resume a coroutine.
...
[yield](../reference/expressions.md#yieldexpr), as usual, pauses execution and returns control
to the caller.
In the example above, the `yield`, on line 3, is called by
`... = await rock` on line 11.
More broadly speaking, `await` calls the [`__await__()`](../reference/datamodel.md#object.__await__) method of
the given object.
...
When a coroutine finishes, it raises a [`StopIteration`](../library/exceptions.md#StopIteration) exception with the
return value attached in the [`value`](../library/exceptions.md#StopIteration.value) attribute.
...
> [async generator function](../reference/expressions.md#asynchronous-generator-functions), a
> different beast entirely.

> 2. What about a [yield from](../reference/expressions.md#yieldexpr) within the coroutine function to a (plain)
> generator?
...
A [future](../library/asyncio-future.md#asyncio-future-obj) is an object meant to represent a
...
[`asyncio.Task`](../library/asyncio-task.md#asyncio.Task) subclasses [`asyncio.Future`](../library/asyncio-future.md#asyncio.Future) in order to gain
...
[`asyncio.sleep()`](../library/asyncio-task.md#asyncio.sleep).
...
marked as done.
If [`future.set_result()`](../library/asyncio-future.md#asyncio.Future.set_result) (the method
...
[`rest of the documentation`](../library/asyncio.md#module-asyncio).
