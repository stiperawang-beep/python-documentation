...
Given a Python object represented by a  pointer,
...
[devguide](https://devguide.python.org)  and the
[Python wiki](https://wiki.python.org/moin/DebuggingWithGdb).
...
- Use a [debug build](../using/configure.md#debug-build) of Python. (When building from source,
  use `configure --with-pydebug`. On Linux distros, install and run a package
  like `python-debug` or `python-dbg`, if available.)
- Use the runtime [development mode](../library/devmode.md#devmode) (`-X dev`).
