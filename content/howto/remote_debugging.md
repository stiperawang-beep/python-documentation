...
* Set the [`PYTHON_DISABLE_REMOTE_DEBUG`](../using/cmdline.md#envvar-PYTHON_DISABLE_REMOTE_DEBUG) environment variable to `1` before
  starting the interpreter.
* Use the [`-X disable_remote_debug`](../using/cmdline.md#cmdoption-X) command-line option.
* Compile Python with the [`--without-remote-debug`](../using/configure.md#cmdoption-without-remote-debug) build flag.

<a id="permission-requirements"></a>
...
#### NOTE
Ensure you’ve satisfied all [Permission requirements](#permission-requirements) before proceeding.
...
This mechanism forms the basis of the [`sys.remote_exec()`](../library/sys.md#sys.remote_exec) function, which
...
You can read more about the ELF file format in the [ELF specification](https://en.wikipedia.org/wiki/Executable_and_Linkable_Format).
...
1. Use the ToolHelp API to enumerate all modules loaded in the target process.
   This is done using functions such as [CreateToolhelp32Snapshot](https://learn.microsoft.com/en-us/windows/win32/api/tlhelp32/nf-tlhelp32-createtoolhelp32snapshot),
   [Module32First](https://learn.microsoft.com/en-us/windows/win32/api/tlhelp32/nf-tlhelp32-module32first),
   and [Module32Next](https://learn.microsoft.com/en-us/windows/win32/api/tlhelp32/nf-tlhelp32-module32next).
2. Identify the module corresponding to `python.exe` or
   `python*XY*.dll`, where `X` and `Y` are the major and minor
...
<a id="remote-debugging-threat-model"></a>
...
The environment variable [`PYTHON_DISABLE_REMOTE_DEBUG`](../using/cmdline.md#envvar-PYTHON_DISABLE_REMOTE_DEBUG) (and the
equivalent [`-X disable_remote_debug`](../using/cmdline.md#cmdoption-X) flag) allows operators to disable
