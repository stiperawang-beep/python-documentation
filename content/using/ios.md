...
Python, projects such as [BeeWare](https://beeware.org) and [Kivy](https://kivy.org) will provide a much more approachable user experience.
...
[`--host`](configure.md#cmdoption-host) option to `configure`. By default, when compiled for iOS,
...
[`platform.ios_ver()`](../library/platform.md#platform.ios_ver). [`platform.system()`](../library/platform.md#platform.system) will report `iOS` or
`iPadOS`, depending on the device.

[`os.uname()`](../library/os.md#os.uname) reports kernel-level details; it will report a name of
`Darwin`.

### Standard library availability

The Python standard library has some notable omissions and restrictions on
iOS. See the [API availability guide for iOS](../library/intro.md#mobile-availability) for
details.
...
[`AppleFrameworkLoader`](../library/importlib.md#importlib.machinery.AppleFrameworkLoader) that is able to read and
...
[`ModuleSpec`](../library/importlib.md#importlib.machinery.ModuleSpec) for the loaded module will report the
...
library like [PythonKit](https://github.com/pvieito/PythonKit) to be
helpful.

To add Python to an iOS Xcode project:

1. Build or obtain a Python `XCFramework`. See the instructions in
   [Apple/iOS/README.md](https://github.com/python/cpython/tree/main/Apple/iOS/README.md) (in the CPython source distribution) for details on
...
8. Add Objective C code to initialize and use a Python interpreter in embedded
   ...
   * UTF-8 mode ([`PyPreConfig.utf8_mode`](../c-api/init_config.md#c.PyPreConfig.utf8_mode)) is *enabled*;
   * Buffered stdio ([`PyConfig.buffered_stdio`](../c-api/init_config.md#c.PyConfig.buffered_stdio)) is *disabled*;
   * Writing bytecode ([`PyConfig.write_bytecode`](../c-api/init_config.md#c.PyConfig.write_bytecode)) is *disabled*;
   * Signal handlers ([`PyConfig.install_signal_handlers`](../c-api/init_config.md#c.PyConfig.install_signal_handlers)) are *enabled*;
   * System logging ([`PyConfig.use_system_logger`](../c-api/init_config.md#c.PyConfig.use_system_logger)) is *enabled*
     (optional, but strongly recommended; this is enabled by default);
   * [`PYTHONHOME`](cmdline.md#envvar-PYTHONHOME) for the interpreter is configured to point at the
     `python` subfolder of your app’s bundle; and
   * The [`PYTHONPATH`](cmdline.md#envvar-PYTHONPATH) for the interpreter includes:
...
* If you’re using a separate folder for third-party packages, ensure that
  ...
  as part of the [`PYTHONPATH`](cmdline.md#envvar-PYTHONPATH) configuration in step 8.
* If any of the folders that contain third-party packages will contain `.pth`
  ...
  [`site.addsitedir()`](../library/site.md#site.addsitedir)), rather than adding to [`PYTHONPATH`](cmdline.md#envvar-PYTHONPATH) or
  [`sys.path`](../library/sys.md#sys.path) directly.

### Testing a Python package

The CPython source tree contains [a testbed project](https://github.com/python/cpython/tree/main/Apple/iOS/testbed) that
...
After building or obtaining an iOS XCFramework (see [Apple/iOS/README.md](https://github.com/python/cpython/tree/main/Apple/iOS/README.md)
...
libraries to provide a Privacy Manifest](https://developer.apple.com/support/third-party-SDK-requirements).
