<a id="extending-simpleexample"></a>

<a id="first-extension-module"></a>
...
the [recommended third party tools](../c-api/intro.md#c-api-tools).
...
See [The Python Tutorial](../tutorial/index.md#tutorial-index) for an introduction to Python itself.
...
This tutorial uses [pip](https://pip.pypa.io/) (`pip install`), but you
can substitute any tool that can build and install `pyproject.toml`-based
projects, like [uv](https://docs.astral.sh/uv/) (`uv pip install`).
Preferably, have a [virtual environment](../library/venv.md#venv-def) activated.
...
Let’s create an extension module called `spam` <sup>[1](#why-spam)</sup>,
...
In production, use [`os.system()`](../library/os.md#os.system) or [`subprocess.run()`](../library/subprocess.md#subprocess.run)
...
Be sure to put `stdlib.h`, and any other standard library includes,
*after* `Python.h`.
...
In this tutorial, we’ll use [meson-python](https://mesonbuild.com/meson-python/).
(If you want to use another one, see [Appendix: Other build tools](#first-extension-other-tools).)
...
#### NOTE
See [meson-python documentation](meson-python) for details on
configuration.
...
[module export hook](../c-api/extension-modules.md#extension-export-hook).
Let’s define one.
...
The [`PyMODEXPORT_FUNC`](../c-api/extension-modules.md#c.PyMODEXPORT_FUNC) macro declares the function’s
...
[`PyModuleDef_Slot`](../c-api/module.md#c.PyModuleDef_Slot) entries, which are essentially key-value pairs.
...
The `PyABIInfo_VAR(abi_info);` macro and the [`Py_mod_abi`](../c-api/module.md#c.Py_mod_abi) slot
...
For both [`Py_mod_name`](../c-api/module.md#c.Py_mod_name) and [`Py_mod_doc`](../c-api/module.md#c.Py_mod_doc), the values are C
...
One of the simplest ways to write glue code is a “[`METH_O`](../c-api/structures.md#c.METH_O)” function,
which takes two Python objects and returns one.
All Python objects – regardless of the Python type – are represented in C
as pointers to the [`PyObject`](../c-api/structures.md#c.PyObject) structure.
...
For now, we ignore the arguments, and use the [`Py_RETURN_NONE`](../c-api/none.md#c.Py_RETURN_NONE)
macro, which expands to a `return` statement that properly returns
a Python [`None`](../library/constants.md#None) object.
...
[`PyMethodDef`](../c-api/structures.md#c.PyMethodDef) <sup>[4](#why-pymethoddef)</sup>:
...
* `ml_flags`: a set of flags describing details like how Python arguments are
  ...
  We’ll use [`METH_O`](../c-api/structures.md#c.METH_O) here – the flag that matches our
...
Add a [`Py_mod_methods`](../c-api/module.md#c.Py_mod_methods) slot to your [`PyMethodDef`](../c-api/structures.md#c.PyMethodDef) array:
...
the [`METH_O`](../c-api/structures.md#c.METH_O) flag:
...
a Python [`int`](../library/functions.md#int) object.
Eventually this will be the exit code of a system command,
but let’s start with a fixed value, say, `3`.

The Python C API provides a function to create a Python [`int`](../library/functions.md#int) object
from a C `int` value: [`PyLong_FromLong()`](../c-api/long.md#c.PyLong_FromLong). <sup>[5](#why-pylongfromlong)</sup>
...
There’s a slight type mismatch here: Python’s [`str`](../library/stdtypes.md#str) objects store
...
[`str.encode()`](../library/stdtypes.md#str.encode) uses by default,
and the C API has special support for it.)

The function to encode a Python string into a UTF-8 buffer is named
[`PyUnicode_AsUTF8AndSize()`](../c-api/unicode.md#c.PyUnicode_AsUTF8AndSize) <sup>[6](#why-pyunicodeasutf8)</sup>.
Call it like this:
...
If [`PyUnicode_AsUTF8AndSize()`](../c-api/unicode.md#c.PyUnicode_AsUTF8AndSize) is successful, *command* will point to the
resulting C string – a zero-terminated array of bytes <sup>[7](#embedded-nul)</sup>.
...
If [`PyUnicode_AsUTF8AndSize()`](../c-api/unicode.md#c.PyUnicode_AsUTF8AndSize) was *not* successful, it returns a `NULL`
pointer.
...
[`PyLong_FromLong()`](../c-api/long.md#c.PyLong_FromLong) correctly.

For the [`PyUnicode_AsUTF8AndSize()`](../c-api/unicode.md#c.PyUnicode_AsUTF8AndSize) call, the correct way to handle
...
Now, all that is left is calling the C library function `system()` with
...
<a id="extending-spammodule-source"></a>
```c
/// Includes
...
<a id="first-extension-other-tools"></a>
...
The Python Packaging User Guide has a [list of recommended tools](https://packaging.python.org/en/latest/guides/tool-recommendations/#build-backends-for-extension-modules);
...
This is a shim for an old-style [initialization function](../c-api/extension-modules.md#extension-export-hook),
...
on [`sys.path`](../library/sys.md#sys.path).

### Footnotes

* <a id='why-spam'>**[1]**</a> `spam` is the favorite food of Monty Python fans…
* <a id='why-spammodule'>**[2]**</a> The source file name is entirely up to you, though some tools can be picky about the `.c` extension. This tutorial uses the traditional `*module.c` suffix. Some people would just use `spam.c` to implement a module named `spam`, projects where Python isn’t the primary language might use `py_spam.c`, and so on.
* <a id='stdlib-h'>**[3]**</a> Including `stdlib.h` is technically not necessary, since `Python.h` includes it and [several other standard headers](../c-api/intro.md#capi-system-includes) for its own use or for backwards compatibility. However, it is good practice to explicitly include what you need.
* <a id='why-pymethoddef'>**[4]**</a> The `PyMethodDef` structure is also used to create methods of classes, so there’s no separate “`PyFunctionDef`”.
* <a id='why-pylongfromlong'>**[5]**</a> The name [`PyLong_FromLong()`](../c-api/long.md#c.PyLong_FromLong) might not seem obvious. `PyLong` refers to a the Python [`int`](../library/functions.md#int), which was originally called `long`; the `FromLong` refers to the C `long` (or `long int`) type.
* <a id='why-pyunicodeasutf8'>**[6]**</a> Here, `PyUnicode` refers to the original name of the Python [`str`](../library/stdtypes.md#str) class: `unicode`.  The `AndSize` part of the name refers to the fact that this function can also retrieve the size of the buffer, using an output argument. We don’t need this, so we set the second argument to NULL.
* <a id='embedded-nul'>**[7]**</a> We’re ignoring the fact that Python strings can also contain NUL bytes, which terminate a C string. In other words, our function will treat `spam.system("foo\0bar")` as `spam.system("foo")`. This possibility can lead to security issues, so the real `os.system` function size checks for this case and raises an error.
