<a id="extending-intro"></a>

# Using the C API: Assorted topics

The [tutorial](first-extension-module.md#first-extension-module) walked you through
...
of the Python tuple returned by [`sys.exc_info()`](../library/sys.md#sys.exc_info).  These are the
...
The most common one is [`PyErr_SetString()`](../c-api/exceptions.md#c.PyErr_SetString).  Its arguments are an exception
object and a C string.  The exception object is usually a predefined object like
[`PyExc_ZeroDivisionError`](../c-api/exceptions.md#c.PyExc_ZeroDivisionError).  The C string indicates the cause of the error
and is converted to a Python string object and stored as the “associated value”
of the exception.

Another useful function is [`PyErr_SetFromErrno()`](../c-api/exceptions.md#c.PyErr_SetFromErrno), which only takes an
exception argument and constructs the associated value by inspection of the
global variable `errno`.  The most general function is
[`PyErr_SetObject()`](../c-api/exceptions.md#c.PyErr_SetObject), which takes two object arguments, the exception and
its associated value.  You don’t need to [`Py_INCREF()`](../c-api/refcounting.md#c.Py_INCREF) the objects passed
to any of these functions.

You can test non-destructively whether an exception has been set with
[`PyErr_Occurred()`](../c-api/exceptions.md#c.PyErr_Occurred).  This returns the current exception object, or `NULL`
if no exception has occurred.  You normally don’t need to call
[`PyErr_Occurred()`](../c-api/exceptions.md#c.PyErr_Occurred) to see whether an error occurred in a function call,
...
condition must be cleared explicitly by calling [`PyErr_Clear()`](../c-api/exceptions.md#c.PyErr_Clear).  The only
time C code should call [`PyErr_Clear()`](../c-api/exceptions.md#c.PyErr_Clear) is if it doesn’t want to pass the
...
[`PyErr_NoMemory()`](../c-api/exceptions.md#c.PyErr_NoMemory) and return a failure indicator itself.  All the
object-creating functions (for example, [`PyLong_FromLong()`](../c-api/long.md#c.PyLong_FromLong)) already do
this, so this note is only relevant to those who call `malloc()` directly.

Also note that, with the important exception of [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple) and
...
Finally, be careful to clean up garbage (by making [`Py_XDECREF()`](../c-api/refcounting.md#c.Py_XDECREF) or
[`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF) calls for objects you have already created) when you return
an error indicator!

The choice of which exception to raise is entirely yours.  There are predeclared
C objects corresponding to all built-in Python exceptions, such as
[`PyExc_ZeroDivisionError`](../c-api/exceptions.md#c.PyExc_ZeroDivisionError), which you can use directly. Of course, you
should choose exceptions wisely — don’t use [`PyExc_TypeError`](../c-api/exceptions.md#c.PyExc_TypeError) to mean
that a file couldn’t be opened (that should probably be [`PyExc_OSError`](../c-api/exceptions.md#c.PyExc_OSError)).
If something’s wrong with the argument list, the [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple)
function usually raises [`PyExc_TypeError`](../c-api/exceptions.md#c.PyExc_TypeError).  If you have an argument whose
value must be in a particular range or must satisfy other conditions,
[`PyExc_ValueError`](../c-api/exceptions.md#c.PyExc_ValueError) is appropriate.
...
and initialize it by calling [`PyErr_NewException()`](../c-api/exceptions.md#c.PyErr_NewException) in the module’s
[`Py_mod_exec`](../c-api/module.md#c.Py_mod_exec) function (`spam_module_exec()`):
...
the module is reinitialized, when the [`Py_mod_exec`](../c-api/module.md#c.Py_mod_exec) function is called.
...
[`PyErr_NewException()`](../c-api/exceptions.md#c.PyErr_NewException) function may create a class with the base class
being [`Exception`](../library/exceptions.md#Exception) (unless another class is passed in instead of `NULL`),
described in [Built-in Exceptions](../library/exceptions.md#bltin-exceptions).
...
For now, the [`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF) call to remove this reference is missing.
...
We discuss the use of [`PyMODINIT_FUNC`](../c-api/extension-modules.md#c.PyMODINIT_FUNC) as a function return type later in this
sample.

The `spam.error` exception can be raised in your extension module using a
call to [`PyErr_SetString()`](../c-api/exceptions.md#c.PyErr_SetString) as shown below:
...
look at the implementation of the [`-c`](../using/cmdline.md#cmdoption-c) command line option in
...
Python function object (be careful to [`Py_INCREF()`](../c-api/refcounting.md#c.Py_INCREF) it!) in a global
...
[`METH_VARARGS`](../c-api/structures.md#c.METH_VARARGS) flag in [`PyMethodDef.ml_flags`](../c-api/structures.md#c.PyMethodDef.ml_flags).  The
[`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple) function and its arguments are documented in section
[Extracting Parameters in Extension Functions](#parsetuple).

The macros [`Py_XINCREF()`](../c-api/refcounting.md#c.Py_XINCREF) and [`Py_XDECREF()`](../c-api/refcounting.md#c.Py_XDECREF) increment/decrement the
...
[`PyObject_CallObject()`](../c-api/call.md#c.PyObject_CallObject).  This function has two arguments, both pointers to
arbitrary Python objects: the Python function, and the argument list.  The
argument list must always be a tuple object, whose length is the number of
arguments.  To call the Python function with no arguments, pass in `NULL`, or
an empty tuple; to call it with one argument, pass a singleton tuple.
[`Py_BuildValue()`](../c-api/arg.md#c.Py_BuildValue) returns a tuple when its format string consists of zero
...
[`PyObject_CallObject()`](../c-api/call.md#c.PyObject_CallObject) returns a Python object pointer: this is the return
value of the Python function.  [`PyObject_CallObject()`](../c-api/call.md#c.PyObject_CallObject) is
“reference-count-neutral” with respect to its arguments.  In the example a new
tuple was created to serve as the argument list, which is
[`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF)-ed immediately after the [`PyObject_CallObject()`](../c-api/call.md#c.PyObject_CallObject)
call.

The return value of [`PyObject_CallObject()`](../c-api/call.md#c.PyObject_CallObject) is “new”: either it is a brand
new object, or it is an existing object whose reference count has been
incremented.  So, unless you want to save it in a global variable, you should
somehow [`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF) the result, even (especially!) if you are not
interested in its value.

Before you do this, however, it is important to check that the return value
isn’t `NULL`.  If it is, the Python function terminated by raising an exception.
If the C code that called [`PyObject_CallObject()`](../c-api/call.md#c.PyObject_CallObject) is called from Python, it
should now return an error indication to its Python caller, so the interpreter
can print a stack trace, or the calling Python code can handle the exception.
If this is not possible or desirable, the exception should be cleared by calling
[`PyErr_Clear()`](../c-api/exceptions.md#c.PyErr_Clear).  For example:
...
have to provide an argument list to [`PyObject_CallObject()`](../c-api/call.md#c.PyObject_CallObject).  In some cases
...
is to call [`Py_BuildValue()`](../c-api/arg.md#c.Py_BuildValue).  For example, if you want to pass an integral
...
[`Py_BuildValue()`](../c-api/arg.md#c.Py_BuildValue) may run out of memory, and this should be checked.

You may also call a function with keyword arguments by using
[`PyObject_Call()`](../c-api/call.md#c.PyObject_Call), which supports arguments and keyword arguments.  As in
the above example, we use [`Py_BuildValue()`](../c-api/arg.md#c.Py_BuildValue) to construct the dictionary.
...
The [tutorial](first-extension-module.md#first-extension-module) uses a “[`METH_O`](../c-api/structures.md#c.METH_O)”
function, which is limited to a single Python argument.
If you want more, you can use [`METH_VARARGS`](../c-api/structures.md#c.METH_VARARGS) instead.
With this flag, the C function will receive a [`tuple`](../library/stdtypes.md#tuple) of arguments
instead of a single object.

For unpacking the tuple, CPython provides the [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple)
...
whose syntax is explained in [Parsing arguments and building values](../c-api/arg.md#arg-parsing) in the Python/C API Reference
Manual.  The remaining arguments must be addresses of variables whose type is
determined by the format string.

For example, to receive a single Python [`str`](../library/stdtypes.md#str) object and turn it
...
[`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple).

Note that while [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple) checks that the Python arguments have
...
<a id="parsetupleandkeywords"></a>

## Keyword Parameters for Extension Functions

If you also want your function to accept
[keyword arguments](../glossary.md#term-keyword-argument), use the [`METH_KEYWORDS`](../c-api/structures.md#c.METH_KEYWORDS)
flag in combination with [`METH_VARARGS`](../c-api/structures.md#c.METH_VARARGS).
...
Use [`PyArg_ParseTupleAndKeywords()`](../c-api/arg.md#c.PyArg_ParseTupleAndKeywords) to parse the arguments to such a
function.

<a id="index-2"></a>

The [`PyArg_ParseTupleAndKeywords()`](../c-api/arg.md#c.PyArg_ParseTupleAndKeywords) function is declared as follows:
...
[`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple) function.  The *kwdict* parameter is the dictionary of
...
right.  On success, [`PyArg_ParseTupleAndKeywords()`](../c-api/arg.md#c.PyArg_ParseTupleAndKeywords) returns true, otherwise
...
passed in which are not present in the *kwlist* will cause [`TypeError`](../library/exceptions.md#TypeError) to
be raised.

<a id="index-3"></a>

Here is an example module which uses keywords, based on an example by Geoff
Philbrick ([philbrick@hks.com](mailto:philbrick@hks.com)):
...
This function is the counterpart to [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple).  It is declared
as follows:
...
[`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple), but the arguments (which are input to the function,
...
One difference with [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple): while the latter requires its
first argument to be a tuple (since Python argument lists are always represented
as tuples internally), [`Py_BuildValue()`](../c-api/arg.md#c.Py_BuildValue) does not always build a tuple.  It
...
The [`gc`](../library/gc.md#module-gc) module exposes a way to run the detector (the
[`collect()`](../library/gc.md#gc.collect) function), as well as configuration
interfaces and the ability to disable the detector at runtime.

<a id="refcountsinpython"></a>

### Reference Counting in Python

There are two macros, `Py_INCREF(x)` and `Py_DECREF(x)`, which handle the
incrementing and decrementing of the reference count. [`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF) also
...
responsible for calling [`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF) when the reference is no longer
needed.  Ownership of a reference can be transferred.  There are three ways to
dispose of an owned reference: pass it on, store it, or call [`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF).
Forgetting to dispose of an owned reference creates a memory leak.

It is also possible to *borrow* <sup>[1](#borrow)</sup> a reference to an object.  The
borrower of a reference should not call [`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF).  The borrower must
not hold on to the object longer than the owner from which it was borrowed.
Using a borrowed reference after the owner has disposed of it risks using freed
memory and should be avoided completely <sup>[2](#dont-check-refcount)</sup>.
...
[`Py_INCREF()`](../c-api/refcounting.md#c.Py_INCREF).  This does not affect the status of the owner from which the
...
object, such as [`PyLong_FromLong()`](../c-api/long.md#c.PyLong_FromLong) and [`Py_BuildValue()`](../c-api/arg.md#c.Py_BuildValue), pass
ownership to the receiver.  Even if the object is not actually new, you still
receive ownership of a new reference to that object.  For instance,
[`PyLong_FromLong()`](../c-api/long.md#c.PyLong_FromLong) maintains a cache of popular values and can return a
reference to a cached item.

Many functions that extract objects from other objects also transfer ownership
with the reference, for instance [`PyObject_GetAttrString()`](../c-api/object.md#c.PyObject_GetAttrString).  The picture
is less clear, here, however, since a few common routines are exceptions:
[`PyTuple_GetItem()`](../c-api/tuple.md#c.PyTuple_GetItem), [`PyList_GetItem()`](../c-api/list.md#c.PyList_GetItem), [`PyDict_GetItem()`](../c-api/dict.md#c.PyDict_GetItem), and
[`PyDict_GetItemString()`](../c-api/dict.md#c.PyDict_GetItemString) all return references that you borrow from the
tuple, list or dictionary.

The function [`PyImport_AddModule()`](../c-api/import.md#c.PyImport_AddModule) also returns a borrowed reference, even
though it may actually create the object it returns: this is possible because an
owned reference to the object is stored in `sys.modules`.

When you pass an object reference into another function, in general, the
function borrows the reference from you — if it needs to store it, it will use
[`Py_INCREF()`](../c-api/refcounting.md#c.Py_INCREF) to become an independent owner.  There are exactly two
important exceptions to this rule: [`PyTuple_SetItem()`](../c-api/tuple.md#c.PyTuple_SetItem) and
[`PyList_SetItem()`](../c-api/list.md#c.PyList_SetItem).  These functions take over ownership of the item passed
to them — even if they fail!  (Note that [`PyDict_SetItem()`](../c-api/dict.md#c.PyDict_SetItem) and friends
...
reference by calling [`Py_INCREF()`](../c-api/refcounting.md#c.Py_INCREF).
...
The first and most important case to know about is using [`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF) on
...
Let’s follow the control flow into [`PyList_SetItem()`](../c-api/list.md#c.PyList_SetItem).  The list owns
...
[`PyList_SetItem()`](../c-api/list.md#c.PyList_SetItem) calls [`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF) on the replaced item,
which invokes replaced item’s corresponding
[`tp_dealloc`](../c-api/typeobj.md#c.PyTypeObject.tp_dealloc) function. During
deallocation, [`tp_dealloc`](../c-api/typeobj.md#c.PyTypeObject.tp_dealloc) calls
[`tp_finalize`](../c-api/typeobj.md#c.PyTypeObject.tp_finalize), which is mapped to the
`__del__()` method for class instances (see [**PEP 442**](https://peps.python.org/pep-0442/)). This entire
sequence happens synchronously within the [`PyList_SetItem()`](../c-api/list.md#c.PyList_SetItem) call.
...
other’s way, because there is a [global lock](../glossary.md#term-global-interpreter-lock)
protecting Python’s entire object space.
However, it is possible to temporarily release this lock using the macro
[`Py_BEGIN_ALLOW_THREADS`](../c-api/threads.md#c.Py_BEGIN_ALLOW_THREADS), and to re-acquire it using
[`Py_END_ALLOW_THREADS`](../c-api/threads.md#c.Py_END_ALLOW_THREADS).  This is common around blocking I/O calls, to
...
The macros [`Py_INCREF()`](../c-api/refcounting.md#c.Py_INCREF) and [`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF) do not check for `NULL`
pointers — however, their variants [`Py_XINCREF()`](../c-api/refcounting.md#c.Py_XINCREF) and [`Py_XDECREF()`](../c-api/refcounting.md#c.Py_XDECREF)
do.
...
that it is always a tuple <sup>[3](#old-calling-convention)</sup>.
...
The function [`PyCapsule_New()`](../c-api/capsule.md#c.PyCapsule_New) takes a name parameter
...
The convenience function [`PyCapsule_Import()`](../c-api/capsule.md#c.PyCapsule_Import) makes it easy to
...
[tutorial](first-extension-module.md#first-extension-module).
...
exporting module, not a client module. Finally, the module’s [`mod_exec`](../c-api/module.md#c.Py_mod_exec) function must take care of initializing the C API pointer array:
...
`import_spam()` in its [`mod_exec`](../c-api/module.md#c.Py_mod_exec) function:
...
Manual in the section [Capsules](../c-api/capsule.md#capsules) and in the implementation of Capsules (files
`Include/pycapsule.h` and `Objects/pycapsule.c` in the Python source
code distribution).

### Footnotes

* <a id='borrow'>**[1]**</a> The metaphor of “borrowing” a reference is not completely correct: the owner still has a copy of the reference.
* <a id='dont-check-refcount'>**[2]**</a> Checking that the reference count is at least 1 **does not work** — the reference count itself could be in freed memory and may thus be reused for another object!
* <a id='old-calling-convention'>**[3]**</a> These guarantees don’t hold when you use the “old” style calling convention — this is still found in much existing code.
