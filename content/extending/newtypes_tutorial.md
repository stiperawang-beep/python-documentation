<a id="defining-new-types"></a>

# Defining Extension Types: Tutorial

Python allows the writer of a C extension module to define new types that
can be manipulated from Python code, much like the built-in [`str`](../library/stdtypes.md#str)
and [`list`](../library/stdtypes.md#list) types.  The code for all extension types follows a
...
The [CPython](../glossary.md#term-CPython) runtime sees all Python objects as variables of type
, which serves as a “base type” for all Python objects.
The [`PyObject`](../c-api/structures.md#c.PyObject) structure itself only contains the object’s
[reference count](../glossary.md#term-reference-count) and a pointer to the object’s “type object”.
...
[`PyType_FromSpec()`](../c-api/type.md#c.PyType_FromSpec) function, which isn’t covered in this tutorial.
...
of type [`PyObject`](../c-api/structures.md#c.PyObject), containing a pointer to a type object and a
reference count (these can be accessed using the macros [`Py_TYPE`](../c-api/structures.md#c.Py_TYPE)
and [`Py_REFCNT`](../c-api/refcounting.md#c.Py_REFCNT) respectively).  The reason for the macro is to
abstract away the layout and to enable additional fields in [debug builds](../using/configure.md#debug-build).

#### NOTE
There is no semicolon above after the [`PyObject_HEAD`](../c-api/structures.md#c.PyObject_HEAD) macro.
...
avoid listing all the [`PyTypeObject`](../c-api/type.md#c.PyTypeObject) fields that you don’t care
about and also to avoid caring about the fields’ declaration order.

The actual definition of [`PyTypeObject`](../c-api/type.md#c.PyTypeObject) in `object.h` has
many more [fields](../c-api/typeobj.md#type-structs) than the definition above.  The
...
with the [`pydoc`](../library/pydoc.md#module-pydoc) and [`pickle`](../library/pickle.md#module-pickle) modules.
...
new `Custom` instances.  [`tp_itemsize`](../c-api/typeobj.md#c.PyTypeObject.tp_itemsize) is
only used for variable-sized objects and should otherwise be zero.

#### NOTE
If you want your type to be subclassable from Python, and your type has the same
[`tp_basicsize`](../c-api/typeobj.md#c.PyTypeObject.tp_basicsize) as its base type, you may have problems with multiple
inheritance.  A Python subclass of your type will have to list your type first
in its [`__bases__`](../reference/datamodel.md#type.__bases__), or else it will not be able to call your type’s
[`__new__()`](../reference/datamodel.md#object.__new__) method without getting an error.  You can avoid this problem by
ensuring that your type has a larger value for [`tp_basicsize`](../c-api/typeobj.md#c.PyTypeObject.tp_basicsize) than its
base type does.  Most of the time, this will be true anyway, because either your
base type will be [`object`](../library/functions.md#object), or else you will be adding data members to
your base type, and therefore increasing its size.

We set the class flags to [`Py_TPFLAGS_DEFAULT`](../c-api/typeobj.md#c.Py_TPFLAGS_DEFAULT).
...
We provide a doc string for the type in [`tp_doc`](../c-api/typeobj.md#c.PyTypeObject.tp_doc).

```c
.tp_doc = PyDoc_STR("Custom objects"),
```

To enable object creation, we have to provide a [`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new)
handler.  This is the equivalent of the Python method [`__new__()`](../reference/datamodel.md#object.__new__), but
has to be specified explicitly.  In this case, we can just use the default
implementation provided by the API function [`PyType_GenericNew()`](../c-api/type.md#c.PyType_GenericNew).
...
to the appropriate default values, including [`ob_type`](../c-api/structures.md#c.PyObject.ob_type) that we initially
set to `NULL`.
...
which is assigned to the [`tp_dealloc`](../c-api/typeobj.md#c.PyTypeObject.tp_dealloc) member:
...
[`Py_XDECREF()`](../c-api/refcounting.md#c.Py_XDECREF) correctly handles the case where its argument is
`NULL` (which might happen here if `tp_new` failed midway).  It then
calls the [`tp_free`](../c-api/typeobj.md#c.PyTypeObject.tp_free) member of the object’s type
...
and install it in the [`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new) member:
...
objects of the type.  It is exposed in Python as the [`__new__()`](../reference/datamodel.md#object.__new__) method.
It is not required to define a `tp_new` member, and indeed many extension
types will simply reuse [`PyType_GenericNew()`](../c-api/type.md#c.PyType_GenericNew) as done in the first
...
The `tp_new` implementation calls the [`tp_alloc`](../c-api/typeobj.md#c.PyTypeObject.tp_alloc)
...
Since memory allocation may fail, we must check the [`tp_alloc`](../c-api/typeobj.md#c.PyTypeObject.tp_alloc)
result against `NULL` before proceeding.

#### NOTE
We didn’t fill the [`tp_alloc`](../c-api/typeobj.md#c.PyTypeObject.tp_alloc) slot ourselves. Rather
[`PyType_Ready()`](../c-api/type.md#c.PyType_Ready) fills it for us by inheriting it from our base class,
which is [`object`](../library/functions.md#object) by default.  Most types use the default allocation
strategy.

#### NOTE
If you are creating a co-operative [`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new) (one
that calls a base type’s [`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new) or [`__new__()`](../reference/datamodel.md#object.__new__)),
...
call, and call its [`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new) directly, or via
`type->tp_base->tp_new`.  If you do not do this, Python subclasses of your
type that also inherit from other Python-defined classes may not work correctly.
(Specifically, you may not be able to create instances of such subclasses
without getting a [`TypeError`](../library/exceptions.md#TypeError).)
...
by filling the [`tp_init`](../c-api/typeobj.md#c.PyTypeObject.tp_init) slot.

```c
.tp_init = Custom_init,
```

The [`tp_init`](../c-api/typeobj.md#c.PyTypeObject.tp_init) slot is exposed in Python as the
[`__init__()`](../reference/datamodel.md#object.__init__) method.  It is used to initialize an object after it’s
...
is called at all (for example, the [`pickle`](../library/pickle.md#module-pickle) module by default
doesn’t call [`__init__()`](../reference/datamodel.md#object.__init__) on unpickled instances).  It can also be
...
[thread state](../glossary.md#term-attached-thread-state) and let arbitrary code run in other
...
* when we know that deallocation of the object <sup>[1](#id5)</sup> will neither detach
  the [thread state](../glossary.md#term-attached-thread-state) nor cause any calls back into our type’s code;
* when decrementing a reference count in a [`tp_dealloc`](../c-api/typeobj.md#c.PyTypeObject.tp_dealloc)
  handler on a type which doesn’t support cyclic garbage collection <sup>[2](#id6)</sup>.
...
and put the definitions in the [`tp_members`](../c-api/typeobj.md#c.PyTypeObject.tp_members) slot:
...
documentation string.  See the [Generic Attribute Management](newtypes.md#generic-attribute-management) section
below for details.
...
(note that we used the [`METH_NOARGS`](../c-api/structures.md#c.METH_NOARGS) flag to indicate that the method
is expecting no arguments other than *self*)

and assign it to the [`tp_methods`](../c-api/typeobj.md#c.PyTypeObject.tp_methods) slot:
...
to add the [`Py_TPFLAGS_BASETYPE`](../c-api/typeobj.md#c.Py_TPFLAGS_BASETYPE) to our class flag definition:
...
module name in the [`PyModuleDef`](../c-api/module.md#c.PyModuleDef) struct, and update the full class
name in the [`PyTypeObject`](../c-api/type.md#c.PyTypeObject) struct.
...
We create an array of [`PyGetSetDef`](../c-api/structures.md#c.PyGetSetDef) structures:
...
and register it in the [`tp_getset`](../c-api/typeobj.md#c.PyTypeObject.tp_getset) slot:

```c
.tp_getset = Custom_getsetters,
```

The last item in a [`PyGetSetDef`](../c-api/structures.md#c.PyGetSetDef) structure is the “closure” mentioned
...
We also need to update the [`tp_init`](../c-api/typeobj.md#c.PyTypeObject.tp_init) handler to only
allow strings <sup>[3](#id7)</sup> to be passed:
...
This means that most of the [`Py_XDECREF()`](../c-api/refcounting.md#c.Py_XDECREF) calls can be converted to
[`Py_DECREF()`](../c-api/refcounting.md#c.Py_DECREF) calls.  The only place we can’t change these calls is in
...
Python has a [cyclic garbage collector (GC)](../glossary.md#term-garbage-collection) that
...
object to be stored in the `first` or `last` attributes <sup>[4](#id8)</sup>.
...
Python provides a [`Py_VISIT()`](../c-api/gcsupport.md#c.Py_VISIT) macro that automates calling visit
functions.  With [`Py_VISIT()`](../c-api/gcsupport.md#c.Py_VISIT), we can minimize the amount of boilerplate
in `Custom_traverse`:
...
#### NOTE
The [`tp_traverse`](../c-api/typeobj.md#c.PyTypeObject.tp_traverse) implementation must name its
arguments exactly *visit* and *arg* in order to use [`Py_VISIT()`](../c-api/gcsupport.md#c.Py_VISIT).
...
Notice the use of the [`Py_CLEAR()`](../c-api/refcounting.md#c.Py_CLEAR) macro.  It is the recommended and safe
way to clear data attributes of arbitrary types while decrementing
their reference counts.  If you were to call [`Py_XDECREF()`](../c-api/refcounting.md#c.Py_XDECREF) instead
...
#### NOTE
You could emulate [`Py_CLEAR()`](../c-api/refcounting.md#c.Py_CLEAR) by writing:
...
use [`Py_CLEAR()`](../c-api/refcounting.md#c.Py_CLEAR) when deleting an attribute.  Don’t
...
from the GC by calling [`PyObject_GC_UnTrack()`](../c-api/gcsupport.md#c.PyObject_GC_UnTrack) before clearing members.
Here is our reimplemented deallocator using [`PyObject_GC_UnTrack()`](../c-api/gcsupport.md#c.PyObject_GC_UnTrack)
and `Custom_clear`:
...
Finally, we add the [`Py_TPFLAGS_HAVE_GC`](../c-api/typeobj.md#c.Py_TPFLAGS_HAVE_GC) flag to the class flags:

```c
.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE | Py_TPFLAGS_HAVE_GC,
```

That’s pretty much it.  If we had written custom [`tp_alloc`](../c-api/typeobj.md#c.PyTypeObject.tp_alloc) or
[`tp_free`](../c-api/typeobj.md#c.PyTypeObject.tp_free) handlers, we’d need to modify them for cyclic
...
easily use the [`PyTypeObject`](../c-api/type.md#c.PyTypeObject) it needs. It can be difficult to share
these [`PyTypeObject`](../c-api/type.md#c.PyTypeObject) structures between extension modules.

In this example we will create a `SubList` type that inherits from the
built-in [`list`](../library/stdtypes.md#list) type. The new type will be completely compatible with
...
the [`PyObject_HEAD()`](../c-api/structures.md#c.PyObject_HEAD) at the beginning of its structure.
...
We see above how to call through to the [`__init__()`](../reference/datamodel.md#object.__init__) method of the base
type.

This pattern is important when writing a type with custom
[`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new) and [`tp_dealloc`](../c-api/typeobj.md#c.PyTypeObject.tp_dealloc)
members.  The [`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new) handler should not actually
create the memory for the object with its [`tp_alloc`](../c-api/typeobj.md#c.PyTypeObject.tp_alloc),
but let the base class handle it by calling its own [`tp_new`](../c-api/typeobj.md#c.PyTypeObject.tp_new).

The [`PyTypeObject`](../c-api/type.md#c.PyTypeObject) struct supports a [`tp_base`](../c-api/typeobj.md#c.PyTypeObject.tp_base)
...
[`PyList_Type`](../c-api/list.md#c.PyList_Type); it should be done in the [`Py_mod_exec`](../c-api/module.md#c.Py_mod_exec)
function:
...
Before calling [`PyType_Ready()`](../c-api/type.md#c.PyType_Ready), the type structure must have the
[`tp_base`](../c-api/typeobj.md#c.PyTypeObject.tp_base) slot filled in.  When we are deriving an
existing type, it is not necessary to fill out the [`tp_alloc`](../c-api/typeobj.md#c.PyTypeObject.tp_alloc)
slot with [`PyType_GenericNew()`](../c-api/type.md#c.PyType_GenericNew) – the allocation function from the base
type will be inherited.

After that, calling [`PyType_Ready()`](../c-api/type.md#c.PyType_Ready) and adding the type object to the
module is the same as with the basic `Custom` examples.

### Footnotes

* <a id='id5'>**[1]**</a> This is true when we know that the object is a basic type, like a string or a float.
* <a id='id6'>**[2]**</a> We relied on this in the [`tp_dealloc`](../c-api/typeobj.md#c.PyTypeObject.tp_dealloc) handler in this example, because our type doesn’t support garbage collection.
...
* <a id='id8'>**[4]**</a> Also, even with our attributes restricted to strings instances, the user could pass arbitrary [`str`](../library/stdtypes.md#str) subclasses and therefore still create reference cycles.
