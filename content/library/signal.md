...
**Source code:** [Lib/signal.py](https://github.com/python/cpython/tree/main/Lib/signal.py)
...
translated into a [`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt) exception if the parent process
has not changed it.
...
[virtual machine](../glossary.md#term-virtual-machine) to execute the corresponding Python signal handler
at a later point (for example, at the next [bytecode](../glossary.md#term-bytecode) instruction).
...
* It makes little sense to catch synchronous errors like [`SIGFPE`](#signal.SIGFPE) or
  ...
  onwards, you can use the [`faulthandler`](faulthandler.md#module-faulthandler) module to report on synchronous
  errors.
...
the synchronization primitives from the [`threading`](threading.md#module-threading) module instead.

Besides, only the main thread of the main interpreter is allowed to set a new signal handler.

#### WARNING
Synchronization primitives such as [`threading.Lock`](threading.md#threading.Lock) should not be used
...
[`enums`](enum.md#enum.IntEnum) ([`Signals`](#signal.Signals), [`Handlers`](#signal.Handlers) and [`Sigmasks`](#signal.Sigmasks) respectively).
...
[`enums`](enum.md#enum.IntEnum) as [`Signals`](#signal.Signals) objects.

The signal module defines three enums:

### *class* signal.Signals

[`enum.IntEnum`](enum.md#enum.IntEnum) collection of SIG\* constants and the CTRL_\* constants.
...
[`enum.IntEnum`](enum.md#enum.IntEnum) collection of the constants [`SIG_DFL`](#signal.SIG_DFL) and [`SIG_IGN`](#signal.SIG_IGN).
...
[`enum.IntEnum`](enum.md#enum.IntEnum) collection of the constants [`SIG_BLOCK`](#signal.SIG_BLOCK), [`SIG_UNBLOCK`](#signal.SIG_UNBLOCK) and [`SIG_SETMASK`](#signal.SIG_SETMASK).

[Availability](intro.md#availability): Unix.

See the man page  and
...
#### SEE ALSO
[`ZeroDivisionError`](exceptions.md#ZeroDivisionError) is raised when the second argument of a division
...
Default action is to raise [`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt).
...
, on others the list is in ). Note that
...
only be used with [`os.kill()`](os.md#os.kill).
...
This error is a subtype of [`OSError`](exceptions.md#OSError).

#### Versionadded
Added in version 3.3: This error used to be a subtype of [`IOError`](exceptions.md#IOError), which is now an
...
[`signal.SIG_IGN`](#signal.SIG_IGN), [`signal.SIG_DFL`](#signal.SIG_DFL) or [`None`](constants.md#None).  Here,
...
for [`SIGINT`](#signal.SIGINT). Returns [`None`](constants.md#None) if *signalnum* has no
description. Raises [`ValueError`](exceptions.md#ValueError) if *signalnum* is invalid.
...
[Availability](intro.md#availability): Linux >= 5.1, Android >= [`build-time`](sys.md#sys.getandroidapilevel) API level 31
...
to fail with [`InterruptedError`](exceptions.md#InterruptedError).
...
Raises an [auditing event](sys.md#auditing) `signal.pthread_kill` with arguments `thread_id`, `signalnum`.

[Availability](intro.md#availability): Unix.

See the man page  for further  information.

See also [`os.kill()`](os.md#os.kill).
...
#### Versionchanged
Changed in version 3.15: Accepts any real numbers as *seconds* and *interval*, not only integers
or floats.
...
attempting to call it from other threads will cause a [`ValueError`](exceptions.md#ValueError)
...
Change system call restart behaviour: if *flag* is [`False`](constants.md#False), system
...
above).  (See the Unix man page  for further information.)
...
see the [description in the type hierarchy](../reference/datamodel.md#frame-objects) or see the
attribute descriptions in the [`inspect`](inspect.md#module-inspect) module).
...
A [`ValueError`](exceptions.md#ValueError) will be raised in any other case.
Note that not all systems define the same set of signal names; an
[`AttributeError`](exceptions.md#AttributeError) will be raised if a signal name is not defined as
...
[`InterruptedError`](exceptions.md#InterruptedError) if it is interrupted by a signal that is not in
*sigset*.
...
performed. Returns [`None`](constants.md#None) if a timeout occurs.
...
[`os.open()`](os.md#os.open) to hang indefinitely.  The solution is to set a 5-second alarm
...
Piping output of your program to tools like  will
...
Do not set [`SIGPIPE`](#signal.SIGPIPE)’s disposition to [`SIG_DFL`](#signal.SIG_DFL) in
order to avoid [`BrokenPipeError`](exceptions.md#BrokenPipeError).  Doing that would cause
your program to exit unexpectedly whenever any socket
connection is interrupted while your program is still writing to
it.

<a id="handlers-and-exceptions"></a>

## Note on Signal Handlers and Exceptions

If a signal handler raises an exception, the exception will be propagated to
the main thread and may be raised after any [bytecode](../glossary.md#term-bytecode) instruction. Most
notably, a [`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt) may appear at any point during execution.
Most Python code, including the standard library, cannot be made robust against
this, and so a [`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt) (or any other exception resulting from
...
[`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt), this is not a problem, but applications that are
complex or require high reliability should avoid raising exceptions from signal
handlers. They should also avoid catching [`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt) as a means
