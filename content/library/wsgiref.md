...
**Source code:** [Lib/wsgiref](https://github.com/python/cpython/tree/main/Lib/wsgiref)
...
[`WSGIEnvironment`](#wsgiref.types.WSGIEnvironment) for a type alias that can be used
in type annotations.
...
Similar to [`request_uri()`](#wsgiref.util.request_uri), except that the `PATH_INFO` and
...
WSGI application at `/foo` calls [`shift_path_info()`](#wsgiref.util.shift_path_info), it will receive the
...
Example usage (see also [`demo_app()`](#wsgiref.simple_server.demo_app)
for another example):
...
A concrete implementation of the [`wsgiref.types.FileWrapper`](#wsgiref.types.FileWrapper)
protocol used to convert a file-like object to an [iterator](../glossary.md#term-iterator).
The resulting objects
are [iterable](../glossary.md#term-iterable)s. As the object is iterated over, the
...
This module provides a single class, [`Headers`](#wsgiref.headers.Headers), for convenient
manipulation of WSGI response headers using a mapping-like interface.

### *class* wsgiref.headers.Headers()

Create a mapping-like object wrapping *headers*, which must be a list of header
name/value tuples as described in [**PEP 3333**](https://peps.python.org/pep-3333/). The default value of *headers* is
an empty list.

[`Headers`](#wsgiref.headers.Headers) objects support typical mapping operations including
[`__getitem__()`](../reference/datamodel.md#object.__getitem__), [`get()`](stdtypes.md#dict.get), [`__setitem__()`](../reference/datamodel.md#object.__setitem__),
[`setdefault()`](stdtypes.md#dict.setdefault),
[`__delitem__()`](../reference/datamodel.md#object.__delitem__) and [`__contains__()`](../reference/datamodel.md#object.__contains__).  For each of
...
Unlike a dictionary, [`Headers`](#wsgiref.headers.Headers) objects do not raise an error when you try
...
[`Headers`](#wsgiref.headers.Headers) objects also support `keys()`, `values()`, and
...
`len()` of a [`Headers`](#wsgiref.headers.Headers) object is the same as the length of its
...
Calling `bytes()` on a [`Headers`](#wsgiref.headers.Headers) object returns a formatted bytestring
...
In addition to their mapping interface and formatting features, [`Headers`](#wsgiref.headers.Headers)
...
This module implements a simple HTTP server (based on [`http.server`](http.server.md#module-http.server))
...
[`wsgiref.util`](#module-wsgiref.util).)
...
The *start_response* callable should follow the [`StartResponse`](#wsgiref.types.StartResponse) protocol.

### *class* wsgiref.simple_server.WSGIServer(server_address, RequestHandlerClass)

Create a [`WSGIServer`](#wsgiref.simple_server.WSGIServer) instance.  *server_address* should be a
`(host,port)` tuple, and *RequestHandlerClass* should be the subclass of
[`http.server.BaseHTTPRequestHandler`](http.server.md#http.server.BaseHTTPRequestHandler) that will be used to process
requests.

You do not normally need to call this constructor, as the [`make_server()`](#wsgiref.simple_server.make_server)
function can handle all the details for you.

[`WSGIServer`](#wsgiref.simple_server.WSGIServer) is a subclass of [`http.server.HTTPServer`](http.server.md#http.server.HTTPServer), so all
of its methods (such as `serve_forever()` and `handle_request()`) are
available. [`WSGIServer`](#wsgiref.simple_server.WSGIServer) also provides these WSGI-specific methods:
...
[`set_app()`](#wsgiref.simple_server.WSGIServer.set_app) is normally called by [`make_server()`](#wsgiref.simple_server.make_server), and the
[`get_app()`](#wsgiref.simple_server.WSGIServer.get_app) exists mainly for the benefit of request handler instances.

### *class* wsgiref.simple_server.WSGIRequestHandler(request, client_address, server)

Create an HTTP handler for the given *request* (i.e. a socket), *client_address*
(a `(host,port)` tuple), and *server* ([`WSGIServer`](#wsgiref.simple_server.WSGIServer) instance).

You do not need to create instances of this class directly; they are
automatically created as needed by [`WSGIServer`](#wsgiref.simple_server.WSGIServer) objects.  You can,
however, subclass this class and supply it as a *handler_class* to the
[`make_server()`](#wsgiref.simple_server.make_server) function.  Some possibly relevant methods for overriding in
subclasses:

#### get_environ()

Return a [`WSGIEnvironment`](#wsgiref.types.WSGIEnvironment) dictionary for a
request.  The default
implementation copies the contents of the [`WSGIServer`](#wsgiref.simple_server.WSGIServer) object’s
...
using a [`wsgiref.handlers`](#module-wsgiref.handlers) class to implement the actual WSGI application
interface.
...
Any detected nonconformance results in an [`AssertionError`](exceptions.md#AssertionError) being raised;
note, however, that how these errors are handled is server-dependent.  For
example, [`wsgiref.simple_server`](#module-wsgiref.simple_server) and other servers based on
[`wsgiref.handlers`](#module-wsgiref.handlers) (that don’t override the error handling methods to do
...
This wrapper may also generate output using the [`warnings`](warnings.md#module-warnings) module to
indicate behaviors that are questionable but which may not actually be
prohibited by [**PEP 3333**](https://peps.python.org/pep-3333/).  Unless they are suppressed using Python command-line
options or the [`warnings`](warnings.md#module-warnings) API, any such warnings will be written to
...
This class is a subclass of [`BaseCGIHandler`](#wsgiref.handlers.BaseCGIHandler) that sets `wsgi.run_once`
to true, `wsgi.multithread` to false, and `wsgi.multiprocess` to true, and
always uses [`sys`](sys.md#module-sys) and [`os`](os.md#module-os) to obtain the necessary CGI streams and
environment.

### *class* wsgiref.handlers.IISCGIHandler

A specialized alternative to [`CGIHandler`](#wsgiref.handlers.CGIHandler), for use when deploying on
...
[`CGIHandler`](#wsgiref.handlers.CGIHandler), i.e., by calling `IISCGIHandler().run(app)`, where
...
Similar to [`CGIHandler`](#wsgiref.handlers.CGIHandler), but instead of using the [`sys`](sys.md#module-sys) and
[`os`](os.md#module-os) modules, the CGI environment and I/O streams are specified explicitly.
...
This class is a subclass of [`SimpleHandler`](#wsgiref.handlers.SimpleHandler) intended for use with
...
instead of [`SimpleHandler`](#wsgiref.handlers.SimpleHandler).

### *class* wsgiref.handlers.SimpleHandler(stdin, stdout, stderr, environ, multithread=True, multiprocess=False)

Similar to [`BaseCGIHandler`](#wsgiref.handlers.BaseCGIHandler), but designed for use with HTTP origin
servers.  If you are writing an HTTP server implementation, you will probably
want to subclass this instead of [`BaseCGIHandler`](#wsgiref.handlers.BaseCGIHandler).

This class is a subclass of [`BaseHandler`](#wsgiref.handlers.BaseHandler).  It overrides the
`__init__()`, [`get_stdin()`](#wsgiref.handlers.BaseHandler.get_stdin),
[`get_stderr()`](#wsgiref.handlers.BaseHandler.get_stderr), [`add_cgi_vars()`](#wsgiref.handlers.BaseHandler.add_cgi_vars),
[`_write()`](#wsgiref.handlers.BaseHandler._write), and [`_flush()`](#wsgiref.handlers.BaseHandler._flush) methods to
support explicitly setting the
environment and streams via the constructor.  The supplied environment and
streams are stored in the `stdin`, `stdout`, `stderr`, and
`environ` attributes.

The [`write()`](io.md#io.BufferedIOBase.write) method of *stdout* should write
each chunk in full, like [`io.BufferedIOBase`](io.md#io.BufferedIOBase).
...
[`BaseHandler`](#wsgiref.handlers.BaseHandler) instances have only one method intended for external use:

#### run(app)

Run the specified WSGI application, *app*.

All of the other [`BaseHandler`](#wsgiref.handlers.BaseHandler) methods are invoked by this method in the
...
method actually transmits the data; [`BaseHandler`](#wsgiref.handlers.BaseHandler) just separates write
...
is a no-op (i.e., if [`_write()`](#wsgiref.handlers.BaseHandler._write) actually sends the data).

#### get_stdin()

Return an object compatible with [`InputStream`](#wsgiref.types.InputStream)
suitable for use as the `wsgi.input` of the
request currently being processed.

#### get_stderr()

Return an object compatible with [`ErrorStream`](#wsgiref.types.ErrorStream)
...
information before attempting to create a customized [`BaseHandler`](#wsgiref.handlers.BaseHandler)
subclass.
...
defaults to true in [`BaseHandler`](#wsgiref.handlers.BaseHandler), but may have a different default (or
...
defaults to false in [`BaseHandler`](#wsgiref.handlers.BaseHandler), but [`CGIHandler`](#wsgiref.handlers.CGIHandler) sets it to
true by default.
...
If the [`origin_server`](#wsgiref.handlers.BaseHandler.origin_server) attribute is set, this attribute’s value is used to
...
as [`BaseCGIHandler`](#wsgiref.handlers.BaseCGIHandler) and [`CGIHandler`](#wsgiref.handlers.CGIHandler)) that are not HTTP origin
servers.
...
implementation uses the `guess_scheme()` function from [`wsgiref.util`](#module-wsgiref.util)
...
[`get_stdin()`](#wsgiref.handlers.BaseHandler.get_stdin), [`get_stderr()`](#wsgiref.handlers.BaseHandler.get_stderr), and [`add_cgi_vars()`](#wsgiref.handlers.BaseHandler.add_cgi_vars) methods and the
[`wsgi_file_wrapper`](#wsgiref.handlers.BaseHandler.wsgi_file_wrapper) attribute.  It also inserts a `SERVER_SOFTWARE` key
if not present, as long as the [`origin_server`](#wsgiref.handlers.BaseHandler.origin_server) attribute is a true value
and the [`server_software`](#wsgiref.handlers.BaseHandler.server_software) attribute is set.
...
[`log_exception()`](#wsgiref.handlers.BaseHandler.log_exception) method.  If `None`, all frames are included.
...
the *start_response* callable should follow the [`StartResponse`](#wsgiref.types.StartResponse)
protocol.

The default implementation just uses the [`error_status`](#wsgiref.handlers.BaseHandler.error_status),
[`error_headers`](#wsgiref.handlers.BaseHandler.error_headers), and [`error_body`](#wsgiref.handlers.BaseHandler.error_body) attributes to generate an output
...
[`wsgiref.types.FileWrapper`](#wsgiref.types.FileWrapper), or `None`.  The default value
of this attribute is the [`wsgiref.util.FileWrapper`](#wsgiref.util.FileWrapper) class.

#### sendfile()

Override to implement platform-specific file transmission.  This method is
called only if the application’s return value is an instance of the class
specified by the [`wsgi_file_wrapper`](#wsgiref.handlers.BaseHandler.wsgi_file_wrapper) attribute.  It should return a true
...
This attribute should be set to a true value if the handler’s [`_write()`](#wsgiref.handlers.BaseHandler._write) and
[`_flush()`](#wsgiref.handlers.BaseHandler._flush) are being used to communicate directly to the client, rather than
via a CGI-like gateway protocol that wants the HTTP status in a special
`Status:` header.

This attribute’s default value is true in [`BaseHandler`](#wsgiref.handlers.BaseHandler), but false in
[`BaseCGIHandler`](#wsgiref.handlers.BaseCGIHandler) and [`CGIHandler`](#wsgiref.handlers.CGIHandler).

#### http_version

If [`origin_server`](#wsgiref.handlers.BaseHandler.origin_server) is true, this string attribute is used to set the HTTP
...
[`CGIHandler`](#wsgiref.handlers.CGIHandler) and [`IISCGIHandler`](#wsgiref.handlers.IISCGIHandler) in place of directly using
...
in [**PEP 3333**](https://peps.python.org/pep-3333/).
...
A [`typing.Protocol`](typing.md#typing.Protocol) describing [**start_response()**](https://peps.python.org/pep-3333/#the-start-response-callable)
callables ([**PEP 3333**](https://peps.python.org/pep-3333/)).
...
A [`typing.Protocol`](typing.md#typing.Protocol) describing a [**WSGI Input Stream**](https://peps.python.org/pep-3333/#input-and-error-streams).

### *class* wsgiref.types.ErrorStream

A [`typing.Protocol`](typing.md#typing.Protocol) describing a [**WSGI Error Stream**](https://peps.python.org/pep-3333/#input-and-error-streams).

### *class* wsgiref.types.FileWrapper

A [`typing.Protocol`](typing.md#typing.Protocol) describing a [**file wrapper**](https://peps.python.org/pep-3333/#optional-platform-specific-file-handling).
See [`wsgiref.util.FileWrapper`](#wsgiref.util.FileWrapper) for a concrete implementation of this
protocol.

## Examples

This is a working “Hello World” WSGI application, where the *start_response*
callable should follow the [`StartResponse`](#wsgiref.types.StartResponse) protocol:
