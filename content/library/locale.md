...
**Source code:** [Lib/locale.py](https://github.com/python/cpython/tree/main/Lib/locale.py)
...
Exception raised when the locale passed to [`setlocale()`](#locale.setlocale) is not
recognized.

### locale.setlocale(category, locale=None)

If *locale* is given and not `None`, [`setlocale()`](#locale.setlocale) modifies the locale
...
[`Error`](#locale.Error) is raised. If successful, the new locale setting is returned.
...
[`setlocale()`](#locale.setlocale) is not thread-safe on most systems. Applications typically
...
#### Versionchanged
Changed in version 3.15: Support language codes with `@`-modifiers.
...
|--------------------------------------|-----------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [`LC_NUMERIC`](#locale.LC_NUMERIC)   | `'decimal_point'`                 | Decimal point character.                                                                                                                                                                                                                                                                                             |
|                                      | `'grouping'`                      | Sequence of numbers specifying<br/>which relative positions the<br/>`'thousands_sep'` is<br/>expected.  If the sequence is<br/>terminated with<br/>[`CHAR_MAX`](#locale.CHAR_MAX), no further<br/>grouping is performed. If the<br/>sequence terminates with a<br/>`0`,  the last group size is<br/>repeatedly used. |
|                                      | `'thousands_sep'`                 | Character used between groups.                                                                                                                                                                                                                                                                                       |
| [`LC_MONETARY`](#locale.LC_MONETARY) | `'int_curr_symbol'`               | International currency symbol.                                                                                                                                                                                                                                                                                       |
|                                      | `'currency_symbol'`               | Local currency symbol.                                                                                                                                                                                                                                                                                               |
|                                      | `'p_cs_precedes/n_cs_precedes'`   | Whether the currency symbol<br/>precedes the value (for<br/>positive resp. negative<br/>values).                                                                                                                                                                                                                     |
|                                      | `'p_sep_by_space/n_sep_by_space'` | Whether the currency symbol is<br/>separated from the value  by a<br/>space (for positive resp.<br/>negative values).                                                                                                                                                                                                |
|                                      | `'mon_decimal_point'`             | Decimal point used for<br/>monetary values.                                                                                                                                                                                                                                                                          |
|                                      | `'frac_digits'`                   | Number of fractional digits<br/>used in local formatting of<br/>monetary values.                                                                                                                                                                                                                                     |
|                                      | `'int_frac_digits'`               | Number of fractional digits<br/>used in international<br/>formatting of monetary values.                                                                                                                                                                                                                             |
|                                      | `'mon_thousands_sep'`             | Group separator used for<br/>monetary values.                                                                                                                                                                                                                                                                        |
|                                      | `'mon_grouping'`                  | Equivalent to `'grouping'`,<br/>used for monetary values.                                                                                                                                                                                                                                                            |
|                                      | `'positive_sign'`                 | Symbol used to annotate a<br/>positive monetary value.                                                                                                                                                                                                                                                               |
|                                      | `'negative_sign'`                 | Symbol used to annotate a<br/>negative monetary value.                                                                                                                                                                                                                                                               |
|                                      | `'p_sign_posn/n_sign_posn'`       | The position of the sign (for<br/>positive resp. negative<br/>values), see below.                                                                                                                                                                                                                                    |

All numeric values can be set to [`CHAR_MAX`](#locale.CHAR_MAX) to indicate that there is no
...
| Value      | Explanation                                                |
|------------|------------------------------------------------------------|
| `0`        | Currency and value are surrounded by<br/>parentheses.      |
| `1`        | The sign should precede the value and<br/>currency symbol. |
| `2`        | The sign should follow the value and<br/>currency symbol.  |
| `3`        | The sign should immediately precede the<br/>value.         |
| `4`        | The sign should immediately follow the<br/>value.          |
...
The [`nl_langinfo()`](#locale.nl_langinfo) function accepts one of the following keys.  Most
...
Get a string that can be used as a format string for [`time.strftime()`](time.md#time.strftime) to
...
Get a format string for [`time.strftime()`](time.md#time.strftime) to represent time in the am/pm
format.
...
#### NOTE
This follows the US convention of [`DAY_1`](#locale.DAY_1) being Sunday, not the
...
#### NOTE
The regular expressions for [`YESEXPR`](#locale.YESEXPR) and
[`NOEXPR`](#locale.NOEXPR) use syntax suitable for the
`regex` function from the C library, which might
differ from the syntax used in [`re`](re.md#module-re).
...
the `E` modifier in their format strings causes the [`time.strftime()`](time.md#time.strftime)
...
Get a format string for [`time.strftime()`](time.md#time.strftime) to represent date and time in a
locale-specific era-based way.

### locale.ERA_D_FMT

Get a format string for [`time.strftime()`](time.md#time.strftime) to represent a date in a
locale-specific era-based way.

### locale.ERA_T_FMT

Get a format string for [`time.strftime()`](time.md#time.strftime) to represent a time in a
locale-specific era-based way.
...
values except [`LC_ALL`](#locale.LC_ALL).  It defaults to [`LC_CTYPE`](#locale.LC_CTYPE).
...
#### Versionchanged
Changed in version 3.15: `@`-modifier are no longer silently removed, but included in
the language code.

### locale.getpreferredencoding(do_setlocale=True)

Return the [locale encoding](../glossary.md#term-locale-encoding) used for text data, according to user
...
On some systems, it is necessary to invoke [`setlocale()`](#locale.setlocale) to obtain the
user preferences, so this function is not thread-safe. If invoking setlocale
is not necessary or desired, *do_setlocale* should be set to `False`.

On Android or if the [Python UTF-8 Mode](os.md#utf8-mode) is enabled, always
return `'utf-8'`, the [locale encoding](../glossary.md#term-locale-encoding) and the *do_setlocale*
argument are ignored.

The [Python preinitialization](../c-api/init_config.md#c-preinit) configures the LC_CTYPE
locale. See also the [filesystem encoding and error handler](../glossary.md#term-filesystem-encoding-and-error-handler).
...
Get the current [locale encoding](../glossary.md#term-locale-encoding):

* On Android and VxWorks, return `"utf-8"`.
* On Unix, return the encoding of the current [`LC_CTYPE`](#locale.LC_CTYPE) locale.
...
[`getpreferredencoding(False)`](#locale.getpreferredencoding) except this
function ignores the [Python UTF-8 Mode](os.md#utf8-mode).
...
code is formatted for use with [`setlocale()`](#locale.setlocale).  If normalization fails, the
original name is returned unchanged.

If the given encoding is not known, the function defaults to the default
encoding for the locale code just like [`setlocale()`](#locale.setlocale).

### locale.strcoll(string1, string2)

Compares two strings according to the current [`LC_COLLATE`](#locale.LC_COLLATE) setting. As
...
Formats a number *val* according to the current [`LC_NUMERIC`](#locale.LC_NUMERIC) setting.
...
Formats a number *val* according to the current [`LC_MONETARY`](#locale.LC_MONETARY) settings.
...
locale via [`setlocale()`](#locale.setlocale) first.
...
[`LC_NUMERIC`](#locale.LC_NUMERIC) settings.
...
Converts a string to a number, following the [`LC_NUMERIC`](#locale.LC_NUMERIC) settings,
by calling *func* on the result of calling [`delocalize()`](#locale.delocalize) on *string*.

### locale.atoi(string)

Converts a string to an integer, following the [`LC_NUMERIC`](#locale.LC_NUMERIC) conventions.
...
equivalents like [`Py_TOLOWER`](../c-api/conversion.md#c.Py_TOLOWER).

### locale.LC_COLLATE

Locale category for sorting strings.  The functions [`strcoll()`](#locale.strcoll) and
[`strxfrm()`](#locale.strxfrm) of the `locale` module are affected.

### locale.LC_TIME

Locale category for the formatting of time.  The function [`time.strftime()`](time.md#time.strftime)
follows these conventions.

### locale.LC_MONETARY

Locale category for formatting of monetary values.  The available options are
available from the [`localeconv()`](#locale.localeconv) function.

### locale.LC_MESSAGES

Locale category for message display. Python currently does not support
application specific locale-aware messages.  Messages displayed by the operating
system, like those returned by [`os.strerror()`](os.md#os.strerror) might be affected by this
category.
...
Locale category for formatting numbers.  The functions [`format_string()`](#locale.format_string),
[`atoi()`](#locale.atoi), [`atof()`](#locale.atof) and [`str()`](#locale.str) of the `locale` module are
...
[`LC_CTYPE`](#locale.LC_CTYPE) category is changed at startup to set the current locale
...
It is generally a bad idea to call [`setlocale()`](#locale.setlocale) in some library routine,
...
certain formats used with [`time.strftime()`](time.md#time.strftime)), you will have to find a way to
...
special functions defined by this module: [`atof()`](#locale.atof), [`atoi()`](#locale.atoi),
[`format_string()`](#locale.format_string), [`str()`](#locale.str).
...
On Posix platforms, it usually has the format <sup>[1](#id4)</sup>:
...
On Windows, several formats are supported. <sup>[2](#id5)</sup> <sup>[3](#id6)</sup>
...
* <a id='id4'>**[1]**</a> [IEEE Std 1003.1-2024; 8.2 Internationalization Variables](https://pubs.opengroup.org/onlinepubs/9799919799/basedefs/V1_chap08.html#tag_08_02)
* <a id='id5'>**[2]**</a> [UCRT Locale names, Languages, and Country/Region strings](https://learn.microsoft.com/en-us/cpp/c-runtime-library/locale-names-languages-and-country-region-strings)
* <a id='id6'>**[3]**</a> [Locale Names](https://learn.microsoft.com/en-us/windows/win32/intl/locale-names)

<a id="embedding-locale"></a>

## For extension writers and programs that embed Python

Extension modules should never call [`setlocale()`](#locale.setlocale), except to find out what
...
provide this interface.  It consists of the functions [`gettext()`](gettext.md#module-gettext),
[`dgettext()`](#locale.dgettext), [`dcgettext()`](#locale.dcgettext), [`textdomain()`](#locale.textdomain), [`bindtextdomain()`](#locale.bindtextdomain),
and [`bind_textdomain_codeset()`](#locale.bind_textdomain_codeset).  These are similar to the same functions in
the [`gettext`](gettext.md#module-gettext) module, but use the C library’s binary format for message
catalogs, and the C library’s search algorithms for locating message catalogs.

Python applications should normally find no need to invoke these functions, and
should use [`gettext`](gettext.md#module-gettext) instead.  A known exception to this rule are
