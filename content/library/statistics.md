...
**Source code:** [Lib/statistics.py](https://github.com/python/cpython/tree/main/Lib/statistics.py)
...
numeric ([`Real`](numbers.md#numbers.Real)-valued) data.
...
Unless explicitly noted, these functions support [`int`](functions.md#int),
[`float`](functions.md#float), [`Decimal`](decimal.md#decimal.Decimal) and [`Fraction`](fractions.md#fractions.Fraction).
...
you may be able to use [`map()`](functions.md#map) to ensure a consistent result, for
...
| [`mean()`](#statistics.mean)                     | Arithmetic mean (“average”) of data.                            |
|--------------------------------------------------|-----------------------------------------------------------------|
| [`fmean()`](#statistics.fmean)                   | Fast, floating-point arithmetic mean, with optional weighting.  |
| [`geometric_mean()`](#statistics.geometric_mean) | Geometric mean of data.                                         |
| [`harmonic_mean()`](#statistics.harmonic_mean)   | Harmonic mean of data.                                          |
| [`kde()`](#statistics.kde)                       | Estimate the probability density distribution of the data.      |
| [`kde_random()`](#statistics.kde_random)         | Random sampling from the PDF generated by kde().                |
| [`median()`](#statistics.median)                 | Median (middle value) of data.                                  |
| [`median_low()`](#statistics.median_low)         | Low median of data.                                             |
| [`median_high()`](#statistics.median_high)       | High median of data.                                            |
| [`median_grouped()`](#statistics.median_grouped) | Median (50th percentile) of grouped data.                       |
| [`mode()`](#statistics.mode)                     | Single mode (most common value) of discrete or nominal data.    |
| [`multimode()`](#statistics.multimode)           | List of modes (most common values) of discrete or nominal data. |
| [`quantiles()`](#statistics.quantiles)           | Divide data into intervals with equal probability.              |
...
| [`pstdev()`](#statistics.pstdev)       | Population standard deviation of data.   |
|----------------------------------------|------------------------------------------|
| [`pvariance()`](#statistics.pvariance) | Population variance of data.             |
| [`stdev()`](#statistics.stdev)         | Sample standard deviation of data.       |
| [`variance()`](#statistics.variance)   | Sample variance of data.                 |

## Statistics for relations between two inputs

These functions calculate statistics regarding relations between two inputs.

| [`covariance()`](#statistics.covariance)               | Sample covariance for two variables.              |
|--------------------------------------------------------|---------------------------------------------------|
| [`correlation()`](#statistics.correlation)             | Pearson and Spearman’s correlation coefficients.  |
| [`linear_regression()`](#statistics.linear_regression) | Slope and intercept for simple linear regression. |
...
If *data* is empty, [`StatisticsError`](#statistics.StatisticsError) will be raised.
...
efficient, measure of [central tendency](https://en.wikipedia.org/wiki/Central_tendency), see [`median()`](#statistics.median).
...
This runs faster than the [`mean()`](#statistics.mean) function and it always returns a
[`float`](functions.md#float).  The *data* may be a sequence or iterable.  If the input
dataset is empty, raises a [`StatisticsError`](#statistics.StatisticsError).
...
a [`ValueError`](exceptions.md#ValueError) will be raised.
...
Raises a [`StatisticsError`](#statistics.StatisticsError) if the input dataset is empty,
...
The harmonic mean is the reciprocal of the arithmetic [`mean()`](#statistics.mean) of the
...
[`StatisticsError`](#statistics.StatisticsError) is raised if *data* is empty, any element
...
[Kernel Density Estimation (KDE)](https://www.itm-conferences.org/articles/itmconf/pdf/2018/08/itmconf_sam2018_00037.pdf):
Create a continuous probability density function or cumulative
distribution function from discrete samples.

The basic idea is to smooth the data using [a kernel function](https://en.wikipedia.org/wiki/Kernel_(statistics)).
...
A [`StatisticsError`](#statistics.StatisticsError) will be raised if the *data* sequence is empty.

[Wikipedia has an example](https://en.wikipedia.org/wiki/Kernel_density_estimation#Example)
where we can use [`kde()`](#statistics.kde) to generate and plot a probability
...
![Scatter plot of the estimated probability density function.](library/kde_example.png)
...
Continuing the example for [`kde()`](#statistics.kde), we can use
[`kde_random()`](#statistics.kde_random) to generate new random selections from an
...
middle two” method.  If *data* is empty, [`StatisticsError`](#statistics.StatisticsError) is raised.
...
support addition), consider using [`median_low()`](#statistics.median_low) or [`median_high()`](#statistics.median_high)
instead.

### statistics.median_low(data)

Return the low median of numeric data.  If *data* is empty,
[`StatisticsError`](#statistics.StatisticsError) is raised.  *data* can be a sequence or iterable.
...
Return the high median of data.  If *data* is empty, [`StatisticsError`](#statistics.StatisticsError)
...
The regular [`median()`](#statistics.median) function would assume that everyone in the
...
If the input *data* is empty, [`StatisticsError`](#statistics.StatisticsError) is raised.
...
Only hashable inputs are supported.  To handle type [`set`](stdtypes.md#set),
consider casting to [`frozenset`](stdtypes.md#frozenset).  To handle type [`list`](stdtypes.md#list),
consider casting to [`tuple`](stdtypes.md#tuple).  For mixed or nested inputs, consider
...
Formerly, it raised [`StatisticsError`](#statistics.StatisticsError) when more than one mode was
found.
...
variance).  See [`pvariance()`](#statistics.pvariance) for arguments and other details.
...
estimate the variance from a sample, the [`variance()`](#statistics.variance) function is usually
a better choice.

Raises [`StatisticsError`](#statistics.StatisticsError) if *data* is empty.
...
variance).  See [`variance()`](#statistics.variance) for arguments and other details.
...
the variance from the entire population, see [`pvariance()`](#statistics.pvariance).

Raises [`StatisticsError`](#statistics.StatisticsError) if *data* has fewer than two values.
...
[`pvariance()`](#statistics.pvariance) function as the *mu* parameter to get the variance of a
sample.
...
*data* into 100 equal sized groups.  Raises [`StatisticsError`](#statistics.StatisticsError) if *n*
is not least 1.

The *data* can be any iterable containing sample data.  For meaningful
results, the number of data points in *data* should be larger than *n*.
Raises [`StatisticsError`](#statistics.StatisticsError) if there is not at least one data point.
...
#### Versionchanged
Changed in version 3.13: No longer raises an exception for an input with only a single data point.
...
If *method* is “ranked”, computes [Spearman’s rank correlation coefficient](https://en.wikipedia.org/wiki/Spearman%27s_rank_correlation_coefficient)
...
not to be constant, otherwise [`StatisticsError`](#statistics.StatisticsError) is raised.
...
otherwise a [`StatisticsError`](#statistics.StatisticsError) is raised.
...
Continuing the example from [`correlation()`](#statistics.correlation), we look to see
...
Subclass of [`ValueError`](exceptions.md#ValueError) for statistics-related exceptions.

## [`NormalDist`](#statistics.NormalDist) objects

[`NormalDist`](#statistics.NormalDist) is a tool for creating and manipulating normal
...
If *sigma* is negative, raises [`StatisticsError`](#statistics.StatisticsError).
...
estimated from the *data* using [`fmean()`](#statistics.fmean) and [`stdev()`](#statistics.stdev).

The *data* can be any [iterable](../glossary.md#term-iterable) and should consist of values
that can be converted to type [`float`](functions.md#float).  If *data* does not
contain at least two elements, raises [`StatisticsError`](#statistics.StatisticsError) because it
...
Returns a [`list`](stdtypes.md#list) of [`float`](functions.md#float) values.
...
versions, use [`random.seed()`](random.md#random.seed) and [`random.gauss()`](random.md#random.gauss).
...
Instances of [`NormalDist`](#statistics.NormalDist) support addition, subtraction,
...
Dividing a constant by an instance of [`NormalDist`](#statistics.NormalDist) is not supported
...
represented as instances of [`NormalDist`](#statistics.NormalDist).  For example:
...
[`NormalDist`](#statistics.NormalDist) readily solves classic probability problems.
...
analytically, [`NormalDist`](#statistics.NormalDist) can generate input samples for a [Monte
...
with [`NormalDist`](#statistics.NormalDist):
