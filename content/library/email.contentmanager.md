...
**Source code:** [Lib/email/contentmanager.py](https://github.com/python/cpython/tree/main/Lib/email/contentmanager.py)
...
If none of these keys produce a handler, raise a [`KeyError`](exceptions.md#KeyError) for the
full MIME type.

#### set_content(msg, obj, \*args, \*\*kw)

If the `maintype` is `multipart`, raise a [`TypeError`](exceptions.md#TypeError); otherwise
look up a handler function based on the type of *obj* (see next
paragraph), call [`clear_content()`](email.message.md#email.message.EmailMessage.clear_content) on the
...
* the type’s fully qualified name (`typ.__module__ + '.' +
  typ.__qualname__`).
* the type’s [`qualname`](../reference/datamodel.md#type.__qualname__) (`typ.__qualname__`)
* the type’s [`name`](../reference/datamodel.md#type.__name__) (`typ.__name__`).

If none of the above match, repeat all of the checks above for each of
the types in the [MRO](../glossary.md#term-MRO) ([`typ.__mro__`](../reference/datamodel.md#type.__mro__)).
Finally, if no other key
yields a handler, check for a handler for the key `None`.  If there is
no handler for `None`, raise a [`KeyError`](exceptions.md#KeyError) for the fully
qualified name of the type.

Also add a *MIME-Version* header if one is not present (see
also [`MIMEPart`](email.message.md#email.message.MIMEPart)).

#### add_get_handler(key, handler)

Record the function *handler* as the handler for *key*.  For the possible
values of *key*, see [`get_content()`](#email.contentmanager.get_content).

#### add_set_handler(typekey, handler)

Record *handler* as the function to call when an object of a type
matching *typekey* is passed to [`set_content()`](#email.contentmanager.set_content).  For the possible
values of *typekey*, see [`set_content()`](#email.contentmanager.set_content).

## Content Manager Instances

Currently the email package provides only one concrete content manager,
[`raw_data_manager`](#email.contentmanager.raw_data_manager), although more may be added in the future.
[`raw_data_manager`](#email.contentmanager.raw_data_manager) is the
[`content_manager`](email.policy.md#email.policy.EmailPolicy.content_manager) provided by
[`EmailPolicy`](email.policy.md#email.policy.EmailPolicy) and its derivatives.

### email.contentmanager.raw_data_manager

This content manager provides only a minimum interface beyond that provided
by [`Message`](email.compat32-message.md#email.message.Message) itself:  it deals only with text, raw
byte strings, and [`Message`](email.compat32-message.md#email.message.Message) objects.  Nevertheless, it
...
[`EmailMessage`](email.message.md#email.message.EmailMessage) object (for `message/rfc822`
parts), or a `bytes` object (for all other non-multipart types).  Raise
a [`KeyError`](exceptions.md#KeyError) if called on a `multipart`.  If the part is a
...
* For `bytes`, use the specified *maintype* and *subtype*, or
  raise a [`TypeError`](exceptions.md#TypeError) if they are not specified.
* For [`EmailMessage`](email.message.md#email.message.EmailMessage) objects, set the maintype
...
* For `str` objects, if *cte* is not set use heuristics to
  ...
  [`str.splitlines()`](stdtypes.md#str.splitlines) is used to normalize all line boundaries,
  ensuring that each line of the payload is terminated by the
  current policy’s [`linesep`](email.policy.md#email.policy.Policy.linesep) property
...
* For [`EmailMessage`](email.message.md#email.message.EmailMessage), per [**RFC 2046**](https://datatracker.ietf.org/doc/html/rfc2046.html), raise
...
correct, but [`BytesGenerator`](email.generator.md#email.generator.BytesGenerator) does not
...
* <a id='id2'>**[1]**</a> Originally added in 3.4 as a [provisional module](../glossary.md#term-provisional-package)
