...
**Source code:** [Lib/shlex.py](https://github.com/python/cpython/tree/main/Lib/shlex.py)
...
Split the string *s* using shell-like syntax. If *comments* is [`False`](constants.md#False)
...
reading [`sys.stdin`](sys.md#sys.stdin).
...
<a id="shlex-quote-warning"></a>

#### WARNING
The `shlex` module is **only designed for Unix shells**.

The [`quote()`](#shlex.quote) function is not guaranteed to be correct on non-POSIX
...
[`subprocess.run()`](subprocess.md#subprocess.run) with `shell=False`.
...
[`quote()`](#shlex.quote) lets you plug the security hole:
...
A [`shlex`](#shlex.shlex) instance or subclass instance is a lexical analyzer
...
[`read()`](io.md#io.TextIOBase.read) and [`readline()`](io.md#io.TextIOBase.readline) methods, or
...
when *posix* is not true (default), the [`shlex`](#shlex.shlex) instance will
operate in compatibility mode.  When operating in POSIX mode,
[`shlex`](#shlex.shlex) will try to be as close as possible to the POSIX shell
...
*punctuation_chars* will be removed from [`wordchars`](#shlex.shlex.wordchars).  See
[Improved Compatibility with Shells](#improved-shell-compatibility) for more information. *punctuation_chars*
can be set only upon [`shlex`](#shlex.shlex) instance creation and can’t be
modified later.
...
#### SEE ALSO
Module [`configparser`](configparser.md#module-configparser)
: Parser for configuration files similar to the Windows `.ini` files.

<a id="shlex-objects"></a>
...
first component of the tuple, with [`open()`](functions.md#open) called on it to yield the second
...
[`close()`](io.md#io.IOBase.close) method of the sourced input stream when it returns
EOF.
...
EOF, at which point the [`close()`](io.md#io.IOBase.close) method of that stream will be
...
* If [`whitespace_split`](#shlex.shlex.whitespace_split) is `False`, any character not
  ...
  a single-character token. If it is `True`, [`shlex`](#shlex.shlex) will only
  split words in whitespaces;
* EOF is signaled with an empty string (`''`);
* It’s not possible to parse empty strings, even if quoted.

When operating in POSIX mode, [`shlex`](#shlex.shlex) will try to obey the
...
* Enclosing characters in quotes which are not part of
  [`escapedquotes`](#shlex.shlex.escapedquotes) (e.g. `"'"`) preserve the literal value
  of all characters within the quotes;
* Enclosing characters in quotes which are part of
  [`escapedquotes`](#shlex.shlex.escapedquotes) (e.g. `'"'`) preserves the literal value
  of all characters within the quotes, with the exception of the characters
  mentioned in [`escape`](#shlex.shlex.escape).  The escape characters retain their
  ...
  normal character.
* EOF is signaled with a [`None`](constants.md#None) value;
* Quoted empty strings (`''`) are allowed.

<a id="improved-shell-compatibility"></a>
...
The [`shlex`](#module-shlex) class provides compatibility with the parsing performed by
...
#### NOTE
When `punctuation_chars` is specified, the [`wordchars`](#shlex.shlex.wordchars)
...
always use `posix` and [`whitespace_split`](#shlex.shlex.whitespace_split) when using
[`punctuation_chars`](#shlex.shlex.punctuation_chars), which will negate
[`wordchars`](#shlex.shlex.wordchars) entirely.
