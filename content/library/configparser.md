...
**Source code:** [Lib/configparser.py](https://github.com/python/cpython/tree/main/Lib/configparser.py)

<a id="index-0"></a>

---

This module provides the [`ConfigParser`](#configparser.ConfigParser) class which implements a basic
...
#### SEE ALSO
Module [`tomllib`](tomllib.md#module-tomllib)
...
The structure of INI files is described [in the following section](#supported-ini-file-structure).  Essentially, the file
...
There are differences, [outlined later](#mapping-protocol-access), but
...
sections <sup>[1](#id16)</sup>.  Note also that keys in sections are
case-insensitive and stored in lowercase <sup>[1](#id16)</sup>.

It is possible to read several configurations into a single
[`ConfigParser`](#configparser.ConfigParser), where the most recently added configuration has the
...
This behaviour is equivalent to a [`ConfigParser.read()`](#configparser.ConfigParser.read) call with several
...
provide [`getboolean()`](#configparser.ConfigParser.getboolean).  This method is case-insensitive and
recognizes Boolean values from `'yes'`/`'no'`, `'on'`/`'off'`,
`'true'`/`'false'` and `'1'`/`'0'` <sup>[1](#id16)</sup>.  For example:
...
Apart from [`getboolean()`](#configparser.ConfigParser.getboolean), config parsers also
provide equivalent [`getint()`](#configparser.ConfigParser.getint) and
[`getfloat()`](#configparser.ConfigParser.getfloat) methods.  You can register your own
converters and customize the provided ones. <sup>[1](#id16)</sup>

## Fallback Values

As with a dictionary, you can use a section’s [`get()`](#configparser.ConfigParser.get) method to
...
One more thing to be aware of is that the parser-level [`get()`](#configparser.ConfigParser.get) method
...
[`getint()`](#configparser.ConfigParser.getint), [`getfloat()`](#configparser.ConfigParser.getfloat) and
[`getboolean()`](#configparser.ConfigParser.getboolean) methods, for example:
...
default <sup>[1](#id16)</sup>).  By default, section names are case sensitive but keys are not
<sup>[1](#id16)</sup>.  Leading and trailing whitespace is removed from keys and values.
Values can be omitted if the parser is configured to allow it <sup>[1](#id16)</sup>,
...
To change this, see [`ConfigParser.SECTCRE`](#configparser.ConfigParser.SECTCRE).
...
the keys/values may be retrieved by [`UNNAMED_SECTION`](#configparser.UNNAMED_SECTION) as in
`config[UNNAMED_SECTION]`.

Configuration files may include comments, prefixed by specific
characters (`#` and `;` by default <sup>[1](#id16)</sup>).  Comments may appear on
their own on an otherwise empty line, possibly indented. <sup>[1](#id16)</sup>
...
<a id="unnamed-sections"></a>

## Unnamed Sections

The name of the first section (or unique) may be omitted and values
retrieved by the [`UNNAMED_SECTION`](#configparser.UNNAMED_SECTION) attribute.
...
On top of the core functionality, [`ConfigParser`](#configparser.ConfigParser) supports
...
The default implementation used by [`ConfigParser`](#configparser.ConfigParser).  It enables
values to contain format strings which refer to other values in the same
section, or values in the special default section <sup>[1](#id16)</sup>.  Additional default
...
In the example above, [`ConfigParser`](#configparser.ConfigParser) with *interpolation* set to
...
[`MutableMapping`](collections.abc.md#collections.abc.MutableMapping) ABC.
However, there are a few differences that should be taken into account:

* By default, all keys in sections are accessible in a case-insensitive manner
  <sup>[1](#id16)</sup>.  E.g. `for option in parser["section"]` yields only `optionxform`’ed
...
* All sections include `DEFAULTSECT` values as well which means that
  ...
  causes a [`KeyError`](exceptions.md#KeyError).
* `DEFAULTSECT` cannot be removed from the parser:
  * trying to delete it raises [`ValueError`](exceptions.md#ValueError),
...
* *defaults*, default value: `None`
  ...
  [`read_dict()`](#configparser.ConfigParser.read_dict) before you read the actual file.
* *dict_type*, default value: [`dict`](stdtypes.md#dict)
...
* *delimiters*, default value: `('=', ':')`
  ...
  [`ConfigParser.write()`](#configparser.ConfigParser.write).
...
* *strict*, default value: `True`
  ...
  duplicates while reading from a single source (using [`read_file()`](#configparser.ConfigParser.read_file),
  [`read_string()`](#configparser.ConfigParser.read_string) or [`read_dict()`](#configparser.ConfigParser.read_dict)).  It is recommended to use strict
...
* *interpolation*, default value: `configparser.BasicInterpolation`
  ...
  [dedicated documentation section](#interpolation-of-values).
  [`RawConfigParser`](#configparser.RawConfigParser) has a default value of `None`.
* *converters*, default value: not set

  Config parsers provide option value getters that perform type conversion.  By
  default [`getint()`](#configparser.ConfigParser.getint), [`getfloat()`](#configparser.ConfigParser.getfloat), and
  [`getboolean()`](#configparser.ConfigParser.getboolean) are implemented.  Should other getters be
...
By default when using [`getboolean()`](#configparser.ConfigParser.getboolean), config parsers
...
To get interpolation, use [`ConfigParser`](#configparser.ConfigParser):
...
string or dictionary), raising [`DuplicateSectionError`](#configparser.DuplicateSectionError) or
[`DuplicateOptionError`](#configparser.DuplicateOptionError).  When *empty_lines_in_values* is `False`
...
[`optionxform()`](#configparser.ConfigParser.optionxform) method just like any other option name reference.  For
example, using the default implementation of [`optionxform()`](#configparser.ConfigParser.optionxform) (which
...
[“Unnamed Sections” section](#unnamed-sections).
...
#### Versionchanged
Changed in version 3.1: The default *dict_type* is [`collections.OrderedDict`](collections.md#collections.OrderedDict).
...
#### Versionchanged
Changed in version 3.7: The *defaults* argument is read with [`read_dict()`](#configparser.ConfigParser.read_dict),
providing consistent behavior across the parser: non-string
keys and values are implicitly converted to strings.

#### Versionchanged
Changed in version 3.8: The default *dict_type* is [`dict`](stdtypes.md#dict), since it now preserves
insertion order.

#### Versionchanged
Changed in version 3.13: Raise a [`MultilineContinuationError`](#configparser.MultilineContinuationError) when *allow_no_value* is
...
name already exists, [`DuplicateSectionError`](#configparser.DuplicateSectionError) is raised.  If the
*default section* name is passed, [`ValueError`](exceptions.md#ValueError) is raised.  The name
of the section must be a string; if not, [`TypeError`](exceptions.md#TypeError) is raised.

#### Versionchanged
Changed in version 3.2: Non-string section names raise [`TypeError`](exceptions.md#TypeError).
...
[`True`](constants.md#True); otherwise return [`False`](constants.md#False).  If the specified
*section* is [`None`](constants.md#None) or an empty string, DEFAULT is assumed.
...
If *filenames* is a string, a [`bytes`](stdtypes.md#bytes) object or a
[path-like object](../glossary.md#term-path-like-object), it is treated as
...
If none of the named files exist, the [`ConfigParser`](#configparser.ConfigParser)
instance will contain an empty dataset.  An application which requires
initial values to be loaded from a file should load the required file or
files using [`read_file()`](#configparser.ConfigParser.read_file) before calling [`read()`](#configparser.ConfigParser.read) for any
optional files:
...
Previously, all files were read using the default encoding for [`open()`](functions.md#open).

#### Versionchanged
Changed in version 3.6.1: The *filenames* parameter accepts a [path-like object](../glossary.md#term-path-like-object).

#### Versionchanged
Changed in version 3.7: The *filenames* parameter accepts a [`bytes`](stdtypes.md#bytes) object.
...
#### getint(section, option, \*, raw=False, vars=None)

A convenience method which coerces the *option* in the specified *section*
to an integer.  See [`get()`](#configparser.ConfigParser.get) for explanation of *raw*, *vars* and
*fallback*.

#### getfloat(section, option, \*, raw=False, vars=None)

A convenience method which coerces the *option* in the specified *section*
to a floating-point number.  See [`get()`](#configparser.ConfigParser.get) for explanation of *raw*,
*vars* and *fallback*.

#### getboolean(section, option, \*, raw=False, vars=None)
...
[`ValueError`](exceptions.md#ValueError).  See [`get()`](#configparser.ConfigParser.get) for explanation of *raw*, *vars* and
*fallback*.
...
[`get()`](#configparser.ConfigParser.get) method.
...
otherwise raise [`NoSectionError`](#configparser.NoSectionError).  *option* and *value* must be
strings; if not, [`TypeError`](exceptions.md#TypeError) is raised.

#### write(fileobject, space_around_delimiters=True)

Write a representation of the configuration to the specified [file
object](../glossary.md#term-file-object), which must be opened in text mode (accepting strings).  This
representation can be parsed by a future [`read()`](#configparser.ConfigParser.read) call.  If
*space_around_delimiters* is true, delimiters between
keys and values are surrounded by spaces.

#### Versionchanged
Changed in version 3.14: Raises InvalidWriteError if this would write a representation which cannot
be accurately parsed by a future [`read()`](#configparser.ConfigParser.read) call from this parser.
...
section does not exist, raise [`NoSectionError`](#configparser.NoSectionError).  If the option
existed to be removed, return [`True`](constants.md#True); otherwise return
...
names is stripped before [`optionxform()`](#configparser.ConfigParser.optionxform) is called.

### configparser.UNNAMED_SECTION

A special object representing a section name used to reference the unnamed section (see [Unnamed Sections](#unnamed-sections)).

### configparser.MAX_INTERPOLATION_DEPTH

The maximum depth for recursive interpolation for [`get()`](#configparser.ConfigParser.get) when the *raw*
parameter is false.  This is relevant only when the default *interpolation*
is used.

<a id="rawconfigparser-objects"></a>
...
Legacy variant of the [`ConfigParser`](#configparser.ConfigParser).  It has interpolation
...
#### NOTE
Consider using [`ConfigParser`](#configparser.ConfigParser) instead which checks types of
the values to be stored internally.  If you don’t want interpolation, you
can use `ConfigParser(interpolation=None)`.

#### add_section(section)

Add a section named *section* or [`UNNAMED_SECTION`](#configparser.UNNAMED_SECTION) to the instance.

If the given section already exists, [`DuplicateSectionError`](#configparser.DuplicateSectionError) is
raised. If the *default section* name is passed, [`ValueError`](exceptions.md#ValueError) is
raised. If [`UNNAMED_SECTION`](#configparser.UNNAMED_SECTION) is passed and support is disabled,
[`UnnamedSectionDisabledError`](#configparser.UnnamedSectionDisabledError) is raised.

Type of *section* is not checked which lets users create non-string named
sections.  This behaviour is unsupported and may cause internal errors.

#### Versionchanged
Changed in version 3.14: Added support for [`UNNAMED_SECTION`](#configparser.UNNAMED_SECTION).

#### set(section, option, value)

If the given section exists, set the given option to the specified value;
otherwise raise [`NoSectionError`](#configparser.NoSectionError).  While it is possible to use
[`RawConfigParser`](#configparser.RawConfigParser) (or [`ConfigParser`](#configparser.ConfigParser) with *raw* parameters
...
Exception raised if [`add_section()`](#configparser.ConfigParser.add_section) is called with the name of a section
...
number of iterations exceeds [`MAX_INTERPOLATION_DEPTH`](#configparser.MAX_INTERPOLATION_DEPTH).  Subclass of
[`InterpolationError`](#configparser.InterpolationError).

### *exception* configparser.InterpolationMissingOptionError

Exception raised when an option referenced from a value does not exist.
Subclass of [`InterpolationError`](#configparser.InterpolationError).

### *exception* configparser.InterpolationSyntaxError

Exception raised when the source text into which substitutions are made does
not conform to the required syntax.  Subclass of [`InterpolationError`](#configparser.InterpolationError).
...
[`UNNAMED_SECTION`](#configparser.UNNAMED_SECTION) without enabling it.
...
Exception raised when an attempted [`ConfigParser.write()`](#configparser.ConfigParser.write) would not be parsed
accurately with a future [`ConfigParser.read()`](#configparser.ConfigParser.read) call.

Ex: Writing a key beginning with the [`ConfigParser.SECTCRE`](#configparser.ConfigParser.SECTCRE) pattern
...
* <a id='id16'>**[1]**</a> Config parsers allow for heavy customization.  If you are interested in changing the behaviour outlined by the footnote reference, consult the [Customizing Parser Behaviour]() section.
