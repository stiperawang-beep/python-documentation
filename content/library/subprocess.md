...
**Source code:** [Lib/subprocess.py](https://github.com/python/cpython/tree/main/Lib/subprocess.py)
...
set *stdout* to [`PIPE`](#subprocess.PIPE)
...
specified *encoding* and *errors* or the [`io.TextIOWrapper`](io.md#io.TextIOWrapper) default.
...
or bytes to bytes on POSIX platforms much like [`os.environ`](os.md#os.environ) or
[`os.environb`](os.md#os.environb).
...
to [`Popen`](#subprocess.Popen) and indicates that the special file [`os.devnull`](os.md#os.devnull)
will be used.
...
[`bytes`](stdtypes.md#bytes) when any output was captured regardless of the
...
Otherwise, `None`.  This is always [`bytes`](stdtypes.md#bytes) when stderr output
...
<a id="frequently-used-arguments"></a>
...
> a single string, either *shell* must be [`True`](constants.md#True) (see below) or else
...
> positive integer), and an existing [file object](../glossary.md#term-file-object) with a valid file
> descriptor.  With the default settings of `None`, no redirection will
> occur.  [`PIPE`](#subprocess.PIPE) indicates that a new pipe to the child should be
> created.  [`DEVNULL`](#subprocess.DEVNULL) indicates that the special file [`os.devnull`](os.md#os.devnull)
...
> defaults for [`io.TextIOWrapper`](io.md#io.TextIOWrapper).

> For *stdin*, line ending characters `'\n'` in the input will be converted
> to the default line separator [`os.linesep`](os.md#os.linesep). For *stdout* and *stderr*,
> all line endings in the output will be converted to `'\n'`.  For more
> information see the documentation of the [`io.TextIOWrapper`](io.md#io.TextIOWrapper) class
...
> #### NOTE
> The newlines attribute of the file objects [`Popen.stdin`](#subprocess.Popen.stdin),
> [`Popen.stdout`](#subprocess.Popen.stdout) and [`Popen.stderr`](#subprocess.Popen.stderr) are not updated by
> the [`Popen.communicate()`](#subprocess.Popen.communicate) method.
...
> implementations of many shell-like features (in particular, [`glob`](glob.md#module-glob),
> [`fnmatch`](fnmatch.md#module-fnmatch), [`os.walk()`](os.md#os.walk), [`os.path.expandvars()`](os.path.md#os.path.expandvars),
> [`os.path.expanduser()`](os.path.md#os.path.expanduser), and [`shutil`](shutil.md#module-shutil)).

> #### Versionchanged
> Changed in version 3.3: When *universal_newlines* is `True`, the class uses the encoding
> [`locale.getpreferredencoding(False)`](locale.md#locale.getpreferredencoding)
> instead of `locale.getpreferredencoding()`.  See the
> [`io.TextIOWrapper`](io.md#io.TextIOWrapper) class for more information on this change.

> #### NOTE
> Read the [Security Considerations]() section before using `shell=True`.

These options, along with all of the other options, are described in more
detail in the [`Popen`](#subprocess.Popen) constructor documentation.

### Popen Constructor

The underlying process creation and management in this module is handled by
the [`Popen`](#subprocess.Popen) class. It offers a lot of flexibility so that developers
...
[`os.execvpe()`](os.md#os.execvpe)-like behavior to execute the child program.  On Windows,
the class uses the Windows `CreateProcess()` function.  The arguments to
[`Popen`](#subprocess.Popen) are as follows.

*args* should be a sequence of program arguments or else a single string
or [path-like object](../glossary.md#term-path-like-object).
...
[`shutil.which()`](shutil.md#shutil.which). On all platforms, passing [`sys.executable`](sys.md#sys.executable)
...
platform dependent. For POSIX, see [`os.execvpe()`](os.md#os.execvpe), and note that
...
especially in complex cases. [`shlex.split()`](shlex.md#shlex.split) can illustrate how to
...
#### Versionchanged
Changed in version 3.6: *args* parameter accepts a [path-like object](../glossary.md#term-path-like-object) if *shell* is
`False` and a sequence containing path-like objects on POSIX.

#### Versionchanged
Changed in version 3.8: *args* parameter accepts a [path-like object](../glossary.md#term-path-like-object) if *shell* is
...
itself.  That is to say, [`Popen`](#subprocess.Popen) does the equivalent of:
...
[`open()`](functions.md#open) function when creating the stdin/stdout/stderr pipe
file objects:
...
#### Versionchanged
Changed in version 3.6: *executable* parameter accepts a [path-like object](../glossary.md#term-path-like-object) on POSIX.

#### Versionchanged
Changed in version 3.8: *executable* parameter accepts a bytes and [path-like object](../glossary.md#term-path-like-object)
on Windows.
...
code using *preexec_fn* to call [`os.setsid()`](os.md#os.setsid) or [`os.setpgid()`](os.md#os.setpgid) in the child.
...
[`RuntimeError`](exceptions.md#RuntimeError). The new restriction may affect applications that
...
as described in [Inheritance of File Descriptors](os.md#fd-inheritance).

On Windows, if *close_fds* is true then no handles will be inherited by the
child process unless explicitly passed in the `handle_list` element of
[`STARTUPINFO.lpAttributeList`](#subprocess.STARTUPINFO.lpAttributeList), or by standard handle redirection.

#### Versionchanged
Changed in version 3.2: The default for *close_fds* was changed from [`False`](constants.md#False) to
what is described above.

#### Versionchanged
Changed in version 3.7: On Windows the default for *close_fds* was changed from [`False`](constants.md#False) to
[`True`](constants.md#True) when redirecting the standard handles. It’s now possible to
set *close_fds* to [`True`](constants.md#True) when redirecting the standard handles.

*pass_fds* is an optional sequence of file descriptors to keep open
between the parent and child.  Providing any *pass_fds* forces
*close_fds* to be [`True`](constants.md#True).  (POSIX only)
...
[path-like](../glossary.md#term-path-like-object) object.  On POSIX, the function
looks for *executable* (or for the first item in *args*) relative to *cwd*
if the executable path is a relative path.

#### Versionchanged
Changed in version 3.6: *cwd* parameter accepts a [path-like object](../glossary.md#term-path-like-object) on POSIX.

#### Versionchanged
Changed in version 3.7: *cwd* parameter accepts a [path-like object](../glossary.md#term-path-like-object) on Windows.
...
[Availability](intro.md#availability): POSIX
...
value is a string, it will be looked up via [`grp.getgrnam()`](grp.md#grp.getgrnam) and
...
[`grp.getgrnam()`](grp.md#grp.getgrnam) and the values in `gr_gid` will be used.
...
child process prior to the execution of the subprocess. If the provided
value is a string, it will be looked up via [`pwd.getpwnam()`](pwd.md#pwd.getpwnam) and
...
[`os.environ`](os.md#os.environ) or [`os.environb`](os.md#os.environb).
...
If given, *startupinfo* will be a [`STARTUPINFO`](#subprocess.STARTUPINFO) object, which is
...
* [`ABOVE_NORMAL_PRIORITY_CLASS`](#subprocess.ABOVE_NORMAL_PRIORITY_CLASS)
* [`BELOW_NORMAL_PRIORITY_CLASS`](#subprocess.BELOW_NORMAL_PRIORITY_CLASS)
* [`HIGH_PRIORITY_CLASS`](#subprocess.HIGH_PRIORITY_CLASS)
* [`IDLE_PRIORITY_CLASS`](#subprocess.IDLE_PRIORITY_CLASS)
* [`NORMAL_PRIORITY_CLASS`](#subprocess.NORMAL_PRIORITY_CLASS)
* [`REALTIME_PRIORITY_CLASS`](#subprocess.REALTIME_PRIORITY_CLASS)
* [`CREATE_NO_WINDOW`](#subprocess.CREATE_NO_WINDOW)
* [`DETACHED_PROCESS`](#subprocess.DETACHED_PROCESS)
* [`CREATE_DEFAULT_ERROR_MODE`](#subprocess.CREATE_DEFAULT_ERROR_MODE)
* [`CREATE_BREAKAWAY_FROM_JOB`](#subprocess.CREATE_BREAKAWAY_FROM_JOB)

*pipesize* can be used to change the size of the pipe when
[`PIPE`](#subprocess.PIPE) is used for *stdin*, *stdout* or *stderr*. The size of the pipe
...
Popen objects are supported as context managers via the [`with`](../reference/compound_stmts.md#with) statement:
...
[auditing event](sys.md#auditing) `subprocess.Popen` with arguments
...
#### Versionchanged
Changed in version 3.6: Popen destructor now emits a [`ResourceWarning`](exceptions.md#ResourceWarning) warning if the child
process is still running.

#### Versionchanged
Changed in version 3.8: Popen can use [`os.posix_spawn()`](os.md#os.posix_spawn) in some cases for better
performance. On Windows Subsystem for Linux and QEMU User Emulation,
Popen constructor using [`os.posix_spawn()`](os.md#os.posix_spawn) no longer raise an
exception on errors like missing program, but the child process fails
with a non-zero [`returncode`](#subprocess.Popen.returncode).
...
The most common exception raised is [`OSError`](exceptions.md#OSError).  This occurs, for example,
when trying to execute a non-existent file.  Applications should prepare for
[`OSError`](exceptions.md#OSError) exceptions. Note that, when `shell=True`, [`OSError`](exceptions.md#OSError)
...
A [`ValueError`](exceptions.md#ValueError) will be raised if [`Popen`](#subprocess.Popen) is called with invalid
arguments.

[`check_call()`](#subprocess.check_call) and [`check_output()`](#subprocess.check_output) will raise
[`CalledProcessError`](#subprocess.CalledProcessError) if the called process returns a non-zero return
code.

All of the functions and methods that accept a *timeout* parameter, such as
[`run()`](#subprocess.run) and [`Popen.communicate()`](#subprocess.Popen.communicate) will raise [`TimeoutExpired`](#subprocess.TimeoutExpired) if
the timeout expires before the process exits.

Exceptions defined in this module all inherit from [`SubprocessError`](#subprocess.SubprocessError).

#### Versionadded
Added in version 3.3: The [`SubprocessError`](#subprocess.SubprocessError) base class was added.

<a id="subprocess-security"></a>
...
vulnerabilities. On [some platforms](shlex.md#shlex-quote-warning), it is possible
to use [`shlex.quote()`](shlex.md#shlex.quote) for this escaping.
...
Instances of the [`Popen`](#subprocess.Popen) class have the following methods:

#### Popen.poll()

Check if child process has terminated.  Set and return
[`returncode`](#subprocess.Popen.returncode) attribute. Otherwise, returns `None`.

#### Popen.wait(timeout=None)

Wait for child process to terminate.  Set and return
[`returncode`](#subprocess.Popen.returncode) attribute.

If the process does not terminate after *timeout* seconds, raise a
[`TimeoutExpired`](#subprocess.TimeoutExpired) exception.  It is safe to catch this exception and
retry the wait.
...
Use [`Popen.communicate()`](#subprocess.Popen.communicate) when using pipes to avoid that.
...
- Linux >= 5.3 uses [`os.pidfd_open()`](os.md#os.pidfd_open) + [`select.poll()`](select.md#select.poll)
- macOS and other BSD variants use [`select.kqueue()`](select.md#select.kqueue) +
...
#### NOTE
Use the [`asyncio`](asyncio.md#module-asyncio) module for an asynchronous wait: see
[`asyncio.create_subprocess_exec`](asyncio-subprocess.md#asyncio.create_subprocess_exec).

#### Versionchanged
Changed in version 3.3: *timeout* was added.

#### Versionchanged
Changed in version 3.15: if *timeout* is not `None`, use efficient event-driven implementation
...
[`returncode`](#subprocess.Popen.returncode) attribute.  The optional *input* argument should be
...
[`communicate()`](#subprocess.Popen.communicate) returns a tuple `(stdout_data, stderr_data)`.
...
[`TimeoutExpired`](#subprocess.TimeoutExpired) exception will be raised.  Catching this exception and
retrying communication will not lose any output.  Supplying *input* to a
subsequent post-timeout [`communicate()`](#subprocess.Popen.communicate) call is in undefined behavior
...
After a call to [`communicate()`](#subprocess.Popen.communicate) raises [`TimeoutExpired`](#subprocess.TimeoutExpired), do
not call [`wait()`](#subprocess.Popen.wait). Use an additional [`communicate()`](#subprocess.Popen.communicate)
call to finish handling pipes and populate the [`returncode`](#subprocess.Popen.returncode)
attribute.
...
#### NOTE
On Windows, SIGTERM is an alias for [`terminate()`](#subprocess.Popen.terminate). CTRL_C_EVENT and
CTRL_BREAK_EVENT can be sent to processes started with a *creationflags*
parameter which includes `CREATE_NEW_PROCESS_GROUP`.

#### Popen.terminate()

Stop the child. On POSIX OSs the method sends [`SIGTERM`](signal.md#signal.SIGTERM) to the
...
On Windows [`kill()`](#subprocess.Popen.kill) is an alias for [`terminate()`](#subprocess.Popen.terminate).
...
The *args* argument as it was passed to [`Popen`](#subprocess.Popen) – a
...
If the *stdin* argument was [`PIPE`](#subprocess.PIPE), this attribute is a writeable
stream object as returned by [`open()`](functions.md#open). If the *encoding* or *errors*
...
If the *stdin* argument was not [`PIPE`](#subprocess.PIPE), this attribute is `None`.

#### Popen.stdout

If the *stdout* argument was [`PIPE`](#subprocess.PIPE), this attribute is a readable
stream object as returned by [`open()`](functions.md#open). Reading from the stream provides
...
argument was not [`PIPE`](#subprocess.PIPE), this attribute is `None`.

#### Popen.stderr

If the *stderr* argument was [`PIPE`](#subprocess.PIPE), this attribute is a readable
stream object as returned by [`open()`](functions.md#open). Reading from the stream provides
...
was not [`PIPE`](#subprocess.PIPE), this attribute is `None`.

#### WARNING
Use [`communicate()`](#subprocess.Popen.communicate) rather than [`.stdin.write`](#subprocess.Popen.stdin),
[`.stdout.read`](#subprocess.Popen.stdout) or [`.stderr.read`](#subprocess.Popen.stderr) to avoid
...
The child return code. Initially `None`, [`returncode`](#subprocess.Popen.returncode) is set by
a call to the [`poll()`](#subprocess.Popen.poll), [`wait()`](#subprocess.Popen.wait), or [`communicate()`](#subprocess.Popen.communicate) methods
...
The [`STARTUPINFO`](#subprocess.STARTUPINFO) class and following constants are only available
on Windows.
...
structure is used for [`Popen`](#subprocess.Popen) creation.  The following attributes can
...
A bit field that determines whether certain [`STARTUPINFO`](#subprocess.STARTUPINFO)
...
If [`dwFlags`](#subprocess.STARTUPINFO.dwFlags) specifies [`STARTF_USESTDHANDLES`](#subprocess.STARTF_USESTDHANDLES), this attribute
is the standard input handle for the process. If
[`STARTF_USESTDHANDLES`](#subprocess.STARTF_USESTDHANDLES) is not specified, the default for standard
...
If [`dwFlags`](#subprocess.STARTUPINFO.dwFlags) specifies [`STARTF_USESHOWWINDOW`](#subprocess.STARTF_USESHOWWINDOW), this attribute
...
[`SW_HIDE`](#subprocess.SW_HIDE) is provided for this attribute. It is used when
[`Popen`](#subprocess.Popen) is called with `shell=True`.
...
: Sequence of handles that will be inherited. *close_fds* must be true if
  ...
  [`os.set_handle_inheritable()`](os.md#os.set_handle_inheritable) when passed to the [`Popen`](#subprocess.Popen)
  constructor, else [`OSError`](exceptions.md#OSError) will be raised with Windows error
  ...
  all handles such as [`os.system()`](os.md#os.system).  This also applies to
...
Specifies that the [`STARTUPINFO.hStdInput`](#subprocess.STARTUPINFO.hStdInput),
[`STARTUPINFO.hStdOutput`](#subprocess.STARTUPINFO.hStdOutput), and [`STARTUPINFO.hStdError`](#subprocess.STARTUPINFO.hStdError) attributes
contain additional information.

### subprocess.STARTF_USESHOWWINDOW

Specifies that the [`STARTUPINFO.wShowWindow`](#subprocess.STARTUPINFO.wShowWindow) attribute contains
additional information.

### subprocess.STARTF_FORCEONFEEDBACK

A [`STARTUPINFO.dwFlags`](#subprocess.STARTUPINFO.dwFlags) parameter to specify that the
...
A [`STARTUPINFO.dwFlags`](#subprocess.STARTUPINFO.dwFlags) parameter to specify that the mouse
...
A [`Popen`](#subprocess.Popen) `creationflags` parameter to specify that a new process
group will be created. This flag is necessary for using [`os.kill()`](os.md#os.kill)
on the subprocess.

This flag is ignored if [`CREATE_NEW_CONSOLE`](#subprocess.CREATE_NEW_CONSOLE) is specified.
...
<a id="call-function-trio"></a>

## Older high-level API

Prior to Python 3.5, these three functions comprised the high level API to
subprocess. You can now use [`run()`](#subprocess.run) in many cases, but lots of existing code
calls these functions.

### subprocess.call(args, , stdin=None, stdout=None, stderr=None, shell=False, cwd=None, timeout=None, \*\*other_popen_kwargs)

Run the command described by *args*.  Wait for command to complete, then
return the [`returncode`](#subprocess.Popen.returncode) attribute.

Code needing to capture stdout or stderr should use [`run()`](#subprocess.run) instead:

```python3
run(...).returncode
```

To suppress stdout or stderr, supply a value of [`DEVNULL`](#subprocess.DEVNULL).

The arguments shown above are merely some common ones.
The full function signature is the
same as that of the [`Popen`](#subprocess.Popen) constructor - this function passes all
...
code was zero then return, otherwise raise [`CalledProcessError`](#subprocess.CalledProcessError). The
[`CalledProcessError`](#subprocess.CalledProcessError) object will have the return code in the
[`returncode`](#subprocess.CalledProcessError.returncode) attribute.
If [`check_call()`](#subprocess.check_call) was unable to start the process it will propagate the exception
that was raised.
...
If the return code was non-zero it raises a [`CalledProcessError`](#subprocess.CalledProcessError). The
[`CalledProcessError`](#subprocess.CalledProcessError) object will have the return code in the
[`returncode`](#subprocess.CalledProcessError.returncode) attribute and any output in the
[`output`](#subprocess.CalledProcessError.output) attribute.
...
The arguments shown above are merely some common ones.
The full function signature is largely the same as that of [`run()`](#subprocess.run) -
most arguments are passed directly through to that interface.
One API deviation from [`run()`](#subprocess.run) behavior exists: passing `input=None`
...
[Frequently Used Arguments](#frequently-used-arguments) and [`run()`](#subprocess.run).
...
#### Versionchanged
Changed in version 3.6: *encoding* and *errors* were added.  See [`run()`](#subprocess.run) for details.
...
<a id="subprocess-replacements"></a>
...
executed program cannot be found; the “b” replacements raise [`OSError`](exceptions.md#OSError)
instead.

In addition, the replacements using [`check_output()`](#subprocess.check_output) will fail with a
[`CalledProcessError`](#subprocess.CalledProcessError) if the requested operation produces a non-zero
return code. The output is still available as the
[`output`](#subprocess.CalledProcessError.output) attribute of the raised exception.
...
### Replacing [`os.system()`](os.md#os.system)
...
* The [`call()`](#subprocess.call) return value is encoded differently to that of
  [`os.system()`](os.md#os.system).
* The [`os.system()`](os.md#os.system) function ignores SIGINT and SIGQUIT signals while
...
### Replacing the [`os.spawn`](os.md#os.spawnl) family
...
### Replacing [`os.popen()`](os.md#os.popen)
...
Execute the string *cmd* in a shell with [`check_output()`](#subprocess.check_output) and
...
Like [`getstatusoutput()`](#subprocess.getstatusoutput), except the exit code is ignored and the return
...
<a id="subprocess-timeout-behavior"></a>

### Timeout Behavior

When using the `timeout` parameter in functions like [`run()`](#subprocess.run),
[`Popen.wait()`](#subprocess.Popen.wait), or [`Popen.communicate()`](#subprocess.Popen.communicate),
...
2. **Extremely Small Timeout Values**: Setting very small timeout values (such as a few
   milliseconds) may result in almost immediate [`TimeoutExpired`](#subprocess.TimeoutExpired) exceptions because
   process creation and system scheduling inherently require time.

<a id="converting-argument-sequence"></a>
...
#### SEE ALSO
[`shlex`](shlex.md#module-shlex)
: Module which provides function to parse and escape command lines.

<a id="disable-posix-spawn"></a>
