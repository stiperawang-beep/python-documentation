...
**Source code:** [Lib/weakref.py](https://github.com/python/cpython/tree/main/Lib/weakref.py)
...
[garbage collection](../glossary.md#term-garbage-collection) is free to destroy the referent and reuse its memory
...
[`WeakKeyDictionary`](#weakref.WeakKeyDictionary) and [`WeakValueDictionary`](#weakref.WeakValueDictionary) classes supplied by
...
[`WeakValueDictionary`](#weakref.WeakValueDictionary), then when the last remaining references to that
...
[`WeakKeyDictionary`](#weakref.WeakKeyDictionary) and [`WeakValueDictionary`](#weakref.WeakValueDictionary) use weak references
in their implementation, setting up callback functions on the weak references
that notify the weak dictionaries when a key or value has been reclaimed by
garbage collection.  [`WeakSet`](#weakref.WeakSet) implements the [`set`](stdtypes.md#set) interface,
but keeps weak references to its elements, just like a
[`WeakKeyDictionary`](#weakref.WeakKeyDictionary) does.

[`finalize`](#weakref.finalize) provides a straight forward way to register a
...
or [`finalize`](#weakref.finalize) is all they need – it’s not usually necessary to
...
sets, frozensets, some [file objects](../glossary.md#term-file-object), [generators](../glossary.md#term-generator),
...
Several built-in types such as [`list`](stdtypes.md#list) and [`dict`](stdtypes.md#dict) do not directly
...
**CPython implementation detail:** Other built-in types such as [`tuple`](stdtypes.md#tuple) and [`int`](functions.md#int) do not support weak
references even when subclassed.

Extension types can easily be made to support weak references; see
[Weak Reference Support](../extending/newtypes.md#weakref-support).
...
See [\_\_slots_\_ documentation](../reference/datamodel.md#slots) for details.

### *class* weakref.ref(object)
...
no longer alive, calling the reference object will cause [`None`](constants.md#None) to be
returned.  If *callback* is provided and not [`None`](constants.md#None), and the returned
...
raised from an object’s [`__del__()`](../reference/datamodel.md#object.__del__) method.

Weak references are [hashable](../glossary.md#term-hashable) if the *object* is hashable.  They will
maintain their hash value even after the *object* was deleted.  If
[`hash()`](functions.md#hash) is called the first time only after the *object* was deleted,
the call will raise [`TypeError`](exceptions.md#TypeError).
...
#### Versionchanged
Changed in version 3.4: Added the [`__callback__`](#weakref.ref.__callback__) attribute.
...
callable.  Proxy objects are not [hashable](../glossary.md#term-hashable) regardless of the referent; this
avoids a number of problems related to their fundamentally mutable nature, and
prevents their use as dictionary keys.  *callback* is the same as the parameter
of the same name to the [`ref()`](#weakref.ref) function.

Accessing an attribute of the proxy object after the referent is
garbage collected raises [`ReferenceError`](exceptions.md#ReferenceError).
...
[`WeakKeyDictionary`](#weakref.WeakKeyDictionary) objects have an additional method that
...
[`WeakValueDictionary`](#weakref.WeakValueDictionary) objects have an additional method that has the
same issues as the [`WeakKeyDictionary.keyrefs()`](#weakref.WeakKeyDictionary.keyrefs) method.
...
A custom [`ref`](#weakref.ref) subclass which simulates a weak reference to a bound
method (i.e., a method defined on a class and looked up on an instance).
Since a bound method is ephemeral, a standard weak reference cannot keep
hold of it.  [`WeakMethod`](#weakref.WeakMethod) has special code to recreate the bound
...
*callback* is the same as the parameter of the same name to the [`ref()`](#weakref.ref) function.
...
whereas calling a dead finalizer returns [`None`](constants.md#None).

Exceptions raised by finalizer callbacks during garbage collection
will be shown on the standard error output, but cannot be
propagated.  They are handled in the same way as exceptions raised
from an object’s [`__del__()`](../reference/datamodel.md#object.__del__) method or a weak reference’s
callback.

When the program exits, each remaining live finalizer is called
unless its [`atexit`](atexit.md#module-atexit) attribute has been set to false.  They
are called in reverse order of creation.

A finalizer will never invoke its callback during the later part of
the [interpreter shutdown](../glossary.md#term-interpreter-shutdown) when module globals are liable to have
been replaced by [`None`](constants.md#None).
...
kwargs)`.  If *self* is dead then return [`None`](constants.md#None).
...
[`atexit`](#weakref.finalize.atexit) is true.  They are called in reverse order of
creation.
...
<a id="weakref-objects"></a>

## Weak Reference Objects

Weak reference objects have no methods and no attributes besides
[`ref.__callback__`](#weakref.ref.__callback__). A weak reference object allows the referent to be
...
Specialized versions of [`ref`](#weakref.ref) objects can be created through subclassing.
This is used in the implementation of the [`WeakValueDictionary`](#weakref.WeakValueDictionary) to reduce
...
This example shows how a subclass of [`ref`](#weakref.ref) can be used to store
...
<a id="finalize-examples"></a>

## Finalizer Objects

The main benefit of using [`finalize`](#weakref.finalize) is that it makes it simple
...
You can unregister a finalizer using its [`detach()`](#weakref.finalize.detach)
...
Unless you set the [`atexit`](#weakref.finalize.atexit) attribute to
[`False`](constants.md#False), a finalizer will be called when the program exits if it
...
## Comparing finalizers with [`__del__()`](../reference/datamodel.md#object.__del__) methods
...
We might try to implement the class using a [`__del__()`](../reference/datamodel.md#object.__del__) method as
follows:
...
Starting with Python 3.4, [`__del__()`](../reference/datamodel.md#object.__del__) methods no longer prevent
reference cycles from being garbage collected, and module globals are
no longer forced to [`None`](constants.md#None) during [interpreter shutdown](../glossary.md#term-interpreter-shutdown).
So this code should work without any issues on CPython.

However, handling of [`__del__()`](../reference/datamodel.md#object.__del__) methods is notoriously implementation
...
[`atexit.register()`](atexit.md#atexit.register), `try: ... finally: ...` and `with: ...`
