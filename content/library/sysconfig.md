...
**Source code:** [Lib/sysconfig](https://github.com/python/cpython/tree/main/Lib/sysconfig)
...
can be accessed using [`get_config_vars()`](#sysconfig.get_config_vars) or [`get_config_var()`](#sysconfig.get_config_var).
...
unique identifiers based on the value returned by [`os.name`](os.md#os.name).
...
- *posix_user*: scheme for POSIX platforms, when the *user* option is used.
  ...
  ([`site.USER_BASE`](site.md#site.USER_BASE)).
- *posix_venv*: scheme for [`Python virtual environments`](venv.md#module-venv) on POSIX
...
- *nt_venv*: scheme for [`Python virtual environments`](venv.md#module-venv) on Windows;
...
Files will be installed into subdirectories of [`site.USER_BASE`](site.md#site.USER_BASE) (written
as `*userbase*` hereafter).  This scheme installs pure Python modules and
extension modules in the same location (also known as [`site.USER_SITE`](site.md#site.USER_SITE)).
...
<a id="sysconfig-prefix-scheme"></a>
...
The return value is a scheme name listed in [`get_scheme_names()`](#sysconfig.get_scheme_names). It
can be passed to `sysconfig` functions that take a *scheme* argument,
such as [`get_paths()`](#sysconfig.get_paths).
...
End users should not use this function, but [`get_default_scheme()`](#sysconfig.get_default_scheme) and
[`get_preferred_scheme()`](#sysconfig.get_preferred_scheme) instead.
...
*name* has to be a value from the list returned by [`get_path_names()`](#sysconfig.get_path_names).

`sysconfig` stores installation paths corresponding to each path name,
for each platform, with variables to be expanded.  For instance the *stdlib*
path for the *nt* scheme is: `{base}/Lib`.

[`get_path()`](#sysconfig.get_path) will use the variables returned by [`get_config_vars()`](#sysconfig.get_config_vars)
to expand the path.  All variables have default values for each platform so
one may call this function and get the default value.

If *scheme* is provided, it must be a value from the list returned by
[`get_scheme_names()`](#sysconfig.get_scheme_names).  Otherwise, the default scheme for the current
platform is used.

If *vars* is provided, it must be a dictionary of variables that will update
the dictionary returned by [`get_config_vars()`](#sysconfig.get_config_vars).

If *expand* is set to `False`, the path will not be expanded using the
variables.

If *name* is not found, raise a [`KeyError`](exceptions.md#KeyError).

### sysconfig.get_paths()

Return a dictionary containing all installation paths corresponding to an
installation scheme. See [`get_path()`](#sysconfig.get_path) for more information.
...
If *scheme* is not an existing scheme, [`get_paths()`](#sysconfig.get_paths) will raise a
...
version and the architecture (as supplied by [`os.uname()`](os.md#os.uname)), although the
...
For other non-POSIX platforms, currently just returns [`sys.platform`](sys.md#sys.platform).
...
<a id="sysconfig-cli"></a>

<a id="using-sysconfig-as-a-script"></a>
...
[`get_platform()`](#sysconfig.get_platform), [`get_python_version()`](#sysconfig.get_python_version), [`get_path()`](#sysconfig.get_path) and
