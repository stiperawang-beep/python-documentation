...
**Source code:** [Lib/xml/dom/minidom.py](https://github.com/python/cpython/tree/main/Lib/xml/dom/minidom.py)
...
[`xml.etree.ElementTree`](xml.etree.elementtree.md#module-xml.etree.ElementTree) module for their XML processing instead.
...
The [`parse()`](#xml.dom.minidom.parse) function can take either a filename or an open file object.
...
If you have XML in a string, you can use the [`parseString()`](#xml.dom.minidom.parseString) function
instead:

### xml.dom.minidom.parseString(string, parser=None)

Return a `Document` that represents the *string*. This method creates an
[`io.StringIO`](io.md#io.StringIO) object for the string and passes that on to [`parse()`](#xml.dom.minidom.parse).

Both functions return a `Document` object representing the content of the
document.

What the [`parse()`](#xml.dom.minidom.parse) and [`parseString()`](#xml.dom.minidom.parseString) functions do is connect an XML
...
`getDOMImplementation()` function in the [`xml.dom`](xml.dom.md#module-xml.dom) package or the
...
<a id="minidom-objects"></a>

## DOM Objects

The definition of the DOM API for Python is given as part of the [`xml.dom`](xml.dom.md#module-xml.dom)
...
You can avoid calling this method explicitly by using the [`with`](../reference/compound_stmts.md#with)
...
#### Versionchanged
Changed in version 3.8: The [`writexml()`](#xml.dom.minidom.Node.writexml) method now preserves the attribute order specified
by the user.
...
With an explicit *encoding* <sup>[1](#id3)</sup> argument, the result is a byte
...
The *standalone* argument behaves exactly as in [`writexml()`](#xml.dom.minidom.Node.writexml).

#### Versionchanged
Changed in version 3.8: The [`toxml()`](#xml.dom.minidom.Node.toxml) method now preserves the attribute order specified
by the user.
...
[`toxml()`](#xml.dom.minidom.Node.toxml).

The *standalone* argument behaves exactly as in [`writexml()`](#xml.dom.minidom.Node.writexml).

#### Versionchanged
Changed in version 3.8: The [`toprettyxml()`](#xml.dom.minidom.Node.toprettyxml) method now preserves the attribute order specified
by the user.
...
* Operations are used as methods. Since the DOM uses only [`in`](../reference/expressions.md#in)
...
* `DOMException` is currently not supported in `xml.dom.minidom`.
  ...
  [`TypeError`](exceptions.md#TypeError) and [`AttributeError`](exceptions.md#AttributeError).
...
* <a id='id3'>**[1]**</a> The encoding name included in the XML output should conform to the appropriate standards. For example, “UTF-8” is valid, but “UTF8” is not valid in an XML document’s declaration, even though Python accepts it as an encoding name. See [https://www.w3.org/TR/2006/REC-xml11-20060816/#NT-EncodingDecl](https://www.w3.org/TR/2006/REC-xml11-20060816/#NT-EncodingDecl) and [https://www.iana.org/assignments/character-sets/character-sets.xhtml](https://www.iana.org/assignments/character-sets/character-sets.xhtml).
