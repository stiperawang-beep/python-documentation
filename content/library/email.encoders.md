...
**Source code:** [Lib/email/encoders.py](https://github.com/python/cpython/tree/main/Lib/email/encoders.py)

---

This module is part of the legacy (`Compat32`) email API.  In the
new API the functionality is provided by the *cte* parameter of
the [`set_content()`](email.message.md#email.message.EmailMessage.set_content) method.

This module is deprecated in Python 3.  The functions provided here
should not be called explicitly since the [`MIMEText`](email.mime.md#email.mime.text.MIMEText)
...
When creating [`Message`](email.compat32-message.md#email.message.Message) objects from scratch, you often
need to encode the payloads for transport through compliant mail servers. This
is especially true for *image/\** and *text/\** type messages
containing binary data.

The [`email`](email.md#module-email) package provides some convenient encoders in its
`encoders` module.  These encoders are actually used by the
[`MIMEAudio`](email.mime.md#email.mime.audio.MIMEAudio) and [`MIMEImage`](email.mime.md#email.mime.image.MIMEImage)
...
[`TypeError`](exceptions.md#TypeError) if passed a message whose type is multipart.
...
*Content-Transfer-Encoding* header to `quoted-printable` <sup>[1](#id2)</sup>.
...
* <a id='id2'>**[1]**</a> Note that encoding with [`encode_quopri()`](#email.encoders.encode_quopri) also encodes all tabs and space characters in the data.
