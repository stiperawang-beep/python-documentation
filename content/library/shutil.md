...
**Source code:** [Lib/shutil.py](https://github.com/python/cpython/tree/main/Lib/shutil.py)
...
[`os`](os.md#module-os) module.

#### WARNING
Even the higher-level file copying functions ([`shutil.copy()`](#shutil.copy),
[`shutil.copy2()`](#shutil.copy2)) cannot copy all file metadata.
...
Copy the contents of the [file-like object](../glossary.md#term-file-object) *fsrc* to the file-like object *fdst*.
...
[`copyfileobj()`](#shutil.copyfileobj) will *not* guarantee that the destination stream has
...
you must ensure that you have called [`flush()`](io.md#io.IOBase.flush) or
[`close()`](io.md#io.IOBase.close) on the file-like object before attempting to read
...
*src* and *dst* are [path-like objects](../glossary.md#term-path-like-object) or path names given as strings.

*dst* must be the complete target file name; look at [`copy()`](#shutil.copy)
for a copy that accepts a target directory path.  If *src* and *dst*
specify the same file, [`SameFileError`](#shutil.SameFileError) is raised.

The destination location must be writable; otherwise, an [`OSError`](exceptions.md#OSError)
...
Raises an [auditing event](sys.md#auditing) `shutil.copyfile` with arguments `src`, `dst`.

#### Versionchanged
Changed in version 3.3: [`IOError`](exceptions.md#IOError) used to be raised instead of [`OSError`](exceptions.md#OSError).
Added *follow_symlinks* argument.
Now returns *dst*.

#### Versionchanged
Changed in version 3.4: Raise [`SameFileError`](#shutil.SameFileError) instead of [`Error`](#shutil.Error).  Since the former is
...
This exception is raised when [`copyfile()`](#shutil.copyfile) or [`copytree()`](#shutil.copytree) attempt
...
This exception is raised if source and destination in [`copyfile()`](#shutil.copyfile)
are the same file.
...
group are unaffected.  *src* and *dst* are [path-like objects](../glossary.md#term-path-like-object) or path names
given as strings.
If *follow_symlinks* is false, and both *src* and *dst* are symbolic links,
[`copymode()`](#shutil.copymode) will attempt to modify the mode of *dst* itself (rather
than the file it points to).  This functionality is not available on every
platform; please see [`copystat()`](#shutil.copystat) for more information.  If
[`copymode()`](#shutil.copymode) cannot modify symbolic links on the local platform, and it
is asked to do so, it will do nothing and return.

Raises an [auditing event](sys.md#auditing) `shutil.copymode` with arguments `src`, `dst`.
...
flags from *src* to *dst*.  On Linux, [`copystat()`](#shutil.copystat) also copies the
“extended attributes” where possible.  The file contents, owner, and
group are unaffected.  *src* and *dst* are [path-like objects](../glossary.md#term-path-like-object) or path
names given as strings.

If *follow_symlinks* is false, and *src* and *dst* both
refer to symbolic links, [`copystat()`](#shutil.copystat) will operate on
...
* If `os.chmod in os.supports_follow_symlinks` is
  `True`, [`copystat()`](#shutil.copystat) can modify the permission
  bits of a symbolic link.
* If `os.utime in os.supports_follow_symlinks` is
  `True`, [`copystat()`](#shutil.copystat) can modify the last access
  and modification times of a symbolic link.
* If `os.chflags in os.supports_follow_symlinks` is
  `True`, [`copystat()`](#shutil.copystat) can modify the flags of
...
[`copystat()`](#shutil.copystat) will copy everything it can.
[`copystat()`](#shutil.copystat) never returns failure.

Please see [`os.supports_follow_symlinks`](os.md#os.supports_follow_symlinks)
for more information.

Raises an [auditing event](sys.md#auditing) `shutil.copystat` with arguments `src`, `dst`.
...
should be [path-like objects](../glossary.md#term-path-like-object) or strings.  If
...
[`copy()`](#shutil.copy) copies the file data and the file’s permission
mode (see [`os.chmod()`](os.md#os.chmod)).  Other metadata, like the
file’s creation and modification times, is not preserved.
To preserve all file metadata from the original, use
[`copy2()`](#shutil.copy2) instead.
...
Identical to [`copy()`](#shutil.copy) except that [`copy2()`](#shutil.copy2)
also attempts to preserve file metadata.

When *follow_symlinks* is false, and *src* is a symbolic
link, [`copy2()`](#shutil.copy2) attempts to copy all metadata from the
*src* symbolic link to the newly created *dst* symbolic link.
However, this functionality is not available on all platforms.
On platforms where some or all of this functionality is
unavailable, [`copy2()`](#shutil.copy2) will preserve all the metadata
it can; [`copy2()`](#shutil.copy2) never raises an exception because it
cannot preserve file metadata.

[`copy2()`](#shutil.copy2) uses [`copystat()`](#shutil.copystat) to copy the file metadata.
Please see [`copystat()`](#shutil.copystat) for more information
...
[`copytree()`](#shutil.copytree)'s *ignore* argument, ignoring files and directories that
...
Permissions and times of directories are copied with [`copystat()`](#shutil.copystat),
individual files are copied using [`copy2()`](#shutil.copy2).
...
an [`Error`](#shutil.Error) exception at the end of the copy process.
You can set the optional *ignore_dangling_symlinks* flag to true if you
want to silence this exception. Notice that this option has no effect
on platforms that don’t support [`os.symlink()`](os.md#os.symlink).

If *ignore* is given, it must be a callable that will receive as its
arguments the directory being visited by [`copytree()`](#shutil.copytree), and a list of its
contents, as returned by [`os.listdir()`](os.md#os.listdir).  Since [`copytree()`](#shutil.copytree) is
called recursively, the *ignore* callable will be called once for each
directory that is copied.  The callable must return a sequence of directory
and file names relative to the current directory (i.e. a subset of the items
in its second argument); these names will then be ignored in the copy
process.  [`ignore_patterns()`](#shutil.ignore_patterns) can be used to create such a callable that
ignores names based on glob-style patterns.

If exception(s) occur, an [`Error`](#shutil.Error) is raised with a list of reasons.

If *copy_function* is given, it must be a callable that will be used to copy
each file. It will be called with the source path and the destination path
as arguments. By default, [`copy2()`](#shutil.copy2) is used, but any function
that supports the same signature (like [`copy()`](#shutil.copy)) can be used.

If *dirs_exist_ok* is false (the default) and *dst* already exists, a
[`FileExistsError`](exceptions.md#FileExistsError) is raised. If *dirs_exist_ok* is true, the copying
operation will continue if it encounters existing directories, and files
within the *dst* tree will be overwritten by corresponding files from the
*src* tree.

Raises an [auditing event](sys.md#auditing) `shutil.copytree` with arguments `src`, `dst`.
...
attack resistant version of [`rmtree()`](#shutil.rmtree) is used by default.  On other
platforms, the [`rmtree()`](#shutil.rmtree) implementation is susceptible to a symlink
...
otherwise.  Applications can use the [`rmtree.avoids_symlink_attacks`](#shutil.rmtree.avoids_symlink_attacks)
...
parameter it receives is the tuple returned from [`sys.exc_info()`](sys.md#sys.exc_info).
...
Raises an [auditing event](sys.md#auditing) `shutil.rmtree` with arguments `path`, `dir_fd`.
...
#### Versionchanged
Changed in version 3.13: `rmtree()` now ignores [`FileNotFoundError`](exceptions.md#FileNotFoundError) exceptions for all
but the top-level path.
Exceptions other than [`OSError`](exceptions.md#OSError) and subclasses of `OSError`
are now always propagated to the caller.

#### rmtree.avoids_symlink_attacks

Indicates whether the current platform and implementation provides a
symlink attack resistant version of [`rmtree()`](#shutil.rmtree).  Currently this is
...
depending on [`os.rename()`](os.md#os.rename) semantics.

If the destination is on the current filesystem, then [`os.rename()`](os.md#os.rename) is
...
if [`os.rename()`](os.md#os.rename) cannot be used.  If the source is a directory,
[`copytree()`](#shutil.copytree) is called, passing it the *copy_function*. The
default *copy_function* is [`copy2()`](#shutil.copy2).  Using [`copy()`](#shutil.copy) as the
*copy_function* allows the move to succeed when it is not possible to also
copy the metadata, at the expense of not copying any of the metadata.

Raises an [auditing event](sys.md#auditing) `shutil.move` with arguments `src`, `dst`.
...
#### Versionchanged
Changed in version 3.9: Accepts a [path-like object](../glossary.md#term-path-like-object) for both *src* and *dst*.

### shutil.disk_usage(path)

Return disk usage statistics about the given path as a [named tuple](../glossary.md#term-named-tuple)
...
See also [`os.chown()`](os.md#os.chown), the underlying function.

Raises an [auditing event](sys.md#auditing) `shutil.chown` with arguments `path`, `user`, `group`.
...
*mode* is a permission mask passed to [`os.access()`](os.md#os.access), by default
determining if the file exists and is executable.

*path* is a “`PATH` string” specifying the directories to look in,
delimited by [`os.pathsep`](os.md#os.pathsep). When no *path* is specified, the
`PATH` environment variable is read from [`os.environ`](os.md#os.environ),
falling back to [`os.defpath`](os.md#os.defpath) if it is not set.
...
if you call `shutil.which("python")`, [`which()`](#shutil.which) will search `PATHEXT`
...
#### Versionchanged
Changed in version 3.8: The [`bytes`](stdtypes.md#bytes) type is now accepted.  If *cmd* type is
[`bytes`](stdtypes.md#bytes), the result type is also [`bytes`](stdtypes.md#bytes).
...
Subclass of [`OSError`](exceptions.md#OSError) collecting exceptions raised during a multi-file
operation. For [`copytree()`](#shutil.copytree), the exception argument is a list of 3-tuples
(*srcname*, *dstname*, *exception*).

<a id="shutil-platform-dependent-efficient-copy-operations"></a>

### Platform-dependent efficient copy operations

Starting from Python 3.8, all functions involving a file copy
([`copyfile()`](#shutil.copyfile), [`copy()`](#shutil.copy), [`copy2()`](#shutil.copy2),
[`copytree()`](#shutil.copytree), and [`move()`](#shutil.move)) may use
...
On Linux [`os.copy_file_range()`](os.md#os.copy_file_range) or [`os.sendfile()`](os.md#os.sendfile) is used.

On Solaris [`os.sendfile()`](os.md#os.sendfile) is used.

On Windows [`shutil.copyfile()`](#shutil.copyfile) uses a bigger default buffer size (1 MiB
instead of 64 KiB) and a [`memoryview()`](stdtypes.md#memoryview)-based variant of
[`shutil.copyfileobj()`](#shutil.copyfileobj) is used, which still reads and writes in a loop.
[`shutil.copy2()`](#shutil.copy2) uses the native `CopyFile2` call on Windows, which is the most
...
[`copyfileobj()`](#shutil.copyfileobj) function internally.

#### Versionchanged
Changed in version 3.8.

#### Versionchanged
Changed in version 3.14: Solaris now uses [`os.sendfile()`](os.md#os.sendfile).

#### Versionchanged
Changed in version 3.14: Copy-on-write or server-side copy may be used internally via
[`os.copy_file_range()`](os.md#os.copy_file_range) on supported Linux filesystems.

<a id="shutil-copytree-example"></a>

### copytree example

An example that uses the [`ignore_patterns()`](#shutil.ignore_patterns) helper:
...
provided.  They rely on the [`zipfile`](zipfile.md#module-zipfile) and [`tarfile`](tarfile.md#module-tarfile) modules.

### shutil.make_archive(base_name, format)

Create an archive file (such as zip or tar) and return its name.

*base_name* is a string or [path-like object](../glossary.md#term-path-like-object) specifying the name of
the file to create, including the path, minus any format-specific extension.

*format* is the archive format: one of
“zip” (if the [`zlib`](zlib.md#module-zlib) module is available), “tar”, “gztar” (if the
[`zlib`](zlib.md#module-zlib) module is available), “bztar” (if the [`bz2`](bz2.md#module-bz2) module is
available), “xztar” (if the [`lzma`](lzma.md#module-lzma) module is available), or “zstdtar”
(if the [`compression.zstd`](compression.zstd.md#module-compression.zstd) module is available).

*root_dir* is a string or [path-like object](../glossary.md#term-path-like-object) specifying a directory
...
*base_dir* is a string or [path-like object](../glossary.md#term-path-like-object) specifying a directory
...
to *root_dir*.  See [Archiving example with base_dir](#shutil-archiving-example-with-basedir) for how to
...
[`logging.Logger`](logging.md#logging.Logger).

The *verbose* argument is unused and deprecated.

Raises an [auditing event](sys.md#auditing) `shutil.make_archive` with arguments `base_name`, `format`, `root_dir`, `base_dir`.

#### NOTE
This function is not thread-safe when custom archivers registered
with [`register_archive_format()`](#shutil.register_archive_format) do not support the *root_dir*
...
#### Versionchanged
Changed in version 3.15: Accepts a [path-like object](../glossary.md#term-path-like-object) for *base_name*, *root_dir* and
*base_dir*.
...
- *zip*: ZIP file (if the [`zlib`](zlib.md#module-zlib) module is available).
- *tar*: Uncompressed tar file. Uses POSIX.1-2001 pax format for new archives.
- *gztar*: gzip’ed tar-file (if the [`zlib`](zlib.md#module-zlib) module is available).
- *bztar*: bzip2’ed tar-file (if the [`bz2`](bz2.md#module-bz2) module is available).
- *xztar*: xz’ed tar-file (if the [`lzma`](lzma.md#module-lzma) module is available).
- *zstdtar*: Zstandard compressed tar-file (if the [`compression.zstd`](compression.zstd.md#module-compression.zstd)
  module is available).

You can register new formats or provide your own archiver for any existing
formats, by using [`register_archive_format()`](#shutil.register_archive_format).
...
*base_dir* (which defaults to [`os.curdir`](os.md#os.curdir)) to start archiving from.
Further arguments are passed as keyword arguments: *owner*, *group*,
*dry_run* and *logger* (as passed in [`make_archive()`](#shutil.make_archive)).
...
In this case [`make_archive()`](#shutil.make_archive) is not thread-safe.

If given, *extra_args* is a sequence of `(name, value)` pairs that will be
used as extra keywords arguments when the archiver callable is used.

*description* is used by [`get_archive_formats()`](#shutil.get_archive_formats) which returns the
...
[`register_unpack_format()`](#shutil.register_unpack_format).  If not provided, [`unpack_archive()`](#shutil.unpack_archive)
will use the archive file name extension and see if an unpacker was
registered for that extension.  In case none is found,
a [`ValueError`](exceptions.md#ValueError) is raised.
...
Raises an [auditing event](sys.md#auditing) `shutil.unpack_archive` with arguments `filename`, `extract_dir`, `format`.
...
Read the [Hints for further verification](tarfile.md#tarfile-further-verification)
section for tar-specific details.

#### Versionchanged
Changed in version 3.7: Accepts a [path-like object](../glossary.md#term-path-like-object) for *filename* and *extract_dir*.
...
by the [`get_unpack_formats()`](#shutil.get_unpack_formats) function.
...
formats, by using [`register_unpack_format()`](#shutil.register_unpack_format).

<a id="shutil-archiving-example"></a>
...
<a id="shutil-archiving-example-with-basedir"></a>
...
<a id="shutil-archiving-example"></a>
,
we show how to use [`make_archive()`](#shutil.make_archive), but this time with the usage of
...
the terminal connected to [`sys.__stdout__`](sys.md#sys.__stdout__) is queried
by invoking [`os.get_terminal_size()`](os.md#os.get_terminal_size).
...
The value returned is a named tuple of type [`os.terminal_size`](os.md#os.terminal_size).
...
#### Versionchanged
Changed in version 3.11: The `fallback` values are also used if [`os.get_terminal_size()`](os.md#os.get_terminal_size)
returns zeroes.
