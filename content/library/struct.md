...
<a id="module-struct"></a>

**Source code:** [Lib/struct.py](https://github.com/python/cpython/tree/main/Lib/struct.py)
...
as Python [`bytes`](stdtypes.md#bytes) objects.  Compact [format strings](#struct-format-strings)
...
Several `struct` functions (and methods of [`Struct`](#struct.Struct)) take a *buffer*
argument.  This refers to objects that implement the [Buffer Protocol](../c-api/buffer.md#bufferobjects) and
provide either a readable or read-writable buffer.  The most common types used
for that purpose are [`bytes`](stdtypes.md#bytes) and [`bytearray`](stdtypes.md#bytearray), but many other types
that can be viewed as an array of bytes implement the buffer protocol, so that
they can be read/filled without additional copying from a [`bytes`](stdtypes.md#bytes) object.
...
size required by the format, as reflected by [`calcsize()`](#struct.calcsize).
...
least the size required by the format, as reflected by [`calcsize()`](#struct.calcsize).
...
required by the format, as reflected by [`calcsize()`](#struct.calcsize).
...
Use [`sys.byteorder`](sys.md#sys.byteorder) to check the endianness of your system.
...
#### SEE ALSO
The [`array`](array.md#module-array) and [ctypes](ctypes.md#ctypes-fundamental-data-types) modules,
...
2. When attempting to pack a non-integer using any of the integer conversion
   codes, if the non-integer has a [`__index__()`](../reference/datamodel.md#object.__index__) method then that method is
   called to convert the argument to an integer before packing.

   #### Versionchanged
   Changed in version 3.2: Added use of the [`__index__()`](../reference/datamodel.md#object.__index__) method for non-integers.
...
6. The IEEE 754 binary16 “half precision” type was introduced in the 2008
   ...
   it’s available as  type, if the compiler supports the Annex H
...
8. The `'p'` format character encodes a “Pascal string”, meaning a short
   ...
   [`pack()`](#struct.pack) is too long (longer than the count minus 1), only the leading
   `count-1` bytes of the string are stored.  If the string is shorter than
   `count-1`, it is padded with null bytes so that exactly count bytes in all
   are used.  Note that for [`unpack()`](#struct.unpack), the `'p'` format character consumes
...
10. For the `'F'` and `'D'` format characters, the packed representation uses
...
then [`struct.error`](#struct.error) is raised.

#### Versionchanged
Changed in version 3.1: Previously, some of the integer formats wrapped out-of-range values and
raised [`DeprecationWarning`](exceptions.md#DeprecationWarning) instead of [`struct.error`](#struct.error).

<a id="index-3"></a>

For the `'?'` format character, the return value is either [`True`](constants.md#True) or
[`False`](constants.md#False). When packing, the truth value of the argument object is used.
Either 0 or 1 in the native or standard bool representation will be packed, and
any non-zero value will be `True` when unpacking.

<a id="struct-examples"></a>
...
#### SEE ALSO
Module [`array`](array.md#module-array)
...
<a id="struct-standard-formats"></a>
...
format strings needn’t worry about reusing a single [`Struct`](#struct.Struct)
instance.
...
Identical to the [`pack()`](#struct.pack) function, using the compiled format.
(`len(result)` will equal [`size`](#struct.Struct.size).)

#### pack_into(buffer, offset, v1, v2, ...)

Identical to the [`pack_into()`](#struct.pack_into) function, using the compiled format.

#### unpack(buffer)

Identical to the [`unpack()`](#struct.unpack) function, using the compiled format.
The buffer’s size in bytes must equal [`size`](#struct.Struct.size).

#### unpack_from(buffer, offset=0)

Identical to the [`unpack_from()`](#struct.unpack_from) function, using the compiled format.
The buffer’s size in bytes, starting at position *offset*, must be at least
[`size`](#struct.Struct.size).

#### iter_unpack(buffer)

Identical to the [`iter_unpack()`](#struct.iter_unpack) function, using the compiled format.
The buffer’s size in bytes must be a multiple of [`size`](#struct.Struct.size).
...
#### Versionchanged
Changed in version 3.7: The format string type is now [`str`](stdtypes.md#str) instead of [`bytes`](stdtypes.md#bytes).

#### size

The calculated size of the struct (and hence of the bytes object produced
by the [`pack()`](#struct.pack) method) corresponding to [`format`](functions.md#format).
