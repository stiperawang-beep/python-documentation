...
**Source code:** [Lib/gettext.py](https://github.com/python/cpython/tree/main/Lib/gettext.py)
...
Like [`gettext()`](#gettext.gettext), but look the message up in the specified *domain*.

### gettext.ngettext(singular, plural, n)

Like [`gettext()`](#gettext.gettext), but consider plural forms. If a translation is found,
...
Like [`ngettext()`](#gettext.ngettext), but look the message up in the specified *domain*.
...
is, [`gettext()`](#module-gettext), [`dgettext()`](#gettext.dgettext), [`ngettext()`](#gettext.ngettext), [`dngettext()`](#gettext.dngettext)),
...
a [`GNUTranslations`](#gettext.GNUTranslations) class which implements the parsing of GNU `.mo` format
...
takes a *domain*, identical to what [`textdomain()`](#gettext.textdomain) takes.  Optional
*localedir* is as in [`bindtextdomain()`](#gettext.bindtextdomain). Optional *languages* is a list of
strings, where each string is a language code.

If *localedir* is not given, then the default system locale directory is used.
<sup>[2](#id4)</sup>  If *languages* is not given, then the following environment variables are
...
[`find()`](#gettext.find) then expands and normalizes the languages, and then iterates
through them, searching for an existing file built of these components:

`*localedir*/*language*/LC_MESSAGES/*domain*.mo`

The first such file name that exists is returned by [`find()`](#gettext.find). If no such
...
and *languages*, which are first passed to [`find()`](#gettext.find) to get a list of the
associated `.mo` file paths.  Instances with identical `.mo` file
names are cached.  The actual class instantiated is *class_* if
provided, otherwise [`GNUTranslations`](#gettext.GNUTranslations).  The class’s constructor must
take a single [file object](../glossary.md#term-file-object) argument.

If multiple files are found, later files are used as fallbacks for earlier ones.
To allow setting the fallback, [`copy.copy()`](copy.md#copy.copy) is used to clone each
translation object from the cache; the actual instance data is still shared with
the cache.

If no `.mo` file is found, this function raises [`OSError`](exceptions.md#OSError) if
*fallback* is false (which is the default), and returns a
[`NullTranslations`](#gettext.NullTranslations) instance if *fallback* is true.
...
*domain* and *localedir* which are passed to the function [`translation()`](#gettext.translation).

For the *names* parameter, please see the description of the translation
object’s [`install()`](#gettext.NullTranslations.install) method.
...
### The [`NullTranslations`](#gettext.NullTranslations) class

Translation classes are what actually implement the translation of original
source file message strings to translated message strings. The base class used
by all translation classes is [`NullTranslations`](#gettext.NullTranslations); this provides the basic
interface you can use to write your own specialized translation classes.  Here
are the methods of `NullTranslations`:

### *class* gettext.NullTranslations(fp=None)

Takes an optional [file object](../glossary.md#term-file-object) *fp*, which is ignored by the base class.
Initializes “protected” instance variables  *\_info* and  *\_charset* which are set
by derived classes, as well as  *\_fallback*, which is set through
[`add_fallback()`](#gettext.NullTranslations.add_fallback).  It then calls `self._parse(fp)` if *fp* is not
`None`.
...
If a fallback has been set, forward [`pgettext()`](#gettext.pgettext) to the fallback.
...
If a fallback has been set, forward [`npgettext()`](#gettext.npgettext) to the fallback.
Otherwise, return the translated message.  Overridden in derived classes.
...
This method installs [`gettext()`](#gettext.NullTranslations.gettext) into the built-in namespace,
binding it to `_`.
...
### The [`GNUTranslations`](#gettext.GNUTranslations) class

The `gettext` module provides one additional class derived from
[`NullTranslations`](#gettext.NullTranslations): [`GNUTranslations`](#gettext.GNUTranslations).  This class overrides
`_parse()` to enable reading GNU **gettext** format `.mo` files
in both big-endian and little-endian format.

[`GNUTranslations`](#gettext.GNUTranslations) parses optional metadata out of the translation
...
[`GNUTranslations`](#gettext.GNUTranslations) class can raise [`OSError`](exceptions.md#OSError).
...
fallback’s [`gettext()`](#gettext.NullTranslations.gettext) method.  Otherwise, the
...
the request is forwarded to the fallback’s [`ngettext()`](#gettext.NullTranslations.ngettext)
...
[`pgettext()`](#gettext.pgettext) method.  Otherwise, the *message* id is returned.
...
[`npgettext()`](#gettext.npgettext) method.  Otherwise, when *n* is 1 *singular* is
...
alias for the [`translation()`](#gettext.translation) function described above.
...
it in `_('...')` — that is, a call to the function [`_`](#module-gettext).  For example:
...
[`install()`](#gettext.install) function:
...
of `_()` in the built-in namespace (until the [`del`](../reference/simple_stmts.md#del) command). Take
...
* <a id='id3'>**[1]**</a> The default locale directory is system dependent; for example, on Red Hat Linux it is `/usr/share/locale`, but on Solaris it is `/usr/lib/locale`. The `gettext` module does not try to support these system dependent defaults; instead its default is `*sys.base_prefix*/share/locale` (see [`sys.base_prefix`](sys.md#sys.base_prefix)). For this reason, it is always best to call [`bindtextdomain()`](#gettext.bindtextdomain) with an explicit absolute path at the start of your application.
* <a id='id4'>**[2]**</a> See the footnote for [`bindtextdomain()`](#gettext.bindtextdomain) above.
