...
**Source code:** [Lib/http/_\_init_\_.py](https://github.com/python/cpython/tree/main/Lib/http/__init__.py)
...
* [`http.client`](http.client.md#module-http.client) is a low-level HTTP protocol client; for high-level URL
  opening use [`urllib.request`](urllib.request.md#module-urllib.request)
* [`http.server`](http.server.md#module-http.server) contains basic HTTP server classes based on [`socketserver`](socketserver.md#module-socketserver)
* [`http.cookies`](http.cookies.md#module-http.cookies) has utilities for implementing state management with cookies
* [`http.cookiejar`](http.cookiejar.md#module-http.cookiejar) provides persistence of cookies
...
A subclass of [`enum.IntEnum`](enum.md#enum.IntEnum) that defines a set of HTTP status codes,
...
Supported,
[IANA-registered status codes](https://www.iana.org/assignments/http-status-codes/http-status-codes.xhtml)
available in [`http.HTTPStatus`](#http.HTTPStatus) are:
...
in the [`http.client`](http.client.md#module-http.client) module in the form of constants. The enum name is
...
#### Versionchanged
Changed in version 3.13: Implemented RFC9110 naming for status constants. Old constant names are preserved for
...
A subclass of [`enum.StrEnum`](enum.md#enum.StrEnum) that defines a set of HTTP methods and descriptions written in English.
...
available in [`http.HTTPMethod`](#http.HTTPMethod) are:
