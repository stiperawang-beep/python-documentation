...
after a timeout, or on a user signal. Call [`faulthandler.enable()`](#faulthandler.enable) to
install fault handlers for the [`SIGSEGV`](signal.md#signal.SIGSEGV),
[`SIGFPE`](signal.md#signal.SIGFPE), [`SIGABRT`](signal.md#signal.SIGABRT), [`SIGBUS`](signal.md#signal.SIGBUS), and
[`SIGILL`](signal.md#signal.SIGILL) signals. You can also
enable them at startup by setting the [`PYTHONFAULTHANDLER`](../using/cmdline.md#envvar-PYTHONFAULTHANDLER) environment
variable or by using the [`-X`](../using/cmdline.md#cmdoption-X) `faulthandler` command line option.
...
By default, the Python traceback is written to [`sys.stderr`](sys.md#sys.stderr). To see
tracebacks, applications must be run in the terminal. A log file can
alternatively be passed to [`faulthandler.enable()`](#faulthandler.enable).

The module is implemented in C, so tracebacks can be dumped on a crash or when
Python is deadlocked.

The [Python Development Mode](devmode.md#devmode) calls [`faulthandler.enable()`](#faulthandler.enable)
at Python startup.
...
#### SEE ALSO
[`traceback.print_tb()`](traceback.md#traceback.print_tb), which can be used to print a traceback object.
...
<a id="c-stack-compatibility"></a>

### C Stack Compatibility

If the system does not support the C-level 
or , then C stack dumps will not work.
An error will be printed instead of the stack.

Additionally, some compilers do not support [CPython’s](../glossary.md#term-CPython)
...
Enable the fault handler: install handlers for the [`SIGSEGV`](signal.md#signal.SIGSEGV),
[`SIGFPE`](signal.md#signal.SIGFPE), [`SIGABRT`](signal.md#signal.SIGABRT), [`SIGBUS`](signal.md#signal.SIGBUS)
and [`SIGILL`](signal.md#signal.SIGILL)
...
traceback, unless the system does not support it. See [`dump_c_stack()`](#faulthandler.dump_c_stack) for
...
#### Versionchanged
Changed in version 3.14: Only the current thread is dumped if the [GIL](../glossary.md#term-GIL) is disabled to
...
[`cancel_dump_traceback_later()`](#faulthandler.cancel_dump_traceback_later) is called: see [issue with file
...
Cancel the last call to [`dump_traceback_later()`](#faulthandler.dump_traceback_later).
...
[`unregister()`](#faulthandler.unregister): see [issue with file descriptors](#faulthandler-fd).
...
installed by [`register()`](#faulthandler.register). Return `True` if the signal was registered,
`False` otherwise.

Not available on Windows.

<a id="faulthandler-fd"></a>

## Issue with file descriptors

[`enable()`](#faulthandler.enable), [`dump_traceback_later()`](#faulthandler.dump_traceback_later) and [`register()`](#faulthandler.register) keep the
file descriptor of their *file* argument. If the file is closed and its file
descriptor is reused by a new file, or if [`os.dup2()`](os.md#os.dup2) is used to replace
