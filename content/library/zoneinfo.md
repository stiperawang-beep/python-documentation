...
**Source code:** [Lib/zoneinfo](https://github.com/python/cpython/tree/main/Lib/zoneinfo)
...
: Provides the [`time`](datetime.md#datetime.time) and [`datetime`](datetime.md#datetime.datetime)
  types with which the [`ZoneInfo`](#zoneinfo.ZoneInfo) class is designed to be used.
...
[`ZoneInfo`](#zoneinfo.ZoneInfo) is a concrete implementation of the [`datetime.tzinfo`](datetime.md#datetime.tzinfo)
abstract base class, and is intended to be attached to `tzinfo`, either via
the constructor, the [`datetime.replace`](datetime.md#datetime.datetime.replace)
method or [`datetime.astimezone`](datetime.md#datetime.datetime.astimezone):
...
These time zones also support the [`fold`](datetime.md#datetime.datetime.fold) attribute
...
tzdata are available, all calls to [`ZoneInfo`](#zoneinfo.ZoneInfo) will raise
[`ZoneInfoNotFoundError`](#zoneinfo.ZoneInfoNotFoundError).

<a id="zoneinfo-data-configuration"></a>

### Configuring the data sources

When `ZoneInfo(key)` is called, the constructor first searches the
directories specified in [`TZPATH`](#zoneinfo.TZPATH) for a file matching `key`, and on
failure looks for a match in the tzdata package. This behavior can be
configured in three ways:

1. The default [`TZPATH`](#zoneinfo.TZPATH) when not otherwise specified can be configured at
   [compile time](#zoneinfo-data-compile-time-config).
2. [`TZPATH`](#zoneinfo.TZPATH) can be configured using [an environment variable](#zoneinfo-data-environment-var).
3. At [runtime](#zoneinfo-data-runtime-config), the search path can be
   manipulated using the [`reset_tzpath()`](#zoneinfo.reset_tzpath) function.

<a id="zoneinfo-data-compile-time-config"></a>

#### Compile-time configuration

The default [`TZPATH`](#zoneinfo.TZPATH) includes several common deployment locations for the
...
flag --with-tzpath`](../using/configure.md#cmdoption-with-tzpath)), which should be a string delimited by
[`os.pathsep`](os.md#os.pathsep).

On all platforms, the configured value is available as the `TZPATH` key in
[`sysconfig.get_config_var()`](sysconfig.md#sysconfig.get_config_var).

<a id="zoneinfo-data-environment-var"></a>

#### Environment configuration

When initializing [`TZPATH`](#zoneinfo.TZPATH) (either at import time or whenever
[`reset_tzpath()`](#zoneinfo.reset_tzpath) is called with no arguments), the `zoneinfo` module will
use the environment variable `PYTHONTZPATH`, if it exists, to set the search
path.

### PYTHONTZPATH

This is an [`os.pathsep`](os.md#os.pathsep)-separated string containing the time zone
...
implementation-defined; CPython will raise [`InvalidTZPathWarning`](#zoneinfo.InvalidTZPathWarning), but
...
<a id="zoneinfo-data-runtime-config"></a>

#### Runtime configuration

The TZ search path can also be configured at runtime using the
[`reset_tzpath()`](#zoneinfo.reset_tzpath) function. This is generally not an advisable operation,
...
A concrete [`datetime.tzinfo`](datetime.md#datetime.tzinfo) subclass that represents an IANA time
...
cache invalidation via [`ZoneInfo.clear_cache()`](#zoneinfo.ZoneInfo.clear_cache), for all values of
...
up-level references. The constructor will raise [`ValueError`](exceptions.md#ValueError) if a
...
(e.g. a file opened in binary mode or an [`io.BytesIO`](io.md#io.BytesIO) object).
...
[`ValueError`](exceptions.md#ValueError) is raised if the data read from *file_obj* is not a valid
TZif file.
...
This is a read-only [attribute](../glossary.md#term-attribute) that returns the value of `key`
...
The string representation returned when calling [`str`](stdtypes.md#str) on a
[`ZoneInfo`](#zoneinfo.ZoneInfo) object defaults to using the [`ZoneInfo.key`](#zoneinfo.ZoneInfo.key) attribute (see
...
`str` falls back to calling [`repr()`](functions.md#repr). `ZoneInfo`’s `repr` is
...
strings. See also the cautionary note on [`ZoneInfo.key`](#zoneinfo.ZoneInfo.key).

### zoneinfo.reset_tzpath(to=None)

Sets or resets the time zone search path ([`TZPATH`](#zoneinfo.TZPATH)) for the module.
When called with no arguments, [`TZPATH`](#zoneinfo.TZPATH) is set to the default value.

Calling `reset_tzpath` will not invalidate the [`ZoneInfo`](#zoneinfo.ZoneInfo) cache,
and so calls to the primary `ZoneInfo` constructor will only use the new
`TZPATH` in the case of a cache miss.

The `to` parameter must be a [sequence](../glossary.md#term-sequence) of strings or
[`os.PathLike`](os.md#os.PathLike) and not a string, all of which must be absolute paths.
[`ValueError`](exceptions.md#ValueError) will be raised if something other than an absolute path
is passed.
...
call to [`reset_tzpath()`](#zoneinfo.reset_tzpath), so it is recommended to use
...
Raised when construction of a [`ZoneInfo`](#zoneinfo.ZoneInfo) object fails because the
