...
**Source code:** [Lib/email/utils.py](https://github.com/python/cpython/tree/main/Lib/email/utils.py)
...
[`datetime`](datetime.md#datetime.datetime) instance, and it is converted to the local time
...
The inverse of [`parseaddr()`](#email.utils.parseaddr), this takes a 2-tuple of the form `(realname,
...
characters.  Can be an instance of [`str`](stdtypes.md#str) or a
[`Charset`](email.charset.md#email.charset.Charset).  Defaults to `utf-8`.
...
mailers don’t follow that format as specified, so [`parsedate()`](#email.utils.parsedate) tries to
...
the date, [`parsedate()`](#email.utils.parsedate) returns a 9-tuple that can be passed directly to
[`time.mktime()`](time.md#time.mktime); otherwise `None` will be returned.  Note that indexes 6,
7, and 8 of the result tuple are not usable.

### email.utils.parsedate_tz(date)

Performs the same function as [`parsedate()`](#email.utils.parsedate), but returns either `None` or
a 10-tuple; the first 9 elements make up a tuple that can be passed directly to
[`time.mktime()`](time.md#time.mktime), and the tenth is the offset of the date’s timezone from UTC
(which is the official term for Greenwich Mean Time) <sup>[1](#id2)</sup>.  If the input string
...
The inverse of [`format_datetime()`](#email.utils.format_datetime).  Performs the same function as
[`parsedate()`](#email.utils.parsedate), but on success returns a [`datetime`](datetime.md#datetime.datetime);
...
corresponding a [`timezone`](datetime.md#datetime.timezone) [`tzinfo`](datetime.md#datetime.tzinfo).
...
Turn a 10-tuple as returned by [`parsedate_tz()`](#email.utils.parsedate_tz) into a UTC
...
[`time.gmtime()`](time.md#time.gmtime) and [`time.localtime()`](time.md#time.localtime), otherwise the current time is
used.
...
Like `formatdate`, but the input is a [`datetime`](datetime.md#module-datetime) instance.  If it is
...
[`Message.get_param`](email.compat32-message.md#email.message.Message.get_param) may return a
3-tuple containing the character set,
language, and value.  [`collapse_rfc2231_value()`](#email.utils.collapse_rfc2231_value) turns this into a unicode
string.  Optional *errors* is passed to the *errors* argument of [`str`](stdtypes.md#str)’s
[`encode()`](stdtypes.md#str.encode) method; it defaults to `'replace'`.  Optional
...
For convenience, if the *value* passed to [`collapse_rfc2231_value()`](#email.utils.collapse_rfc2231_value) is not
...
* <a id='id2'>**[1]**</a> Note that the sign of the timezone offset is the opposite of the sign of the `time.timezone` variable for the same timezone; the latter variable follows the POSIX standard while this module follows [**RFC 2822**](https://datatracker.ietf.org/doc/html/rfc2822.html).
