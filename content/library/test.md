...
modules [`test.support`](#module-test.support) and [`test.regrtest`](#module-test.regrtest).
[`test.support`](#module-test.support) is used to enhance your tests while
[`test.regrtest`](#module-test.regrtest) drives the testing suite.

Each module in the `test` package whose name starts with `test_` is a
testing suite for a specific module or feature. All new tests should be written
using the [`unittest`](unittest.md#module-unittest) or [`doctest`](doctest.md#module-doctest) module.  Some older tests are
written using a “traditional” testing style that compares output printed to
`sys.stdout`; this style of test is considered deprecated.

#### SEE ALSO
Module [`unittest`](unittest.md#module-unittest)
...
It is preferred that tests that use the [`unittest`](unittest.md#module-unittest) module follow a few
...
This code pattern allows the testing suite to be run by [`test.regrtest`](#module-test.regrtest),
on its own as a script that supports the [`unittest`](unittest.md#module-unittest) CLI, or via the
...
* Try to maximize code reuse. On occasion, tests will vary by something as small
  ...
  [`unittest.TestCase`](unittest.md#unittest.TestCase) are run as tests.  The `TestFuncAcceptsSequencesMixin` class in the example above
  does not have any data and so can’t be run by itself, thus it does not
  inherit from [`unittest.TestCase`](unittest.md#unittest.TestCase).
...
test suite, thanks to the [`-m`](../using/cmdline.md#cmdoption-m) option: **python -m test**. Under
...
[`unittest`](unittest.md#module-unittest)-based tests and [`unittest.TestCase`](unittest.md#unittest.TestCase)’s assertion
methods.

### *exception* test.support.ResourceDenied

Subclass of [`unittest.SkipTest`](unittest.md#unittest.SkipTest). Raised when a resource (such as a
network connection) is not available. Raised by the [`requires()`](#test.support.requires)
function.
...
[`test.regrtest`](#module-test.regrtest).
...
The timeout should be long enough for [`connect()`](socket.md#socket.socket.connect),
[`recv()`](socket.md#socket.socket.recv) and [`send()`](socket.md#socket.socket.send) methods of
[`socket.socket`](socket.md#socket.socket).

Its default value is 5 seconds.

See also [`INTERNET_TIMEOUT`](#test.support.INTERNET_TIMEOUT).
...
Usually, a timeout using [`INTERNET_TIMEOUT`](#test.support.INTERNET_TIMEOUT) should not mark a test as
failed, but skip the test instead: see
[`transient_internet()`](#test.support.socket_helper.transient_internet).

Its default value is 1 minute.

See also [`LOOPBACK_TIMEOUT`](#test.support.LOOPBACK_TIMEOUT).
...
If a test using [`SHORT_TIMEOUT`](#test.support.SHORT_TIMEOUT) starts to fail randomly on slow
buildbots, use [`LONG_TIMEOUT`](#test.support.LONG_TIMEOUT) instead.
...
See also [`LOOPBACK_TIMEOUT`](#test.support.LOOPBACK_TIMEOUT), [`INTERNET_TIMEOUT`](#test.support.INTERNET_TIMEOUT) and
[`SHORT_TIMEOUT`](#test.support.SHORT_TIMEOUT).
...
`True` if Python was built with the [`Py_DEBUG`](../c-api/intro.md#c.Py_DEBUG) macro
defined, that is, if
Python was [built in debug mode](../using/configure.md#debug-build).
...
Set to [`sys.maxsize`](sys.md#sys.maxsize) for big memory tests.

### test.support.max_memuse

Set by [`set_memlimit()`](#test.support.set_memlimit) as the memory limit for big memory tests.
Limited by [`MAX_Py_ssize_t`](#test.support.MAX_Py_ssize_t).

### test.support.real_max_memuse

Set by [`set_memlimit()`](#test.support.set_memlimit) as the memory limit for big memory tests.  Not
limited by [`MAX_Py_ssize_t`](#test.support.MAX_Py_ssize_t).
...
See the [`configure --without-doc-strings`](../using/configure.md#cmdoption-without-doc-strings) option.

See also the [`HAVE_DOCSTRINGS`](#test.support.HAVE_DOCSTRINGS) variable.

### test.support.HAVE_DOCSTRINGS

Set to `True` if function docstrings are available.
See the [`python -OO`](../using/cmdline.md#cmdoption-O) option, which strips docstrings of functions implemented in Python.

See also the [`MISSING_C_DOCSTRINGS`](#test.support.MISSING_C_DOCSTRINGS) variable.
...
Object that is not equal to anything (even to [`ALWAYS_EQ`](#test.support.ALWAYS_EQ)).
...
After *timeout* seconds, raise an [`AssertionError`](exceptions.md#AssertionError) if *error* is true,
...
See [`busy_retry()`](#test.support.busy_retry) documentation for the parameters usage.
...
available resources is only set when [`test.regrtest`](#module-test.regrtest) is executing the
tests.

### test.support.get_resource_value(resource)

Return the value specified for *resource* (as `-u *resource*=*value*`).
...
Raise [`ResourceDenied`](#test.support.ResourceDenied) if *resource* is not available. *msg* is the
argument to [`ResourceDenied`](#test.support.ResourceDenied) if it is raised. Always returns
`True` if called by a function whose `__name__` is `'__main__'`.
Used when tests are executed by [`test.regrtest`](#module-test.regrtest).
...
Set the [`sys.setswitchinterval()`](sys.md#sys.setswitchinterval) to the given *interval*.  Defines
...
Set the values for [`max_memuse`](#test.support.max_memuse) and [`real_max_memuse`](#test.support.real_max_memuse) for big
memory tests.
...
Return the original stdout set by [`record_original_stdout()`](#test.support.record_original_stdout) or
...
[`io.StringIO`](io.md#io.StringIO) object.
...
A context manager that temporary disables [`faulthandler`](faulthandler.md#module-faulthandler).
...
Call the `flush()` method on [`sys.stdout`](sys.md#sys.stdout) and then on
[`sys.stderr`](sys.md#sys.stderr). It can be used to make sure that the logs order is
...
Print a warning into [`sys.__stderr__`](sys.md#sys.__stderr__). Format the message as:
...
Raise an [`AssertionError`](exceptions.md#AssertionError) if the process exit code is not equal to
*exitcode*.

If the process runs longer than *timeout* seconds ([`SHORT_TIMEOUT`](#test.support.SHORT_TIMEOUT) by
default), kill the process and raise an [`AssertionError`](exceptions.md#AssertionError). The timeout
...
Return the size of the [`PyObject`](../c-api/structures.md#c.PyObject) whose structure members are
defined by *fmt*. The returned value includes the size of the Python object header and alignment.

### test.support.calcvobjsize(fmt)

Return the size of the [`PyVarObject`](../c-api/structures.md#c.PyVarObject) whose structure members are
defined by *fmt*. The returned value includes the size of the Python object header and alignment.
...
[`unittest.expectedFailure()`](unittest.md#unittest.expectedFailure). Any use of this decorator should
...
[GIL](../glossary.md#term-GIL) is disabled, the test is skipped.
...
Decorator for skipping tests if [`zlib`](zlib.md#module-zlib) doesn’t exist.

### @test.support.requires_gzip

Decorator for skipping tests if [`gzip`](gzip.md#module-gzip) doesn’t exist.

### @test.support.requires_bz2

Decorator for skipping tests if [`bz2`](bz2.md#module-bz2) doesn’t exist.

### @test.support.requires_lzma

Decorator for skipping tests if [`lzma`](lzma.md#module-lzma) doesn’t exist.
...
Decorator for only running the test if [`HAVE_DOCSTRINGS`](#test.support.HAVE_DOCSTRINGS).

### @test.support.requires_limited_api

Decorator for only running the test if [Limited C API](../c-api/stable.md#limited-c-api)
is available.
...
Decorator for invoking [`check_impl_detail()`](#test.support.check_impl_detail) on *guards*.  If that
...
*testcase* is the [`unittest`](unittest.md#module-unittest) instance for the test.  *errtext* is the
regular expression which should match the string representation of the
raised [`SyntaxError`](exceptions.md#SyntaxError).  If *lineno* is not `None`, compares to
the line of the exception.  If *offset* is not `None`, compares to
the offset of the exception.

### test.support.open_urlresource(url, \*args, \*\*kw)

Open *url*.  If open fails, raises [`TestFailed`](#test.support.TestFailed).
...
Get an attribute, raising [`unittest.SkipTest`](unittest.md#unittest.SkipTest) if [`AttributeError`](exceptions.md#AttributeError)
is raised.

### test.support.catch_unraisable_exception()

Context manager catching unraisable exception using
[`sys.unraisablehook()`](sys.md#sys.unraisablehook).
...
Generic implementation of the [`unittest`](unittest.md#module-unittest) `load_tests` protocol for
...
includes all magic methods, i.e. those starting and ending in ‘_\_’.
...
Run *code* in subinterpreter.  Raise [`unittest.SkipTest`](unittest.md#unittest.SkipTest) if
[`tracemalloc`](tracemalloc.md#module-tracemalloc) is enabled.
...
### test.support.check_\_all_\_(test_case, module, name_of_module=None, extra=(), not_exported=())
...
detected as “public”, like objects without a proper [`__module__`](stdtypes.md#definition.__module__)
...
an [`OSError`](exceptions.md#OSError).
...
[`sys.set_int_max_str_digits()`](sys.md#sys.set_int_max_str_digits) setting for the duration of the
...
On UNIX, [`resource.setrlimit()`](resource.md#resource.setrlimit) is used to set
[`resource.RLIMIT_CORE`](resource.md#resource.RLIMIT_CORE)’s soft limit to 0 to prevent coredump file
creation.

On both platforms, the old value is restored by [`__exit__()`](../reference/datamodel.md#object.__exit__).
...
Set the signal numbers from the [`save()`](#test.support.SaveSignals.save) dictionary to the saved
handler.
...
the `sock` parameter (default is [`AF_INET`](socket.md#socket.AF_INET),
...
Either this method or [`bind_port()`](#test.support.socket_helper.bind_port) should be used for any tests
where a server socket needs to be bound to a particular port for the
duration of the test.
Which one to use depends on whether the calling code is creating a Python
socket, or if an unused port needs to be provided in a constructor
or passed to an external program (i.e. the `-accept` argument to
openssl’s s_server mode).  Always prefer [`bind_port()`](#test.support.socket_helper.bind_port) over
[`find_unused_port()`](#test.support.socket_helper.find_unused_port) where possible.  Using a hard coded port is
...
`sock.family` is [`AF_INET`](socket.md#socket.AF_INET) and `sock.type` is
[`SOCK_STREAM`](socket.md#socket.SOCK_STREAM), and the socket has
...
Bind a Unix socket, raising [`unittest.SkipTest`](unittest.md#unittest.SkipTest) if
[`PermissionError`](exceptions.md#PermissionError) is raised.
...
A context manager that raises [`ResourceDenied`](#test.support.ResourceDenied) when
...
Setting [`PYTHONHOME`](../using/cmdline.md#envvar-PYTHONHOME) is one way to get most of the testsuite to run
in that situation.  [`PYTHONPATH`](../using/cmdline.md#envvar-PYTHONPATH) or `PYTHONUSERSITE` are
...
If the  *\_\_cleanenv* keyword-only parameter is set, *env_vars* is used as a fresh
environment.

Python is started in isolated mode (command line option `-I`),
except if the  *\_\_isolated* keyword-only parameter is set to `False`.
...
See [`assert_python_ok()`](#test.support.script_helper.assert_python_ok) for more options.
...
*kw* is extra keyword args to pass to [`subprocess.Popen()`](subprocess.md#subprocess.Popen). Returns a
[`subprocess.Popen`](subprocess.md#subprocess.Popen) object.

### test.support.script_helper.kill_python(p)

Run the given [`subprocess.Popen`](subprocess.md#subprocess.Popen) process until completion and return
stdout.
...
Return instr if *opname* is found, otherwise throws [`AssertionError`](exceptions.md#AssertionError).

#### BytecodeTestCase.assertNotInBytecode(x, opname, argval=\_UNSPECIFIED)

Throws [`AssertionError`](exceptions.md#AssertionError) if *opname* is found.
...
Join a *thread* within *timeout*.  Raise an [`AssertionError`](exceptions.md#AssertionError) if thread
...
exception was raised; an example would be [`threading.Event.set()`](threading.md#threading.Event.set).
...
Context manager catching [`threading.Thread`](threading.md#threading.Thread) exception using
[`threading.excepthook()`](threading.md#threading.excepthook).
...
See [`threading.excepthook()`](threading.md#threading.excepthook) documentation.
...
A non-ASCII character encodable by [`os.fsencode()`](os.md#os.fsencode).

### test.support.os_helper.SAVEDCWD

Set to [`os.getcwd()`](os.md#os.getcwd).
...
Set to a filename containing the [`FS_NONASCII`](#test.support.os_helper.FS_NONASCII) character, if it exists.
...
Simple [path-like object](../glossary.md#term-path-like-object).  It implements the
[`__fspath__()`](os.md#os.PathLike.__fspath__)
...
Call [`os.rmdir()`](os.md#os.rmdir) on *filename*.  On Windows platforms, this is
...
Call [`shutil.rmtree()`](shutil.md#shutil.rmtree) on *path* or call [`os.lstat()`](os.md#os.lstat) and
[`os.rmdir()`](os.md#os.rmdir) to remove a path and its contents.  As with [`rmdir()`](#test.support.os_helper.rmdir),
...
created using [`tempfile.mkdtemp()`](tempfile.md#tempfile.mkdtemp).
...
[`tempfile.mkdtemp()`](tempfile.md#tempfile.mkdtemp).  If *quiet* is `False`, the context manager
...
Call [`os.unlink()`](os.md#os.unlink) on *filename*.  As with [`rmdir()`](#test.support.os_helper.rmdir),
...
them raise [`ImportError`](exceptions.md#ImportError).
...
This function will raise [`ImportError`](exceptions.md#ImportError) if the named module cannot be
imported.
...
import, this function raises [`unittest.SkipTest`](unittest.md#unittest.SkipTest) if the module
cannot be imported.
...
which will be compared against [`sys.platform`](sys.md#sys.platform).
...
Return a copy of [`sys.modules`](sys.md#sys.modules).
...
[`DeprecationWarning`](exceptions.md#DeprecationWarning) on import.  Example usage:
...
A context manager to temporarily add directories to [`sys.path`](sys.md#sys.path).

This makes a copy of [`sys.path`](sys.md#sys.path), appends any directories given
as positional arguments, then reverts [`sys.path`](sys.md#sys.path) to the copied
settings when the context ends.

Note that *all* [`sys.path`](sys.md#sys.path) modifications in the body of the
...
which must be [`Warning`](exceptions.md#Warning) or a subclass.
Roughly equivalent to [`warnings.catch_warnings()`](warnings.md#warnings.catch_warnings)
with [`warnings.simplefilter('ignore', category=category)`](warnings.md#warnings.simplefilter).
For example:
...
Context manager to check that no [`ResourceWarning`](exceptions.md#ResourceWarning) was raised.  You
must remove the object which may emit [`ResourceWarning`](exceptions.md#ResourceWarning) before the
...
Test also that the [`SyntaxWarning`](exceptions.md#SyntaxWarning) is emitted only once, and that it
will be converted to a [`SyntaxError`](exceptions.md#SyntaxError) when turned into error.
*testcase* is the [`unittest`](unittest.md#module-unittest) instance for the test.  *errtext* is the
regular expression which should match the string representation of the
emitted [`SyntaxWarning`](exceptions.md#SyntaxWarning) and raised [`SyntaxError`](exceptions.md#SyntaxError).  If *lineno*
...
A convenience wrapper for [`warnings.catch_warnings()`](warnings.md#warnings.catch_warnings) that makes it
easier to test that a warning was correctly raised.  It is approximately
equivalent to calling `warnings.catch_warnings(record=True)` with
[`warnings.simplefilter()`](warnings.md#warnings.simplefilter) set to `always` and with the option to
...
[`catch_warnings()`](warnings.md#warnings.catch_warnings) is available via the recorder object’s
[`warnings`](warnings.md#module-warnings) attribute.  As a convenience, the attributes of the object
...
raised, [`check_warnings()`](#test.support.warnings_helper.check_warnings) would raise an error.
...
[`check_warnings()`](#test.support.warnings_helper.check_warnings) above for more details.
