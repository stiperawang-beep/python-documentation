...
**Source code:** [Lib/tkinter/_\_init_\_.py](https://github.com/python/cpython/tree/main/Lib/tkinter/__init__.py)
...
threaded. See the source code for the [`_tkinter`](#module-_tkinter) module
...
: Tk is a [Tcl package](https://wiki.tcl-lang.org/37432) implemented in C
  ...
  [`Tk`](#tkinter.Tk) object embeds its own Tcl interpreter instance with Tk loaded into
...
: Themed Tk (Ttk) is a newer family of Tk widgets that provide a much better
  ...
  bindings are provided in a separate module, [`tkinter.ttk`](tkinter.ttk.md#module-tkinter.ttk).
...
Tcl command string to an internal [`_tkinter`](#module-_tkinter) binary module, which then
...
main `tkinter` module, as well as the [`tkinter.ttk`](tkinter.ttk.md#module-tkinter.ttk) module, which provides
...
The [`Tk`](#tkinter.Tk) class is typically instantiated using all default values.
...
*useTk*
: If `True`, initialize the Tk subsystem.  The [`tkinter.Tcl()`](#tkinter.Tcl)
...
[`Tk`](#tkinter.Tk) reads and interprets profile files, named
`.*className*.tcl` and `.*baseName*.tcl`, into the Tcl
interpreter and calls [`exec()`](functions.md#exec) on the contents of
`.*className*.py` and `.*baseName*.py`.  The path for the
profile files is the `HOME` environment variable or, if that
isn’t defined, then [`os.curdir`](os.md#os.curdir).

#### tk

The Tk application object created by instantiating [`Tk`](#tkinter.Tk).  This
provides access to the Tcl interpreter.  Each widget that is attached
the same instance of [`Tk`](#tkinter.Tk) has the same value for its [`tk`](#tkinter.Tk.tk)
attribute.

#### master

The widget object that contains this widget.  For [`Tk`](#tkinter.Tk), the
`master` is [`None`](constants.md#None) because it is the main window.  The terms
*master* and *parent* are similar and sometimes used interchangeably
as argument names; however, calling `winfo_parent()` returns a
string of the widget name whereas `master` returns the object.
*parent*/*child* reflects the tree-like relationship while
*master* (or *container*)/*content* reflects the container structure.

#### children

The immediate descendants of this widget as a [`dict`](stdtypes.md#dict) with the
child widget names as the keys and the child instance objects as the
values.

### tkinter.Tcl(screenName=None, baseName=None, className='Tk', useTk=False)

The [`Tcl()`](#tkinter.Tcl) function is a factory function which creates an object much like
that created by the [`Tk`](#tkinter.Tk) class, except that it does not initialize the Tk
...
created by the [`Tcl()`](#tkinter.Tcl) object can have a Toplevel window created (and the Tk
...
[`tkinter.colorchooser`](tkinter.colorchooser.md#module-tkinter.colorchooser)
: Dialog to let the user choose a color.

[`tkinter.commondialog`](dialog.md#module-tkinter.commondialog)
: Base class for the dialogs defined in the other modules listed here.

[`tkinter.filedialog`](dialog.md#module-tkinter.filedialog)
: Common dialogs to allow the user to specify a file to open or save.

[`tkinter.font`](tkinter.font.md#module-tkinter.font)
: Utilities to help work with fonts.

[`tkinter.messagebox`](tkinter.messagebox.md#module-tkinter.messagebox)
: Access to standard Tk dialog boxes.

[`tkinter.scrolledtext`](tkinter.scrolledtext.md#module-tkinter.scrolledtext)
: Text widget with a vertical scroll bar built in.

[`tkinter.simpledialog`](dialog.md#module-tkinter.simpledialog)
: Basic dialogs and convenience functions.

[`tkinter.ttk`](tkinter.ttk.md#module-tkinter.ttk)
: Themed widget set introduced in Tk 8.5, providing modern alternatives
  for many of the classic widgets in the main `tkinter` module.

Additional modules:

<a id="module-_tkinter"></a>

[`_tkinter`](#module-_tkinter)
: A binary module that contains the low-level interface to Tcl/Tk.
  It is automatically imported by the main `tkinter` module,
...
[`tkinter.dnd`](tkinter.dnd.md#module-tkinter.dnd)
: (experimental) Drag-and-drop support for `tkinter`. This will
  become deprecated when it is replaced with the Tk DND.

[`turtle`](turtle.md#module-turtle)
...
interactive Python shell or with [`print()`](functions.md#print), can help you identify what
you need.
...
standard [`dir()`](functions.md#dir) function. If you try it, you’ll see there are over 200
...
* Tcl/Tk libraries can be built so they are not thread-aware. In this case,
  `tkinter` calls the library from the originating Python thread, even
...
* While `tkinter` allows you to create more than one instance of a `Tk`
...
* There are a few select `tkinter` functions that presently work only when
  called from the thread that created the Tcl interpreter.

## Handy Reference

<a id="tkinter-setting-options"></a>
...
widget.  The Standard Options are documented on the  man
page.
...
|       2 | option class for database<br/>lookup | `'Relief'` |
...
want to use the [`BufferedIOBase`](io.md#io.BufferedIOBase) or [`TextIOBase`](io.md#io.TextIOBase)
[`read()`](io.md#io.BufferedIOBase.read) or [`readline()`](io.md#io.IOBase.readline) methods,
since these will insist on reading a predefined number of bytes.
For sockets, the [`recv()`](socket.md#socket.socket.recv) or
[`recvfrom()`](socket.md#socket.socket.recvfrom) methods will work fine; for other files,
...
may either be an object with a [`fileno()`](io.md#io.IOBase.fileno) method (such as
