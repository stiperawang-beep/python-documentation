...
**Source code:** [Lib/profiling/sampling/](https://github.com/python/cpython/tree/main/Lib/profiling/sampling/)

---
![Tachyon logo](library/tachyon-logo.png)
...
collect enough samples for reliable results. Use [`profiling.tracing`](profiling.tracing.md#module-profiling.tracing)
...
times a function was called, use [`profiling.tracing`](profiling.tracing.md#module-profiling.tracing).

When comparing two implementations where the difference might be only 1-2%,
sampling noise can obscure real differences. Use [`timeit`](timeit.md#module-timeit) for
micro-benchmarks or [`profiling.tracing`](profiling.tracing.md#module-profiling.tracing) for precise measurements.

The key difference from [`profiling.tracing`](profiling.tracing.md#module-profiling.tracing) is how measurement happens.
...
and complete call graphs, use [`profiling.tracing`](profiling.tracing.md#module-profiling.tracing) instead.
...
See [Platform requirements](#profiling-permissions) for platform-specific requirements.

<a id="replay-command"></a>
...
The [`--sampling-rate`](#cmdoption-profiling.sampling-r) option ([`-r`](#cmdoption-profiling.sampling-r)) sets how frequently samples
...
The [`--duration`](#cmdoption-profiling.sampling-d) option ([`-d`](#cmdoption-profiling.sampling-d)) sets how long to profile in seconds. By
...
[`threading`](threading.md#module-threading) module or implicitly through libraries that manage thread
pools.

By default, the profiler samples only the main thread. The [`--all-threads`](#cmdoption-profiling.sampling-a)
option ([`-a`](#cmdoption-profiling.sampling-a)) enables sampling of all threads in the process:
...
For these cases, the [`--blocking`](#cmdoption-profiling.sampling-blocking) option stops the target process during
each sample:
...
The [`--native`](#cmdoption-profiling.sampling-native) option adds `<native>` frames to indicate when Python has
...
active. The [`--no-gc`](#cmdoption-profiling.sampling-no-gc) option suppresses these frames:
...
The [`--opcodes`](#cmdoption-profiling.sampling-opcodes) option enables instruction-level profiling that captures
...
The [`--opcodes`](#cmdoption-profiling.sampling-opcodes) option is compatible with [`--live`](#cmdoption-profiling.sampling-live), [`--flamegraph`](#cmdoption-profiling.sampling-flamegraph),
[`--heatmap`](#cmdoption-profiling.sampling-heatmap), and [`--gecko`](#cmdoption-profiling.sampling-gecko) formats. It requires additional memory to store
...
The [`--realtime-stats`](#cmdoption-profiling.sampling-realtime-stats) option displays sampling rate statistics during
profiling:
...
[Sampling efficiency](#sampling-efficiency) for details on interpreting these metrics.

### Subprocess profiling

The [`--subprocesses`](#cmdoption-profiling.sampling-subprocesses) option enables automatic profiling of subprocesses
...
applications that use [`multiprocessing`](multiprocessing.md#module-multiprocessing), [`subprocess`](subprocess.md#module-subprocess),
[`concurrent.futures`](concurrent.futures.md#module-concurrent.futures) with [`ProcessPoolExecutor`](concurrent.futures.md#concurrent.futures.ProcessPoolExecutor),
...
The [`--subprocesses`](#cmdoption-profiling.sampling-subprocesses) option is incompatible with [`--live`](#cmdoption-profiling.sampling-live) mode
...
Wall-clock mode ([`--mode`](#cmdoption-profiling.sampling-mode)`=wall`) captures all samples regardless of what the
...
CPU mode ([`--mode`](#cmdoption-profiling.sampling-mode)`=cpu`) records samples only when the thread is actually
...
GIL mode ([`--mode`](#cmdoption-profiling.sampling-mode)`=gil`) records samples only when the thread holds Python’s
...
The pstats format ([`--pstats`](#cmdoption-profiling.sampling-pstats)) produces a text table similar to what
...
![Tachyon pstats terminal output](library/tachyon-pstats.png)
...
Use [`--no-summary`](#cmdoption-profiling.sampling-no-summary) to suppress both the legend and summary sections.
...
The [`--sort`](#cmdoption-profiling.sampling-sort) option determines the column used for ordering results:
...
The [`--limit`](#cmdoption-profiling.sampling-l) option restricts output to the top N entries:

```sh
python -m profiling.sampling run --limit=30 script.py
```

The [`--no-summary`](#cmdoption-profiling.sampling-no-summary) option suppresses the header summary that precedes the
statistics table.

### Collapsed stacks format

Collapsed stacks format ([`--collapsed`](#cmdoption-profiling.sampling-collapsed)) produces one line per unique call
...
Flame graph format ([`--flamegraph`](#cmdoption-profiling.sampling-flamegraph)) produces a self-contained HTML file with
...
![Tachyon interactive flame graph](library/tachyon-flamegraph.png)
...
- A sidebar showing profile summary, thread statistics, sampling efficiency
  metrics (see [Sampling efficiency](#sampling-efficiency)), and top hotspot functions
...
Gecko format ([`--gecko`](#cmdoption-profiling.sampling-gecko)) produces JSON output compatible with the Firefox
Profiler:
...
The [Firefox Profiler](https://profiler.firefox.com) is a sophisticated
...
[Firefox Profiler documentation](https://profiler.firefox.com/docs/#/) for
...
For this reason, the [`--mode`](#cmdoption-profiling.sampling-mode) option is not available with Gecko format;
all relevant data is captured automatically.

![Firefox Profiler Call Tree view](library/tachyon-gecko-calltree.png)![Firefox Profiler Flame Graph view](library/tachyon-gecko-flamegraph.png)![Firefox Profiler Marker Chart with opcodes](library/tachyon-gecko-opcodes.png)

### Heatmap format

Heatmap format ([`--heatmap`](#cmdoption-profiling.sampling-heatmap)) generates an interactive HTML visualization
...
![Tachyon heatmap visualization](library/tachyon-heatmap.png)
...
When opcode-level profiling is enabled with [`--opcodes`](#cmdoption-profiling.sampling-opcodes), each hot line
can be expanded to show which bytecode instructions consumed time:

![Heatmap with expanded bytecode panel](library/tachyon-heatmap-with-opcodes.png)
...
Binary format ([`--binary`](#cmdoption-profiling.sampling-binary)) produces a compact binary file for efficient
...
The [`--compression`](#cmdoption-profiling.sampling-compression) option controls data compression:

- `auto` (default): Use zstd compression if available, otherwise no
  compression
- `zstd`: Force zstd compression (requires [`compression.zstd`](compression.zstd.md#module-compression.zstd) support)
...
To analyze binary profiles, use the [The replay command](#replay-command) to convert them to
...
Live mode ([`--live`](#cmdoption-profiling.sampling-live)) provides a terminal-based real-time view of profiling
...
![Tachyon live mode showing all threads](library/tachyon-live-mode-2.gif)
...
hottest functions. This mode requires the [`curses`](curses.md#module-curses) module, which is
...
When [`--opcodes`](#cmdoption-profiling.sampling-opcodes) is enabled, an additional opcode panel appears below the
...
![Tachyon live mode with opcode panel](library/tachyon-live-mode-1.gif)
...
still navigate, sort, and filter the results before pressing `q` to exit.

Live mode is incompatible with output format options ([`--collapsed`](#cmdoption-profiling.sampling-collapsed),
[`--flamegraph`](#cmdoption-profiling.sampling-flamegraph), and so on) because it uses an interactive terminal
interface rather than producing file output.

## Async-aware profiling

For programs using [`asyncio`](asyncio.md#module-asyncio), the profiler offers async-aware mode
([`--async-aware`](#cmdoption-profiling.sampling-async-aware)) that reconstructs call stacks based on the task structure
...
#### NOTE
Async-aware profiling requires the target process to have the [`asyncio`](asyncio.md#module-asyncio)
...
The [`--async-mode`](#cmdoption-profiling.sampling-async-mode) option controls which tasks appear in the profile:
...
With [`--async-mode`](#cmdoption-profiling.sampling-async-mode)`=running` (the default), only the task currently executing
...
With [`--async-mode`](#cmdoption-profiling.sampling-async-mode)`=all`, tasks that are suspended (awaiting I/O, locks, or
...
incompatible with: [`--native`](#cmdoption-profiling.sampling-native), [`--no-gc`](#cmdoption-profiling.sampling-no-gc), [`--all-threads`](#cmdoption-profiling.sampling-a), and
[`--mode`](#cmdoption-profiling.sampling-mode)`=cpu` or [`--mode`](#cmdoption-profiling.sampling-mode)`=gil`.
...
intervals (1000 µs or higher) to minimize impact. See [Blocking mode](#blocking-mode)
for details.
...
When written to stdout, the output is a text table; with [`-o`](#cmdoption-profiling.sampling-o),
...
[`--pstats`](#cmdoption-profiling.sampling-pstats) prints a text table to stdout, while `-o` writes a
binary pstats file. Other formats generate a file named
`<format>_<PID>.<ext>` (for example, `flamegraph_12345.html`).
[`--heatmap`](#cmdoption-profiling.sampling-heatmap) creates a directory named `heatmap_<PID>`.

### --browser

Automatically open HTML output ([`--flamegraph`](#cmdoption-profiling.sampling-flamegraph) and
[`--heatmap`](#cmdoption-profiling.sampling-heatmap)) in your default web browser after generation.
When profiling with [`--subprocesses`](#cmdoption-profiling.sampling-subprocesses), only the main process
...
: Web-based profiler that accepts Gecko format output. See the
  [documentation](https://profiler.firefox.com/docs/#/) for usage details.
