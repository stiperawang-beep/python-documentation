...
**Source code:** [Lib/compression/zstd/_\_init_\_.py](https://github.com/python/cpython/tree/main/Lib/compression/zstd/__init__.py)
...
* The [`open()`](#compression.zstd.open) function and [`ZstdFile`](#compression.zstd.ZstdFile) class for reading and
  writing compressed files.
* The [`ZstdCompressor`](#compression.zstd.ZstdCompressor) and [`ZstdDecompressor`](#compression.zstd.ZstdDecompressor) classes for
  incremental (de)compression.
* The [`compress()`](#compression.zstd.compress) and [`decompress()`](#compression.zstd.decompress) functions for one-shot
  (de)compression.
* The [`train_dict()`](#compression.zstd.train_dict) and [`finalize_dict()`](#compression.zstd.finalize_dict) functions and the
  [`ZstdDict`](#compression.zstd.ZstdDict) class to train and manage Zstandard dictionaries.
* The [`CompressionParameter`](#compression.zstd.CompressionParameter), [`DecompressionParameter`](#compression.zstd.DecompressionParameter), and
  [`Strategy`](#compression.zstd.Strategy) classes for setting advanced (de)compression parameters.
...
[file object](../glossary.md#term-file-object).

The *file* argument can be either a file name (given as a
[`str`](stdtypes.md#str), [`bytes`](stdtypes.md#bytes) or [path-like](../glossary.md#term-path-like-object)
...
decompression parameters; see [`DecompressionParameter`](#compression.zstd.DecompressionParameter) for detailed
information about supported
parameters. The *zstd_dict* argument is a [`ZstdDict`](#compression.zstd.ZstdDict) instance to be
...
[`CompressionParameter`](#compression.zstd.CompressionParameter) for detailed information about supported
...
The *zstd_dict* argument is a [`ZstdDict`](#compression.zstd.ZstdDict) instance to be used during
compression.

In binary mode, this function is equivalent to the [`ZstdFile`](#compression.zstd.ZstdFile)
constructor: `ZstdFile(file, mode, ...)`. In this case, the
*encoding*, *errors*, and *newline* parameters must not be provided.

In text mode, a [`ZstdFile`](#compression.zstd.ZstdFile) object is created, and wrapped in an
...
A [`ZstdFile`](#compression.zstd.ZstdFile) can wrap an already-open [file object](../glossary.md#term-file-object), or operate
directly on a named file. The *file* argument specifies either the file
object to wrap, or the name of the file to open (as a [`str`](stdtypes.md#str),
[`bytes`](stdtypes.md#bytes) or [path-like](../glossary.md#term-path-like-object) object). If
wrapping an existing file object, the wrapped file will not be closed when
the [`ZstdFile`](#compression.zstd.ZstdFile) is closed.
...
[`DecompressionParameter`](#compression.zstd.DecompressionParameter) for detailed information about supported
parameters. The *zstd_dict* argument is a [`ZstdDict`](#compression.zstd.ZstdDict) instance to be
used during decompression. When reading, if the *level*
argument is not None, a `TypeError` will be raised.
...
*zstd_dict* argument is a [`ZstdDict`](#compression.zstd.ZstdDict) instance to be used during
compression.
...
#### NOTE
While calling [`peek()`](#compression.zstd.ZstdFile.peek) does not change the file position of
the [`ZstdFile`](#compression.zstd.ZstdFile), it may change the position of the underlying
file object (for example, if the [`ZstdFile`](#compression.zstd.ZstdFile) was constructed by
...
The name of the Zstandard file. Equivalent to the [`name`](io.md#io.FileIO.name)
...
Compress *data* (a [bytes-like object](../glossary.md#term-bytes-like-object)), returning the compressed
data as a [`bytes`](stdtypes.md#bytes) object.

The *level* argument is an integer controlling the level of
compression. *level* is an alternative to setting
[`CompressionParameter.compression_level`](#compression.zstd.CompressionParameter.compression_level) in *options*. Use
[`bounds()`](#compression.zstd.CompressionParameter.bounds) on
[`compression_level`](#compression.zstd.CompressionParameter.compression_level) to get the values that can
...
are documented as part of the [`CompressionParameter`](#compression.zstd.CompressionParameter) documentation.

The *zstd_dict* argument is an instance of [`ZstdDict`](#compression.zstd.ZstdDict)
containing trained data to improve compression efficiency. The
function [`train_dict()`](#compression.zstd.train_dict) can be used to generate a Zstandard dictionary.

### compression.zstd.decompress(data, zstd_dict=None, options=None)

Decompress *data* (a [bytes-like object](../glossary.md#term-bytes-like-object)), returning the uncompressed
data as a [`bytes`](stdtypes.md#bytes) object.

The *options* argument is a Python dictionary containing advanced
decompression parameters. The valid keys and values for compression
parameters are documented as part of the [`DecompressionParameter`](#compression.zstd.DecompressionParameter)
documentation.
...
module-level function [`compress()`](#compression.zstd.compress).
...
The *zstd_dict* argument is an optional instance of [`ZstdDict`](#compression.zstd.ZstdDict)
containing trained data to improve compression efficiency. The
function [`train_dict()`](#compression.zstd.train_dict) can be used to generate a Zstandard dictionary.

#### compress(data, mode=ZstdCompressor.CONTINUE)

Compress *data* (a [bytes-like object](../glossary.md#term-bytes-like-object)), returning a [`bytes`](stdtypes.md#bytes)
object with compressed data if possible, or otherwise an empty
`bytes` object. Some of *data* may be buffered internally, for
use in later calls to `compress()` and [`flush()`](#compression.zstd.ZstdCompressor.flush). The returned
data should be concatenated with the output of any previous calls to
[`compress()`](#compression.zstd.ZstdCompressor.compress).

The *mode* argument is a [`ZstdCompressor`](#compression.zstd.ZstdCompressor) attribute, either
[`CONTINUE`](#compression.zstd.ZstdCompressor.CONTINUE), [`FLUSH_BLOCK`](#compression.zstd.ZstdCompressor.FLUSH_BLOCK),
or [`FLUSH_FRAME`](#compression.zstd.ZstdCompressor.FLUSH_FRAME).

When all data has been provided to the compressor, call the
[`flush()`](#compression.zstd.ZstdCompressor.flush) method to finish the compression process. If
[`compress()`](#compression.zstd.ZstdCompressor.compress) is called with *mode* set to [`FLUSH_FRAME`](#compression.zstd.ZstdCompressor.FLUSH_FRAME),
[`flush()`](#compression.zstd.ZstdCompressor.flush) should not be called, as it would write out a new empty
frame.

#### flush(mode=ZstdCompressor.FLUSH_FRAME)

Finish the compression process, returning a [`bytes`](stdtypes.md#bytes) object
containing any data stored in the compressor’s internal buffers.

The *mode* argument is a [`ZstdCompressor`](#compression.zstd.ZstdCompressor) attribute, either
[`FLUSH_BLOCK`](#compression.zstd.ZstdCompressor.FLUSH_BLOCK), or [`FLUSH_FRAME`](#compression.zstd.ZstdCompressor.FLUSH_FRAME).
...
frame unless [`CompressionParameter.content_size_flag`](#compression.zstd.CompressionParameter.content_size_flag) is `False`
...
If [`last_mode`](#compression.zstd.ZstdCompressor.last_mode) is not [`FLUSH_FRAME`](#compression.zstd.ZstdCompressor.FLUSH_FRAME), a
[`ValueError`](exceptions.md#ValueError) is raised as the compressor is not at the start of
a frame. If the pledged size does not match the actual size of data
provided to [`compress()`](#compression.zstd.ZstdCompressor.compress), future calls to `compress()` or
[`flush()`](#compression.zstd.ZstdCompressor.flush) may raise [`ZstdError`](#compression.zstd.ZstdError) and the last chunk of data may
be lost.

After [`flush()`](#compression.zstd.ZstdCompressor.flush) or [`compress()`](#compression.zstd.ZstdCompressor.compress) are called with mode
[`FLUSH_FRAME`](#compression.zstd.ZstdCompressor.FLUSH_FRAME), the next frame will not include the frame size into
...
future blocks generated by calls to [`compress()`](#compression.zstd.ZstdCompressor.compress),
improving compression.

#### FLUSH_FRAME

Complete and write out a frame. Future data provided to
[`compress()`](#compression.zstd.ZstdCompressor.compress) will be written into a new frame and
*cannot* reference past data.

#### last_mode

The last mode passed to either [`compress()`](#compression.zstd.ZstdCompressor.compress) or [`flush()`](#compression.zstd.ZstdCompressor.flush).
The value can be one of [`CONTINUE`](#compression.zstd.ZstdCompressor.CONTINUE), [`FLUSH_BLOCK`](#compression.zstd.ZstdCompressor.FLUSH_BLOCK), or
[`FLUSH_FRAME`](#compression.zstd.ZstdCompressor.FLUSH_FRAME). The initial value is [`FLUSH_FRAME`](#compression.zstd.ZstdCompressor.FLUSH_FRAME),
...
once, see the module-level function [`decompress()`](#compression.zstd.decompress).
...
compressed frames, unlike the [`decompress()`](#compression.zstd.decompress) function and
[`ZstdFile`](#compression.zstd.ZstdFile) class. To decompress a multi-frame input, you should
use [`decompress()`](#compression.zstd.decompress), [`ZstdFile`](#compression.zstd.ZstdFile) if working with a
[file object](../glossary.md#term-file-object), or multiple `ZstdDecompressor` instances.
...
output can be produced, the [`needs_input`](#compression.zstd.ZstdDecompressor.needs_input) attribute will
be set to `False`. In this case, the next call to
[`decompress()`](#compression.zstd.ZstdDecompressor.decompress) may provide *data* as `b''` to obtain
more of the output.

If all of the input data was decompressed and returned (either
because this was less than *max_length* bytes, or because
*max_length* was negative), the [`needs_input`](#compression.zstd.ZstdDecompressor.needs_input) attribute
will be set to `True`.

Attempting to decompress data after the end of a frame will raise a
[`ZstdError`](#compression.zstd.ZstdError). Any data found after the end of the frame is ignored
and saved in the [`unused_data`](#compression.zstd.ZstdDecompressor.unused_data) attribute.
...
`False` if the [`decompress()`](#compression.zstd.ZstdDecompressor.decompress) method can provide more
...
Train a Zstandard dictionary, returning a [`ZstdDict`](#compression.zstd.ZstdDict) instance.
...
The *samples* argument (an iterable of [`bytes`](stdtypes.md#bytes) objects), is the
...
The *zstd_dict* argument is a [`ZstdDict`](#compression.zstd.ZstdDict) instance with
the [`dict_content`](#compression.zstd.ZstdDict.dict_content) containing the raw dictionary contents.

The *samples* argument (an iterable of [`bytes`](stdtypes.md#bytes) objects), contains
sample data for generating the Zstandard dictionary.

The *dict_size* argument, an integer, is the maximum size (in bytes) the
Zstandard dictionary should be. See [`train_dict()`](#compression.zstd.train_dict) for
...
the compression of many small chunks of data. Use [`train_dict()`](#compression.zstd.train_dict) if you
need to train a new dictionary from sample data.

The *dict_content* argument (a [bytes-like object](../glossary.md#term-bytes-like-object)), is the already
...
for example, [`train_dict()`](#compression.zstd.train_dict) or the external **zstd** CLI.
...
> |--------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------|
> | Advanced parameters of the compressor which may be overridden by<br/>the dictionary’s parameters | `window_log`, `hash_log`, `chain_log`, `search_log`,<br/>`min_match`, `target_length`, `strategy`,<br/>`enable_long_distance_matching`, `ldm_hash_log`,<br/>`ldm_min_match`, `ldm_bucket_size_log`, `ldm_hash_rate_log`,<br/>and some non-public parameters. | None                                                                                                       |
> | `ZstdDict` internally caches the dictionary                                                      | Yes. It’s faster when loading a digested dictionary again with the<br/>same compression level.                                                                                                                                                               | No. If you wish to load an undigested dictionary multiple times,<br/>consider reusing a compressor object. |
...
> #### NOTE
> The meaning of `0` for `ZstdDict.dict_id` is different
> from the `dictionary_id` attribute to the [`get_frame_info()`](#compression.zstd.get_frame_info)
> function.
...
An [`IntEnum`](enum.md#enum.IntEnum) containing the advanced compression parameter
keys that can be used when compressing data.

The [`bounds()`](#compression.zstd.CompressionParameter.bounds) method can be used on any attribute to get the valid
values for that parameter.

Parameters are optional; any omitted parameter will have it’s value selected
automatically.

Example getting the lower and upper bound of [`compression_level`](#compression.zstd.CompressionParameter.compression_level):

```python3
lower, upper = CompressionParameter.compression_level.bounds()
```

Example setting the [`window_log`](#compression.zstd.CompressionParameter.window_log) to the maximum size:
...
[`compression_level`](#compression.zstd.CompressionParameter.compression_level), one may check the result of
...
Setting the level to zero uses [`COMPRESSION_LEVEL_DEFAULT`](#compression.zstd.COMPRESSION_LEVEL_DEFAULT).
...
ratio of strategies <= [`dfast`](#compression.zstd.Strategy.dfast), and improve compression
speed of strategies > [`dfast`](#compression.zstd.Strategy.dfast).
...
[`fast`](#compression.zstd.Strategy.fast) strategy. It’s still useful when using
[`dfast`](#compression.zstd.Strategy.dfast) strategy, in which case it defines a secondary
probe table.
...
[`fast`](#compression.zstd.Strategy.fast) and [`dfast`](#compression.zstd.Strategy.dfast) strategies.
...
for this size and larger. For all strategies < [`btopt`](#compression.zstd.Strategy.btopt),
the effective minimum is `4`; for all strategies
> [`fast`](#compression.zstd.Strategy.fast), the effective maximum is `6`.

A value of zero causes the value to be selected automatically.

#### target_length

The impact of this field depends on the selected [`Strategy`](#compression.zstd.Strategy).

For strategies [`btopt`](#compression.zstd.Strategy.btopt), [`btultra`](#compression.zstd.Strategy.btultra) and
[`btultra2`](#compression.zstd.Strategy.btultra2), the value is the length of a match
considered “good enough” to stop searching. Larger values make
compression ratios better, but compresses slower.

For strategy [`fast`](#compression.zstd.Strategy.fast), it is the distance between match
...
#### SEE ALSO
[`Strategy`](#compression.zstd.Strategy)
...
[`window_log`](#compression.zstd.CompressionParameter.window_log) to 128 MiB except when expressly
set to a different value. This setting is enabled by default if
`window_log` >= 128 MiB and the compression
strategy >= [`btopt`](#compression.zstd.Strategy.btopt) (compression level 16+).
...
* Calling [`compress()`](#compression.zstd.compress) for one-shot compression
* Providing all of the data to be compressed in the frame in a single
  [`ZstdCompressor.compress()`](#compression.zstd.ZstdCompressor.compress) call, with the
  [`ZstdCompressor.FLUSH_FRAME`](#compression.zstd.ZstdCompressor.FLUSH_FRAME) mode.
* Calling [`ZstdCompressor.set_pledged_input_size()`](#compression.zstd.ZstdCompressor.set_pledged_input_size) with the exact
  amount of data that will be provided to the compressor prior to any
  calls to [`ZstdCompressor.compress()`](#compression.zstd.ZstdCompressor.compress) for the current frame.
...
the checksum. If there is a mismatch a [`ZstdError`](#compression.zstd.ZstdError) exception is
raised.
...
When compressing with a [`ZstdDict`](#compression.zstd.ZstdDict), the dictionary’s ID is written
...
[`nb_workers`](#compression.zstd.CompressionParameter.nb_workers) >= 1. Each compression job is
...
only used when [`nb_workers`](#compression.zstd.CompressionParameter.nb_workers) >= 1. Acceptable
...
An [`IntEnum`](enum.md#enum.IntEnum) containing the advanced decompression parameter
...
The [`bounds()`](#compression.zstd.DecompressionParameter.bounds) method can be used on any attribute to get the valid
values for that parameter.

Example setting the [`window_log_max`](#compression.zstd.DecompressionParameter.window_log_max) to the maximum size:
...
An [`IntEnum`](enum.md#enum.IntEnum) containing strategies for compression.
...
Retrieve a [`FrameInfo`](#compression.zstd.FrameInfo) object containing metadata about a Zstandard
