...
**Source code:** [Lib/heapq.py](https://github.com/python/cpython/tree/main/Lib/heapq.py)
...
favors min-heaps as they better correspond to Python [`lists`](stdtypes.md#list).

These two aspects make it possible to view the heap as a regular Python list
without surprises: `heap[0]` is the smallest item, and `heap.sort()`
maintains the heap invariant!

Like [`list.sort()`](stdtypes.md#list.sort), this implementation uses only the `<` operator
...
into a min-heap or max-heap using the [`heapify()`](#heapq.heapify) or [`heapify_max()`](#heapq.heapify_max)
...
invariant.  If the heap is empty, [`IndexError`](exceptions.md#IndexError) is raised.  To access the
smallest item without popping it, use `heap[0]`.

### heapq.heappushpop(heap, item)

Push *item* on the heap, then pop and return the smallest item from the
*heap*.  The combined action runs more efficiently than [`heappush()`](#heapq.heappush)
followed by a separate call to [`heappop()`](#heapq.heappop).

### heapq.heapreplace(heap, item)

Pop and return the smallest item from the *heap*, and also push the new *item*.
The heap size doesn’t change. If the heap is empty, [`IndexError`](exceptions.md#IndexError) is raised.

This one step operation is more efficient than a [`heappop()`](#heapq.heappop) followed by
[`heappush()`](#heapq.heappush) and can be more appropriate when using a fixed-size heap.
The pop/push combination always returns an element from the heap and replaces
it with *item*.

The value returned may be larger than the *item* added.  If that isn’t
desired, consider using [`heappushpop()`](#heapq.heappushpop) instead.  Its push/pop
...
max-heap invariant.  If the max-heap is empty, [`IndexError`](exceptions.md#IndexError) is raised.
...
from *heap*.
The combined action runs more efficiently than [`heappush_max()`](#heapq.heappush_max)
followed by a separate call to [`heappop_max()`](#heapq.heappop_max).
...
[`IndexError`](exceptions.md#IndexError) is raised.

The value returned may be smaller than the *item* added.  Refer to the
analogous function [`heapreplace()`](#heapq.heapreplace) for detailed usage notes.
...
timestamped entries from multiple log files).  Returns an [iterator](../glossary.md#term-iterator)
...
*key* specifies a [key function](../glossary.md#term-key-function) of one argument that is used to
...
values, it is more efficient to use the [`sorted()`](functions.md#sorted) function.  Also, when
`n==1`, it is more efficient to use the built-in [`min()`](functions.md#min) and [`max()`](functions.md#max)
...
This is similar to `sorted(iterable)`, but unlike [`sorted()`](functions.md#sorted), this
...
![Example (min-heap) binary tree.](library/heapq-binary-tree.svg)
...
these runs, which merging is often very cleverly organised <sup>[1](#id2)</sup>. It is very
