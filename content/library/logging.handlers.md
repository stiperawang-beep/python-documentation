...
queue-like object; it’s used as-is by the [`enqueue()`](#logging.handlers.QueueHandler.enqueue) method, which
...
message (obtained by calling the handler’s [`format()`](functions.md#format) method), and
...
that a handler on the [`QueueListener`](#logging.handlers.QueueListener) side won’t have the
...
object; it’s passed as-is to the [`dequeue()`](#logging.handlers.QueueListener.dequeue) method, which needs
