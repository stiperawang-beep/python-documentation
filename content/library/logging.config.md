...
**Source code:** [Lib/logging/config.py](https://github.com/python/cpython/tree/main/Lib/logging/config.py)

---

This section describes the API for configuring the logging module.

<a id="logging-config-api"></a>
...
in [`logging`](logging.md#module-logging) itself) and defining handlers which are declared either in
[`logging`](logging.md#module-logging) or [`logging.handlers`](logging.handlers.md#module-logging.handlers).

### logging.config.dictConfig(config)

Takes the logging configuration from a dictionary.  The contents of
this dictionary are described in [Configuration dictionary schema](#logging-config-dictschema)
below.

If an error is encountered during configuration, this function will
raise a [`ValueError`](exceptions.md#ValueError), [`TypeError`](exceptions.md#TypeError), [`AttributeError`](exceptions.md#AttributeError)
or [`ImportError`](exceptions.md#ImportError) with a suitably descriptive message.  The
...
[`dictConfig()`](#logging.config.dictConfig) calls `dictConfigClass` passing
...
this new subclass, and then [`dictConfig()`](#logging.config.dictConfig) could be called exactly as
...
Reads the logging configuration from a [`configparser`](configparser.md#module-configparser)-format file. The
format of the file should be as described in
[Configuration file format](#logging-config-fileformat).
...
It will raise [`FileNotFoundError`](exceptions.md#FileNotFoundError) if the file
doesn’t exist and [`RuntimeError`](exceptions.md#RuntimeError) if the file is invalid or
empty.

* **Parameters:**
  * **fname** – A filename, or a file-like object, or an instance derived
    from [`RawConfigParser`](configparser.md#configparser.RawConfigParser). If a
    `RawConfigParser`-derived instance is passed, it is used as
    is. Otherwise, a [`ConfigParser`](configparser.md#configparser.ConfigParser) is
    instantiated, and the configuration read by it from the
    object passed in `fname`. If that has a [`readline()`](readline.md#module-readline)
    method, it is assumed to be a file-like object and read using
    [`read_file()`](configparser.md#configparser.ConfigParser.read_file); otherwise,
    it is assumed to be a filename and passed to
    [`read()`](configparser.md#configparser.ConfigParser.read).
...
#### Versionchanged
Changed in version 3.4: An instance of a subclass of [`RawConfigParser`](configparser.md#configparser.RawConfigParser) is
...
sent as a file suitable for processing by [`dictConfig()`](#logging.config.dictConfig) or
[`fileConfig()`](#logging.config.fileConfig). Returns a [`Thread`](threading.md#threading.Thread) instance on which
you can call [`start()`](threading.md#threading.Thread.start) to start the server, and which
you can [`join()`](threading.md#threading.Thread.join) when appropriate. To stop the server,
call [`stopListening()`](#logging.config.stopListening).
...
<a id="logging-eval-security"></a>

#### NOTE
Because portions of the configuration are passed through
[`eval()`](functions.md#eval), use of this function may open its users to a security risk.
...
[`listen()`](#logging.config.listen). Specifically, if the process calling [`listen()`](#logging.config.listen) runs
...
[`listen()`](#logging.config.listen) socket and sending a configuration which runs whatever
code the attacker wants to have executed in the victim’s process. This is
especially easy to do if the default port is used, but not hard even if a
different port is used. To avoid the risk of this happening, use the
`verify` argument to [`listen()`](#logging.config.listen) to prevent unrecognised
...
the configuration, which will use [`dictConfig()`](#logging.config.dictConfig) for configuration.
This method allows you to specify `disable_existing_loggers` as
`False` in the configuration you send.

### logging.config.stopListening()

Stops the listening server which was created with a call to [`listen()`](#logging.config.listen).
...
[User-defined objects](#logging-config-dict-userdef). However, these same mechanisms (importing
...
<a id="logging-config-dictschema"></a>
...
These objects aren’t limited to those provided by the [`logging`](logging.md#module-logging)
...
The dictionary passed to [`dictConfig()`](#logging.config.dictConfig) must contain the following
keys:
...
<a id="logging-config-dictschema-formatters"></a>
* *formatters* - the corresponding value will be a dict in which each
  key is a formatter id and each value is a dict describing how to
  configure the corresponding [`Formatter`](logging.md#logging.Formatter) instance.

  The configuring dict is searched for the following optional keys
  which correspond to the arguments passed to create a
  [`Formatter`](logging.md#logging.Formatter) object:
  * `format`
  * `datefmt`
  * `style`
  ...
  * `defaults` (since version >=3.12)
  ...
  arguments are as for [`Formatter`](logging.md#logging.Formatter), thus this key is
  most useful for instantiating a customised subclass of
  [`Formatter`](logging.md#logging.Formatter).  For example, the alternative class
...
* *filters* - the corresponding value will be a dict in which each key
  ...
  empty string) and this is used to construct a [`logging.Filter`](logging.md#logging.Filter)
  instance.
* *handlers* - the corresponding value will be a dict in which each
  ...
  * `filters` (optional).  A list of ids of the filters for this
  ...
  [`logging.StreamHandler`](logging.handlers.md#logging.StreamHandler), using `sys.stdout` as the underlying
  stream.  The handler with id `file` is instantiated as a
  [`logging.handlers.RotatingFileHandler`](logging.handlers.md#logging.handlers.RotatingFileHandler) with the keyword arguments
...
* *incremental* - whether the configuration is to be interpreted as
  ...
  existing [`fileConfig()`](#logging.config.fileConfig) API.
...
* *disable_existing_loggers* - whether any existing non-root loggers are
  ...
  [`fileConfig()`](#logging.config.fileConfig). If absent, this parameter defaults to `True`.
  This value is ignored if *incremental* is `True`.

<a id="logging-config-dict-incremental"></a>
...
<a id="logging-config-dict-connections"></a>
...
<a id="logging-config-dict-userdef"></a>
...
`brief`, is a standard [`logging.Formatter`](logging.md#logging.Formatter) instance with the
specified format string.  The second, with id `default`, has a
longer format and also defines the time format explicitly, and will
result in a [`logging.Formatter`](logging.md#logging.Formatter) initialized with those two format
...
standard [`logging.Formatter`](logging.md#logging.Formatter) instances are created.  The
...
<a id="handler-config-dict-order"></a>
...
<a id="logging-config-dict-externalobj"></a>
...
<a id="logging-config-dict-internalobj"></a>
...
objects which are not known to the [`logging`](logging.md#module-logging) module.  For
example, consider [`logging.handlers.MemoryHandler`](logging.handlers.md#logging.handlers.MemoryHandler), which takes
...
<a id="logging-import-resolution"></a>

### Import resolution and custom importers

Import resolution, by default, uses the builtin [`__import__()`](functions.md#import__) function
...
it with [`staticmethod()`](functions.md#staticmethod). For example:
...
You don’t need to wrap with [`staticmethod()`](functions.md#staticmethod) if you’re setting the import
callable on a configurator *instance*.

<a id="configure-queue"></a>

### Configuring QueueHandler and QueueListener

If you want to configure a [`QueueHandler`](logging.handlers.md#logging.handlers.QueueHandler), noting that this
is normally used in conjunction with a [`QueueListener`](logging.handlers.md#logging.handlers.QueueListener), you
can configure both together. After the configuration, the `QueueListener` instance
will be available as the [`listener`](logging.handlers.md#logging.handlers.QueueHandler.listener) attribute of
the created handler, and that in turn will be available to you using
[`getHandlerByName()`](logging.md#logging.getHandlerByName) and passing the name you have used for the
...
* An object implementing the [`Queue.put_nowait`](queue.md#queue.Queue.put_nowait)
  and [`Queue.get`](queue.md#queue.Queue.get) public API. For instance, this may be
  an actual instance of [`queue.Queue`](queue.md#queue.Queue) or a subclass thereof, or a proxy
  obtained by [`multiprocessing.managers.SyncManager.Queue()`](multiprocessing.md#multiprocessing.managers.SyncManager.Queue).
...
* A string that resolves to a callable which, when called with no arguments, returns
  the queue instance to use. That callable could be a [`queue.Queue`](queue.md#queue.Queue) subclass
...
* A dict with a `'()'` key which is constructed in the usual way as discussed in
  ...
  [`queue.Queue`](queue.md#queue.Queue) instance.

If the  `queue` key is absent, a standard unbounded [`queue.Queue`](queue.md#queue.Queue) instance is
created and used.

If the `listener` key is present, the corresponding value can be one of the following:

* A subclass of [`logging.handlers.QueueListener`](logging.handlers.md#logging.handlers.QueueListener). This is of course only
...
If the `listener` key is absent, [`logging.handlers.QueueListener`](logging.handlers.md#logging.handlers.QueueListener) is used.
...
initialization signatures as [`QueueHandler`](logging.handlers.md#logging.handlers.QueueHandler) and
[`QueueListener`](logging.handlers.md#logging.handlers.QueueListener).

#### Versionadded
Added in version 3.12.

<a id="logging-config-fileformat"></a>

## Configuration file format

The configuration file format understood by [`fileConfig()`](#logging.config.fileConfig) is based on
[`configparser`](configparser.md#module-configparser) functionality. The file must contain sections called
...
#### NOTE
The [`fileConfig()`](#logging.config.fileConfig) API is older than the [`dictConfig()`](#logging.config.dictConfig) API and does
not provide functionality to cover certain aspects of logging. For example,
you cannot configure [`Filter`](logging.md#logging.Filter) objects, which provide for
filtering of messages beyond simple integer levels, using [`fileConfig()`](#logging.config.fileConfig).
If you need to have instances of [`Filter`](logging.md#logging.Filter) in your logging
configuration, you will need to use [`dictConfig()`](#logging.config.dictConfig). Note that future
enhancements to configuration functionality will be added to
[`dictConfig()`](#logging.config.dictConfig), so it’s worth considering transitioning to this newer
...
logged. Level values are [evaluated](functions.md#func-eval) in the context of the `logging`
package’s namespace.
...
The `class` entry indicates the handler’s class (as determined by [`eval()`](functions.md#eval)
...
The `args` entry, when [evaluated](functions.md#func-eval) in the context of the `logging`
package’s namespace, is the list of arguments to the constructor for the handler
class. Refer to the constructors for the relevant handlers, or to the examples
below, to see how typical entries are constructed. If not provided, it defaults
to `()`.

The optional `kwargs` entry, when [evaluated](functions.md#func-eval) in the context of the
...
The `defaults` entry, when [evaluated](functions.md#func-eval) in the context of
...
#### NOTE
Due to the use of [`eval()`](functions.md#eval) as described above, there are
potential security risks which result from using the [`listen()`](#logging.config.listen) to send
and receive configurations via sockets. The risks are limited to where
multiple users with no mutual trust run code on the same machine; see the
[`listen()`](#logging.config.listen) documentation for more information.
