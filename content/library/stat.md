# `stat` — Interpreting [`stat()`](os.md#os.stat) results

**Source code:** [Lib/stat.py](https://github.com/python/cpython/tree/main/Lib/stat.py)

---

The `stat` module defines constants and functions for interpreting the
results of [`os.stat()`](os.md#os.stat), [`os.fstat()`](os.md#os.fstat) and [`os.lstat()`](os.md#os.lstat) (if they
...
[`os.chmod()`](os.md#os.chmod)—that is, the file’s permission bits, plus the sticky
...
that isn’t handled by [`os.path`](os.path.md#module-os.path), like the tests for block and character
devices.
...
#### Versionchanged
Changed in version 3.4: The function supports [`S_IFDOOR`](#stat.S_IFDOOR), [`S_IFPORT`](#stat.S_IFPORT) and
[`S_IFWHT`](#stat.S_IFWHT).

All the variables below are simply symbolic indexes into the 10-tuple returned
by [`os.stat()`](os.md#os.stat), [`os.fstat()`](os.md#os.fstat) or [`os.lstat()`](os.md#os.lstat).
...
bytes waiting to be read at the time of the call to [`os.stat()`](os.md#os.stat),
[`os.fstat()`](os.md#os.fstat), or [`os.lstat()`](os.md#os.lstat); this can sometimes be useful, especially
...
The variables below define the flags used in the [`ST_MODE`](#stat.ST_MODE) field.
...
#### NOTE
[`S_IFDOOR`](#stat.S_IFDOOR), [`S_IFPORT`](#stat.S_IFPORT) or [`S_IFWHT`](#stat.S_IFWHT) are defined as
0 when the platform does not have support for the file types.

The following flags can also be used in the *mode* argument of [`os.chmod()`](os.md#os.chmod):
...
created there will also get the [`S_ISGID`](#stat.S_ISGID) bit set.  For a
file that does not have the group execution bit ([`S_IXGRP`](#stat.S_IXGRP))
set, the set-group-ID bit indicates mandatory file/record locking
(see also [`S_ENFMT`](#stat.S_ENFMT)).
...
System V file locking enforcement.  This flag is shared with [`S_ISGID`](#stat.S_ISGID):
file/record locking is enforced on files that do not have the group
execution bit ([`S_IXGRP`](#stat.S_IXGRP)) set.

### stat.S_IREAD

Unix V7 synonym for [`S_IRUSR`](#stat.S_IRUSR).

### stat.S_IWRITE

Unix V7 synonym for [`S_IWUSR`](#stat.S_IWUSR).

### stat.S_IEXEC

Unix V7 synonym for [`S_IXUSR`](#stat.S_IXUSR).

The following flags can be used in the *flags* argument of [`os.chflags()`](os.md#os.chflags):
...
See the \*BSD or macOS systems man page  for more information.

On Windows, the following file attribute constants are available for use when
testing bits in the `st_file_attributes` member returned by [`os.stat()`](os.md#os.stat).
...
`st_reparse_tag` member returned by [`os.lstat()`](os.md#os.lstat). These are well-known
...
testing bits in the [`stx_attributes`](os.md#os.statx_result.stx_attributes) and
[`stx_attributes_mask`](os.md#os.statx_result.stx_attributes_mask) members returned by
[`os.statx()`](os.md#os.statx).  See the  man page for more detail on the
