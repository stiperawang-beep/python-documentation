...
**Source code:** [Lib/random.py](https://github.com/python/cpython/tree/main/Lib/random.py)
...
Almost all module functions depend on the basic function [`random()`](#random.random), which
...
instance of the [`random.Random`](#random.Random) class.  You can instantiate your own
instances of [`Random`](#random.Random) to get generators that don’t share state.

Class [`Random`](#random.Random) can also be subclassed if you want to use a different
basic generator of your own devising: see the documentation on that class for
more details.

The `random` module also provides the [`SystemRandom`](#random.SystemRandom) class which
uses the system function [`os.urandom()`](os.md#os.urandom) to generate random numbers
from sources provided by the operating system.

#### WARNING
The pseudo-random generators of this module should not be used for
security purposes.  For security or cryptographic uses, see the
[`secrets`](secrets.md#module-secrets) module.
...
#### NOTE
The global random number generator and instances of [`Random`](#random.Random) are thread-safe.
However, in the free-threaded build, concurrent calls to the global generator or
to the same instance of [`Random`](#random.Random) may encounter contention and poor performance.
Consider using separate instances of [`Random`](#random.Random) per thread instead.
...
instead of the system time (see the [`os.urandom()`](os.md#os.urandom) function for details
on availability).

If *a* is an int, its absolute value is used directly.

With version 2 (the default), a [`str`](stdtypes.md#str), [`bytes`](stdtypes.md#bytes), or [`bytearray`](stdtypes.md#bytearray)
object gets converted to an [`int`](functions.md#int) and all of its bits are used.

With version 1 (provided for reproducing random sequences from older versions
of Python), the algorithm for [`str`](stdtypes.md#str) and [`bytes`](stdtypes.md#bytes) generates a
...
`None`, [`int`](functions.md#int), [`float`](functions.md#float), [`str`](stdtypes.md#str),
[`bytes`](stdtypes.md#bytes), or [`bytearray`](stdtypes.md#bytearray).
...
Use [`secrets.token_bytes()`](secrets.md#secrets.token_bytes) instead.
...
The positional argument pattern matches the [`range()`](stdtypes.md#range) function.
...
#### Versionchanged
Changed in version 3.2: [`randrange()`](#random.randrange) is more sophisticated about producing equally distributed
...
now raise a [`TypeError`](exceptions.md#TypeError).
...
[`getrandbits()`](#random.getrandbits) enables [`randrange()`](#random.randrange) to handle arbitrarily large
ranges.
...
raises [`IndexError`](exceptions.md#IndexError).

### random.choices(population, weights=None, , cum_weights=None, k=1)

Return a *k* sized list of elements chosen from the *population* with replacement.
If the *population* is empty, raises [`IndexError`](exceptions.md#IndexError).

If a *weights* sequence is specified, selections are made according to the
relative weights.  Alternatively, if a *cum_weights* sequence is given, the
selections are made according to the cumulative weights (perhaps computed
using [`itertools.accumulate()`](itertools.md#itertools.accumulate)).  For example, the relative weights
...
the same length as the *population* sequence.  It is a [`TypeError`](exceptions.md#TypeError)
to specify both *weights* and *cum_weights*.

The *weights* or *cum_weights* can use any numeric type that interoperates
with the [`float`](functions.md#float) values returned by [`random()`](#module-random) (that includes
integers, floats, and fractions but excludes decimals).  Weights are assumed
to be non-negative and finite.  A [`ValueError`](exceptions.md#ValueError) is raised if all
weights are zero.

For a given seed, the [`choices()`](#random.choices) function with equal weighting
typically produces a different sequence than repeated calls to
[`choice()`](#random.choice).  The algorithm used by [`choices()`](#random.choices) uses floating-point
arithmetic for internal consistency and speed.  The algorithm used
by [`choice()`](#random.choice) defaults to integer arithmetic with repeated selections
...
#### Versionchanged
Changed in version 3.9: Raises a [`ValueError`](exceptions.md#ValueError) if all weights are zero.
...
Members of the population need not be [hashable](../glossary.md#term-hashable) or unique.  If the population
...
To choose a sample from a range of integers, use a [`range()`](stdtypes.md#range) object as an
argument.  This is especially fast and space efficient for sampling from a large
population:  `sample(range(10000000), k=60)`.

If the sample size is larger than the population size, a [`ValueError`](exceptions.md#ValueError)
is raised.
...
<a id="real-valued-distributions"></a>
...
the [`normalvariate()`](#random.normalvariate) function defined below.
...
slower, but thread-safe [`normalvariate()`](#random.normalvariate) function instead.
...
Override this method in subclasses to customise the [`seed()`](#random.seed)
behaviour of `Random` instances.

#### getstate()

Override this method in subclasses to customise the [`getstate()`](#random.getstate)
behaviour of `Random` instances.

#### setstate(state)

Override this method in subclasses to customise the [`setstate()`](#random.setstate)
behaviour of `Random` instances.

#### random()

Override this method in subclasses to customise the [`random()`](#random.random)
behaviour of `Random` instances.
...
[`getrandbits()`](#random.getrandbits) behaviour of `Random` instances.

#### randbytes(n)

Override this method in subclasses to customise the
[`randbytes()`](#random.randbytes) behaviour of `Random` instances.

### *class* random.SystemRandom()

Class that uses the [`os.urandom()`](os.md#os.urandom) function for generating random numbers
from sources provided by the operating system. Not available on all systems.
Does not rely on software state, and sequences are not reproducible. Accordingly,
the [`seed()`](#random.seed) method has no effect and is ignored.
The [`getstate()`](#random.getstate) and [`setstate()`](#random.setstate) methods raise
[`NotImplementedError`](exceptions.md#NotImplementedError) if called.
...
* If a new seeding method is added, then a backward compatible seeder will be
  offered.
* The generator’s [`random()`](#random.Random.random) method will continue to produce the same
  sequence when the compatible seeder is given the same seed.

<a id="random-examples"></a>
...
from the combinatoric iterators in the [`itertools`](itertools.md#module-itertools) module
or the [more-itertools](https://pypi.org/project/more-itertools/) project:
...
The default [`random()`](#random.random) returns multiples of 2⁻⁵³ in the range
...
fine-grained floats than normally generated by [`random()`](#random.random).
...
Print a random choice, using [`choice()`](#random.choice).

### -i <N>

### --integer <N>

Print a random integer between 1 and N inclusive, using [`randint()`](#random.randint).
