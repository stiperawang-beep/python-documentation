...
**Source code:** [Lib/pty.py](https://github.com/python/cpython/tree/main/Lib/pty.py)
...
The returned file descriptor *fd* is [non-inheritable](os.md#fd-inheritance).
...
Open a new pseudo-terminal pair, using [`os.openpty()`](os.md#os.openpty) if possible, or
...
Return the exit status value from [`os.waitpid()`](os.md#os.waitpid) on the child process.

[`os.waitstatus_to_exitcode()`](os.md#os.waitstatus_to_exitcode) can be used to convert the exit status into
an exit code.

Raises an [auditing event](sys.md#auditing) `pty.spawn` with argument `argv`.

#### Versionchanged
Changed in version 3.4: [`spawn()`](#pty.spawn) now returns the status value from [`os.waitpid()`](os.md#os.waitpid)
on the child process.

## Example

The following program acts like the Unix command , using a
