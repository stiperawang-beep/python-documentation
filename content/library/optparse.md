...
* options that have a default value can include `%default` in the help
  string—`optparse` will replace it with [`str()`](stdtypes.md#str) of the option’s
...
there’s no such [`OptionGroup`](#optparse.OptionGroup), return `None`.
...
* `"help"`
  ...
  OptionParser’s constructor and the [`help`](#optparse.Option.help) string passed to every
  option.
...
: a [`Values`](#optparse.Values) object to store option arguments in (default: a
  new instance of [`Values`](#optparse.Values)) – if you give an existing object, the
...
As an example, let’s define an [`OptionParser`](#optparse.OptionParser) that resolves conflicts
...
: is the argument to this option seen on the command-line.  `optparse` will
  ...
  the type implied by the option’s type.  If [`type`](#optparse.Option.type) for this option is
...
`optparse`’s types: [`TYPES`](#optparse.Option.TYPES) and [`TYPE_CHECKER`](#optparse.Option.TYPE_CHECKER).
...
argument, which is passed as-is to [`OptionParser`](#optparse.OptionParser)’s `error()`
...
(If we didn’t make a [`copy()`](copy.md#module-copy) of [`Option.TYPE_CHECKER`](#optparse.Option.TYPE_CHECKER), we would end
up modifying the [`TYPE_CHECKER`](#optparse.Option.TYPE_CHECKER) attribute of `optparse`’s
