...
**Source code:** [Lib/lzma.py](https://github.com/python/cpython/tree/main/Lib/lzma.py)
...
The interface provided by this module is very similar to that of the [`bz2`](bz2.md#module-bz2)
module. Note that [`LZMAFile`](#lzma.LZMAFile) and [`bz2.BZ2File`](bz2.md#bz2.BZ2File) are *not*
thread-safe, so if you need to use a single [`LZMAFile`](#lzma.LZMAFile) instance
...
[`str`](stdtypes.md#str), [`bytes`](stdtypes.md#bytes) or [path-like](../glossary.md#term-path-like-object) object), in
...
the same meanings as for [`LZMADecompressor`](#lzma.LZMADecompressor). In this case, the *check*
and *preset* arguments should not be used.

When opening a file for writing, the *format*, *check*, *preset* and
*filters* arguments have the same meanings as for [`LZMACompressor`](#lzma.LZMACompressor).

For binary mode, this function is equivalent to the [`LZMAFile`](#lzma.LZMAFile)
constructor: `LZMAFile(filename, mode, ...)`. In this case, the *encoding*,
*errors* and *newline* arguments must not be provided.

For text mode, a [`LZMAFile`](#lzma.LZMAFile) object is created, and wrapped in an
...
An [`LZMAFile`](#lzma.LZMAFile) can wrap an already-open [file object](../glossary.md#term-file-object), or operate
...
[`bytes`](stdtypes.md#bytes) or [path-like](../glossary.md#term-path-like-object) object). When wrapping an
existing file object, the wrapped file will not be closed when the
[`LZMAFile`](#lzma.LZMAFile) is closed.
...
[`LZMAFile`](#lzma.LZMAFile) supports all the members specified by
...
#### NOTE
While calling [`peek()`](#lzma.LZMAFile.peek) does not change the file position of
the [`LZMAFile`](#lzma.LZMAFile), it may change the position of the underlying
file object (e.g. if the [`LZMAFile`](#lzma.LZMAFile) was constructed by passing a
...
The lzma file name.  Equivalent to the [`name`](io.md#io.FileIO.name)
...
* `FORMAT_RAW`: A raw data stream, not using any container format.
  : This format specifier does not support integrity checks, and requires that
    ...
    decompressed using `FORMAT_AUTO` (see [`LZMADecompressor`](#lzma.LZMADecompressor)).
...
If the specified check is not supported, an [`LZMAError`](#lzma.LZMAError) is raised.
...
[`LZMACompressor`](#lzma.LZMACompressor) object can be as high as 800 MiB. For this reason,
...
Compress *data* (a [`bytes`](stdtypes.md#bytes) object), returning a [`bytes`](stdtypes.md#bytes)
...
[`compress()`](#lzma.compress) and [`flush()`](#lzma.LZMACompressor.flush). The returned data should be
concatenated with the output of any previous calls to [`compress()`](#lzma.compress).
...
once, see [`decompress()`](#lzma.decompress).
...
fail with an [`LZMAError`](#lzma.LZMAError) if it is not possible to decompress the input
...
compressed streams, unlike [`decompress()`](#lzma.decompress) and [`LZMAFile`](#lzma.LZMAFile). To
decompress a multi-stream input with [`LZMADecompressor`](#lzma.LZMADecompressor), you must
...
internally, for use in later calls to [`decompress()`](#lzma.decompress). The
returned data should be concatenated with the output of any
previous calls to [`decompress()`](#lzma.decompress).

If *max_length* is nonnegative, returns at most *max_length*
bytes of decompressed data. If this limit is reached and further
output can be produced, the [`needs_input`](#lzma.LZMADecompressor.needs_input) attribute will
be set to `False`. In this case, the next call to
[`decompress()`](#lzma.LZMADecompressor.decompress) may provide *data* as `b''` to obtain
more of the output.

If all of the input data was decompressed and returned (either
because this was less than *max_length* bytes, or because
*max_length* was negative), the [`needs_input`](#lzma.LZMADecompressor.needs_input) attribute
will be set to `True`.

Attempting to decompress data after the end of stream is reached
raises an [`EOFError`](exceptions.md#EOFError).  Any data found after the end of the
stream is ignored and saved in the [`unused_data`](#lzma.LZMADecompressor.unused_data) attribute.
...
`False` if the [`decompress()`](#lzma.LZMADecompressor.decompress) method can provide more
...
Compress *data* (a [`bytes`](stdtypes.md#bytes) object), returning the compressed data as a
[`bytes`](stdtypes.md#bytes) object.

See [`LZMACompressor`](#lzma.LZMACompressor) above for a description of the *format*, *check*,
*preset* and *filters* arguments.

### lzma.decompress(data, format=FORMAT_AUTO, memlimit=None, filters=None)

Decompress *data* (a [`bytes`](stdtypes.md#bytes) object), returning the uncompressed data
as a [`bytes`](stdtypes.md#bytes) object.

If *data* is the concatenation of multiple distinct compressed streams,
decompress all of these streams, and return the concatenation of the results.

See [`LZMADecompressor`](#lzma.LZMADecompressor) above for a description of the *format*,
