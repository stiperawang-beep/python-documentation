...
**Source code:** [Lib/ast.py](https://github.com/python/cpython/tree/main/Lib/ast.py)
...
An abstract syntax tree can be generated by passing [`ast.PyCF_ONLY_AST`](#ast.PyCF_ONLY_AST) as
a flag to the [`compile()`](functions.md#compile) built-in function, or using the [`parse()`](#ast.parse)
helper provided in this module.  The result will be a tree of objects whose
classes all inherit from [`ast.AST`](#ast.AST).  An abstract syntax tree can be
compiled into a Python code object using the built-in [`compile()`](functions.md#compile) function.

<a id="abstract-grammar"></a>
...
[above](#abstract-grammar).  They are defined in the `_ast` C
...
[`ast.BinOp`](#ast.BinOp) inherits from `ast.expr`.  For production rules
...
of the type as defined in the grammar.  For example, [`ast.BinOp`](#ast.BinOp)
...
values when compiling an AST with [`compile()`](functions.md#compile).

#### \_field_types

The `_field_types` attribute on each concrete class is a dictionary
mapping field names (as also listed in [`_fields`](#ast.AST._fields)) to their types.
...
[`lineno`](#ast.AST.lineno), [`col_offset`](#ast.AST.col_offset), [`end_lineno`](#ast.AST.end_lineno), and
[`end_col_offset`](#ast.AST.end_col_offset) attributes.  The [`lineno`](#ast.AST.lineno) and [`end_lineno`](#ast.AST.end_lineno)
are the first and last line numbers of source text span (1-indexed so the
first line is line 1) and the [`col_offset`](#ast.AST.col_offset) and [`end_col_offset`](#ast.AST.end_col_offset)
...
For example, to create and populate an [`ast.UnaryOp`](#ast.UnaryOp) node, you could
use
...
list. If a field of type `ast.expr_context` is omitted, it defaults to
[`Load()`](#ast.Load). If any other field is omitted, a [`DeprecationWarning`](exceptions.md#DeprecationWarning) is raised
and the AST node will not have this field. In Python 3.15, this condition will
raise an error.

#### Versionchanged
Changed in version 3.8: Class [`ast.Constant`](#ast.Constant) is now used for all constants.
...
#### Versionchanged
Changed in version 3.13: AST node constructors were changed to provide sensible defaults for omitted
...
[`Load()`](#ast.Load). Previously, omitted attributes would not exist on constructed
nodes (accessing them raised [`AttributeError`](exceptions.md#AttributeError)).

#### Versionchanged
Changed in version 3.14: The [`__repr__()`](../reference/datamodel.md#object.__repr__) output of [`AST`](#ast.AST) nodes includes
...
`ast.Str`, `ast.Bytes`, `ast.NameConstant` and
`ast.Ellipsis`, which were deprecated in Python 3.8. These classes
...
#### Deprecated-removed
Deprecated since version 3.13, removed in version 3.15: Previous versions of Python allowed the creation of AST nodes that were missing
...
<a id="ast-root-nodes"></a>

### Root nodes

### *class* ast.Module(body, type_ignores)

A Python module, as with [file input](../reference/toplevel_components.md#file-input).
Node type generated by [`ast.parse()`](#ast.parse) in the default `"exec"` *mode*.

`body` is a [`list`](stdtypes.md#list) of the module’s [Statements](#ast-statements).

`type_ignores` is a [`list`](stdtypes.md#list) of the module’s type ignore comments;
see [`ast.parse()`](#ast.parse) for more details.
...
A single Python [expression input](../reference/toplevel_components.md#expression-input).
Node type generated by [`ast.parse()`](#ast.parse) when *mode* is `"eval"`.

`body` is a single node,
one of the [expression types](#ast-expressions).
...
A single [interactive input](../reference/toplevel_components.md#interactive), like in [Interactive Mode](../tutorial/appendix.md#tut-interac).
Node type generated by [`ast.parse()`](#ast.parse) when *mode* is `"single"`.

`body` is a [`list`](stdtypes.md#list) of [statement nodes](#ast-statements).
...
Node type generated by [`ast.parse()`](#ast.parse) when *mode* is `"func_type"`.
...
`argtypes` is a [`list`](stdtypes.md#list) of [expression nodes](#ast-expressions).

`returns` is a single [expression node](#ast-expressions).
...
Python object it represents. The values represented can be instances of [`str`](stdtypes.md#str),
[`bytes`](stdtypes.md#bytes), [`int`](functions.md#int), [`float`](functions.md#float), [`complex`](functions.md#complex), and [`bool`](functions.md#bool),
and the constants [`None`](constants.md#None) and [`Ellipsis`](constants.md#Ellipsis).
...
isolated otherwise it appears in [`JoinedStr`](#ast.JoinedStr).

* `value` is any expression node (such as a literal, a variable, or a
  function call).
* `conversion` is an integer:
  * -1: no formatting
  * 97 (`ord('a')`): `!a` [`ASCII`](functions.md#ascii) formatting
  * 114 (`ord('r')`): `!r` [`repr()`](functions.md#repr) formatting
  * 115 (`ord('s')`): `!s` [`string`](stdtypes.md#str) formatting
* `format_spec` is a [`JoinedStr`](#ast.JoinedStr) node representing the formatting
  of the value, or `None` if no format was specified. Both
  `conversion` and `format_spec` can be set at the same time.

### *class* ast.JoinedStr(values)

An f-string, comprising a series of [`FormattedValue`](#ast.FormattedValue) and [`Constant`](#ast.Constant)
nodes.
...
[`Interpolation`](#ast.Interpolation) and [`Constant`](#ast.Constant) nodes.
...
* `str` is a constant containing the text of the interpolation expression.
  ...
  when calling [`ast.unparse()`](#ast.unparse). This no longer guarantees that the
  generated code is identical to the original and is intended for code
  generation.
* `conversion` is an integer:
  * -1: no conversion
  * 97 (`ord('a')`): `!a` [`ASCII`](functions.md#ascii) conversion
  * 114 (`ord('r')`): `!r` [`repr()`](functions.md#repr) conversion
  * 115 (`ord('s')`): `!s` [`string`](stdtypes.md#str) conversion
...
`ctx` is [`Store`](#ast.Store) if the container is an assignment target (i.e.
`(x,y)=something`), and [`Load`](#ast.Load) otherwise.
...
[`Name`](#ast.Name) node. This type must be used when building a [`Call`](#ast.Call)
node with `*args`.
...
`value` holds one of the other nodes in this section, a [`Constant`](#ast.Constant), a
[`Name`](#ast.Name), a [`Lambda`](#ast.Lambda), a [`Yield`](#ast.Yield) or [`YieldFrom`](#ast.YieldFrom) node.
...
Unary operator tokens. [`Not`](#ast.Not) is the `not` keyword, [`Invert`](#ast.Invert)
is the `~` operator.
...
A boolean operation, ‘or’ or ‘and’. `op` is [`Or`](#ast.Or) or [`And`](#ast.And).
...
This doesn’t include `not`, which is a [`UnaryOp`](#ast.UnaryOp).
...
[`Name`](#ast.Name) or [`Attribute`](#ast.Attribute) object. Of the arguments:

* `args` holds a list of the arguments passed by position.
* `keywords` holds a list of [`keyword`](#ast.keyword) objects representing
...
in the following example, all three are [`Name`](#ast.Name) nodes.
...
[`Name`](#ast.Name). `attr` is a bare string giving the name of the attribute,
and `ctx` is [`Load`](#ast.Load), [`Store`](#ast.Store) or [`Del`](#ast.Del) according to how
...
operator (also known as the walrus operator). As opposed to the [`Assign`](#ast.Assign)
...
It can be a [`Tuple`](#ast.Tuple) and contain a [`Slice`](#ast.Slice).
`ctx` is [`Load`](#ast.Load), [`Store`](#ast.Store) or [`Del`](#ast.Del)
...
Can occur only inside the *slice* field of [`Subscript`](#ast.Subscript), either
directly or as an element of [`Tuple`](#ast.Tuple).
...
`generators` is a list of [`comprehension`](#ast.comprehension) nodes.
...
each element - typically a [`Name`](#ast.Name) or [`Tuple`](#ast.Tuple) node. `iter`
...
Unpacking is represented by putting a [`Tuple`](#ast.Tuple) or [`List`](#ast.List)
within `targets`.
...
be a [`Name`](#ast.Name), an [`Attribute`](#ast.Attribute) or a [`Subscript`](#ast.Subscript).
`annotation` is the annotation, such as a [`Constant`](#ast.Constant) or [`Name`](#ast.Name)
...
[`Name`](#ast.Name) node that does not appear between parentheses; all other
targets are considered complex. Only simple targets appear in
the [`__annotations__`](../reference/datamodel.md#object.__annotations__) dictionary of modules and classes.
...
`target` is a [`Name`](#ast.Name) node for `x` (with the [`Store`](#ast.Store)
context), `op` is [`Add`](#ast.Add), and `value` is a [`Constant`](#ast.Constant) with
value for 1.

The `target` attribute cannot be of class [`Tuple`](#ast.Tuple) or [`List`](#ast.List),
unlike the targets of [`Assign`](#ast.Assign).
...
[`Call`](#ast.Call) or [`Name`](#ast.Name), or `None` for a standalone `raise`.
...
An assertion. `test` holds the condition, such as a [`Compare`](#ast.Compare) node.
...
[`Name`](#ast.Name), [`Attribute`](#ast.Attribute) or [`Subscript`](#ast.Subscript) nodes.
...
A [type alias](typing.md#type-aliases) created through the [`type`](../reference/simple_stmts.md#type)
statement. `name` is the name of the alias, `type_params` is a list of
[type parameters](#ast-type-params), and `value` is the value of the
type alias.
...
An import statement. `names` is a list of [`alias`](#ast.alias) nodes.
...
An `if` statement. `test` holds a single node, such as a [`Compare`](#ast.Compare)
node. `body` and `orelse` each hold a list of nodes.

`elif` clauses don’t have a special representation in the AST, but rather
appear as extra [`If`](#ast.If) nodes within the `orelse` section of the
previous one.
...
single [`Name`](#ast.Name), [`Tuple`](#ast.Tuple), [`List`](#ast.List), [`Attribute`](#ast.Attribute) or
[`Subscript`](#ast.Subscript) node. `iter` holds the item to be looped over, again
...
A `while` loop. `test` holds the condition, such as a [`Compare`](#ast.Compare)
node.
...
`handlers`, which is a list of [`ExceptHandler`](#ast.ExceptHandler) nodes.
...
same as for [`Try`](#ast.Try) but the [`ExceptHandler`](#ast.ExceptHandler) nodes in `handlers`
...
typically a [`Name`](#ast.Name) node (or `None` for a catch-all `except:` clause).
...
A `with` block. `items` is a list of [`withitem`](#ast.withitem) nodes representing
...
manager, often a [`Call`](#ast.Call) node. `optional_vars` is a [`Name`](#ast.Name),
[`Tuple`](#ast.Tuple) or [`List`](#ast.List) for the `as foo` part, or `None` if that
isn’t used.
...
[`match_case`](#ast.match_case) nodes with the different cases.
...
[`AST`](#ast.AST) nodes produced for patterns differ from those produced for
...
is set to `False` (default).  See [`ast.parse()`](#ast.parse) for more details.

#### Versionadded
Added in version 3.8.

<a id="ast-type-params"></a>

### Type parameters

[Type parameters](../reference/compound_stmts.md#type-params) can exist on classes, functions, and type
aliases.

### *class* ast.TypeVar(name, bound, default_value)

A [`typing.TypeVar`](typing.md#typing.TypeVar). `name` is the name of the type variable.
`bound` is the bound or constraints, if any. If `bound` is a [`Tuple`](#ast.Tuple),
...
A [`typing.ParamSpec`](typing.md#typing.ParamSpec). `name` is the name of the parameter specification.
...
A [`typing.TypeVarTuple`](typing.md#typing.TypeVarTuple). `name` is the name of the type variable tuple.
...
* `args` is an [`arguments`](#ast.arguments) node.
* `body` is the list of nodes inside the function.
* `decorator_list` is the list of decorators to be applied, stored outermost
  first (i.e. the first in the list will be applied last).
* `returns` is the return annotation.
* `type_params` is a list of [type parameters](#ast-type-params).
...
expression. Unlike [`FunctionDef`](#ast.FunctionDef), `body` holds a single node.
...
* `posonlyargs`, `args` and `kwonlyargs` are lists of [`arg`](#ast.arg) nodes.
* `vararg` and `kwarg` are single [`arg`](#ast.arg) nodes, referring to the
...
name; `annotation` is its annotation, such as a [`Name`](#ast.Name) node.
...
must be wrapped in an [`Expr`](#ast.Expr) node if the value sent back is not used.
...
* `keywords` is a list of [`keyword`](#ast.keyword) nodes, principally for ‘metaclass’.
  Other keywords will be passed to the metaclass, as per [**PEP 3115**](https://peps.python.org/pep-3115/).
* `body` is a list of nodes representing the code within the class
  definition.
* `decorator_list` is a list of nodes, as in [`FunctionDef`](#ast.FunctionDef).
* `type_params` is a list of [type parameters](#ast-type-params).
...
Only valid in the body of an [`AsyncFunctionDef`](#ast.AsyncFunctionDef).
...
fields as [`For`](#ast.For) and [`With`](#ast.With), respectively. Only valid in the
body of an [`AsyncFunctionDef`](#ast.AsyncFunctionDef).

#### NOTE
When a string is parsed by [`ast.parse()`](#ast.parse), operator nodes (subclasses
of `ast.operator`, `ast.unaryop`, `ast.cmpop`,
`ast.boolop` and `ast.expr_context`) on the returned tree
will be singletons. Changes to one will be reflected in all other
occurrences of the same value (for example, [`ast.Add`](#ast.Add)).
...
This is equivalent to adding [`ast.PyCF_TYPE_COMMENTS`](#ast.PyCF_TYPE_COMMENTS) to the
flags passed to [`compile()`](functions.md#compile).  This will report syntax errors
...
attribute of [`Module`](#ast.Module) (otherwise it is always an empty list).
...
parsing of [`match`](../reference/compound_stmts.md#match) statements.
...
If source contains a null character (`\0`), [`ValueError`](exceptions.md#ValueError) is raised.

#### WARNING
Note that successfully parsing source code into an AST object doesn’t
guarantee that the source code provided is valid Python code that can
be executed as the compilation step can raise further [`SyntaxError`](exceptions.md#SyntaxError)
...
In particular, [`ast.parse()`](#ast.parse) won’t do any scoping checks, which the
...
#### Versionchanged
Changed in version 3.13: The minimum supported version for `feature_version` is now `(3, 7)`.
...
Unparse an [`ast.AST`](#ast.AST) object and generate a string with code
that would produce an equivalent [`ast.AST`](#ast.AST) object if parsed
back with [`ast.parse()`](#ast.parse).

#### WARNING
The produced code string will not necessarily be equal to the original
code that generated the [`ast.AST`](#ast.AST) object (without any compiler
optimizations, such as constant tuples/frozensets).

#### WARNING
Trying to unparse a highly complex expression would result with
[`RecursionError`](exceptions.md#RecursionError).
...
execute Python code, unlike the more general [`eval()`](functions.md#eval). There is no
...
It can raise [`ValueError`](exceptions.md#ValueError), [`TypeError`](exceptions.md#TypeError), [`SyntaxError`](exceptions.md#SyntaxError),
[`MemoryError`](exceptions.md#MemoryError) and [`RecursionError`](exceptions.md#RecursionError) depending on the malformed
input.
...
[`FunctionDef`](#ast.FunctionDef), [`AsyncFunctionDef`](#ast.AsyncFunctionDef), [`ClassDef`](#ast.ClassDef),
or [`Module`](#ast.Module) node), or `None` if it has no docstring.
If *clean* is true, clean up the docstring’s indentation with
[`inspect.cleandoc()`](inspect.md#inspect.cleandoc).

#### Versionchanged
Changed in version 3.5: [`AsyncFunctionDef`](#ast.AsyncFunctionDef) is now supported.

### ast.get_source_segment(source, node, , padded=False)

Get source code segment of the *source* that generated *node*.
If some location information ([`lineno`](#ast.AST.lineno), [`end_lineno`](#ast.AST.end_lineno),
[`col_offset`](#ast.AST.col_offset), or [`end_col_offset`](#ast.AST.end_col_offset)) is missing, return `None`.
...
When you compile a node tree with [`compile()`](functions.md#compile), the compiler expects
[`lineno`](#ast.AST.lineno) and [`col_offset`](#ast.AST.col_offset) attributes for every node that supports
...
Copy source location ([`lineno`](#ast.AST.lineno), [`col_offset`](#ast.AST.col_offset), [`end_lineno`](#ast.AST.end_lineno),
and [`end_col_offset`](#ast.AST.end_col_offset)) from *old_node* to *new_node* if possible,
and return *new_node*.
...
which is forwarded by the [`visit()`](#ast.NodeVisitor.visit) method.
...
class, or [`generic_visit()`](#ast.NodeVisitor.generic_visit) if that method doesn’t exist.

#### generic_visit(node)

This visitor calls [`visit()`](#ast.NodeVisitor.visit) on all children of the node.

Note that child nodes of nodes that have a custom visitor method won’t be
visited unless the visitor calls [`generic_visit()`](#ast.NodeVisitor.generic_visit) or visits them
itself.
...
Don’t use the [`NodeVisitor`](#ast.NodeVisitor) if you want to apply changes to nodes
during traversal.  For this a special visitor exists
([`NodeTransformer`](#ast.NodeTransformer)) that allows modifications.

#### Deprecated-removed
Deprecated since version 3.8, removed in version 3.14: Methods `visit_Num()`, `visit_Str()`, `visit_Bytes()`,
`visit_NameConstant()` and `visit_Ellipsis()` will not be called
in Python 3.14+.  Add the [`visit_Constant()`](#ast.NodeVisitor.visit_Constant) method instead to handle
all constant nodes.

### *class* ast.NodeTransformer

A [`NodeVisitor`](#ast.NodeVisitor) subclass that walks the abstract syntax tree and
allows modification of nodes.

The [`NodeTransformer`](#ast.NodeTransformer) will walk the AST and use the return value of
...
either transform the child nodes yourself or call the [`generic_visit()`](#ast.NodeVisitor.generic_visit)
...
If [`NodeTransformer`](#ast.NodeTransformer) introduces new nodes (that weren’t part of
original tree) without giving them location information (such as
[`lineno`](#ast.AST.lineno)), [`fix_missing_locations()`](#ast.fix_missing_locations) should be called with
...
#### Versionchanged
Changed in version 3.15: Omit optional `Load()` values by default.

<a id="ast-compiler-flags"></a>

## Compiler flags

The following flags may be passed to [`compile()`](functions.md#compile) in order to change
...
in [`compile()`](functions.md#compile) or [`ast.parse()`](#ast.parse).
