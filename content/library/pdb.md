...
**Source code:** [Lib/pdb.py](https://github.com/python/cpython/tree/main/Lib/pdb.py)
...
The debugger is extensible – it is actually defined as the class [`Pdb`](#pdb.Pdb).
This is currently undocumented but easily understood by reading the source.  The
extension interface uses the modules [`bdb`](bdb.md#module-bdb) and [`cmd`](cmd.md#module-cmd).
...
Module [`traceback`](traceback.md#module-traceback)
...
#### Versionchanged
Changed in version 3.7: The built-in [`breakpoint()`](functions.md#breakpoint), when called with defaults, can be used
...
#### Versionchanged
Changed in version 3.3: Tab-completion via the [`readline`](readline.md#module-readline) module is available for commands and
...
[optimized scope](../glossary.md#term-optimized-scope).
...
module [`__main__`](__main__.md#module-__main__) is used.  (See the explanation of the built-in
[`exec()`](functions.md#exec) or [`eval()`](functions.md#eval) functions.)

### pdb.runeval(expression, globals=None, locals=None)

Evaluate the *expression* (given as a string or a code object) under debugger
control.  When [`runeval()`](#pdb.runeval) returns, it returns the value of the
*expression*.  Otherwise this function is similar to [`run()`](#pdb.run).

### pdb.runcall(function, \*args, \*\*kwds)

Call the *function* (a function or method object, not a string) with the
given arguments.  When [`runcall()`](#pdb.runcall) returns, it returns whatever the
...
#### Versionchanged
Changed in version 3.13: [`set_trace()`](#pdb.set_trace) will enter the debugger immediately, rather than
...
async version of [`set_trace()`](#pdb.set_trace). This function should be used inside an
async function with [`await`](../reference/expressions.md#await).
...
[`await`](../reference/expressions.md#await) statements are supported if the debugger is invoked by this function.
...
[traceback object](../reference/datamodel.md#traceback-objects). If no value is given, it uses
...
[`sys.last_exc`](sys.md#sys.last_exc).

### pdb.set_default_backend(backend)

There are two supported backends for pdb: `'settrace'` and `'monitoring'`.
See [`bdb.Bdb`](bdb.md#bdb.Bdb) for details. The user can set the default backend to
use if none is specified when instantiating [`Pdb`](#pdb.Pdb). If no backend is
specified, the default is `'settrace'`.

#### NOTE
[`breakpoint()`](functions.md#breakpoint) and [`set_trace()`](#pdb.set_trace) will not be affected by this
...
The `run*` functions and [`set_trace()`](#pdb.set_trace) are aliases for instantiating the
[`Pdb`](#pdb.Pdb) class and calling the method of the same name.  If you want to
...
[`Pdb`](#pdb.Pdb) is the debugger class.

The *completekey*, *stdin* and *stdout* arguments are passed to the
underlying [`cmd.Cmd`](cmd.md#cmd.Cmd) class; see the description there.

The *skip* argument, if given, must be an iterable of glob-style module name
patterns.  The debugger will not step into frames that originate in a module
that matches one of these patterns. <sup>[1](#id3)</sup>
...
is passed, the default backend will be used. See [`set_default_backend()`](#pdb.set_default_backend).
...
Raises an [auditing event](sys.md#auditing) `pdb.Pdb` with no arguments.
...
#### Versionchanged
Changed in version 3.14: Inline breakpoints like [`breakpoint()`](functions.md#breakpoint) or [`pdb.set_trace()`](#pdb.set_trace) will
...
documentation (the docstring of the [`pdb`](#module-pdb) module).  Since the *command*
...
yet).  The file is searched on [`sys.path`](sys.md#sys.path).  Acceptable forms of *filename*
...
[`return`](#pdbcommand-return), [`until`](#pdbcommand-until), [`jump`](#pdbcommand-jump), [`quit`](#pdbcommand-quit) and their abbreviations)
...
#### Versionchanged
Changed in version 3.14: Frame information will not be displayed if a command that resumes execution
...
possible to jump into the middle of a [`for`](../reference/compound_stmts.md#for) loop or out of a
[`finally`](../reference/compound_stmts.md#finally) clause.
...
Python [`print()`](functions.md#print) function.

### pp expression

Like the [`p`](#pdbcommand-p) command, except the value of *expression* is
pretty-printed using the [`pprint`](pprint.md#module-pprint) module.
...
Start an interactive interpreter (using the [`code`](code.md#module-code) module) in a new
...
#### Versionchanged
Changed in version 3.13: [`interact`](#pdbcommand-interact) directs its output to the debugger’s
output channel rather than [`sys.stderr`](sys.md#sys.stderr).
...
[`global`](../reference/simple_stmts.md#global) statement on the same line, e.g.:
...
with [`shlex`](shlex.md#module-shlex) and the result is used as the new [`sys.argv`](sys.md#sys.argv).
History, breakpoints, actions and debugger options are preserved.
[`restart`](#pdbcommand-restart) is an alias for [`run`](#pdbcommand-run).

#### Versionchanged
Changed in version 3.14: [`run`](#pdbcommand-run) and [`restart`](#pdbcommand-restart) commands are disabled when the
debugger is invoked in `'inline'` mode.

### q(uit)

Quit from the debugger.  The program being executed is aborted.
An end-of-file input is equivalent to [`quit`](#pdbcommand-quit).
...
#### Versionchanged
Changed in version 3.14: A confirmation prompt will be shown if the debugger is invoked in
`'inline'` mode. After the confirmation, the debugger will call
[`sys.exit()`](sys.md#sys.exit) immediately, instead of raising [`bdb.BdbQuit`](bdb.md#bdb.BdbQuit)
...
* <a id='id3'>**[1]**</a> Whether a frame is considered to originate in a certain module is determined by the `__name__` in the frame globals.
