...
An [`OSError`](exceptions.md#OSError) is raised on syscall failure.
...
Resources usage can be limited using the [`setrlimit()`](#resource.setrlimit) function described
...
described in the  man page.  The resources listed below
...
#### Versionchanged
Changed in version 3.15: It is now always positive.
...
Can be equal to [`RLIM_INFINITY`](#resource.RLIM_INFINITY).
...
*resource*. Raises [`ValueError`](exceptions.md#ValueError) if an invalid resource is specified, or
[`error`](#resource.error) if the underlying system call fails unexpectedly.
...
[`RLIM_INFINITY`](#resource.RLIM_INFINITY) can be used to request a limit that is
unlimited.

Raises [`ValueError`](exceptions.md#ValueError) if an invalid resource is specified, if the new soft
limit exceeds the hard limit, or if a process tries to raise its hard limit.
Specifying a limit of [`RLIM_INFINITY`](#resource.RLIM_INFINITY) when the hard or
system limit for that resource is not unlimited will result in a
[`ValueError`](exceptions.md#ValueError).  A process with the effective UID of super-user can
request any valid limit value, including unlimited, but [`ValueError`](exceptions.md#ValueError)
will still be raised if the requested limit exceeds the system imposed
limit.

`setrlimit` may also raise [`error`](#resource.error) if the underlying system call
fails.

VxWorks only supports setting [`RLIMIT_NOFILE`](#resource.RLIMIT_NOFILE).

Raises an [auditing event](sys.md#auditing) `resource.setrlimit` with arguments `resource`, `limits`.

### resource.prlimit(pid, resource)

Combines [`setrlimit()`](#resource.setrlimit) and [`getrlimit()`](#resource.getrlimit) in one function and
...
*limits* have the same meaning as in [`setrlimit()`](#resource.setrlimit), except that
*limits* is optional.
...
Raises [`ProcessLookupError`](exceptions.md#ProcessLookupError) when *pid* can’t be found and
[`PermissionError`](exceptions.md#PermissionError) when the user doesn’t have `CAP_SYS_RESOURCE` for
the process.

Raises an [auditing event](sys.md#auditing) `resource.prlimit` with arguments `pid`, `resource`, `limits`.

[Availability](intro.md#availability): Linux >= 2.6.36 with glibc >= 2.13.

#### Versionadded
Added in version 3.4.

These symbols define resources whose consumption can be controlled using the
[`setrlimit()`](#resource.setrlimit) and [`getrlimit()`](#resource.getrlimit) functions described below. The values of
these symbols are exactly the constants used by C programs.

The Unix man page for  lists the available resources.
...
this limit is exceeded, a [`SIGXCPU`](signal.md#signal.SIGXCPU) signal is sent to the process. (See
the [`signal`](signal.md#module-signal) module documentation for information about how to catch this
...
The BSD name for [`RLIMIT_NOFILE`](#resource.RLIMIT_NOFILE).
...
Usually an alias of [`RLIMIT_AS`](#resource.RLIMIT_AS).

[Availability](intro.md#availability): Solaris, FreeBSD, NetBSD.
...
[Availability](intro.md#availability): FreeBSD, NetBSD.
...
[Availability](intro.md#availability): FreeBSD >= 8.
...
[Availability](intro.md#availability): NetBSD >= 7.0.
...
[Availability](intro.md#availability): FreeBSD >= 14.2.
...
[Availability](intro.md#availability): AIX.
...
remaining values are integers. Consult the  man page for
...
This function will raise a [`ValueError`](exceptions.md#ValueError) if an invalid *who* parameter is
specified. It may also raise [`error`](#resource.error) exception in unusual circumstances.
...
The following `RUSAGE_*` symbols are passed to the [`getrusage()`](#resource.getrusage)
function to specify which processes information should be provided for.

### resource.RUSAGE_SELF

Pass to [`getrusage()`](#resource.getrusage) to request resources consumed by the calling
process, which is the sum of resources used by all threads in the process.

### resource.RUSAGE_CHILDREN

Pass to [`getrusage()`](#resource.getrusage) to request resources consumed by child processes
of the calling process which have been terminated and waited for.

### resource.RUSAGE_BOTH

Pass to [`getrusage()`](#resource.getrusage) to request resources consumed by both the current
process and child processes.  May not be available on all systems.

### resource.RUSAGE_THREAD

Pass to [`getrusage()`](#resource.getrusage) to request resources consumed by the current
