...
[Modules](../tutorial/modules.md#tut-modules) for an introduction.

<a id="name-equals-main"></a>
...
* the Python module or package passed to the Python interpreter with the
  [`-m`](../using/cmdline.md#cmdoption-m) argument:
  ```shell-session
  $ python -m tarfile
...
* Python code read by the Python interpreter from standard input:
  ...
  ...
  ```
* Python code passed to the Python interpreter with the [`-c`](../using/cmdline.md#cmdoption-c) argument:
  ```shell-session
...
the tutorial section [Modules](../tutorial/modules.md#tut-modules).
...
where the return value of `main` is passed into [`sys.exit()`](sys.md#sys.exit).
For example:
...
Since the call to `main` is wrapped in [`sys.exit()`](sys.md#sys.exit), the expectation is
that your function will return some value acceptable as an input to
[`sys.exit()`](sys.md#sys.exit); typically, an integer or `None` (which is implicitly
...
[`sys.exit()`](sys.md#sys.exit) will interpret a string argument as a failure message, so
your program will have an exit code of `1`, indicating failure, and the
string will be written to [`sys.stderr`](sys.md#sys.stderr).  The `echo.py` example from
...
If you are not familiar with Python packages, see section [Packages](../tutorial/modules.md#tut-packages)
...
directly from the command line using the [`-m`](../using/cmdline.md#cmdoption-m) flag. For example:
...
package.  For more details, see [Intra-package References](../tutorial/modules.md#intra-package-references) in the
[Modules](../tutorial/modules.md#tut-modules) section of the tutorial.
...
#### SEE ALSO
See [`venv`](venv.md#module-venv) for an example of a package with a minimal `__main__.py`
in the standard library. It doesn’t contain a `if __name__ == '__main__'`
block. You can invoke it with `python -m venv [directory]`.

See [`runpy`](runpy.md#module-runpy) for more details on the [`-m`](../using/cmdline.md#cmdoption-m) flag to the
interpreter executable.

See [`zipapp`](zipapp.md#module-zipapp) for how to run applications packaged as  *.zip* files. In
...
([namespace](../glossary.md#term-namespace)) by importing the `__main__` module.  This doesn’t import
...
Python inserts an empty `__main__` module in [`sys.modules`](sys.md#sys.modules) at
...
module is present in [`sys.modules`](sys.md#sys.modules), Python passes that to `namely`.
See [Special considerations for \_\_main_\_](../reference/import.md#import-dunder-main) in the
...
The `__main__` scope is used in the implementation of [`pdb`](pdb.md#module-pdb) and
[`rlcompleter`](rlcompleter.md#module-rlcompleter).
