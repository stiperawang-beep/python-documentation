...
**Source code:** [Lib/email/headerregistry.py](https://github.com/python/cpython/tree/main/Lib/email/headerregistry.py)
...
Headers are represented by customized subclasses of [`str`](stdtypes.md#str).  The
particular class used to represent a given header is determined by the
[`header_factory`](email.policy.md#email.policy.EmailPolicy.header_factory) of the [`policy`](email.policy.md#module-email.policy) in
effect when the headers are created.  This section documents the particular
`header_factory` implemented by the email package for handling [**RFC 5322**](https://datatracker.ietf.org/doc/html/rfc5322.html)
...
[`EmailPolicy`](email.policy.md#email.policy.EmailPolicy), all headers are produced by
[`HeaderRegistry`](#email.headerregistry.HeaderRegistry) and have [`BaseHeader`](#email.headerregistry.BaseHeader) as their last base
...
[`UnstructuredHeader`](#email.headerregistry.UnstructuredHeader) as their other base class.  The specialized second
class for a header is determined by the name of the header, using a lookup
table stored in the [`HeaderRegistry`](#email.headerregistry.HeaderRegistry).  All of this is managed
...
followed by the API for modifying the behavior of [`HeaderRegistry`](#email.headerregistry.HeaderRegistry), and
...
[`header_factory`](email.policy.md#email.policy.EmailPolicy.header_factory) call.  The string value of
...
[`header_factory`](email.policy.md#email.policy.EmailPolicy.header_factory) call for *name*; that
is, case is preserved.

#### defects

A tuple of [`HeaderDefect`](email.errors.md#email.errors.HeaderDefect) instances reporting any
RFC compliance problems found during parsing.  The email package tries to
be complete about detecting compliance issues.  See the [`errors`](email.errors.md#module-email.errors)
...
Return a string containing [`linesep`](email.policy.md#email.policy.Policy.linesep)
characters as required to correctly fold the header according to
*policy*.  A [`cte_type`](email.policy.md#email.policy.Policy.cte_type) of `8bit` will be
treated as if it were `7bit`, since headers may not contain arbitrary
binary data.  If [`utf8`](email.policy.md#email.policy.EmailPolicy.utf8) is `False`,
non-ASCII data will be [**RFC 2047**](https://datatracker.ietf.org/doc/html/rfc2047.html) encoded.
...
the specialized class provide a [`classmethod()`](functions.md#classmethod) named `parse`.  This
...
In [**RFC 5322**](https://datatracker.ietf.org/doc/html/rfc5322.html), an unstructured header is a run of arbitrary text in the
ASCII character set.  [**RFC 2047**](https://datatracker.ietf.org/doc/html/rfc2047.html), however, has an [**RFC 5322**](https://datatracker.ietf.org/doc/html/rfc5322.html) compatible
...
into unicode, following the [**RFC 2047**](https://datatracker.ietf.org/doc/html/rfc2047.html) rules for unstructured text.  The
...
another, this attribute will contain a [`datetime`](datetime.md#datetime.datetime)
...
information about the source timezone), then [`datetime`](#email.headerregistry.DateHeader.datetime) will be a
naive [`datetime`](datetime.md#datetime.datetime).  If a specific timezone offset is
found (including `+0000`), then [`datetime`](#email.headerregistry.DateHeader.datetime) will contain an aware
`datetime` that uses [`datetime.timezone`](datetime.md#datetime.timezone) to record the timezone
offset.

The `decoded` value of the header is determined by formatting the
`datetime` according to the [**RFC 5322**](https://datatracker.ietf.org/doc/html/rfc5322.html) rules; that is, it is set to:
...
[`datetime`](datetime.md#datetime.datetime) instance.  This means, for example, that
...
more useful is to use the [`localtime()`](email.utils.md#email.utils.localtime) function from the
[`utils`](email.utils.md#module-email.utils) module:
...
A tuple of [`Group`](#email.headerregistry.Group) objects encoding the
...
`Groups` whose [`display_name`](#email.headerregistry.Group.display_name) is `None`.

#### addresses

A tuple of [`Address`](#email.headerregistry.Address) objects encoding all
...
unicode.  [`idna`](codecs.md#module-encodings.idna) encoded domain names are also decoded to
unicode.  The `decoded` value is set by [joining](stdtypes.md#meth-str-join) the
[`str`](stdtypes.md#str) value of the elements of the `groups` attribute with `',
'`.

A list of [`Address`](#email.headerregistry.Address) and [`Group`](#email.headerregistry.Group) objects in any combination
...
A subclass of [`AddressHeader`](#email.headerregistry.AddressHeader) that adds one
...
the RFC under the default [`policy`](email.policy.md#module-email.policy)), accessing this attribute
will result in a [`ValueError`](exceptions.md#ValueError).
...
variant, [`max_count`](#email.headerregistry.BaseHeader.max_count) is set to 1.
...
#### Versionchanged
Changed in version 3.15: It is now a [`frozendict`](stdtypes.md#frozendict) instead of a
[`types.MappingProxyType`](types.md#types.MappingProxyType).

### *class* email.headerregistry.ContentTypeHeader

A [`ParameterizedMIMEHeader`](#email.headerregistry.ParameterizedMIMEHeader) class that handles the
*Content-Type* header.
...
This is the factory used by [`EmailPolicy`](email.policy.md#email.policy.EmailPolicy) by default.
...
class’s [`__bases__`](../reference/datamodel.md#type.__bases__) list.
...
To support SMTP ([**RFC 5321**](https://datatracker.ietf.org/doc/html/rfc5321.html)), `Address` handles one special case: if
...
A possibly empty tuple of [`Address`](#email.headerregistry.Address) objects representing the
...
* <a id='id2'>**[1]**</a> Originally added in 3.3 as a [provisional module](../glossary.md#term-provisional-package)
