...
**Source code:** [Lib/hashlib.py](https://github.com/python/cpython/tree/main/Lib/hashlib.py)
...
the [`zlib`](zlib.md#module-zlib) module.

<a id="hash-algorithms"></a>

## Hash algorithms

There is one constructor method named for each type of *hash*.  All return
a hash object with the same simple interface. For example: use [`sha256()`](#hashlib.sha256)
to create a SHA-256 hash object. You can now feed this object with
[bytes-like objects](../glossary.md#term-bytes-like-object) (normally [`bytes`](stdtypes.md#bytes)) using
the [`update`](#hashlib.hash.update) method.  At any point you can ask it for the
*digest* of the concatenation of the data fed to it so far using the
[`digest()`](#hashlib.hash.digest) or [`hexdigest()`](#hashlib.hash.hexdigest) methods.

To allow multithreading, the Python [GIL](../glossary.md#term-GIL) is released while computing a
hash supplied more than 2047 bytes of data at once in its constructor or
[`.update`](#hashlib.hash.update) method.

<a id="index-2"></a>

Constructors for hash algorithms that are always present in this module are
[`sha1()`](#hashlib.sha1), [`sha224()`](#hashlib.sha224), [`sha256()`](#hashlib.sha256), [`sha384()`](#hashlib.sha384), [`sha512()`](#hashlib.sha512),
[`sha3_224()`](#hashlib.sha3_224), [`sha3_256()`](#hashlib.sha3_256), [`sha3_384()`](#hashlib.sha3_384), [`sha3_512()`](#hashlib.sha3_512),
[`shake_128()`](#hashlib.shake_128), [`shake_256()`](#hashlib.shake_256), [`blake2b()`](#hashlib.blake2b), and [`blake2s()`](#hashlib.blake2s).
[`md5()`](#hashlib.md5) is normally available as well, though it may be missing or blocked
if you are using a rare “FIPS compliant” build of Python.
These correspond to [`algorithms_guaranteed`](#hashlib.algorithms_guaranteed).
...
accessible by name via [`new()`](#hashlib.new).  See [`algorithms_available`](#hashlib.algorithms_available).
...
#### Versionadded
Added in version 3.6: SHA3 (Keccak) and SHAKE constructors [`sha3_224()`](#hashlib.sha3_224), [`sha3_256()`](#hashlib.sha3_256),
[`sha3_384()`](#hashlib.sha3_384), [`sha3_512()`](#hashlib.sha3_512), [`shake_128()`](#hashlib.shake_128), [`shake_256()`](#hashlib.shake_256)
were added.
[`blake2b()`](#hashlib.blake2b) and [`blake2s()`](#hashlib.blake2s) were added.

<a id="hashlib-usedforsecurity"></a>
...
#### Deprecated-removed
Deprecated since version 3.15, will be removed in version 3.19: The undocumented `string` keyword parameter in `_hashlib.new()`
...
Using [`new()`](#hashlib.new) with an algorithm name:
...
### hashlib.sha224(\*, usedforsecurity=True)

### hashlib.sha256(\*, usedforsecurity=True)

### hashlib.sha384(\*, usedforsecurity=True)

### hashlib.sha512(\*, usedforsecurity=True)

### hashlib.sha3_224(\*, usedforsecurity=True)

### hashlib.sha3_256(\*, usedforsecurity=True)

### hashlib.sha3_384(\*, usedforsecurity=True)

### hashlib.sha3_512(\*, usedforsecurity=True)
...
[`new()`](#hashlib.new).  [`algorithms_guaranteed`](#hashlib.algorithms_guaranteed) will always be a subset.  The
...
parameter to [`new()`](#hashlib.new) to create another hash of this type.
...
Update the hash object with the [bytes-like object](../glossary.md#term-bytes-like-object).
...
Return the digest of the data passed to the [`update()`](#hashlib.hash.update) method so far.
This is a bytes object of size [`digest_size`](#hashlib.hash.digest_size) which may contain bytes in
the whole range from 0 to 255.

#### hash.hexdigest()

Like [`digest()`](#hashlib.hash.digest) except the digest is returned as a string object of
...
### hashlib.shake_128(\*, usedforsecurity=True)

### hashlib.shake_256(\*, usedforsecurity=True)

The [`shake_128()`](#hashlib.shake_128) and [`shake_256()`](#hashlib.shake_256) algorithms provide variable
...
Like [`digest()`](#hashlib.shake.digest) except the digest is returned as a string object of
double length, containing only hexadecimal digits.  This may be used to
...
It accepts file objects from  builtin [`open()`](functions.md#open), [`BytesIO`](io.md#io.BytesIO)
instances, SocketIO objects from [`socket.socket.makefile()`](socket.md#socket.socket.makefile), and
similar. *fileobj* must be opened in blocking mode, otherwise a
[`BlockingIOError`](exceptions.md#BlockingIOError) may be raised.

The function may bypass Python’s I/O and use the file descriptor
from [`fileno()`](io.md#io.IOBase.fileno) directly. *fileobj* must be assumed to be
...
#### Versionchanged
Changed in version 3.14: Now raises a [`BlockingIOError`](exceptions.md#BlockingIOError) if the file is opened in non-blocking
...
a proper source, e.g. [`os.urandom()`](os.md#os.urandom).
...
*password* and *salt* must be [bytes-like objects](../glossary.md#term-bytes-like-object).  Applications and libraries should limit *password*
to a sensible length (e.g. 1024).  *salt* should be about 16 or more
bytes from a proper source, e.g. [`os.urandom()`](os.md#os.urandom).
...
* *data*: initial chunk of data to hash, which must be
  [bytes-like object](../glossary.md#term-bytes-like-object).  It can be passed only as positional argument.
...
![Explanation of tree mode parameters.](library/hashlib-blake2-tree.png)
...
calling the appropriate constructor function ([`blake2b()`](#hashlib.blake2b) or
[`blake2s()`](#hashlib.blake2s)), then update it with the data by calling [`update()`](#hashlib.hash.update) on the
object, and, finally, get the digest out of the object by calling
[`digest()`](#hashlib.hash.digest) (or [`hexdigest()`](#hashlib.hash.hexdigest) for hex-encoded string).
...
You can call [`hash.update()`](#hashlib.hash.update) as many times as you need to iteratively
update the hash:
...
in HMAC construction with [`hmac`](hmac.md#module-hmac) module:
...
<a id="hashlib-seealso"></a>

#### SEE ALSO
Module [`hmac`](hmac.md#module-hmac)
...
[https://csrc.nist.gov/pubs/fips/202/final](https://csrc.nist.gov/pubs/fips/202/final)
