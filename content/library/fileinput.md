...
**Source code:** [Lib/fileinput.py](https://github.com/python/cpython/tree/main/Lib/fileinput.py)

---

This module implements a helper class and functions to quickly write a
loop over standard input or a list of files. If you just want to read or
write one file see [`open()`](functions.md#open).
...
first argument to [`input()`](#fileinput.input).  A single file name is also allowed.

All files are opened in text mode by default, but you can override this by
specifying the *mode* parameter in the call to [`input()`](#fileinput.input) or
[`FileInput`](#fileinput.FileInput).  If an I/O error occurs during opening or reading a file,
[`OSError`](exceptions.md#OSError) is raised.
...
*openhook* parameter to [`fileinput.input()`](#fileinput.input) or [`FileInput()`](#fileinput.FileInput). The
...
This module provides a [`hook_compressed()`](#fileinput.hook_compressed) to support compressed files.

The following function is the primary interface of this module:

### fileinput.input(files=None, inplace=False, backup='', , mode='r', openhook=None, encoding=None, errors=None)

Create an instance of the [`FileInput`](#fileinput.FileInput) class.  The instance will be used
as global state for the functions of this module, and is also returned to use
during iteration.  The parameters to this function will be passed along to the
constructor of the [`FileInput`](#fileinput.FileInput) class.

The [`FileInput`](#fileinput.FileInput) instance can be used as a context manager in the
[`with`](../reference/compound_stmts.md#with) statement.  In this example, *input* is closed after the
...
The following functions use the global state created by [`fileinput.input()`](#fileinput.input);
if there is no active state, [`RuntimeError`](exceptions.md#RuntimeError) is raised.
...
Class [`FileInput`](#fileinput.FileInput) is the implementation; its methods [`filename()`](#fileinput.filename),
[`fileno()`](#fileinput.fileno), [`lineno()`](#fileinput.lineno), [`filelineno()`](#fileinput.filelineno), [`isfirstline()`](#fileinput.isfirstline),
[`isstdin()`](#fileinput.isstdin), [`nextfile()`](#fileinput.nextfile) and [`close()`](#fileinput.close) correspond to the
functions of the same name in the module. In addition it is [iterable](../glossary.md#term-iterable)
and has a [`readline()`](io.md#io.TextIOBase.readline) method which returns the next
input line. The sequence must be accessed in strictly sequential order;
random access and [`readline()`](io.md#io.TextIOBase.readline) cannot be mixed.

With *mode* you can specify which file mode will be passed to [`open()`](functions.md#open). It
...
You can specify *encoding* and *errors* that is passed to [`open()`](functions.md#open) or *openhook*.

A [`FileInput`](#fileinput.FileInput) instance can be used as a context manager in the
[`with`](../reference/compound_stmts.md#with) statement.  In this example, *input* is closed after the
`with` statement is exited, even if an exception occurs:
...
passed to [`fileinput.input()`](#fileinput.input) or to the [`FileInput`](#fileinput.FileInput) constructor, the
...
extensions `'.gz'` and `'.bz2'`) using the [`gzip`](gzip.md#module-gzip) and [`bz2`](bz2.md#module-bz2)
modules.  If the filename extension is not `'.gz'` or `'.bz2'`, the file is
opened normally (ie, using [`open()`](functions.md#open) without any decompression).

The *encoding* and *errors* values are passed to [`io.TextIOWrapper`](io.md#io.TextIOWrapper)
...
Returns a hook which opens each file with [`open()`](functions.md#open), using the given
...
#### Deprecated
Deprecated since version 3.10: This function is deprecated since [`fileinput.input()`](#fileinput.input) and [`FileInput`](#fileinput.FileInput)
