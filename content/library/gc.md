...
[`ValueError`](exceptions.md#ValueError) is raised if the generation number is invalid. The sum of
...
particular implementation, in particular [`float`](functions.md#float).
...
* `uncollectable` is the total number of objects which were found
  to be uncollectable (and were therefore moved to the [`garbage`](#gc.garbage)
...
#### Versionchanged
Changed in version 3.15: Add `duration` and `candidates`.
...
See [Garbage collector design](https://devguide.python.org/garbage_collector) for more information.
...
resulting referrers.  To get only currently live objects, call [`collect()`](#gc.collect)
before calling [`get_referrers()`](#gc.get_referrers).

#### WARNING
Care must be taken when using objects returned by [`get_referrers()`](#gc.get_referrers) because
some of them could still be under construction and hence in a temporarily
invalid state. Avoid using [`get_referrers()`](#gc.get_referrers) for any purpose other than
debugging.
...
[`tp_traverse`](../c-api/typeobj.md#c.PyTypeObject.tp_traverse) methods (if any), and may not be all objects actually
directly reachable.  [`tp_traverse`](../c-api/typeobj.md#c.PyTypeObject.tp_traverse) methods are supported only by objects
...
If [`DEBUG_SAVEALL`](#gc.DEBUG_SAVEALL) is set, then all unreachable objects will be
added to this list rather than freed.

#### Versionchanged
Changed in version 3.2: If this list is non-empty at [interpreter shutdown](../glossary.md#term-interpreter-shutdown), a
[`ResourceWarning`](exceptions.md#ResourceWarning) is emitted, which is silent by default.  If
[`DEBUG_UNCOLLECTABLE`](#gc.DEBUG_UNCOLLECTABLE) is set, in addition all uncollectable objects
are printed.

#### Versionchanged
Changed in version 3.4: Following [**PEP 442**](https://peps.python.org/pep-0442/), objects with a [`__del__()`](../reference/datamodel.md#object.__del__) method don’t end
up in [`gc.garbage`](#gc.garbage) anymore.
...
> that could not be collected and were put in [`garbage`](#gc.garbage).
...
> types when they appear in [`garbage`](#gc.garbage).
...
The following constants are provided for use with [`set_debug()`](#gc.set_debug):
...
#### Versionchanged
Changed in version 3.2: Also print the contents of the [`garbage`](#gc.garbage) list at
[interpreter shutdown](../glossary.md#term-interpreter-shutdown), if it isn’t empty.
