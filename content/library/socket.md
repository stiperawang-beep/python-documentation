...
**Source code:** [Lib/socket.py](https://github.com/python/cpython/tree/main/Lib/socket.py)
...
#### SEE ALSO
Module [`socketserver`](socketserver.md#module-socketserver)
: Classes that simplify writing network servers.

Module [`ssl`](ssl.md#module-ssl)
...
- The address of an [`AF_UNIX`](#socket.AF_UNIX) socket bound to a file system node
  ...
  Linux’s abstract namespace is returned as a [bytes-like object](../glossary.md#term-bytes-like-object) with
...
- [`AF_BLUETOOTH`](#socket.AF_BLUETOOTH) supports the following protocols and address
  ...
  - [`BTPROTO_SCO`](#socket.BTPROTO_SCO) accepts `bdaddr` where `bdaddr` is
    the Bluetooth address as a string or a [`bytes`](stdtypes.md#bytes) object.
...
- `IPPROTO_UDPLITE` is a variant of UDP which allows you to specify
  ...
  [Availability](intro.md#availability): Linux >= 2.6.20, FreeBSD >= 10.1
...
related to socket or address semantics raise [`OSError`](exceptions.md#OSError) or one of its
subclasses.
...
A deprecated alias of [`OSError`](exceptions.md#OSError).

#### Versionchanged
Changed in version 3.3: Following [**PEP 3151**](https://peps.python.org/pep-3151/), this class was made an alias of [`OSError`](exceptions.md#OSError).

### *exception* socket.herror

A subclass of [`OSError`](exceptions.md#OSError), this exception is raised for
address-related errors, i.e. for functions that use *h_errno* in the POSIX
C API, including [`gethostbyname_ex()`](#socket.gethostbyname_ex) and [`gethostbyaddr()`](#socket.gethostbyaddr).
...
#### Versionchanged
Changed in version 3.3: This class was made a subclass of [`OSError`](exceptions.md#OSError).

### *exception* socket.gaierror

A subclass of [`OSError`](exceptions.md#OSError), this exception is raised for
address-related errors by [`getaddrinfo()`](#socket.getaddrinfo) and [`getnameinfo()`](#socket.getnameinfo).
...
A subclass of [`OSError`](exceptions.md#OSError), this exception is raised when a timeout
occurs on a socket which has had timeouts enabled via a prior call to
[`settimeout()`](#socket.socket.settimeout) (or implicitly through
[`setdefaulttimeout()`](#socket.setdefaulttimeout)).  The accompanying value is a string
...
#### Versionchanged
Changed in version 3.10: This class was made an alias of [`TimeoutError`](exceptions.md#TimeoutError).

### Constants

The AF_\* and SOCK_\* constants are now `AddressFamily` and
`SocketKind` [`IntEnum`](enum.md#enum.IntEnum) collections.
...
first argument to [`socket()`](#socket.socket).  If the [`AF_UNIX`](#socket.AF_UNIX) constant is not
defined then this protocol is unsupported.  More constants may be available
depending on the system.

### socket.AF_UNSPEC

[`AF_UNSPEC`](#socket.AF_UNSPEC) means that
[`getaddrinfo()`](#socket.getaddrinfo) should return socket addresses for any
...
[`socket()`](#socket.socket).  More constants may be available depending on the system.
(Only [`SOCK_STREAM`](#socket.SOCK_STREAM) and [`SOCK_DGRAM`](#socket.SOCK_DGRAM) appear to be generally
useful.)
...
generally used in arguments to the [`setsockopt()`](#socket.socket.setsockopt) and [`getsockopt()`](#socket.socket.getsockopt)
...
#### Versionchanged
Changed in version 3.15: `IPV6_HDRINCL` was added.
...
[Availability](intro.md#availability): Linux >= 2.6.25, NetBSD >= 8.
...
[Availability](intro.md#availability): Linux >= 2.6.25.
...
[Availability](intro.md#availability): Linux >= 3.6.
...
[Availability](intro.md#availability): Linux >= 4.1.
...
[Availability](intro.md#availability): Linux >= 5.4.
...
[Availability](intro.md#availability): FreeBSD >= 14.0.
...
`ETH_P_ALL` can be used in the [`socket`](#socket.socket)
constructor as *proto* for the [`AF_PACKET`](#socket.AF_PACKET) family in order to
...
[`ioctl()`](#socket.socket.ioctl) method of socket objects.
...
meanings. For example, [`BDADDR_ANY`](#socket.BDADDR_ANY) can be used to indicate
...
connecting a [`BTPROTO_L2CAP`](#socket.BTPROTO_L2CAP) socket.

[Availability](intro.md#availability): Linux, FreeBSD
...
Used in the level argument to the [`setsockopt()`](#socket.socket.setsockopt) and
[`getsockopt()`](#socket.socket.getsockopt) methods of Bluetooth socket objects.

[`SOL_BLUETOOTH`](#socket.SOL_BLUETOOTH) is only available on Linux. Other constants
...
Used in the option name and value argument to the [`setsockopt()`](#socket.socket.setsockopt)
and [`getsockopt()`](#socket.socket.getsockopt) methods of Bluetooth socket objects.

`BT_*` and [`L2CAP_LM`](#socket.L2CAP_LM) are only available on Linux.
...
Option names for use with [`BTPROTO_HCI`](#socket.BTPROTO_HCI).
...
Possible values for `channel` field in the [`BTPROTO_HCI`](#socket.BTPROTO_HCI) address.
...
[Availability](intro.md#availability): FreeBSD >= 12.0
...
[Availability](intro.md#availability): Linux, FreeBSD, macOS.
...
These constants are used by the [`shutdown()`](#socket.socket.shutdown) method of socket objects.
...
number.  The address family should be [`AF_INET`](#socket.AF_INET) (the default),
[`AF_INET6`](#socket.AF_INET6), [`AF_UNIX`](#socket.AF_UNIX), [`AF_CAN`](#socket.AF_CAN), [`AF_PACKET`](#socket.AF_PACKET),
or [`AF_RDS`](#socket.AF_RDS). The socket type should be [`SOCK_STREAM`](#socket.SOCK_STREAM) (the
default), [`SOCK_DGRAM`](#socket.SOCK_DGRAM), [`SOCK_RAW`](#socket.SOCK_RAW) or perhaps one of the other
`SOCK_` constants. The protocol number is usually zero and may be omitted
or in the case where the address family is [`AF_CAN`](#socket.AF_CAN) the protocol
should be one of `CAN_RAW`, [`CAN_BCM`](#socket.CAN_BCM), [`CAN_ISOTP`](#socket.CAN_ISOTP) or
[`CAN_J1939`](#socket.CAN_J1939).
...
of [`socket.getpeername()`](#socket.socket.getpeername) but not the actual OS resource.  Unlike
[`socket.fromfd()`](#socket.fromfd), *fileno* will return the same socket and not a
duplicate. This may help close a detached socket using
[`socket.close()`](#socket.close).

The newly created socket is [non-inheritable](os.md#fd-inheritance).

Raises an [auditing event](sys.md#auditing) `socket.__new__` with arguments `self`, `family`, `type`, `protocol`.
...
#### Versionchanged
Changed in version 3.7: When [`SOCK_NONBLOCK`](#socket.SOCK_NONBLOCK) or [`SOCK_CLOEXEC`](#socket.SOCK_CLOEXEC)
bit flags are applied to *type* they are cleared, and
[`socket.type`](#socket.socket.type) will not reflect them.  They are still passed
...
as for the [`socket()`](#socket.socket) function. The default family is [`AF_UNIX`](#socket.AF_UNIX)
if defined on the platform; otherwise, the default is [`AF_INET`](#socket.AF_INET).

The newly created sockets are [non-inheritable](os.md#fd-inheritance).
...
function than [`socket.connect()`](#socket.socket.connect): if *host* is a non-numeric hostname,
it will try to resolve it for both [`AF_INET`](#socket.AF_INET) and [`AF_INET6`](#socket.AF_INET6),
...
[`getdefaulttimeout()`](#socket.getdefaulttimeout) is used.
...
is `True`, it is an [`ExceptionGroup`](exceptions.md#ExceptionGroup) containing the errors of all
attempts.
...
*family* should be either [`AF_INET`](#socket.AF_INET) or [`AF_INET6`](#socket.AF_INET6).
*backlog* is the queue size passed to [`socket.listen()`](#socket.socket.listen); if not specified
, a default reasonable value is chosen.
*reuse_port* dictates whether to set the `SO_REUSEPORT` socket option.

If *dualstack_ipv6* is true, *family* is [`AF_INET6`](#socket.AF_INET6) and the platform
supports it the socket will be able to accept both IPv4 and IPv6 connections,
else it will raise [`ValueError`](exceptions.md#ValueError). Most POSIX platforms and Windows are
supposed to support this functionality.
When this functionality is enabled the address returned by
[`socket.getpeername()`](#socket.socket.getpeername) when an IPv4 connection occurs will be an IPv6
...
This parameter can be used in conjunction with [`has_dualstack_ipv6()`](#socket.has_dualstack_ipv6):
...
[`fileno()`](io.md#io.IOBase.fileno) method) and build a socket object from the result.  Address
family, socket type and protocol number are as for the [`socket()`](#socket.socket) function.
...
Instantiate a socket from data obtained from the [`socket.share()`](#socket.socket.share)
...
Close a socket file descriptor. This is like [`os.close()`](os.md#os.close), but for
sockets. On some platforms (most notably Windows) [`os.close()`](os.md#os.close)
...
Pass their default values ([`AF_UNSPEC`](#socket.AF_UNSPEC), 0, and 0, respectively)
...
meant to be passed to the [`socket()`](#socket.socket) function.  *canonname* will be
...
and is meant to be passed to the [`socket.connect()`](#socket.socket.connect) method.

*sockaddr* can be one of the following:

* a `(address, port)` 2-tuple for [`AF_INET`](#socket.AF_INET)
* a `(address, port, flowinfo, scope_id)` 4-tuple for [`AF_INET6`](#socket.AF_INET6) if
  Python was compiled with `--enable-ipv6` (the default)
* a 2-tuple containing raw data for [`AF_INET6`](#socket.AF_INET6) if Python was
...
consider limiting the results by *type* (e.g. [`SOCK_STREAM`](#socket.SOCK_STREAM) or
[`SOCK_DGRAM`](#socket.SOCK_DGRAM)) and/or *proto* (e.g. `IPPROTO_TCP` or
...
Raises an [auditing event](sys.md#auditing) `socket.getaddrinfo` with arguments `host`, `port`, `family`, `type`, `protocol`.
...
hostname returned by [`gethostbyaddr()`](#socket.gethostbyaddr) is checked, followed by aliases for the
...
the hostname from [`gethostname()`](#socket.gethostname) is returned.

### socket.gethostbyname(hostname)

Translate a host name to IPv4 address format.  The IPv4 address is returned as a
string, such as  `'100.50.200.5'`.  If the host name is an IPv4 address itself
it is returned unchanged.  See [`gethostbyname_ex()`](#socket.gethostbyname_ex) for a more complete
interface. [`gethostbyname()`](#socket.gethostbyname) does not support IPv6 name resolution, and
[`getaddrinfo()`](#socket.getaddrinfo) should be used instead for IPv4/v6 dual stack support.

Raises an [auditing event](sys.md#auditing) `socket.gethostbyname` with argument `hostname`.
...
always a single address). [`gethostbyname_ex()`](#socket.gethostbyname_ex) does not support IPv6 name
resolution, and [`getaddrinfo()`](#socket.getaddrinfo) should be used instead for IPv4/v6 dual
stack support.
...
Raises an [auditing event](sys.md#auditing) `socket.gethostname` with no arguments.

Note: [`gethostname()`](#socket.gethostname) doesn’t always return the fully qualified domain
name; use [`getfqdn()`](#socket.getfqdn) for that.
...
domain name, use the function [`getfqdn()`](#socket.getfqdn). [`gethostbyaddr()`](#socket.gethostbyaddr) supports
both IPv4 and IPv6.

Raises an [auditing event](sys.md#auditing) `socket.gethostbyaddr` with argument `ip_address`.
...
Raises an [auditing event](sys.md#auditing) `socket.getnameinfo` with argument `sockaddr`.
...
([`SOCK_RAW`](#socket.SOCK_RAW)); for the normal socket modes, the correct protocol is chosen
...
Raises an [auditing event](sys.md#auditing) `socket.getservbyname` with arguments `servicename`, `protocolname`.
...
Raises an [auditing event](sys.md#auditing) `socket.getservbyport` with arguments `port`, `protocolname`.
...
#### Versionchanged
Changed in version 3.10: Raises [`OverflowError`](exceptions.md#OverflowError) if *x* does not fit in a 16-bit unsigned
integer.
...
[`inet_aton()`](#socket.inet_aton) also accepts strings with less than three dots; see the
Unix manual page  for details.

If the IPv4 address string passed to this function is invalid,
[`OSError`](exceptions.md#OSError) will be raised. Note that exactly what is valid depends on
the underlying C implementation of `inet_aton()`.

[`inet_aton()`](#socket.inet_aton) does not support IPv6, and [`inet_pton()`](#socket.inet_pton) should be used
instead for IPv4/v6 dual stack support.

### socket.inet_ntoa(packed_ip)

Convert a 32-bit packed IPv4 address (a [bytes-like object](../glossary.md#term-bytes-like-object) four
...
length, [`OSError`](exceptions.md#OSError) will be raised. [`inet_ntoa()`](#socket.inet_ntoa) does not
support IPv6, and [`inet_ntop()`](#socket.inet_ntop) should be used instead for IPv4/v6 dual
stack support.
...
binary format. [`inet_pton()`](#socket.inet_pton) is useful when a library or network protocol
calls for an object of type `in_addr` (similar to
[`inet_aton()`](#socket.inet_aton)) or `in6_addr`.

Supported values for *address_family* are currently [`AF_INET`](#socket.AF_INET) and
[`AF_INET6`](#socket.AF_INET6). If the IP address string *ip_string* is invalid,
[`OSError`](exceptions.md#OSError) will be raised. Note that exactly what is valid depends on
...
Convert a packed IP address (a [bytes-like object](../glossary.md#term-bytes-like-object) of some number of
bytes) to its standard, family-specific string representation (for
example, `'7.10.0.5'` or `'5aef:2b::8'`).
[`inet_ntop()`](#socket.inet_ntop) is useful when a library or network protocol returns an
object of type `in_addr` (similar to [`inet_ntoa()`](#socket.inet_ntoa)) or
`in6_addr`.

Supported values for *address_family* are currently [`AF_INET`](#socket.AF_INET) and
[`AF_INET6`](#socket.AF_INET6). If the bytes object *packed_ip* is not the correct
length for the specified address family, [`ValueError`](exceptions.md#ValueError) will be raised.
[`OSError`](exceptions.md#OSError) is raised for errors from the call to [`inet_ntop()`](#socket.inet_ntop).
...
can often be used as the buffer size for [`recvmsg()`](#socket.socket.recvmsg) to
receive a single item of ancillary data, but [**RFC 3542**](https://datatracker.ietf.org/doc/html/rfc3542.html) requires
portable applications to use [`CMSG_SPACE()`](#socket.CMSG_SPACE) and thus include
space for padding, even when the item will be the last in the
buffer.  Raises [`OverflowError`](exceptions.md#OverflowError) if *length* is outside the
...
Return the buffer size needed for [`recvmsg()`](#socket.socket.recvmsg) to
...
to receive multiple items is the sum of the [`CMSG_SPACE()`](#socket.CMSG_SPACE)
values for their associated data lengths.  Raises
[`OverflowError`](exceptions.md#OverflowError) if *length* is outside the permissible range
of values.
...
[`settimeout()`](#socket.socket.settimeout) for possible values and their respective
meanings.
...
[`OSError`](exceptions.md#OSError) if you don’t have enough rights.

Raises an [auditing event](sys.md#auditing) `socket.sethostname` with argument `name`.

[Availability](intro.md#availability): Unix, not Android.
...
[`OSError`](exceptions.md#OSError) if the system call fails.
...
interface name.
[`OSError`](exceptions.md#OSError) if no interface with the given name exists.
...
#### SEE ALSO
“Interface name” is a name as documented in [`if_nameindex()`](#socket.if_nameindex).

### socket.if_indextoname(if_index)

Return a network interface name corresponding to an
interface index number.
[`OSError`](exceptions.md#OSError) if no interface with the given index exists.
...
### socket.send_fds(sock, buffers, fds)

Send the list of file descriptors *fds* over an [`AF_UNIX`](#socket.AF_UNIX) socket *sock*.
The *fds* parameter is a sequence of file descriptors.
Consult [`sendmsg()`](#socket.socket.sendmsg) for the documentation of these parameters.

[Availability](intro.md#availability): Unix, not WASI.

Unix platforms supporting [`sendmsg()`](#socket.socket.sendmsg)
and `SCM_RIGHTS` mechanism.

#### Versionadded
Added in version 3.9.

### socket.recv_fds(sock, bufsize, maxfds)

Receive up to *maxfds* file descriptors from an [`AF_UNIX`](#socket.AF_UNIX) socket *sock*.
Return `(msg, list(fds), flags, addr)`.
Consult [`recvmsg()`](#socket.socket.recvmsg) for the documentation of these parameters.

[Availability](intro.md#availability): Unix, not WASI.

Unix platforms supporting [`recvmsg()`](#socket.socket.recvmsg)
and `SCM_RIGHTS` mechanism.
...
<a id="socket-objects"></a>

## Socket Objects

Socket objects have the following methods.  Except for
[`makefile()`](#socket.socket.makefile), these correspond to Unix system calls applicable
to sockets.

#### Versionchanged
Changed in version 3.2: Support for the [context manager](../glossary.md#term-context-manager) protocol was added.  Exiting the
...
an [`InterruptedError`](exceptions.md#InterruptedError) exception (see [**PEP 475**](https://peps.python.org/pep-0475/) for the rationale).

#### socket.bind(address)

Bind the socket to *address*.  The socket must not already be bound. The format
of *address* depends on the address family — see [Socket families](#socket-addresses).

Raises an [auditing event](sys.md#auditing) `socket.bind` with arguments `self`, `address`.
...
descriptor) is also closed when all file objects from [`makefile()`](#socket.socket.makefile)
...
it is recommended to [`close()`](#socket.close) them explicitly, or to use a
[`with`](../reference/compound_stmts.md#with) statement around them.

#### Versionchanged
Changed in version 3.6: [`OSError`](exceptions.md#OSError) is now raised if an error occurs when the underlying
`close()` call is made.

#### NOTE
[`close()`](#socket.close) releases the resource associated with a connection but
does not necessarily close the connection immediately.  If you want
to close the connection in a timely fashion, call [`shutdown()`](#socket.socket.shutdown)
before [`close()`](#socket.close).

#### socket.connect(address)

Connect to a remote socket at *address*. The format of *address* depends on the
address family — see [Socket families](#socket-addresses).

If the connection is interrupted by a signal, the method waits until the
connection completes, or raises a [`TimeoutError`](exceptions.md#TimeoutError) on timeout, if the
signal handler doesn’t raise an exception and the socket is blocking or has
a timeout. For non-blocking sockets, the method raises an
[`InterruptedError`](exceptions.md#InterruptedError) exception if the connection is interrupted by a
signal (or the exception raised by the signal handler).

Raises an [auditing event](sys.md#auditing) `socket.connect` with arguments `self`, `address`.
...
is useful with [`select.select()`](select.md#select.select).

Under Windows the small integer returned by this method cannot be used where a
file descriptor can be used (such as [`os.fdopen()`](os.md#os.fdopen)).  Unix does not have
this limitation.
...
of the address returned depends on the address family — see [Socket families](#socket-addresses).
...
the address family — see [Socket families](#socket-addresses).

#### socket.getsockopt(level, optname)

Return the value of the given socket option (see the Unix man page
).  The needed symbolic constants ([SO_\* etc.](#socket-unix-constants))
...
contents of the buffer (see the optional built-in module [`struct`](struct.md#module-struct) for a way
...
[`setblocking()`](#socket.socket.setblocking) or [`settimeout()`](#socket.socket.settimeout).

#### socket.ioctl(control, option)

The [`ioctl()`](#socket.socket.ioctl) method is a limited interface to the WSAIoctl system
...
On other platforms, the generic [`fcntl.fcntl()`](fcntl.md#fcntl.fcntl) and [`fcntl.ioctl()`](fcntl.md#fcntl.ioctl)
...
Return a [file object](../glossary.md#term-file-object) associated with the socket.  The exact returned
type depends on the arguments given to [`makefile()`](#socket.socket.makefile).  These arguments are
interpreted the same way as by the built-in [`open()`](functions.md#open) function, except
...
Closing the file object returned by [`makefile()`](#socket.socket.makefile) won’t close the
original socket unless all other file objects have been closed and
[`socket.close()`](#socket.close) has been called on the socket object.

#### NOTE
On Windows, the file-like object created by [`makefile()`](#socket.socket.makefile) cannot be
used where a file object with a file descriptor is expected, such as the
stream arguments of [`subprocess.Popen()`](subprocess.md#subprocess.Popen).
...
See the Unix manual page  for the meaning of the optional argument
...
address of the socket sending the data.  See the Unix manual page
 for the meaning of the optional argument *flags*; it defaults
to zero. The format of *address* depends on the address family — see
[Socket families](#socket-addresses).
...
[`CMSG_SPACE()`](#socket.CMSG_SPACE) or [`CMSG_LEN()`](#socket.CMSG_LEN), and items which do not fit
...
address)`.  The *data* item is a [`bytes`](stdtypes.md#bytes) object holding the
...
[`bytes`](stdtypes.md#bytes) object holding the associated data.  The *msg_flags*
...
On some systems, [`sendmsg()`](#socket.socket.sendmsg) and [`recvmsg()`](#socket.socket.recvmsg) can be used to
pass file descriptors between processes over an [`AF_UNIX`](#socket.AF_UNIX)
socket.  When this facility is used (it is often restricted to
[`SOCK_STREAM`](#socket.SOCK_STREAM) sockets), [`recvmsg()`](#socket.socket.recvmsg) will return, in its
ancillary data, items of the form `(socket.SOL_SOCKET,
socket.SCM_RIGHTS, fds)`, where *fds* is a [`bytes`](stdtypes.md#bytes) object
representing the new file descriptors as a binary array of the
native C  type.  If [`recvmsg()`](#socket.socket.recvmsg) raises an
...
to extend beyond the end of the buffer, [`recvmsg()`](#socket.socket.recvmsg) will issue
a [`RuntimeWarning`](exceptions.md#RuntimeWarning), and will return the part of it which is
...
messages being received).  See also [`sendmsg()`](#socket.socket.sendmsg).
...
#### socket.recvmsg_into(buffers)

Receive normal data and ancillary data from the socket, behaving as
[`recvmsg()`](#socket.socket.recvmsg) would, but scatter the non-ancillary data into a
series of buffers instead of returning a new bytes object.  The
*buffers* argument must be an iterable of objects that export
writable buffers (e.g. [`bytearray`](stdtypes.md#bytearray) objects); these will be
...
system may set a limit ([`sysconf()`](os.md#os.sysconf) value `SC_IOV_MAX`)
on the number of buffers that can be used.  The *ancbufsize* and
*flags* arguments have the same meaning as for [`recvmsg()`](#socket.socket.recvmsg).
...
*msg_flags* and *address* are the same as for [`recvmsg()`](#socket.socket.recvmsg).
...
the data.  See the Unix manual page  for the meaning of the
optional argument *flags*; it defaults to zero. The format of *address*
depends on the address family — see [Socket families](#socket-addresses).
...
bytes received.  See the Unix manual page  for the meaning
of the optional argument *flags*; it defaults to zero.

#### socket.send(bytes)

Send data to the socket.  The socket must be connected to a remote socket.  The
optional *flags* argument has the same meaning as for [`recv()`](#socket.socket.recv).
Returns the number of bytes sent. Applications are responsible for checking that
all data has been sent; if only some of the data was transmitted, the
application needs to attempt delivery of the remaining data. For further
information on this topic, consult the [Socket Programming HOWTO](../howto/sockets.md#socket-howto).
...
Send data to the socket.  The socket must be connected to a remote socket.  The
optional *flags* argument has the same meaning as for [`recv()`](#socket.socket.recv).
Unlike [`send()`](#socket.socket.send), this method continues to send data from *bytes* until
...
argument has the same meaning as for [`recv()`](#socket.socket.recv).  Return the number of
bytes sent. The format of *address* depends on the address family — see
[Socket families](#socket-addresses).

Raises an [auditing event](sys.md#auditing) `socket.sendto` with arguments `self`, `address`.
...
(e.g. [`bytes`](stdtypes.md#bytes) objects); the operating system may set a limit
([`sysconf()`](os.md#os.sysconf) value `SC_IOV_MAX`) on the number of buffers
...
some systems (in particular, systems without [`CMSG_SPACE()`](#socket.CMSG_SPACE))
might support sending only one control message per call.  The
*flags* argument defaults to 0 and has the same meaning as for
[`send()`](#socket.socket.send).  If *address* is supplied and not `None`, it sets a
...
over an [`AF_UNIX`](#socket.AF_UNIX) socket, on systems which support the
`SCM_RIGHTS` mechanism.  See also [`recvmsg()`](#socket.socket.recvmsg).
...
Raises an [auditing event](sys.md#auditing) `socket.sendmsg` with arguments `self`, `address`.
...
#### socket.sendmsg_afalg(\*, op)

Specialized version of [`sendmsg()`](#socket.socket.sendmsg) for [`AF_ALG`](#socket.AF_ALG) socket.
Set mode, IV, AEAD associated data length and flags for [`AF_ALG`](#socket.AF_ALG) socket.
...
[`os.sendfile`](os.md#os.sendfile) and return the total number of bytes which were sent.
*file* must be a regular file object opened in binary mode. If
[`os.sendfile`](os.md#os.sendfile) is not available (e.g. Windows) or *file* is not a
regular file [`send()`](#socket.socket.send) will be used instead. *offset* tells from where to
start reading the file. If specified, *count* is the total number of bytes
to transmit as opposed to sending the file until EOF is reached. File
position is updated on return or also in case of error in which case
[`file.tell()`](io.md#io.IOBase.tell) can be used to figure out the number of
bytes which were sent. The socket must be of [`SOCK_STREAM`](#socket.SOCK_STREAM) type.
...
This method is a shorthand for certain [`settimeout()`](#socket.socket.settimeout) calls:

* `sock.setblocking(True)` is equivalent to `sock.settimeout(None)`
* `sock.setblocking(False)` is equivalent to `sock.settimeout(0.0)`

#### Versionchanged
Changed in version 3.7: The method no longer applies [`SOCK_NONBLOCK`](#socket.SOCK_NONBLOCK) flag on
...
[`timeout`](#socket.timeout) exception if the timeout period *value* has elapsed before
...
#### Versionchanged
Changed in version 3.7: The method no longer toggles [`SOCK_NONBLOCK`](#socket.SOCK_NONBLOCK) flag on
[`socket.type`](#socket.socket.type).

#### Versionchanged
Changed in version 3.15: Accepts any real number, not only integer or float.

#### socket.setsockopt(level, optname, value: [int](functions.md#int) | [Buffer](collections.abc.md#collections.abc.Buffer))

#### socket.setsockopt(level, optname, None, optlen: [int](functions.md#int))

<a id="index-14"></a>

Set the value of the given socket option (see the Unix manual page
).  The needed symbolic constants are defined in this
module (SO_\* etc. <socket-unix-constants>).  The value can be an integer,
`None` or a [bytes-like object](../glossary.md#term-bytes-like-object) representing a buffer. In the latter
case it is up to the caller to ensure that the bytestring contains the
proper bits (see the optional built-in module [`struct`](struct.md#module-struct) for a way to
...
Shut down one or both halves of the connection.  If *how* is [`SHUT_RD`](#socket.SHUT_RD),
further receives are disallowed.  If *how* is [`SHUT_WR`](#socket.SHUT_WR), further sends
are disallowed.  If *how* is [`SHUT_RDWR`](#socket.SHUT_RDWR), further sends and receives are
disallowed.
...
communication and the socket can be recreated there using [`fromshare()`](#socket.fromshare).
...
[`recv()`](#socket.socket.recv) and [`send()`](#socket.socket.send) without *flags* argument instead.
...
<a id="socket-timeouts"></a>

## Notes on socket timeouts

A socket object can be in one of three modes: blocking, non-blocking, or
timeout.  Sockets are by default always created in blocking mode, but this
can be changed by calling [`setdefaulttimeout()`](#socket.setdefaulttimeout).
...
* In *non-blocking mode*, operations fail (with an error that is unfortunately
  ...
  [`select`](select.md#module-select) module can be used to know when and whether a socket is available
  for reading or writing.
* In *timeout mode*, operations fail if they cannot be completed within the
  timeout specified for the socket (they raise a [`timeout`](#socket.timeout) exception)
...
to use the [`fileno()`](#socket.socket.fileno) of a socket.

### Timeouts and the `connect` method

The [`connect()`](#socket.socket.connect) operation is also subject to the timeout
setting, and in general it is recommended to call [`settimeout()`](#socket.socket.settimeout)
before calling [`connect()`](#socket.socket.connect) or pass a timeout parameter to
[`create_connection()`](#socket.create_connection).  However, the system network stack may also
return a connection timeout error of its own regardless of any Python socket
timeout setting.

### Timeouts and the `accept` method

If [`getdefaulttimeout()`](#socket.getdefaulttimeout) is not [`None`](constants.md#None), sockets returned by
the [`accept()`](#socket.socket.accept) method inherit that timeout.  Otherwise, the
behaviour depends on settings of the listening socket:

* if the listening socket is in *blocking mode* or in *timeout mode*,
  the socket returned by [`accept()`](#socket.socket.accept) is in *blocking mode*;
* if the listening socket is in *non-blocking mode*, whether the socket
  returned by [`accept()`](#socket.socket.accept) is in blocking or non-blocking mode
...
<a id="socket-example"></a>

## Example

Here are four minimal example programs using the TCP/IP protocol: a server that
echoes all data that it receives back (servicing only one client), and a client
using it.  Note that a server must perform the sequence [`socket()`](#socket.socket),
[`bind()`](#socket.socket.bind), [`listen()`](#socket.socket.listen), [`accept()`](#socket.socket.accept) (possibly
repeating the [`accept()`](#socket.socket.accept) to service more than one client), while a
client only needs the sequence [`socket()`](#socket.socket), [`connect()`](#socket.socket.connect).  Also
note that the server does not [`sendall()`](#socket.socket.sendall)/[`recv()`](#socket.socket.recv) on
...
After binding (`CAN_RAW`) or connecting ([`CAN_BCM`](#socket.CAN_BCM)) the socket, you
can use the [`socket.send()`](#socket.socket.send) and [`socket.recv()`](#socket.socket.recv) operations (and
