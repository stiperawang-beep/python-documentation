...
**Source code:** [Lib/curses](https://github.com/python/cpython/tree/main/Lib/curses)
...
#### SEE ALSO
Module [`curses.ascii`](curses.ascii.md#module-curses.ascii)
: Utilities for working with ASCII characters, regardless of your locale settings.

Module [`curses.panel`](curses.panel.md#module-curses.panel)
: A panel stack extension that adds depth to  curses windows.

Module [`curses.textpad`](#module-curses.textpad)
: Editable text widget for curses supporting  **Emacs**-like bindings.

[Curses Programming with Python](../howto/curses.md#curses-howto)
: Tutorial material on using curses with Python, by Andrew Kuchling and Eric
  Raymond.

<a id="curses-functions"></a>
...
first [`raw()`](#curses.raw) then [`cbreak()`](#curses.cbreak) leaves the terminal in cbreak mode.
...
attribute value can be combined with [`A_STANDOUT`](#curses.A_STANDOUT), [`A_REVERSE`](#curses.A_REVERSE),
and the other `A_*` attributes.  [`pair_number()`](#curses.pair_number) is the counterpart
to this function.
...
program is not in curses.)  Subsequent calls to [`reset_prog_mode()`](#curses.reset_prog_mode) will
restore this mode.
...
[`reset_shell_mode()`](#curses.reset_shell_mode) will restore this mode.
...
representing the desired next state.  The [`doupdate()`](#curses.doupdate) ground updates the
physical screen to match the virtual screen.

The virtual screen may be updated by a [`noutrefresh()`](#curses.window.noutrefresh) call after write
operations such as [`addstr()`](#curses.window.addstr) have been performed on a window.  The normal
[`refresh()`](#curses.window.refresh) call is simply `noutrefresh()` followed by `doupdate()`;
...
The [`filter()`](#curses.filter) routine, if used, must be called before [`initscr()`](#curses.initscr) is
...
attention signal produced by [`beep()`](#curses.beep).
...
After [`getch()`](#curses.window.getch) returns [`KEY_MOUSE`](#curses.KEY_MOUSE) to signal a mouse event, this
...
[`BUTTONn_PRESSED`](#curses.BUTTONn_PRESSED), [`BUTTONn_RELEASED`](#curses.BUTTONn_RELEASED), [`BUTTONn_CLICKED`](#curses.BUTTONn_CLICKED),
[`BUTTONn_DOUBLE_CLICKED`](#curses.BUTTONn_DOUBLE_CLICKED), [`BUTTONn_TRIPLE_CLICKED`](#curses.BUTTONn_TRIPLE_CLICKED),
[`BUTTON_SHIFT`](#curses.BUTTON_SHIFT), [`BUTTON_CTRL`](#curses.BUTTON_CTRL), [`BUTTON_ALT`](#curses.BUTTON_ALT).
...
`(y, x)`.  If [`leaveok`](#curses.window.leaveok) is currently `True`, then return `(-1, -1)`.

### curses.getwin(file)

Read window related data stored in the file by an earlier [`window.putwin()`](#curses.window.putwin) call.
...
`1000`.  When [`init_color()`](#curses.init_color) is used, all occurrences of that color on the
screen immediately change to the new definition.  This function is a no-op on
most terminals; it is active only if [`can_change_color()`](#curses.can_change_color) returns `True`.
...
[`use_default_colors()`](#curses.use_default_colors) and [`assume_default_colors()`](#curses.assume_default_colors)).
...
Return `True` if [`resize_term()`](#curses.resize_term) would modify the window structure,
`False` otherwise.

### curses.isendwin()

Return `True` if [`endwin()`](#curses.endwin) has been called (that is, the  curses library has
been deinitialized).
...
defined only after the call to [`initscr()`](#curses.initscr).
...
echoing of input) do not occur.  The [`refresh()`](#curses.window.refresh) and [`noutrefresh()`](#curses.window.noutrefresh)
...
[`color_pair()`](#curses.color_pair) is the counterpart to this function.

### curses.putp(str)

Equivalent to `tputs(str, 1, putchar)`; emit the value of a specified
terminfo capability for the current terminal.  Note that the output of [`putp()`](#curses.putp)
always goes to standard output.

### curses.qiflush()

If *flag* is `False`, the effect is the same as calling [`noqiflush()`](#curses.noqiflush). If
...
[`savetty()`](#curses.savetty).

### curses.resize_term(nlines, ncols)

Backend function used by [`resizeterm()`](#curses.resizeterm), performing most of the work;
when resizing the windows, [`resize_term()`](#curses.resize_term) blank-fills the areas that are
...
[`resetty()`](#curses.resetty).

### curses.get_escdelay()

Retrieves the value set by [`set_escdelay()`](#curses.set_escdelay).
...
Retrieves the value set by [`set_tabsize()`](#curses.set_tabsize).
...
[`leaveok`](#curses.window.leaveok) is set `True`.
...
after [`initscr()`](#curses.initscr).

[`start_color()`](#curses.start_color) initializes eight basic colors (black, red,  green, yellow,
blue, magenta, cyan, and white), and two global variables in the `curses`
module, [`COLORS`](#curses.COLORS) and [`COLOR_PAIRS`](#curses.COLOR_PAIRS), containing the maximum number
...
Push *ch* so the next [`getch()`](#curses.window.getch) will return it.
...
Update the [`LINES`](#curses.LINES) and [`COLS`](#curses.COLS) module variables.
...
Push *ch* so the next [`get_wch()`](#curses.window.get_wch) will return it.
...
Push a [`KEY_MOUSE`](#curses.KEY_MOUSE) event onto the input queue, associating the given
state data with it.

### curses.use_env(flag)

If used, this function should be called before [`initscr()`](#curses.initscr) or newterm are
...
<a id="curses-window-objects"></a>

## Window Objects

### *class* curses.window

Window objects, as returned by [`initscr()`](#curses.initscr) and [`newwin()`](#curses.newwin) above, have
...
#### NOTE
Writing outside the window, subwindow, or pad raises a [`curses.error`](#curses.error).
...
#### NOTE
* Writing outside the window, subwindow, or pad raises [`curses.error`](#curses.error).
  Attempting to write to the lower right corner of a window, subwindow,
...
* A [bug in ncurses](https://bugs.python.org/issue35924), the backend
  ...
  ncurses, you can avoid triggering this if you do not call [`addstr()`](#curses.window.addstr)
  with a *str* that has embedded newlines.  Instead, call [`addstr()`](#curses.window.addstr)
...
|-------------|---------------------|----------------------------------------|
| *ls*        | Left side           | [`ACS_VLINE`](#curses.ACS_VLINE)       |
| *rs*        | Right side          | [`ACS_VLINE`](#curses.ACS_VLINE)       |
| *ts*        | Top                 | [`ACS_HLINE`](#curses.ACS_HLINE)       |
| *bs*        | Bottom              | [`ACS_HLINE`](#curses.ACS_HLINE)       |
| *tl*        | Upper-left corner   | [`ACS_ULCORNER`](#curses.ACS_ULCORNER) |
| *tr*        | Upper-right corner  | [`ACS_URCORNER`](#curses.ACS_URCORNER) |
| *bl*        | Bottom-left corner  | [`ACS_LLCORNER`](#curses.ACS_LLCORNER) |
| *br*        | Bottom-right corner | [`ACS_LRCORNER`](#curses.ACS_LRCORNER) |

#### window.box()

Similar to [`border()`](#curses.window.border), but both *ls* and *rs* are *vertch* and both *ts* and
...
will be touched using the [`touchline()`](#curses.window.touchline) method so that the contents will
be redisplayed by the next window refresh.

#### window.clear()

Like [`erase()`](#curses.window.erase), but also cause the whole window to be repainted upon next
call to [`refresh()`](#curses.window.refresh).

#### window.clearok(flag)

If *flag* is `True`, the next call to [`refresh()`](#curses.window.refresh) will clear the window
completely.

#### window.clrtobot()

Erase from cursor to the end of the window: all lines below the cursor are
deleted, and then the equivalent of [`clrtoeol()`](#curses.window.clrtoeol) is performed.
...
An abbreviation for “derive window”, [`derwin()`](#curses.window.derwin) is the same as calling
[`subwin()`](#curses.window.subwin), except that *begin_y* and *begin_x* are relative to the origin
of the window, rather than relative to the entire screen.  Return a window
object for the derived window.

#### window.echochar(ch)

Add character *ch* with attribute *attr*, and immediately  call [`refresh()`](#curses.window.refresh)
on the window.
...
is created, for example with [`window.subwin()`](#curses.window.subwin).
By default, current locale encoding is used (see [`locale.getencoding()`](locale.md#locale.getencoding)).
...
Get a character, returning a string instead of an integer, as [`getch()`](#curses.window.getch)
...
window to be refreshed; you no longer have to call [`refresh()`](#curses.window.refresh) yourself.
...
from the characters.  If *n* is specified, [`instr()`](#curses.window.instr) returns a string
...
[`refresh()`](#curses.window.refresh); otherwise return `False`.  Raise a [`curses.error`](#curses.error)
exception if *line* is not valid for the given window.

#### window.is_wintouched()

Return `True` if the specified window was modified since the last call to
[`refresh()`](#curses.window.refresh); otherwise return `False`.
...
If *flag* is `True`, [`getch()`](#curses.window.getch) will be non-blocking.
...
the physical screen.  To accomplish that, call  [`doupdate()`](#curses.doupdate).
...
[`overlay()`](#curses.window.overlay) can be used. *sminrow* and *smincol* are the upper-left
...
To get fine-grained control over the copied region, the second form of
[`overwrite()`](#curses.window.overwrite) can be used. *sminrow* and *smincol* are the upper-left
...
information can be later retrieved using the [`getwin()`](#curses.getwin) function.

#### window.redrawln(beg, num)

Indicate that the *num* screen lines, starting at line *beg*, are corrupted and
should be completely redrawn on the next [`refresh()`](#curses.window.refresh) call.

#### window.redrawwin()

Touch the entire window, causing it to be completely redrawn on the next
[`refresh()`](#curses.window.refresh) call.
...
with [`newpad()`](#curses.newpad).  The additional parameters are needed to indicate what part
...
rendition (as set by [`bkgdset()`](#curses.window.bkgdset)) merged into them.
...
terminal, it is also necessary to call [`idlok()`](#curses.window.idlok).
...
windows.  This routine is called by [`refresh()`](#curses.window.refresh), so it should almost never
be necessary to call it manually.

#### window.syncok(flag)

If *flag* is `True`, then [`syncup()`](#curses.window.syncup) is called automatically
...
*delay* is zero, then non-blocking read is used, and [`getch()`](#curses.window.getch) will
return `-1` if no input is waiting.  If *delay* is positive, then
[`getch()`](#curses.window.getch) will block for *delay* milliseconds, and return `-1` if there is
...
Some curses routines  that  return  an integer, such as [`getch()`](#curses.window.getch), return
[`ERR`](#curses.ERR) upon failure.

### curses.OK

Some curses routines  that  return  an integer, such as  [`napms()`](#curses.napms), return
[`OK`](#curses.OK) upon success.
...
It is defined only after the call to [`start_color()`](#curses.start_color).
...
It is defined only after the call to [`initscr()`](#curses.initscr).
Updated by [`update_lines_cols()`](#curses.update_lines_cols), [`resizeterm()`](#curses.resizeterm) and
...
| ### curses.A_REVERSE    | Reverse background and<br/>foreground colors |
...
| Bit-mask                | Meaning                                              |
|-------------------------|------------------------------------------------------|
| ### curses.A_ATTRIBUTES | Bit-mask to extract<br/>attributes                   |
| ### curses.A_CHARTEXT   | Bit-mask to extract a<br/>character                  |
| ### curses.A_COLOR      | Bit-mask to extract<br/>color-pair field information |
...
| ### curses.KEY_F0        | Function keys.  Up to 64 function keys are<br/>supported. |
...
normally at least four function keys ([`KEY_F1`](#curses.KEY_Fn), [`KEY_F2`](#curses.KEY_Fn),
[`KEY_F3`](#curses.KEY_Fn), [`KEY_F4`](#curses.KEY_Fn)) available, and the arrow keys mapped to
[`KEY_UP`](#curses.KEY_UP), [`KEY_DOWN`](#curses.KEY_DOWN), [`KEY_LEFT`](#curses.KEY_LEFT) and [`KEY_RIGHT`](#curses.KEY_RIGHT) in
...
#### NOTE
These are available only after [`initscr()`](#curses.initscr) has  been called.
...
The following table lists mouse button constants used by [`getmouse()`](#curses.getmouse):
...
The `curses.textpad` module provides a [`Textbox`](#curses.textpad.Textbox) class that handles
...
<a id="curses-textpad-objects"></a>

## Textbox objects

You can instantiate a [`Textbox`](#curses.textpad.Textbox) object as follows:
...
The instance’s [`stripspaces`](#curses.textpad.Textbox.stripspaces) flag is initially on.

[`Textbox`](#curses.textpad.Textbox) objects have the following methods:
...
[`stripspaces`](#curses.textpad.Textbox.stripspaces) attribute.
...
| `Control`-`B` | Cursor left, wrapping to previous line if<br/>appropriate.              |
| `Control`-`D` | Delete character under cursor.                                          |
| `Control`-`E` | Go to right edge (stripspaces off) or end<br/>of line (stripspaces on). |
| `Control`-`F` | Cursor right, wrapping to next line when<br/>appropriate.               |
| `Control`-`G` | Terminate, returning the window contents.                               |
| `Control`-`H` | Delete character backward.                                              |
| `Control`-`J` | Terminate if the window is 1 line,<br/>otherwise insert newline.        |
| `Control`-`K` | If line is blank, delete it, otherwise<br/>clear to end of line.        |
...
| Constant                                 | Keystroke     |
|------------------------------------------|---------------|
| [`KEY_LEFT`](#curses.KEY_LEFT)           | `Control`-`B` |
| [`KEY_RIGHT`](#curses.KEY_RIGHT)         | `Control`-`F` |
| [`KEY_UP`](#curses.KEY_UP)               | `Control`-`P` |
| [`KEY_DOWN`](#curses.KEY_DOWN)           | `Control`-`N` |
| [`KEY_BACKSPACE`](#curses.KEY_BACKSPACE) | `Control`-`h` |
...
window are included is affected by the [`stripspaces`](#curses.textpad.Textbox.stripspaces) member.
