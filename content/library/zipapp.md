...
**Source code:** [Lib/zipapp.py](https://github.com/python/cpython/tree/main/Lib/zipapp.py)
...
Python code, which can be  [executed directly by the Python interpreter](../using/cmdline.md#using-on-interface-options).  The module provides both a
[Command-Line Interface](#zipapp-command-line-interface) and a [Python API](#zipapp-python-api).

## Basic Example

The following example shows how the [Command-Line Interface](#zipapp-command-line-interface)
...
<a id="zipapp-command-line-interface"></a>
...
[`--main`](#cmdoption-zipapp-m) cannot be specified when copying an archive.
...
[`--compress`](#cmdoption-zipapp-c) has no effect when copying an archive.
...
* The name of a directory, or a [path-like object](../glossary.md#term-path-like-object) referring
  to a directory, in which case a new application archive will be
  created from the content of that directory.
* The name of an existing application archive file, or a [path-like object](../glossary.md#term-path-like-object)
...
* If it is the name of a file, or a [path-like object](../glossary.md#term-path-like-object),
...
archive.  If there is no `#!` line, return [`None`](constants.md#None).
...
The same can be done using the [`create_archive()`](#zipapp.create_archive) function:
...
using the [`create_archive()`](#zipapp.create_archive) function:
...
To update the file in place, do the replacement in memory using a [`BytesIO`](io.md#io.BytesIO)
...
<a id="zipapp-specifying-the-interpreter"></a>
...
the appropriate interpreter available. See [Specifying the Interpreter](#zipapp-specifying-the-interpreter)
...
[`sys.path`](sys.md#sys.path) and thus further modules can be imported from the zip file.
...
1. An optional shebang line, containing the characters `b'#!'` followed by an
   ...
   and in [`sys.getfilesystemencoding()`](sys.md#sys.getfilesystemencoding) on POSIX.
2. Standard zipfile data, as generated by the [`zipfile`](zipfile.md#module-zipfile) module.  The
