...
This module can be used directly, or via the [`rlcompleter`](rlcompleter.md#module-rlcompleter) module, which
supports completion of Python identifiers at the interactive prompt.  Settings
made using  this module affect the behaviour of both the interpreter’s
interactive prompt  and the prompts offered by the built-in [`input()`](functions.md#input)
function.
...
you can use [`backend`](#readline.backend) to determine which library is being used.
...
It raises an [auditing event](sys.md#auditing) `open` with the file name
...
and raises an [auditing event](sys.md#auditing) `open` with the file
...
[auditing event](sys.md#auditing) `open` with the file name if given and
`"~/.history"` otherwise.

#### Versionchanged
Changed in version 3.14: The auditing event was added.

### readline.append_history_file(nelements)
...
that supports it. It raises an [auditing event](sys.md#auditing) `open`
...
The [`write_history_file()`](#readline.write_history_file) function uses this value to truncate
...
[`get_history_length()`](#readline.get_history_length), which returns the maximum number of lines that will
...
to be used by [`rlcompleter`](rlcompleter.md#module-rlcompleter) to complete Python identifiers for
...
from the user’s [`PYTHONSTARTUP`](../using/cmdline.md#envvar-PYTHONSTARTUP) file.
...
[interactive mode](../tutorial/interpreter.md#tut-interactive) (see [Readline configuration](site.md#rlcompleter-config)).
...
The following example extends the [`code.InteractiveConsole`](code.md#code.InteractiveConsole) class to
...
#### NOTE
The new [REPL](../glossary.md#term-REPL) introduced in version 3.13 doesn’t support readline.
However, readline can still be used by setting the [`PYTHON_BASIC_REPL`](../using/cmdline.md#envvar-PYTHON_BASIC_REPL)
