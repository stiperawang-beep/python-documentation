...
**Source code:** [Lib/email/header.py](https://github.com/python/cpython/tree/main/Lib/email/header.py)
...
dictionary-like API of the [`EmailMessage`](email.message.md#email.message.EmailMessage) class.  In
...
[**RFC 2047**](https://datatracker.ietf.org/doc/html/rfc2047.html), and [**RFC 2231**](https://datatracker.ietf.org/doc/html/rfc2231.html). The [`email`](email.md#module-email) package supports these standards
in its `email.header` and [`email.charset`](email.charset.md#module-email.charset) modules.

If you want to include non-ASCII characters in your email headers, say in the
*Subject* or *To* fields, you should use the
[`Header`](#email.header.Header) class and assign the field in the [`Message`](email.compat32-message.md#email.message.Message)
object to an instance of [`Header`](#email.header.Header) instead of using a string for the header
value.  Import the [`Header`](#email.header.Header) class from the `email.header` module.
For example:
...
character?  We did this by creating a [`Header`](#email.header.Header) instance and passing in
the character set that the byte string was encoded in.  When the subsequent
[`Message`](email.compat32-message.md#email.message.Message) instance was flattened, the *Subject*
...
Here is the [`Header`](#email.header.Header) class description:
...
[`append()`](#email.header.Header.append) method calls.  *s* may be an instance of [`bytes`](stdtypes.md#bytes) or
[`str`](stdtypes.md#str), but see the [`append()`](#email.header.Header.append) documentation for semantics.

Optional *charset* serves two purposes: it has the same meaning as the *charset*
argument to the [`append()`](#email.header.Header.append) method.  It also sets the default character set
for all subsequent [`append()`](#email.header.Header.append) calls that omit the *charset* argument.  If
*charset* is not provided in the constructor (the default), the `us-ascii`
character set is used both as *s*’s initial charset and as the default for
subsequent [`append()`](#email.header.Header.append) calls.
...
Optional *errors* is passed straight through to the [`append()`](#email.header.Header.append) method.

#### append(s, charset=None, errors='strict')

Append the string *s* to the MIME header.

Optional *charset*, if given, should be a [`Charset`](email.charset.md#email.charset.Charset)
instance (see [`email.charset`](email.charset.md#module-email.charset)) or the name of a character set, which
will be converted to a [`Charset`](email.charset.md#email.charset.Charset) instance.  A value
of `None` (the default) means that the *charset* given in the constructor
is used.

*s* may be an instance of [`bytes`](stdtypes.md#bytes) or [`str`](stdtypes.md#str).  If it is an
instance of [`bytes`](stdtypes.md#bytes), then *charset* is the encoding of that byte
string, and a [`UnicodeError`](exceptions.md#UnicodeError) will be raised if the string cannot be
decoded with that character set.

If *s* is an instance of [`str`](stdtypes.md#str), then *charset* is a hint specifying
...
The [`Header`](#email.header.Header) class also provides a number of methods to support
standard operators and built-in functions.

#### \_\_str_\_()

Returns an approximation of the [`Header`](#email.header.Header) as a string, using an
...
This method allows you to compare two [`Header`](#email.header.Header) instances for
equality.
...
1. A list of pairs containing each of the decoded parts of the header,
   ...
   [`bytes`](stdtypes.md#bytes), and *charset* is either:
...
2. A list of length 1 containing a pair `(string, None)`, where
   *string* is always an instance of [`str`](stdtypes.md#str).

An [`email.errors.HeaderParseError`](email.errors.md#email.errors.HeaderParseError) may be raised when certain decoding
...
new code, we recommend using [`email.headerregistry.HeaderRegistry`](email.headerregistry.md#email.headerregistry.HeaderRegistry).

### email.header.make_header(decoded_seq, maxlinelen=None, header_name=None, continuation_ws=' ')

Create a [`Header`](#email.header.Header) instance from a sequence of pairs as returned by
[`decode_header()`](#email.header.decode_header).

[`decode_header()`](#email.header.decode_header) takes a header value string and returns a sequence of
pairs of the format `(decoded_string, charset)` where *charset* is the name of
the character set.

This function takes one of those sequence of pairs and returns a
[`Header`](#email.header.Header) instance.  Optional *maxlinelen*, *header_name*, and
*continuation_ws* are as in the [`Header`](#email.header.Header) constructor.
