...
**Source code:** [Lib/dis.py](https://github.com/python/cpython/tree/main/Lib/dis.py)
...
The `dis` module supports the analysis of CPython [bytecode](../glossary.md#term-bytecode) by
...
#### Versionchanged
Changed in version 3.10: The argument of jump, exception handling and loop instructions is now
...
which take the form of [`CACHE`](#opcode-CACHE) instructions. These instructions
...
[`CACHE`](#opcode-CACHE) entries.

As a consequence, the presence of the [`CACHE`](#opcode-CACHE) instructions is
...
#### Versionchanged
Changed in version 3.14: The [`-P`](#cmdoption-dis-P) command-line option
and the `show_positions` argument were added.

The [`-S`](#cmdoption-dis-S) command-line option is added.
...
[`Bytecode`](#dis.Bytecode) object that provides easy access to details of the compiled
code.
...
returned by [`compile()`](functions.md#compile)).

This is a convenience wrapper around many of the functions listed below, most
notably [`get_instructions()`](#dis.get_instructions), as iterating over a [`Bytecode`](#dis.Bytecode)
instance yields the bytecode operations as [`Instruction`](#dis.Instruction) instances.
...
disassembled code. Setting this means [`dis()`](#dis.Bytecode.dis) will display a “current
instruction” marker against the specified opcode.

If *show_caches* is `True`, [`dis()`](#dis.Bytecode.dis) will display inline cache
entries used by the interpreter to specialize the bytecode.

If *adaptive* is `True`, [`dis()`](#dis.Bytecode.dis) will display specialized bytecode
that may be different from the original bytecode.

If *show_offsets* is `True`, [`dis()`](#dis.Bytecode.dis) will include instruction
offsets in the output.

If *show_positions* is `True`, [`dis()`](#dis.Bytecode.dis) will include instruction
source code positions in the output.

#### *classmethod* from_traceback(tb, , show_caches=False)

Construct a [`Bytecode`](#dis.Bytecode) instance from the given traceback, setting
...
code object, like [`code_info()`](#dis.code_info).
...
and the code objects used for [annotation scopes](../reference/executionmodel.md#annotation-scopes).
Strings are first compiled to code objects with the [`compile()`](functions.md#compile)
...
1. the source code location of the instruction. Complete location information
...
The iterator generates a series of [`Instruction`](#dis.Instruction) named tuples giving
...
The *adaptive* parameter works as it does in [`dis()`](#module-dis).
...
generates the [`Instruction`](#dis.Instruction) instances with the *cache_info*
...
This generator function uses the [`co_lines()`](../reference/datamodel.md#codeobject.co_lines) method
of the [code object](../reference/datamodel.md#code-objects) *code* to find the offsets which
are starts of
...
#### Versionchanged
Changed in version 3.10: The [**PEP 626**](https://peps.python.org/pep-0626/) [`co_lines()`](../reference/datamodel.md#codeobject.co_lines) method is used instead of the
[`co_firstlineno`](../reference/datamodel.md#codeobject.co_firstlineno) and [`co_lnotab`](../reference/datamodel.md#codeobject.co_lnotab)
attributes of the [code object](../reference/datamodel.md#code-objects).
...
If the code has a jump target and *jump* is `True`, [`stack_effect()`](#dis.stack_effect)
...
The [`get_instructions()`](#dis.get_instructions) function and [`Bytecode`](#dis.Bytecode) class provide
details of bytecode instructions as [`Instruction`](#dis.Instruction) instances:
...
below and the bytecode values in the [Opcode collections](#opcode-collections).
...
otherwise equal to [`opcode`](#dis.Instruction.opcode)

#### baseopname

human readable name for the base operation if operation is specialized;
otherwise equal to [`opname`](#dis.opname)
...
alias for [`arg`](#dis.Instruction.arg)
...
`EXTENDED_ARG` operations if present; otherwise equal to [`offset`](#dis.Instruction.offset)
...
[`dis.Positions`](#dis.Positions) object holding the
...
Do nothing code.
Used by the interpreter to record [`BRANCH_LEFT`](sys.monitoring.md#monitoring-event-BRANCH_LEFT)
and [`BRANCH_RIGHT`](sys.monitoring.md#monitoring-event-BRANCH_RIGHT) events for [`sys.monitoring`](sys.monitoring.md#module-sys.monitoring).
...
#### Versionchanged
Changed in version 3.13: This instruction now requires an exact [`bool`](functions.md#bool) operand.
...
If `STACK[-1]` is a [generator iterator](../glossary.md#term-generator-iterator) or [coroutine](../glossary.md#term-coroutine) object
...
the [`CO_ITERABLE_COROUTINE`](inspect.md#inspect.CO_ITERABLE_COROUTINE) flag, or resolves
`o.__await__`.
...
Terminates an [`async for`](../reference/compound_stmts.md#async-for) loop.  Handles an exception raised
...
If the exception is not [`StopAsyncIteration`](exceptions.md#StopAsyncIteration), it is re-raised.
...
Handles an exception raised during a [`throw()`](../reference/expressions.md#generator.throw) or
[`close()`](../reference/expressions.md#generator.close) call through the current frame.  If `STACK[-1]` is an
instance of [`StopIteration`](exceptions.md#StopIteration), pop three values from the stack and push
...
For all of the [`SET_ADD`](#opcode-SET_ADD), [`LIST_APPEND`](#opcode-LIST_APPEND) and [`MAP_ADD`](#opcode-MAP_ADD)
...
or module body contains [variable annotations](../glossary.md#term-variable-annotation)
statically.
...
[`f_lasti`](../reference/datamodel.md#frame.f_lasti) of the current frame.
...
has occurred in a [`with`](../reference/compound_stmts.md#with) statement.
...
list of constants supported by this instruction.  Used by the [`assert`](../reference/simple_stmts.md#assert)
statement to load [`AssertionError`](exceptions.md#AssertionError).
...
Perform `STACK.append(len(STACK[-1]))`. Used in [`match`](../reference/compound_stmts.md#match) statements where
...
If `STACK[-1]` is an instance of [`collections.abc.Mapping`](collections.abc.md#collections.abc.Mapping) (or, more
technically: if it has the [`Py_TPFLAGS_MAPPING`](../c-api/typeobj.md#c.Py_TPFLAGS_MAPPING) flag set in its
[`tp_flags`](../c-api/typeobj.md#c.PyTypeObject.tp_flags)), push `True` onto the stack.  Otherwise,
push `False`.
...
If `STACK[-1]` is an instance of [`collections.abc.Sequence`](collections.abc.md#collections.abc.Sequence) and is *not* an instance
of [`str`](stdtypes.md#str)/[`bytes`](stdtypes.md#bytes)/[`bytearray`](stdtypes.md#bytearray) (or, more technically: if it has
the [`Py_TPFLAGS_SEQUENCE`](../c-api/typeobj.md#c.Py_TPFLAGS_SEQUENCE) flag set in its [`tp_flags`](../c-api/typeobj.md#c.PyTypeObject.tp_flags)),
...
If `STACK[-2]` contains all of the keys in `STACK[-1]`, push a [`tuple`](stdtypes.md#tuple)
...
[`co_names`](../reference/datamodel.md#codeobject.co_names) of the [code object](../reference/datamodel.md#code-objects).
The compiler tries to use [`STORE_FAST`](#opcode-STORE_FAST) or [`STORE_GLOBAL`](#opcode-STORE_GLOBAL) if possible.

### DELETE_NAME(namei)

Implements `del name`, where *namei* is the index into [`co_names`](../reference/datamodel.md#codeobject.co_names)
attribute of the [code object](../reference/datamodel.md#code-objects).
...
where *namei* is the index of name in [`co_names`](../reference/datamodel.md#codeobject.co_names) of the
...
where *namei* is the index of name into [`co_names`](../reference/datamodel.md#codeobject.co_names) of the
[code object](../reference/datamodel.md#code-objects).

### STORE_GLOBAL(namei)

Works as [`STORE_NAME`](#opcode-STORE_NAME), but stores the name as a global.

### DELETE_GLOBAL(namei)

Works as [`DELETE_NAME`](#opcode-DELETE_NAME), but deletes a global name.
...
to prepare namespace dictionaries for [`LOAD_FROM_DICT_OR_DEREF`](#opcode-LOAD_FROM_DICT_OR_DEREF)
and [`LOAD_FROM_DICT_OR_GLOBALS`](#opcode-LOAD_FROM_DICT_OR_GLOBALS).
...
similar to [`LOAD_GLOBAL`](#opcode-LOAD_GLOBAL).
This is used for loading global variables in
[annotation scopes](../reference/executionmodel.md#annotation-scopes) within class bodies.
...
Constructs a new [`Template`](string.templatelib.md#string.templatelib.Template) instance from a tuple
...
Constructs a new [`Interpolation`](string.templatelib.md#string.templatelib.Interpolation) instance from a
...
Works as [`BUILD_TUPLE`](#opcode-BUILD_TUPLE), but creates a list.

### BUILD_SET(count)

Works as [`BUILD_TUPLE`](#opcode-BUILD_TUPLE), but creates a set.
...
Like [`DICT_UPDATE`](#opcode-DICT_UPDATE) but raises an exception for duplicate keys.
...
`STACK[-1]` will be used as the first argument (`self`) by [`CALL`](#opcode-CALL)
or [`CALL_KW`](#opcode-CALL_KW) when calling the unbound method.
...
This opcode implements [`super()`](functions.md#super), both in its zero-argument and
...
With respect to its argument, it works similarly to [`LOAD_ATTR`](#opcode-LOAD_ATTR),
except that `namei` is shifted left by 2 bits instead of 1.

The low bit of `namei` signals to attempt a method load, as with
[`LOAD_ATTR`](#opcode-LOAD_ATTR), which results in pushing `NULL` and the loaded method.
When it is unset a single value is pushed to the stack.

The second-low bit of `namei`, if set, means that this was a two-argument
call to [`super()`](functions.md#super) (unset means zero-argument).
...
popped and provide the *fromlist* and *level* arguments of [`__import__()`](functions.md#import__).
The module object is pushed onto the stack.  The current namespace is not affected: for a proper import statement, a subsequent [`STORE_FAST`](#opcode-STORE_FAST) instruction
...
[`STORE_FAST`](#opcode-STORE_FAST) instruction.
...
`STACK[-1]` is an [iterator](../glossary.md#term-iterator).  Call its [`__next__()`](stdtypes.md#iterator.__next__) method.
...
guaranteed to be initialized. It cannot raise [`UnboundLocalError`](exceptions.md#UnboundLocalError).
...
Pushes a reference to the local `co_varnames[var_num]` onto the stack,
raising an [`UnboundLocalError`](exceptions.md#UnboundLocalError) if the local variable has not been
initialized.
...
#### Versionchanged
Changed in version 3.11: `i` is no longer offset by the length of [`co_varnames`](../reference/datamodel.md#codeobject.co_varnames).
...
slot `i`, similar to [`LOAD_DEREF`](#opcode-LOAD_DEREF). This is used for loading
[closure variables](../glossary.md#term-closure-variable) in class bodies (which previously used
...
Used by the [`del`](../reference/simple_stmts.md#del) statement.
...
Copies the `n` [free (closure) variables](../glossary.md#term-closure-variable) from the closure
...
#### Versionchanged
Changed in version 3.13: Calls with keyword arguments are now handled by [`CALL_KW`](#opcode-CALL_KW).
...
* The remaining positional arguments
* The named arguments
* A [`tuple`](stdtypes.md#tuple) of keyword argument names
...
[`LOAD_ATTR`](#opcode-LOAD_ATTR) for non-method calls.
...
removed. They now use [`SET_FUNCTION_ATTRIBUTE`](#opcode-SET_FUNCTION_ATTRIBUTE).
...
* `0x10` the [annotate function](../glossary.md#term-annotate-function) for the function object
...
See the [`slice()`](functions.md#slice) built-in function for more information.
...
* `oparg == 1`: call [`str()`](stdtypes.md#str) on *value*
* `oparg == 2`: call [`repr()`](functions.md#repr) on *value*
* `oparg == 3`: call [`ascii()`](functions.md#ascii) on *value*
...
If the call raises [`StopIteration`](exceptions.md#StopIteration), pop the top value from the stack,
...
use the [`hasarg`](#dis.hasarg) collection instead.
...
#### Deprecated
Deprecated since version 3.13: Use [`hasarg`](#dis.hasarg) instead.
...
| `INTRINSIC_PRINT`               | Prints the argument to standard<br/>out. Used in the REPL.                                                                                                                                                                               |
| `INTRINSIC_IMPORT_STAR`         | Performs `import *` for the<br/>named module.                                                                                                                                                                                            |
| `INTRINSIC_STOPITERATION_ERROR` | Extracts the return value from a<br/>`StopIteration` exception.                                                                                                                                                                          |
| `INTRINSIC_ASYNC_GEN_WRAP`      | Wraps an async generator value                                                                                                                                                                                                           |
| `INTRINSIC_UNARY_POSITIVE`      | Performs the unary `+`<br/>operation                                                                                                                                                                                                     |
| `INTRINSIC_LIST_TO_TUPLE`       | Converts a list to a tuple                                                                                                                                                                                                               |
| `INTRINSIC_TYPEVAR`             | Creates a [`typing.TypeVar`](typing.md#typing.TypeVar)                                                                                                                                                                                   |
| `INTRINSIC_PARAMSPEC`           | Creates a<br/>[`typing.ParamSpec`](typing.md#typing.ParamSpec)                                                                                                                                                                           |
| `INTRINSIC_TYPEVARTUPLE`        | Creates a<br/>[`typing.TypeVarTuple`](typing.md#typing.TypeVarTuple)                                                                                                                                                                     |
| `INTRINSIC_SUBSCRIPT_GENERIC`   | Returns [`typing.Generic`](typing.md#typing.Generic)<br/>subscripted with the argument                                                                                                                                                   |
| `INTRINSIC_TYPEALIAS`           | Creates a<br/>[`typing.TypeAliasType`](typing.md#typing.TypeAliasType);<br/>used in the [`type`](../reference/simple_stmts.md#type)<br/>statement. The argument is a tuple<br/>of the type alias’s name,<br/>type parameters, and value. |
...
| `INTRINSIC_PREP_RERAISE_STAR`        | Calculates the<br/>[`ExceptionGroup`](exceptions.md#ExceptionGroup) to raise<br/>from a `try-except*`. |
| `INTRINSIC_TYPEVAR_WITH_BOUND`       | Creates a [`typing.TypeVar`](typing.md#typing.TypeVar)<br/>with a bound.                               |
| `INTRINSIC_TYPEVAR_WITH_CONSTRAINTS` | Creates a<br/>[`typing.TypeVar`](typing.md#typing.TypeVar) with<br/>constraints.                       |
| `INTRINSIC_SET_FUNCTION_TYPE_PARAMS` | Sets the `__type_params__`<br/>attribute of a function.                                                |
...
This variant is used in [`with`](../reference/compound_stmts.md#with) and [`async with`](../reference/compound_stmts.md#async-with)
constructs, which push the return value of the context manager’s
[`__enter__()`](../reference/datamodel.md#object.__enter__) or [`__aenter__()`](../reference/datamodel.md#object.__aenter__) to the stack.
...
Sequence of bytecodes that access a [free (closure) variable](../glossary.md#term-closure-variable).
...
#### Deprecated
Deprecated since version 3.13: All jumps are now relative. Use [`hasjump`](#dis.hasjump).
