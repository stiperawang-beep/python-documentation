...
**Source code:** [Lib/email/message.py](https://github.com/python/cpython/tree/main/Lib/email/message.py)

---

#### Versionadded
Added in version 3.6: <sup>[1](#id3)</sup>

The central class in the [`email`](email.md#module-email) package is the [`EmailMessage`](#email.message.EmailMessage)
class, imported from the `email.message` module.  It is the base class for
the [`email`](email.md#module-email) object model.  [`EmailMessage`](#email.message.EmailMessage) provides the core
...
The conceptual model provided by an [`EmailMessage`](#email.message.EmailMessage) object is that of an
...
The [`EmailMessage`](#email.message.EmailMessage) dictionary-like interface is indexed by the header
...
objects, or a list of [`EmailMessage`](#email.message.EmailMessage) objects, for MIME container
...
[`default`](email.policy.md#email.policy.default) policy, which follows the rules of the email
RFCs except for line endings (instead of the RFC mandated `\r\n`, it uses
the Python standard `\n` line endings).  For more information see the
[`policy`](email.policy.md#module-email.policy) documentation. <sup>[2](#id4)</sup>
...
with the base [`Message`](email.compat32-message.md#email.message.Message) class *maxheaderlen* is
accepted, but defaults to `None`, which means that by default the line
length is controlled by the
[`max_line_length`](email.policy.md#email.policy.Policy.max_line_length) of the policy.  The
...
passed to the [`Generator`](email.generator.md#email.generator.Generator).

Flattening the message may trigger changes to the [`EmailMessage`](#email.message.EmailMessage)
...
[`email.generator.Generator`](email.generator.md#email.generator.Generator) for a more flexible API for
serializing messages.  Note also that this method is restricted to
producing messages serialized as “7 bit clean” when
[`utf8`](email.policy.md#email.policy.EmailPolicy.utf8) is `False`, which is the default.
...
being a direct alias for [`as_string()`](#email.message.EmailMessage.as_string).
...
most useful way to serialize messages in your application, especially if
you are dealing with multiple messages.  See
[`email.generator.BytesGenerator`](email.generator.md#email.generator.BytesGenerator) for a more flexible API for
serializing messages.

#### \_\_bytes_\_()

Equivalent to [`as_bytes()`](#email.message.EmailMessage.as_bytes).  Allows `bytes(msg)` to produce a
bytes object containing the serialized message.

#### is_multipart()

Return `True` if the message’s payload is a list of
sub-[`EmailMessage`](#email.message.EmailMessage) objects, otherwise return `False`.  When
[`is_multipart()`](#email.message.EmailMessage.is_multipart) returns `False`, the payload should be a string
object (which might be a CTE encoded binary payload).  Note that
[`is_multipart()`](#email.message.EmailMessage.is_multipart) returning `True` does not necessarily mean that
“msg.get_content_maintype() == ‘multipart’” will return the `True`.
For example, `is_multipart` will return `True` when the
[`EmailMessage`](#email.message.EmailMessage) is of type `message/rfc822`.

#### set_unixfrom(unixfrom)

Set the message’s envelope header to *unixfrom*, which should be a
string.  (See [`mboxMessage`](mailbox.md#mailbox.mboxMessage) for a brief description of
this header.)
...
order to the keys returned by [`keys()`](#email.message.EmailMessage.keys), but in an [`EmailMessage`](#email.message.EmailMessage)
...
undefined.  Use the [`get_all()`](#email.message.EmailMessage.get_all) method to get the values of all the
extant headers named *name*.

Using the standard (non-`compat32`) policies, the returned value is an
instance of a subclass of [`email.headerregistry.BaseHeader`](email.headerregistry.md#email.headerregistry.BaseHeader).
...
If the [`policy`](email.policy.md#module-email.policy) defines certain headers to be unique (as the standard
policies do), this method may raise a [`ValueError`](exceptions.md#ValueError) when an attempt
...
Extended header setting.  This method is similar to [`__setitem__()`](#email.message.EmailMessage.__setitem__)
...
[`KeyError`](exceptions.md#KeyError).
...
[`get_default_type()`](#email.message.EmailMessage.get_default_type).  If the *Content-Type* header is
...
[`get_content_type()`](#email.message.EmailMessage.get_content_type) will always return a value.  [**RFC 2045**](https://datatracker.ietf.org/doc/html/rfc2045.html) defines
...
part of the string returned by [`get_content_type()`](#email.message.EmailMessage.get_content_type).
...
Use of the *requote* parameter with [`EmailMessage`](#email.message.EmailMessage) objects is
deprecated.

Note that existing parameter values of headers may be accessed through
the [`params`](email.headerregistry.md#email.headerregistry.ParameterizedMIMEHeader.params) attribute of the
...
*boundary*.  [`set_boundary()`](#email.message.EmailMessage.set_boundary) will always quote *boundary* if
...
boundary via [`add_header()`](#email.message.EmailMessage.add_header), because [`set_boundary()`](#email.message.EmailMessage.set_boundary) preserves
...
with [`is_multipart()`](email.compat32-message.md#email.message.Message.is_multipart).
...
The [`walk()`](#email.message.EmailMessage.walk) method is an all-purpose generator which can be used to
iterate over all the parts and subparts of a message object tree, in
depth-first traversal order.  You will typically use [`walk()`](#email.message.EmailMessage.walk) as the
...
[`is_multipart()`](#email.message.EmailMessage.is_multipart) returns `True`, even though
...
[`walk()`](#email.message.EmailMessage.walk).)

#### get_content(\*args, content_manager=None, \*\*kw)

Call the [`get_content()`](email.contentmanager.md#email.contentmanager.ContentManager.get_content) method
...
by the current [`policy`](email.policy.md#module-email.policy).

#### set_content(\*args, content_manager=None, \*\*kw)

Call the [`set_content()`](email.contentmanager.md#email.contentmanager.ContentManager.set_content) method
of the *content_manager*, passing self as the message object, and passing
along any other arguments or keywords as additional arguments.  If
*content_manager* is not specified, use the `content_manager` specified
by the current [`policy`](email.policy.md#module-email.policy).
...
object, pass all of the arguments to its [`set_content()`](#email.message.EmailMessage.set_content) method,
and [`attach()`](email.compat32-message.md#email.message.Message.attach) it to the `multipart`.  If
the message is a non-`multipart`, call [`make_related()`](#email.message.EmailMessage.make_related) and then
proceed as above.  If the message is any other type of `multipart`,
raise a [`TypeError`](exceptions.md#TypeError). If *content_manager* is not specified, use
the `content_manager` specified by the current [`policy`](email.policy.md#module-email.policy).
...
object, pass all of the arguments to its [`set_content()`](#email.message.EmailMessage.set_content) method, and
[`attach()`](email.compat32-message.md#email.message.Message.attach) it to the `multipart`.  If the
message is a non-`multipart` or `multipart/related`, call
[`make_alternative()`](#email.message.EmailMessage.make_alternative) and then proceed as above.  If the message is
any other type of `multipart`, raise a [`TypeError`](exceptions.md#TypeError). If
*content_manager* is not specified, use the `content_manager` specified
by the current [`policy`](email.policy.md#module-email.policy).
...
pass all of the arguments to its [`set_content()`](#email.message.EmailMessage.set_content) method, and
[`attach()`](email.compat32-message.md#email.message.Message.attach) it to the `multipart`.  If the
message is a non-`multipart`, `multipart/related`, or
`multipart/alternative`, call [`make_mixed()`](#email.message.EmailMessage.make_mixed) and then proceed as
above. If *content_manager* is not specified, use the `content_manager`
specified by the current [`policy`](email.policy.md#module-email.policy).  If the added part
...
[`EmailMessage`](#email.message.EmailMessage) objects have the following instance attributes:
...
the end of the message.  As with the [`preamble`](#email.message.EmailMessage.preamble),
...
[`EmailMessage`](#email.message.EmailMessage), except that no *MIME-Version* headers are
added when [`set_content()`](#email.message.EmailMessage.set_content) is called, since sub-parts do
not need their own *MIME-Version* headers.

### Footnotes

* <a id='id3'>**[1]**</a> Originally added in 3.4 as a [provisional module](../glossary.md#term-provisional-package).  Docs for legacy message class moved to [email.message.Message: Representing an email message using the compat32 API](email.compat32-message.md#compat32-message).
* <a id='id4'>**[2]**</a> The [`EmailMessage`](#email.message.EmailMessage) class requires a policy that provides a `content_manager` attribute for content management methods like `set_content()` and `get_content()` to work. The legacy [`compat32`](email.policy.md#email.policy.compat32) policy does not support these methods and should not be used with [`EmailMessage`](#email.message.EmailMessage).
