...
**Source code:** [Lib/profiling/](https://github.com/python/cpython/tree/main/Lib/profiling/)
...
The `profiling` package organizes Python’s built-in profiling tools under
...
[`timeit`](timeit.md#module-timeit) module, which provides reasonably accurate timing
...
<a id="choosing-a-profiler"></a>

## Choosing a profiler

For most performance analysis, use the statistical profiler
([`profiling.sampling`](profiling.sampling.md#module-profiling.sampling)). It has minimal overhead, works for both development
and production, and provides rich visualization options including flame graphs,
heatmaps, GIL analysis, and more.

Use the deterministic profiler ([`profiling.tracing`](profiling.tracing.md#module-profiling.tracing)) when you need **exact
...
| Feature             | Statistical sampling<br/>([`profiling.sampling`](profiling.sampling.md#module-profiling.sampling))   | Deterministic<br/>([`profiling.tracing`](profiling.tracing.md#module-profiling.tracing))   |
|---------------------|------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------|
| **Overhead**        | Virtually none                                                                                       | Moderate                                                                                   |
| **Accuracy**        | Statistical estimate                                                                                 | Exact call counts                                                                          |
| **Output formats**  | pstats, flame graph, heatmap,<br/>gecko, collapsed                                                   | pstats                                                                                     |
...
The statistical profiler ([`profiling.sampling`](profiling.sampling.md#module-profiling.sampling)) is recommended for most
...
The deterministic profiler ([`profiling.tracing`](profiling.tracing.md#module-profiling.tracing)) instruments every function
...
To profile a script, use the [`profiling.sampling`](profiling.sampling.md#module-profiling.sampling) module with the `run`
command:
...
: Which functions called a given function (callers) and which functions it
  ...
  can display it via the [`print_callers()`](pstats.md#pstats.Stats.print_callers) and
  [`print_callees()`](pstats.md#pstats.Stats.print_callees) methods.

## Legacy compatibility

For backward compatibility, the `cProfile` module remains available as an
alias to [`profiling.tracing`](profiling.tracing.md#module-profiling.tracing). Existing code using `import cProfile` will
continue to work without modification in all future Python versions.

#### Deprecated
Deprecated since version 3.15: The pure Python [`profile`](profile.md#module-profile) module is deprecated and will be removed in
Python 3.17. Use [`profiling.tracing`](profiling.tracing.md#module-profiling.tracing) (or its alias `cProfile`)
instead. See [`profile`](profile.md#module-profile) for migration guidance.
...
[`timeit`](timeit.md#module-timeit)
