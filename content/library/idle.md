...
**Source code:** [Lib/idlelib/](https://github.com/python/cpython/tree/main/Lib/idlelib/)
...
The IDLE application is implemented in the [`idlelib`](#module-idlelib) package.
...
: After an unclosed parenthesis for a function, open a small window with
  function parameter hints.  See [Calltips](#calltips) in the
...
: Rewrap the text block containing the text insert cursor.
  Avoid code lines.  See [Format block](#format-block) in the
  Editing and Navigation section below.
...
: Remove trailing space and other whitespace characters after the last
  non-whitespace character of a line by applying [`str.rstrip()`](stdtypes.md#str.rstrip) to each line,
...
Run Module
: Do [Check Module](#check-module).  If no error, restart the shell to clean the
...
Run… Customized
: Same as [Run Module](#run-module), but run the module with customized
  settings.  *Command Line Arguments* extend [`sys.argv`](sys.md#sys.argv) as if passed
...
: Open a pane at the top of the edit window which shows the block context
  ...
  [Code Context](#code-context) in the Editing and Navigation section
  below.
...
: Open a column to the left of the edit window which shows the number
  ...
  preferences (see [Setting preferences](#preferences)).
...
the General tab. See the [Help sources](#help-sources) subsection below
...
For filenames in the root directory, type [`os.sep`](os.md#os.sep) or
[`os.altsep`](os.md#os.altsep) immediately after an opening quote.  (On Windows,
...
modules, not included in ‘_\_all_\_’.  The hidden names can be accessed
...
[`SyntaxError`](exceptions.md#SyntaxError) when multiple statements are compiled as if they were one.
...
IDLE also highlights the [soft keywords](../reference/lexical_analysis.md#soft-keywords) [`match`](../reference/compound_stmts.md#match),
[`case`](../reference/compound_stmts.md#match), and [`_`](../reference/compound_stmts.md#wildcard-patterns) in
...
the environment variables `IDLESTARTUP` or [`PYTHONSTARTUP`](../using/cmdline.md#envvar-PYTHONSTARTUP).
...
Run the startup file (as defined by the environment variables `IDLESTARTUP` or [`PYTHONSTARTUP`](../using/cmdline.md#envvar-PYTHONSTARTUP)) before opening the shell window.
...
and [`str.join()`](stdtypes.md#str.join) can help combine fields and lines.
...
sense that feature changes can be backported (see [**PEP 434**](https://peps.python.org/pep-0434/)).
