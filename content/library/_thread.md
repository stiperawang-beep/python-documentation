...
The [`threading`](threading.md#module-threading) module provides an easier to use and higher-level
...
#### Versionchanged
Changed in version 3.3: This is now a synonym of the built-in [`RuntimeError`](exceptions.md#RuntimeError).
...
[`sys.unraisablehook()`](sys.md#sys.unraisablehook) is called to handle the exception. The *object*
...
When the function raises a [`SystemExit`](exceptions.md#SystemExit) exception, it is silently
ignored.

Raises an [auditing event](sys.md#auditing) `_thread.start_new_thread` with arguments `function`, `args`, `kwargs`.

#### Versionchanged
Changed in version 3.8: [`sys.unraisablehook()`](sys.md#sys.unraisablehook) is now used to handle unhandled exceptions.
...
If *signum* is not given, [`signal.SIGINT`](signal.md#signal.SIGINT) is simulated.

If the given signal isn’t handled by Python (it was set to
[`signal.SIG_DFL`](signal.md#signal.SIG_DFL) or [`signal.SIG_IGN`](signal.md#signal.SIG_IGN)), this function does
nothing.
...
If you want to truly emit the signal, use [`signal.raise_signal()`](signal.md#signal.raise_signal).

### \_thread.exit()

Raise the [`SystemExit`](exceptions.md#SystemExit) exception.  When not caught, this will cause the
...
[`Lock.acquire`](threading.md#threading.Lock.acquire). Specifying a timeout greater
than this value will raise an [`OverflowError`](exceptions.md#OverflowError).
...
[`with`](../reference/compound_stmts.md#with) statement, e.g.:
...
<a id="index-2"></a>
* Interrupts always go to the main thread (the [`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt)
  exception will be received by that thread.)
* Calling [`sys.exit()`](sys.md#sys.exit) or raising the [`SystemExit`](exceptions.md#SystemExit) exception is
  equivalent to calling [`_thread.exit()`](#thread.exit).
* When the main thread exits, it is system defined whether the other threads
  survive.  On most systems, they are killed without executing
  [`try`](../reference/compound_stmts.md#try) … [`finally`](../reference/compound_stmts.md#finally) clauses or executing object
  destructors.
