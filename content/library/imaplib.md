...
**Source code:** [Lib/imaplib.py](https://github.com/python/cpython/tree/main/Lib/imaplib.py)

<a id="index-0"></a>

---

This module defines three classes, [`IMAP4`](#imaplib.IMAP4), [`IMAP4_SSL`](#imaplib.IMAP4_SSL) and
[`IMAP4_stream`](#imaplib.IMAP4_stream), which encapsulate a connection to an IMAP4 server and
...
Three classes are provided by the `imaplib` module, [`IMAP4`](#imaplib.IMAP4) is the
base class:
...
The [`IMAP4`](#imaplib.IMAP4) class supports the [`with`](../reference/compound_stmts.md#with) statement.  When used
...
#### Versionchanged
Changed in version 3.5: Support for the [`with`](../reference/compound_stmts.md#with) statement was added.

#### Versionchanged
Changed in version 3.9: The optional *timeout* parameter was added.

Three exceptions are defined as attributes of the [`IMAP4`](#imaplib.IMAP4) class:
...
[`IMAP4.error`](#imaplib.IMAP4.error).  Note that closing the instance and instantiating a new one
...
server.  This is a sub-class of [`IMAP4.error`](#imaplib.IMAP4.error).  Some other client now has
...
This is a subclass derived from [`IMAP4`](#imaplib.IMAP4) that connects over an SSL
encrypted socket (to use this class you need a socket module that was compiled
with SSL support).  If *host* is not specified, `''` (the local host) is used.
If *port* is omitted, the standard IMAP4-over-SSL port (993) is used.
*ssl_context* is a [`ssl.SSLContext`](ssl.md#ssl.SSLContext) object which allows bundling
SSL configuration options, certificates and private keys into a single
(potentially long-lived) structure.  Please read [Security considerations](ssl.md#ssl-security) for
best practices.
...
This is a subclass derived from [`IMAP4`](#imaplib.IMAP4) that connects to the
...
time.  The return value is a [`time.struct_time`](time.md#time.struct_time) tuple or
...
returned by [`time.time()`](time.md#time.time)), a 9-tuple representing local time
an instance of [`time.struct_time`](time.md#time.struct_time) (as returned by
[`time.localtime()`](time.md#time.localtime)), an aware instance of
[`datetime.datetime`](datetime.md#datetime.datetime), or a double-quoted string.  In the last
...
can all be found at (**Source Code**) [https://github.com/uw-imap/imap](https://github.com/uw-imap/imap) (**Not Maintained**).

<a id="imap4-objects"></a>
...
An [`IMAP4`](#imaplib.IMAP4) instance has the following methods:
...
#### Versionadded
Added in version 3.5: The [`enable()`](#imaplib.IMAP4.enable) method itself, and [**RFC 6855**](https://datatracker.ietf.org/doc/html/rfc6855.html) support.
...
[`with`](../reference/compound_stmts.md#with) statement, produces IMAP untagged responses via the
[iterator](../glossary.md#term-iterator) protocol, and sends `DONE` upon context exit.
...
the [`with`](../reference/compound_stmts.md#with) context) can be retrieved in the usual way after
`IDLE` ends, using [`IMAP4.response()`](#imaplib.IMAP4.response).
...
after which any ongoing iteration will stop. It can be an [`int`](functions.md#int) or
[`float`](functions.md#float), or `None` for no time limit.
...
[`IMAP4_stream`](#imaplib.IMAP4_stream) connections.
...
(expressed as an [`int`](functions.md#int) or [`float`](functions.md#float)).

This [generator](../glossary.md#term-generator) is an alternative to iterating one response at a
...
Requires a socket connection; does not work on [`IMAP4_stream`](#imaplib.IMAP4_stream)
connections.
...
#### NOTE
The iterator returned by [`IMAP4.idle()`](#imaplib.IMAP4.idle) is usable only within a
[`with`](../reference/compound_stmts.md#with) statement. Before or after that context, unsolicited
responses are collected internally whenever a command finishes, and can
be retrieved with [`IMAP4.response()`](#imaplib.IMAP4.response).
...
#### Versionchanged
Changed in version 3.15: An [`IMAP4.error`](#imaplib.IMAP4.error) is raised if MD5 support is not available.
...
it will raise a [`ValueError`](exceptions.md#ValueError) to reject creating a non-blocking socket.
This method is implicitly called by the [`IMAP4`](#imaplib.IMAP4) constructor.
The connection objects established by this method will be used in
the [`IMAP4.read()`](#imaplib.IMAP4.read), [`IMAP4.readline()`](#imaplib.IMAP4.readline), [`IMAP4.send()`](#imaplib.IMAP4.send),
and [`IMAP4.shutdown()`](#imaplib.IMAP4.shutdown) methods. You may override this method.

Raises an [auditing event](sys.md#auditing) `imaplib.open` with arguments `self`, `host`, `port`.
...
the `UTF8=ACCEPT` capability was enabled using the [`enable()`](#imaplib.IMAP4.enable)
command.
...
called by [`IMAP4.logout()`](#imaplib.IMAP4.logout).  You may override this method.
...
and should be a [`ssl.SSLContext`](ssl.md#ssl.SSLContext) object.  This will enable
...
[`imaplib.IMAP4.unselect()`](#imaplib.IMAP4.unselect) frees server’s resources associated with the
selected mailbox and returns the server to the authenticated
state. This command performs the same actions as [`imaplib.IMAP4.close()`](#imaplib.IMAP4.close), except
...
The following attributes are defined on instances of [`IMAP4`](#imaplib.IMAP4):
...
[`enable()`](#imaplib.IMAP4.enable) command is successfully issued for the `UTF8=ACCEPT`
capability.

#### Versionadded
Added in version 3.5.

<a id="imap4-example"></a>
