...
**Source code:** [Lib/runpy.py](https://github.com/python/cpython/tree/main/Lib/runpy.py)

---

The `runpy` module is used to locate and run Python modules without
importing them first. Its main use is to implement the [`-m`](../using/cmdline.md#cmdoption-m) command
...
If that limitation is not acceptable for a given use case, [`importlib`](importlib.md#module-importlib)
...
module, then that package is imported and the [`__main__`](__main__.md#module-__main__) submodule within
...
overridden by [`run_module()`](#runpy.run_module).
...
[`None`](constants.md#None), to `mod_name + '.__main__'` if the named module is a
...
[set as normal](../reference/datamodel.md#import-mod-attrs) based on the module spec.

If the argument *alter_sys* is supplied and evaluates to [`True`](constants.md#True),
then `sys.argv[0]` is updated with the value of `__file__` and
`sys.modules[__name__]` is updated with a temporary module object for the
module being executed. Both `sys.argv[0]` and `sys.modules[__name__]`
are restored to their original values before the function returns.

Note that this manipulation of [`sys`](sys.md#module-sys) is not thread-safe. Other threads
may see the partially initialised module, as well as the altered list of
arguments. It is recommended that the `sys` module be left alone when
invoking this function from threaded code.

#### SEE ALSO
The [`-m`](../using/cmdline.md#cmdoption-m) option offering equivalent functionality from the
command line.

#### Versionchanged
Changed in version 3.1: Added ability to execute packages by looking for a [`__main__`](__main__.md#module-__main__) submodule.
...
[`ModuleSpec`](importlib.md#importlib.machinery.ModuleSpec) for alternatives.
...
compiled bytecode file or a valid [`sys.path`](sys.md#sys.path) entry containing a
[`__main__`](__main__.md#module-__main__) module
(e.g. a zipfile containing a top-level `__main__.py` file).

For a simple script, the specified code is simply executed in a fresh
module namespace. For a valid [`sys.path`](sys.md#sys.path) entry (typically a zipfile or
directory), the entry is first added to the beginning of `sys.path`. The
function then looks for and executes a [`__main__`](__main__.md#module-__main__) module using the
...
*init_globals* will not be modified. If any of the special global variables
below are defined in *init_globals*, those definitions are
overridden by [`run_path()`](#runpy.run_path).
...
`__name__` is set to *run_name* if this optional argument is not
[`None`](constants.md#None) and to `'<run_path>'` otherwise.
...
`__package__` will all be set to [`None`](constants.md#None).

If *file_path* is a reference to a valid [`sys.path`](sys.md#sys.path) entry, then
`__spec__` will be set appropriately for the imported [`__main__`](__main__.md#module-__main__)
module (that is, `__spec__.name` will always be `__main__`).
`__file__`, `__loader__` and `__package__` will be
[set as normal](../reference/datamodel.md#import-mod-attrs) based on the module spec.

A number of alterations are also made to the [`sys`](sys.md#module-sys) module. Firstly,
[`sys.path`](sys.md#sys.path) may be altered as described above. `sys.argv[0]` is updated
with the value of *file_path* and `sys.modules[__name__]` is updated
with a temporary module object for the module being executed. All
modifications to items in [`sys`](sys.md#module-sys) are reverted before the function
returns.

Note that, unlike [`run_module()`](#runpy.run_module), the alterations made to [`sys`](sys.md#module-sys)
are not optional in this function as these adjustments are essential to
allowing the execution of [`sys.path`](sys.md#sys.path) entries. As the thread-safety
limitations still apply, use of this function in threaded code should be
either serialised with the import lock or delegated to a separate process.

#### SEE ALSO
[Interface options](../using/cmdline.md#using-on-interface-options) for equivalent functionality on the
...
case where `__main__` is imported from a valid [`sys.path`](sys.md#sys.path) entry rather
...
[Command line and environment](../using/cmdline.md#using-on-general) - CPython command line details

The [`importlib.import_module()`](importlib.md#importlib.import_module) function
