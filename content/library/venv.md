...
**Source code:** [Lib/venv/](https://github.com/python/cpython/tree/main/Lib/venv/)
...
their [`site`](site.md#module-site) directories.
...
available. See [Virtual Environments](sys_path_init.md#sys-path-init-virtual-environments) and [`site`](site.md#module-site)’s
...
[Virtual environments](#venv-def) are created by executing the `venv`
module:
...
Unless the `--without-pip` option is given, [`ensurepip`](ensurepip.md#module-ensurepip) will be
...
<a id="venv-explanation"></a>

## How venvs work

When a Python interpreter is running from a virtual environment,
[`sys.prefix`](sys.md#sys.prefix) and [`sys.exec_prefix`](sys.md#sys.exec_prefix)
point to the directories of the virtual environment,
whereas [`sys.base_prefix`](sys.md#sys.base_prefix) and [`sys.base_exec_prefix`](sys.md#sys.base_exec_prefix)
...
you have the [Python install manager](../using/windows.md#launcher) installed. Thus, double-clicking an installed
...
creation according to their needs, the [`EnvBuilder`](#venv.EnvBuilder) class.
...
The [`EnvBuilder`](#venv.EnvBuilder) class accepts the following keyword arguments on
instantiation:
...
* *with_pip* – a boolean value which, if true, ensures pip is
  installed in the virtual environment. This uses [`ensurepip`](ensurepip.md#module-ensurepip) with
...
* *scm_ignore_files* – Create ignore files based for the specified source
  ...
  default is `"git"` via [`create_git_ignore_file()`](#venv.EnvBuilder.create_git_ignore_file).
...
[`EnvBuilder`](#venv.EnvBuilder) may be used as a base class.
...
The `create` method of the [`EnvBuilder`](#venv.EnvBuilder) class illustrates the
...
[`create_configuration()`](#venv.EnvBuilder.create_configuration), [`setup_python()`](#venv.EnvBuilder.setup_python),
[`setup_scripts()`](#venv.EnvBuilder.setup_scripts) and [`post_setup()`](#venv.EnvBuilder.post_setup) can be overridden.
...
methods.  If the [`EnvBuilder`](#venv.EnvBuilder) is created with the arg
`clear=True`, contents of the environment directory will be cleared
and then all necessary subdirectories will be recreated.

The returned context object is a [`types.SimpleNamespace`](types.md#types.SimpleNamespace) with the
following attributes:

* `env_dir` - The location of the virtual environment. Used for
  `__VENV_DIR__` in activation scripts (see [`install_scripts()`](#venv.EnvBuilder.install_scripts)).
* `env_name` - The name of the virtual environment. Used for
  `__VENV_NAME__` in activation scripts (see [`install_scripts()`](#venv.EnvBuilder.install_scripts)).
* `prompt` - The prompt to be used by the activation scripts. Used for
  `__VENV_PROMPT__` in activation scripts (see [`install_scripts()`](#venv.EnvBuilder.install_scripts)).
...
#### Versionchanged
Changed in version 3.11: The *venv*
[sysconfig installation scheme](sysconfig.md#installation-paths)
...
#### Versionchanged
Changed in version 3.15: The attribute `platlib_path` was added to the context.
...
This method can be
called from [`setup_scripts()`](#venv.EnvBuilder.setup_scripts) or [`post_setup()`](#venv.EnvBuilder.post_setup) in subclasses to
...
directory corresponding to [`os.name`](os.md#os.name) are copied after some text
...
copying the actual binaries. In 3.7.2 only [`setup_python()`](#venv.EnvBuilder.setup_python) does
nothing unless running from a build in the source tree.

#### Versionchanged
Changed in version 3.7.3: Windows copies the redirector scripts as part of [`setup_python()`](#venv.EnvBuilder.setup_python)
instead of [`setup_scripts()`](#venv.EnvBuilder.setup_scripts). This was not the case in 3.7.2.
...
Create an [`EnvBuilder`](#venv.EnvBuilder) with the given keyword arguments, and call its
[`create()`](#venv.EnvBuilder.create) method with the *env_dir* argument.
...
The following script shows how to extend [`EnvBuilder`](#venv.EnvBuilder) by implementing a
