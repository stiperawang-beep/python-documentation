...
**Source code:** [Lib/profiling/tracing/](https://github.com/python/cpython/tree/main/Lib/profiling/tracing/)
...
*Deterministic profiling* captures every function call, function return,
...
In contrast to [statistical profiling](profiling.sampling.md#profiling-sampling), which samples
...
<a id="profiling-tracing-cli"></a>

## Command-line interface

The `profiling.tracing` module can be invoked as a script to profile
...
file can be read by the [`pstats`](pstats.md#module-pstats) module for later analysis.

### -s <sort_order>

Sort the output by the specified key. This accepts any of the sort keys
recognized by [`pstats.Stats.sort_stats()`](pstats.md#pstats.Stats.sort_stats), such as `cumulative`,
`time`, `calls`, or `name`. This option only applies when
[`-o`](#cmdoption-profiling.tracing-o) is not specified.
...
#### Versionadded
Added in version 3.8: The `-m` option for [`profile`](profile.md#module-profile).
...
This function executes the *command* string using [`exec()`](functions.md#exec) in the
...
If *filename* is not provided, the function creates a [`pstats.Stats`](pstats.md#pstats.Stats)
...
with [`pstats`](pstats.md#module-pstats).

The *sort* argument specifies the sort order for printed output, accepting
any value recognized by [`pstats.Stats.sort_stats()`](pstats.md#pstats.Stats.sort_stats).
...
Like [`run()`](#profiling.tracing.run), but executes the command with the specified *globals*
...
Create a [`pstats.Stats`](pstats.md#pstats.Stats) object from the current profile and print
...
as [`pstats.Stats.sort_stats()`](pstats.md#pstats.Stats.sort_stats).
...
[`pstats.Stats`](pstats.md#pstats.Stats) for later analysis.

#### run(cmd)

Profile the command string via [`exec()`](functions.md#exec).

#### runctx(cmd, globals, locals)

Profile the command string via [`exec()`](functions.md#exec) with the specified
namespaces.
...
interpreter terminates (for example, via [`sys.exit()`](sys.md#sys.exit)) during
profiling, no results will be available.

## Using a custom timer

The [`Profile`](#profiling.tracing.Profile) class accepts a custom timing function, allowing you to
...
The [`time`](time.md#module-time) module provides several functions suitable for use as custom
timers:

- [`time.perf_counter()`](time.md#time.perf_counter) for high-resolution wall-clock time
- [`time.process_time()`](time.md#time.process_time) for CPU time (excluding sleep)
- [`time.monotonic()`](time.md#time.monotonic) for monotonic clock time
...
less pronounced than with the deprecated pure Python [`profile`](profile.md#module-profile) module.
...
[`profile`](profile.md#module-profile)
