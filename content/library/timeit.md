...
**Source code:** [Lib/timeit.py](https://github.com/python/cpython/tree/main/Lib/timeit.py)
...
Create a [`Timer`](#timeit.Timer) instance with the given statement, *setup* code and
*timer* function and run its [`timeit()`](#timeit.Timer.timeit) method with *number* executions.
...
Create a [`Timer`](#timeit.Timer) instance with the given statement, *setup* code and
*timer* function and run its [`repeat()`](#timeit.Timer.repeat) method with the given *repeat*
...
#### Versionchanged
Changed in version 3.3: [`time.perf_counter()`](time.md#time.perf_counter) is now the default timer.
...
To measure the execution time of the first statement, use the [`timeit()`](#timeit.Timer.timeit)
method.  The [`repeat()`](#timeit.Timer.repeat) and [`autorange()`](#timeit.Timer.autorange) methods are convenience
methods to call [`timeit()`](#timeit.Timer.timeit) multiple times.

The execution time of *setup* is excluded from the overall timed execution run.

The *stmt* and *setup* parameters can also take objects that are callable
without arguments.  This will embed calls to them in a timer function that
will then be executed by [`timeit()`](#timeit.Timer.timeit).  Note that the timing overhead is a
...
#### NOTE
By default, [`timeit()`](#timeit.Timer.timeit) temporarily turns off [garbage
collection](../glossary.md#term-garbage-collection) during the timing.  The advantage of this approach is that
...
Automatically determine how many times to call [`timeit()`](#timeit.Timer.timeit).

This is a convenience function that calls [`timeit()`](#timeit.Timer.timeit) repeatedly
so that the total time >= *Timer.target_time* seconds, returning the eventual
(number of loops, time taken for that number of loops). It calls
[`timeit()`](#timeit.Timer.timeit) with increasing numbers from the sequence 1, 2, 5,
...
#### Versionchanged
Changed in version 3.15: The optional *target_time* parameter was added.

#### repeat(repeat=5, number=1000000)

Call [`timeit()`](#timeit.Timer.timeit) a few times.

This is a convenience function that calls the [`timeit()`](#timeit.Timer.timeit) repeatedly,
returning a list of results.  The first argument specifies how many times
to call [`timeit()`](#timeit.Timer.timeit).  The second argument specifies the *number*
argument for [`timeit()`](#timeit.Timer.timeit).
...
So the [`min()`](functions.md#min) of the result is probably the only number you
...
where the traceback is sent; it defaults to [`sys.stderr`](sys.md#sys.stderr).

<a id="timeit-command-line-interface"></a>
...
measure process time, not wallclock time, using [`time.process_time()`](time.md#time.process_time)
instead of [`time.perf_counter()`](time.md#time.perf_counter), which is the default
...
if [`--number`](#cmdoption-timeit-n) is 0, the code will run until it takes at
...
time is at least [`--target-time`](#cmdoption-timeit-t) seconds (default: 0.2).

[`default_timer()`](#timeit.default_timer) measurements can be affected by other programs running on
...
most cases.  You can use [`time.process_time()`](time.md#time.process_time) to measure CPU time.
...
The same can be done using the [`Timer`](#timeit.Timer) class and its methods:
...
Here we compare the cost of using [`hasattr()`](functions.md#hasattr) vs. [`try`](../reference/compound_stmts.md#try)/[`except`](../reference/compound_stmts.md#except)
...
Another option is to pass [`globals()`](functions.md#globals) to the  *globals* parameter, which will cause the code
