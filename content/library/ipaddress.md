...
**Source code:** [Lib/ipaddress.py](https://github.com/python/cpython/tree/main/Lib/ipaddress.py)
...
[An introduction to the ipaddress module](../howto/ipaddress.md#ipaddress-howto).
...
Return an [`IPv4Address`](#ipaddress.IPv4Address) or [`IPv6Address`](#ipaddress.IPv6Address) object depending on
the IP address passed as argument.  Either IPv4 or IPv6 addresses may be
supplied; integers less than `2**32` will be considered to be IPv4 by default.
A [`ValueError`](exceptions.md#ValueError) is raised if *address* does not represent a valid IPv4
or IPv6 address.
...
Return an [`IPv4Network`](#ipaddress.IPv4Network) or [`IPv6Network`](#ipaddress.IPv6Network) object depending on
the IP address passed as argument.  *address* is a string or integer
representing the IP network.  Either IPv4 or IPv6 networks may be supplied;
integers less than `2**32` will be considered to be IPv4 by default.  *strict*
is passed to [`IPv4Network`](#ipaddress.IPv4Network) or [`IPv6Network`](#ipaddress.IPv6Network) constructor.  A
[`ValueError`](exceptions.md#ValueError) is raised if *address* does not represent a valid IPv4 or
...
Return an [`IPv4Interface`](#ipaddress.IPv4Interface) or [`IPv6Interface`](#ipaddress.IPv6Interface) object depending
...
The [`IPv4Address`](#ipaddress.IPv4Address) and [`IPv6Address`](#ipaddress.IPv6Address) objects share a lot of common
attributes.  Some attributes that are only meaningful for IPv6 addresses are
also implemented by [`IPv4Address`](#ipaddress.IPv4Address) objects, in order to make it easier to
write code that handles both IP versions correctly.  Address objects are
[hashable](../glossary.md#term-hashable), so they can be used as keys in dictionaries.

### *class* ipaddress.IPv4Address(address)

Construct an IPv4 address.  An [`AddressValueError`](#ipaddress.AddressValueError) is raised if
...
3. An integer packed into a [`bytes`](stdtypes.md#bytes) object of length 4 (most
...
The binary representation of this address - a [`bytes`](stdtypes.md#bytes) object of
...
* For IPv4-mapped IPv6-addresses the `is_private` value is determined by the
  ...
  (see [`IPv6Address.ipv4_mapped`](#ipaddress.IPv6Address.ipv4_mapped)):
  ```python3
  address.is_private == address.ipv4_mapped.is_private
  ```

`is_private` has value opposite to [`is_global`](#ipaddress.IPv4Address.is_global), except for the shared address space
...
`is_global` has value opposite to [`is_private`](#ipaddress.IPv4Address.is_private), except for the shared address space
(`100.64.0.0/10` range) where they are both `False`.
...
#### Versionchanged
Changed in version 3.13: Fixed some false positives and false negatives, see [`is_private`](#ipaddress.IPv4Address.is_private) for details.
...
For IPv6, this is all addresses [allocated](https://www.iana.org/assignments/ipv6-address-space/ipv6-address-space.xhtml) as
`Reserved by IETF` for future use.

#### NOTE
For IPv4, `is_reserved` is not related to the address block value of the
`Reserved-by-Protocol` column in [iana-ipv4-special-registry](https://www.iana.org/assignments/iana-ipv4-special-registry/iana-ipv4-special-registry.xhtml).
...
[`IPv4Address`](#ipaddress.IPv4Address) object representing the IPv4-mapped IPv6 address. See [**RFC 4291**](https://datatracker.ietf.org/doc/html/rfc4291.html).

#### Versionadded
Added in version 3.13.

#### IPv4Address.\_\_format_\_(fmt)
...
equivalent to [`str()`](stdtypes.md#str), `'b'` for a zero-padded binary string,
...
Construct an IPv6 address.  An [`AddressValueError`](#ipaddress.AddressValueError) is raised if
...
1. A string consisting of eight groups of four hexadecimal digits, each
   ...
   [**RFC 4291**](https://datatracker.ietf.org/doc/html/rfc4291.html) for details.  For example,
   ...
   and may not contain `%`.
   See [**RFC 4007**](https://datatracker.ietf.org/doc/html/rfc4007.html) for details.
   For example, `fe80::1234%1` might identify address `fe80::1234` on the first link of the node.
2. An integer that fits into 128 bits.
3. An integer packed into a [`bytes`](stdtypes.md#bytes) object of length 16, big-endian.
...
documentation of the [`IPv4Address`](#ipaddress.IPv4Address) class:
...
[`is_private`](#ipaddress.IPv4Address.is_private) to test if this address is in the
...
the [`str()`](stdtypes.md#str) and [`int()`](functions.md#int) builtin functions:
...
The [`IPv4Network`](#ipaddress.IPv4Network) and [`IPv6Network`](#ipaddress.IPv6Network) objects provide a mechanism
...
All of these are common between [`IPv4Network`](#ipaddress.IPv4Network) and [`IPv6Network`](#ipaddress.IPv6Network),
so to avoid duplication they are only documented for [`IPv4Network`](#ipaddress.IPv4Network).
Network objects are [hashable](../glossary.md#term-hashable), so they can be used as keys in
dictionaries.
...
2. An integer that fits into 32 bits.  This is equivalent to a
   ...
   the mask being `/32`.
3. An integer packed into a [`bytes`](stdtypes.md#bytes) object of length 4, big-endian.
   The interpretation is similar to an integer *address*.
4. A two-tuple of an address description and a netmask, where the address
   description is either a string, a 32-bits integer, a 4-bytes packed
   integer, or an existing [`IPv4Address`](#ipaddress.IPv4Address) object; and the netmask is either
   an integer representing the prefix length (e.g. `24`) or a string
   representing the prefix mask (e.g. `255.255.255.0`).

An [`AddressValueError`](#ipaddress.AddressValueError) is raised if *address* is not a valid IPv4
address.  A [`NetmaskValueError`](#ipaddress.NetmaskValueError) is raised if the mask is not valid for
an IPv4 address.

If *strict* is `True` and host bits are set in the supplied address,
then [`ValueError`](exceptions.md#ValueError) is raised.  Otherwise, the host bits are masked out
to determine the appropriate network address.

Unless stated otherwise, all network methods accepting other network/address
objects will raise [`TypeError`](exceptions.md#TypeError) if the argument’s IP version is
...
The host mask, as an [`IPv4Address`](#ipaddress.IPv4Address) object.

#### netmask

The net mask, as an [`IPv4Address`](#ipaddress.IPv4Address) object.
...
Raises [`ValueError`](exceptions.md#ValueError) if *network* is not completely contained in
this network.
...
4. A two-tuple of an address description and a netmask, where the address
   ...
   integer, or an existing [`IPv6Address`](#ipaddress.IPv6Address) object; and the netmask is an
   integer representing the prefix length.

An [`AddressValueError`](#ipaddress.AddressValueError) is raised if *address* is not a valid IPv6
address.  A [`NetmaskValueError`](#ipaddress.NetmaskValueError) is raised if the mask is not valid for
an IPv6 address.
...
(for usable hosts, use the [`hosts()`](#ipaddress.IPv4Network.hosts) method).  An
example:
...
Interface objects are [hashable](../glossary.md#term-hashable), so they can be used as keys in
dictionaries.

### *class* ipaddress.IPv4Interface(address)

Construct an IPv4 interface.  The meaning of *address* is as in the
constructor of [`IPv4Network`](#ipaddress.IPv4Network), except that arbitrary host addresses
are always accepted.

[`IPv4Interface`](#ipaddress.IPv4Interface) is a subclass of [`IPv4Address`](#ipaddress.IPv4Address), so it inherits
all the attributes from that class.  In addition, the following attributes
are available:

#### ip

The address ([`IPv4Address`](#ipaddress.IPv4Address)) without network information.
...
The network ([`IPv4Network`](#ipaddress.IPv4Network)) this interface belongs to.
...
constructor of [`IPv6Network`](#ipaddress.IPv6Network), except that arbitrary host addresses
are always accepted.

[`IPv6Interface`](#ipaddress.IPv6Interface) is a subclass of [`IPv6Address`](#ipaddress.IPv6Address), so it inherits
all the attributes from that class.  In addition, the following attributes
are available:
...
[`ValueError`](exceptions.md#ValueError) is raised if the integer is negative or too large to be an
IPv4 IP address.
...
IP addresses.  *first* is the first [`IPv4Address`](#ipaddress.IPv4Address) or
[`IPv6Address`](#ipaddress.IPv6Address) in the range and *last* is the last [`IPv4Address`](#ipaddress.IPv4Address)
or [`IPv6Address`](#ipaddress.IPv6Address) in the range.  A [`TypeError`](exceptions.md#TypeError) is raised if
*first* or *last* are not IP addresses or are not of the same version.  A
[`ValueError`](exceptions.md#ValueError) is raised if *last* is not greater than *first* or if
...
Return an iterator of the collapsed [`IPv4Network`](#ipaddress.IPv4Network) or
[`IPv6Network`](#ipaddress.IPv6Network) objects.  *addresses* is an [iterable](../glossary.md#term-iterable) of
[`IPv4Network`](#ipaddress.IPv4Network) or [`IPv6Network`](#ipaddress.IPv6Network) objects.  A [`TypeError`](exceptions.md#TypeError) is
...
this function as the *key* argument to [`sorted()`](functions.md#sorted).
