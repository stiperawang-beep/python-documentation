...
**Source code:** [Lib/email/policy.py](https://github.com/python/cpython/tree/main/Lib/email/policy.py)

---

The [`email`](email.md#module-email) package’s prime focus is the handling of email messages as
...
A [`Policy`](#email.policy.Policy) object encapsulates a set of attributes and methods that
control the behavior of various components of the email package during use.
[`Policy`](#email.policy.Policy) instances can be passed to various classes and methods in the
email package to alter the default behavior.  The settable values and their
defaults are described below.

There is a default policy used by all classes in the email package.  For all of
the [`parser`](email.parser.md#module-email.parser) classes and the related convenience functions, and for
the [`Message`](email.compat32-message.md#email.message.Message) class, this is the [`Compat32`](#email.policy.Compat32)
policy, via its corresponding pre-defined instance [`compat32`](#email.policy.compat32).  This
...
[`EmailMessage`](email.message.md#email.message.EmailMessage) is the [`EmailPolicy`](#email.policy.EmailPolicy) policy, via
its pre-defined instance [`default`](#email.policy.default).

When a [`Message`](email.compat32-message.md#email.message.Message) or [`EmailMessage`](email.message.md#email.message.EmailMessage)
object is created, it acquires a policy.  If the message is created by a
[`parser`](email.parser.md#module-email.parser), a policy passed to the parser will be the policy used by
...
[`generator`](email.generator.md#module-email.generator), the generator uses the policy from the message by
default, but you can also pass a specific policy to the generator that will
override the one stored on the message object.

The default value for the *policy* keyword for the [`email.parser`](email.parser.md#module-email.parser) classes
...
[`parser`](email.parser.md#module-email.parser) module.

The first part of this documentation covers the features of [`Policy`](#email.policy.Policy), an
[abstract base class](../glossary.md#term-abstract-base-class) that defines the features that are common to all
policy objects, including [`compat32`](#email.policy.compat32).  This includes certain hook
...
concrete classes [`EmailPolicy`](#email.policy.EmailPolicy) and [`Compat32`](#email.policy.Compat32), which implement
the hooks that provide the standard behavior and the backward compatible
behavior and features, respectively.

[`Policy`](#email.policy.Policy) instances are immutable, but they can be cloned, accepting the
same keyword arguments as the class constructor and returning a new
[`Policy`](#email.policy.Policy) instance that is a copy of the original but with the specified
...
Here we are telling [`BytesGenerator`](email.generator.md#email.generator.BytesGenerator) to use the RFC
...
the [`as_bytes()`](email.compat32-message.md#email.message.Message.as_bytes) method of the *msg* object from
...
This is the [abstract base class](../glossary.md#term-abstract-base-class) for all policy classes.  It provides
default implementations for a couple of trivial methods, as well as the
implementation of the immutability property, the [`clone()`](#email.policy.Policy.clone) method, and
...
`0` or [`None`](constants.md#None) indicates that no line wrapping should be
done at all.
...
| `7bit`   | all data must be “7 bit clean” (ASCII-only).  This means that<br/>where necessary data will be encoded using either<br/>quoted-printable or base64 encoding.                                                                                                                                          |
|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `8bit`   | data is not constrained to be 7 bit clean.  Data in headers is<br/>still required to be ASCII-only and so will be encoded (see<br/>[`fold_binary()`](#email.policy.Policy.fold_binary) and [`utf8`](#email.policy.EmailPolicy.utf8) below for<br/>exceptions), but body parts may use the `8bit` CTE. |
...
If [`True`](constants.md#True), any defects encountered will be raised as errors.  If
[`False`](constants.md#False) (the default), defects will be passed to the
[`register_defect()`](#email.policy.Policy.register_defect) method.

#### mangle_from_

If [`True`](constants.md#True), lines starting with  *“From “* in the body are
escaped by putting a `>` in front of them. This parameter is used when
the message is being serialized by a generator.
Default: [`False`](constants.md#False).
...
which case [`Message`](email.compat32-message.md#email.message.Message) is used.
...
[`HeaderWriteError`](email.errors.md#email.errors.HeaderWriteError) instead of writing a header
...
[`Compat32`](#email.policy.Compat32) policy.
...
The following [`Policy`](#email.policy.Policy) method is intended to be called by code using
the email library to create policy instances with custom settings:

#### clone(\*\*kw)

Return a new [`Policy`](#email.policy.Policy) instance whose attributes have the same
values as the current instance, except where those attributes are
given new values by the keyword arguments.

The remaining [`Policy`](#email.policy.Policy) methods are called by the email package code,
...
[`MessageDefect`](email.errors.md#email.errors.MessageDefect).

The default implementation checks the [`raise_on_defect`](#email.policy.Policy.raise_on_defect) flag.  If
it is `True`, *defect* is raised as an exception.  If it is `False`
(the default), *obj* and *defect* are passed to [`register_defect()`](#email.policy.Policy.register_defect).

#### register_defect(obj, defect)

Register a *defect* on *obj*.  In the email package, *defect* will always
be a subclass of [`MessageDefect`](email.errors.md#email.errors.MessageDefect).

The default implementation calls the `append` method of the `defects`
attribute of *obj*.  When the email package calls [`handle_defect`](#email.policy.Policy.handle_defect),
...
Called when a header is added to an [`EmailMessage`](email.message.md#email.message.EmailMessage)
or [`Message`](email.compat32-message.md#email.message.Message) object.  If the returned value is not
...
[`linesep`](#email.policy.Policy.linesep) characters at the appropriate places.  See [**RFC 5322**](https://datatracker.ietf.org/doc/html/rfc5322.html)
...
The same as [`fold()`](#email.policy.Policy.fold), except that the returned value should be a
...
This concrete [`Policy`](#email.policy.Policy) provides behavior that is intended to be fully
...
The default value for the [`message_factory`](#email.policy.Policy.message_factory)
attribute is [`EmailMessage`](email.message.md#email.message.EmailMessage).

In addition to the settable attributes listed above that apply to all
policies, this policy adds the following additional attributes:

#### Versionadded
Added in version 3.6: <sup>[1](#id2)</sup>
...
[`parser`](email.parser.md#module-email.parser) (as opposed to being set by a program), this
...
|----------|---------------------------------------------------------------------------------------------|
| `long`   | source values that have any line that is longer than<br/>`max_line_length` will be refolded |
...
default `header_factory` (see [`headerregistry`](email.headerregistry.md#module-email.headerregistry)) is provided
...
the [`get_content()`](email.message.md#email.message.EmailMessage.get_content) or
[`set_content()`](email.message.md#email.message.EmailMessage.set_content) method of an
[`EmailMessage`](email.message.md#email.message.EmailMessage) object is called, it calls the
...
methods of [`Policy`](#email.policy.Policy):

#### header_max_count(name)

Returns the value of the
[`max_count`](email.headerregistry.md#email.headerregistry.BaseHeader.max_count) attribute of the
...
Header folding is controlled by the [`refold_source`](#email.policy.EmailPolicy.refold_source) policy setting.
...
Source values are split into lines using [`splitlines()`](stdtypes.md#str.splitlines).  If
...
The same as [`fold()`](#email.policy.EmailPolicy.fold) if [`cte_type`](#email.policy.Policy.cte_type) is `7bit`, except
that the returned value is bytes.

If [`cte_type`](#email.policy.Policy.cte_type) is `8bit`, non-ASCII binary data is
converted back
...
The following instances of [`EmailPolicy`](#email.policy.EmailPolicy) provide defaults suitable for
...
The same as `SMTP` except that [`utf8`](#email.policy.EmailPolicy.utf8) is `True`.
Useful for serializing messages to a message store without using encoded
words in the headers.  Should only be used for SMTP transmission if the
sender or recipient addresses have non-ASCII characters (the
[`smtplib.SMTP.send_message()`](smtplib.md#smtplib.SMTP.send_message) method handles this automatically).
...
With all of these [`EmailPolicies`](#email.policy.EmailPolicy), the effective API of
the email package is changed from the Python 3.2 API in the following ways:

* Setting a header on a [`Message`](email.compat32-message.md#email.message.Message) results in that
  header being parsed and a header object created.
* Fetching a header value from a [`Message`](email.compat32-message.md#email.message.Message) results
...
[`EmailMessage`](email.message.md#email.message.EmailMessage) is a header object with extra
...
[`headerregistry`](email.headerregistry.md#module-email.headerregistry).

### *class* email.policy.Compat32(\*\*kw)

This concrete [`Policy`](#email.policy.Policy) is the backward compatibility policy.  It
replicates the behavior of the email package in Python 3.2.  The
`policy` module also defines an instance of this class,
[`compat32`](#email.policy.compat32), that is used as the default policy.  Thus the default
behavior of the email package is to maintain compatibility with Python 3.2.

The following attributes have values that are different from the
[`Policy`](#email.policy.Policy) default:
...
abstract methods of [`Policy`](#email.policy.Policy):
...
[`Header`](email.header.md#email.header.Header) object using the `unknown-8bit` charset.
Otherwise it is returned unmodified.

#### fold(name, value)

Headers are folded using the [`Header`](email.header.md#email.header.Header) folding
...
An instance of [`Compat32`](#email.policy.Compat32), providing  backward compatibility with the
behavior of the email package in Python 3.2.

#### NOTE
The [`compat32`](#email.policy.compat32) policy should not be used as a policy for
[`EmailMessage`](email.message.md#email.message.EmailMessage) objects, and should only be used
to serialize messages that were created using the [`compat32`](#email.policy.compat32)
policy.

### Footnotes

* <a id='id2'>**[1]**</a> Originally added in 3.3 as a [provisional feature](../glossary.md#term-provisional-package).
