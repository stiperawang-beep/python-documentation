...
**Source code:** [Lib/bisect.py](https://github.com/python/cpython/tree/main/Lib/bisect.py)
...
insertion point. Accordingly, the functions never call an [`__eq__()`](../reference/datamodel.md#object.__eq__)
method to determine whether a value has been found.  Instead, the
functions only call the [`__lt__()`](../reference/datamodel.md#object.__lt__) method and will return an insertion
...
[`insort_left()`](#bisect.insort_left) on the same list from multiple threads
may result in the list becoming unsorted.

<a id="bisect-functions"></a>
...
Similar to [`bisect_left()`](#bisect.bisect_left), but returns an insertion point which comes
...
This function first runs [`bisect_left()`](#bisect.bisect_left) to locate an insertion point.
Next, it runs the [`insert()`](stdtypes.md#sequence.insert) method on *a* to insert *x* at the
...
Similar to [`insort_left()`](#bisect.insort_left), but inserting *x* in *a* after any existing
entries of *x*.

This function first runs [`bisect_right()`](#bisect.bisect_right) to locate an insertion point.
Next, it runs the [`insert()`](stdtypes.md#sequence.insert) method on *a* to insert *x* at the
appropriate position to maintain sort order.
...
* The search functions are stateless and discard key function results after
  ...
  [`functools.cache()`](functools.md#functools.cache) to avoid duplicate computations.  Alternatively,
...
<a id="bisect-example"></a>

The [`bisect()`](#bisect.bisect) function can be useful for numeric table lookups. This
example uses [`bisect()`](#bisect.bisect) to look up a letter grade for an exam score (say)
...
The [`bisect()`](#bisect.bisect) and [`insort()`](#bisect.insort) functions also work with
