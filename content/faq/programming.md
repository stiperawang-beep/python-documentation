...
[`breakpoint()`](../library/functions.md#breakpoint) allows you to drop into any of them.

The pdb module is a simple but adequate console-mode debugger for Python. It is
part of the standard Python library, and is [`documented in the Library
Reference Manual`](../library/pdb.md#module-pdb). You can also write your own debugger by using the code
for pdb as an example.

The IDLE interactive development environment, which is part of the standard
Python distribution (normally available as [`idlelib`](../library/idle.md#module-idlelib)),
...
[Pylint](https://pylint.readthedocs.io/) and
...
<a id="faq-create-standalone-binary"></a>
...
<a id="faq-unboundlocalerror"></a>

### Why am I getting an UnboundLocalError when the variable has a value?

It can be a surprise to get the [`UnboundLocalError`](../library/exceptions.md#UnboundLocalError) in previously working
...
You can do a similar thing in a nested scope using the [`nonlocal`](../reference/simple_stmts.md#nonlocal)
keyword:
...
one hand, requiring [`global`](../reference/simple_stmts.md#global) for assigned variables provides a bar
...
1. standard library modules – such as [`sys`](../library/sys.md#module-sys), [`os`](../library/os.md#module-os), [`argparse`](../library/argparse.md#module-argparse), [`re`](../library/re.md#module-re)
2. third-party library modules (anything installed in Python’s site-packages
   directory) – such as [dateutil](https://pypi.org/project/dateutil/), [requests](https://pypi.org/project/requests/), [tzdata](https://pypi.org/project/tzdata/)
...
the module is probably available in [`sys.modules`](../library/sys.md#sys.modules).
...
<a id="faq-argument-vs-parameter"></a>

### What is the difference between arguments and parameters?

[Parameters](../glossary.md#term-parameter) are defined by the names that appear in a
function definition, whereas [arguments](../glossary.md#term-argument) are the values
actually passed to a function when calling it.  Parameters define what
[kind of arguments](../glossary.md#term-parameter) a function can accept.  For
...
2. Lists are [mutable](../glossary.md#term-mutable), which means that you can change their content.

After the call to [`append()`](../library/stdtypes.md#sequence.append), the content of the mutable object has
...
because integers are [immutable](../glossary.md#term-immutable), and when we do `x = x + 1` we are not
...
and [`sorted(y)`](../library/functions.md#sorted)) create a new object.  In general in Python (and in all cases
...
* If we have a mutable object (such as [`list`](../library/stdtypes.md#list), [`dict`](../library/stdtypes.md#dict), [`set`](../library/stdtypes.md#set)),
  we can use some specific operations to mutate it and all the variables
  that refer to it will see the change.
* If we have an immutable object (such as [`str`](../library/stdtypes.md#str), [`int`](../library/functions.md#int), [`tuple`](../library/stdtypes.md#tuple)),
...
use the [`is`](../reference/expressions.md#is) operator, or the built-in function [`id()`](../library/functions.md#id).
...
In general, try [`copy.copy()`](../library/copy.md#copy.copy) or [`copy.deepcopy()`](../library/copy.md#copy.deepcopy) for the general case.
Not all objects can be copied, but most can.

Some objects can be copied more easily.  Dictionaries have a [`copy()`](../library/stdtypes.md#dict.copy)
method:
...
For an instance `x` of a user-defined class, [`dir(x)`](../library/functions.md#dir) returns an alphabetized
...
Yes.  Usually this is done by nesting [`lambda`](../reference/expressions.md#lambda) within
...
<a id="faq-positional-only-arguments"></a>
...
For example, [`divmod()`](../library/functions.md#divmod) is a function that accepts positional-only
...
positional-only. Thus, calling [`divmod()`](../library/functions.md#divmod) with keyword arguments would lead
to an error:
...
a [`SyntaxError`](../library/exceptions.md#SyntaxError) because the period is seen as a decimal point:
...
For integers, use the built-in [`int()`](../library/functions.md#int) type constructor, for example, `int('144')
== 144`.  Similarly, [`float()`](../library/functions.md#float) converts to a floating-point number,
for example, `float('144') == 144.0`.

By default, these interpret the number as decimal, so that `int('0144') ==
144` holds true, and `int('0x144')` raises [`ValueError`](../library/exceptions.md#ValueError). `int(string,
...
Do not use the built-in function [`eval()`](../library/functions.md#eval) if all you need is to convert
strings to numbers.  [`eval()`](../library/functions.md#eval) will be significantly slower and it presents a
...
[`eval()`](../library/functions.md#eval) also has the effect of interpreting numbers as Python expressions,
...
constructor [`str()`](../library/stdtypes.md#str).  If you want a hexadecimal or octal representation, use
the built-in functions [`hex()`](../library/functions.md#hex) or [`oct()`](../library/functions.md#oct).  For fancy formatting, see
the [f-strings](../reference/lexical_analysis.md#f-strings) and [Format string syntax](../library/string.md#formatstrings) sections.
...
Unicode data, try using an [`io.StringIO`](../library/io.md#io.StringIO) object or the [`array`](../library/array.md#module-array)
module:
...
* The best is to use a dictionary that maps strings to functions.  The primary
  ...
  ```
* Use the built-in function [`getattr()`](../library/functions.md#getattr):
  ```python3
  import foo
  getattr(foo, 'bar')()
  ```

  Note that [`getattr()`](../library/functions.md#getattr) works on any object, including classes, class
  ...
  f()
  ```
* Use [`locals()`](../library/functions.md#locals) to resolve the function name:
  ```python3
  def myFunc():
      print("hello")
...
whitespace-delimited words using the [`split()`](../library/stdtypes.md#str.split) method of string objects
and then convert decimal strings to numeric values using [`int()`](../library/functions.md#int) or
[`float()`](../library/functions.md#float).  `split()` supports an optional “sep” parameter which is useful
...
See the [Unicode HOWTO](../howto/unicode.md#unicode-howto).

<a id="faq-programming-raw-string-backslash"></a>
...
It is also possible to use [`os.path.join()`](../library/os.path.md#os.path.join) to append a backslash on Windows:
...
Also see the specification in the [language reference](../reference/lexical_analysis.md#strings).
...
* Performance characteristics vary across Python implementations.  This FAQ
  focuses on [CPython](../glossary.md#term-CPython).
* Behaviour can vary across operating systems, especially when talking about
  I/O or multi-threading.
* You should always find the hot spots in your program *before* attempting to
  optimize any code (see the [`profile`](../library/profile.md#module-profile) module).
* Writing benchmark scripts will allow you to iterate quickly when searching
  for improvements (see the [`timeit`](../library/timeit.md#module-timeit) module).
...
* Making your algorithms faster (or changing to faster ones) can yield
  ...
  all over your code.
* Use the right data structures.  Study documentation for the [Built-in Types](../library/stdtypes.md#bltin-types)
  and the [`collections`](../library/collections.md#module-collections) module.
* When the standard library provides a primitive for doing something, it is
  ...
  either the [`list.sort()`](../library/stdtypes.md#list.sort) built-in method or the related [`sorted()`](../library/functions.md#sorted)
  function to do sorting (and see the [Sorting Techniques](../howto/sorting.md#sortinghowto) for examples
...
skills, you can also [write a C extension module](../extending/index.md#extending-index)
yourself.

#### SEE ALSO
The wiki page devoted to [performance tips](https://wiki.python.org/moin/PythonSpeed/PerformanceTips).

<a id="efficient-string-concatenation"></a>

### What is the most efficient way to concatenate many strings together?

[`str`](../library/stdtypes.md#str) and [`bytes`](../library/stdtypes.md#bytes) objects are immutable, therefore concatenating
...
To accumulate many [`str`](../library/stdtypes.md#str) objects, the recommended idiom is to place
them into a list and call [`str.join()`](../library/stdtypes.md#str.join) at the end:
...
(Another reasonably efficient idiom is to use [`io.StringIO`](../library/io.md#io.StringIO).)

To accumulate many [`bytes`](../library/stdtypes.md#bytes) objects, the recommended idiom is to extend
a [`bytearray`](../library/stdtypes.md#bytearray) object using in-place concatenation (the `+=` operator):
...
but returns the same object, so it is cheap to call [`tuple()`](../library/stdtypes.md#tuple) when you
...
Use the [`reversed()`](../library/functions.md#reversed) built-in function:
...
[hashable](../glossary.md#term-hashable)) this is often faster:
...
<a id="faq-multidimensional-list"></a>
...
a [list comprehension](../glossary.md#term-list-comprehension) is an elegant solution:
...
a plain [`for`](../reference/compound_stmts.md#for) loop will suffice:
...
<a id="faq-augmented-assignment-tuple-error"></a>
...
[`__iadd__()`](../reference/datamodel.md#object.__iadd__) magic method, it gets called when the `+=` augmented
assignment
is executed, and its return value is what gets used in the assignment statement;
and (b) for lists, `__iadd__()` is equivalent to calling
[`extend()`](../library/stdtypes.md#sequence.extend) on the list and returning the list.
That’s why we say that for lists, `+=` is a “shorthand” for [`list.extend()`](../library/stdtypes.md#list.extend):
...
Python, use the `key` argument for the [`list.sort()`](../library/stdtypes.md#list.sort) method:
...
See also [Why must ‘self’ be used explicitly in method definitions and calls?](design.md#why-self).

### How do I check if an object is an instance of a given class or of a subclass of it?

Use the built-in function [`isinstance(obj, cls)`](../library/functions.md#isinstance).  You can
check if an object
...
Note that [`isinstance()`](../library/functions.md#isinstance) also checks for virtual inheritance from an
[abstract base class](../glossary.md#term-abstract-base-class).  So, the test will return `True` for a
registered class even if hasn’t directly or indirectly inherited from it.  To
test for “true inheritance”, scan the [method resolution order](../glossary.md#term-method-resolution-order) (MRO) of
the class:
...
Note that most programs do not use [`isinstance()`](../library/functions.md#isinstance) on user-defined classes
...
[`__getattr__()`](../reference/datamodel.md#object.__getattr__) method; consult [the language reference](../reference/datamodel.md#attribute-access)
for more information about controlling attribute access.

Note that for more general cases delegation can get trickier. When attributes
must be set as well as retrieved, the class must define a [`__setattr__()`](../reference/datamodel.md#object.__setattr__)
...
Many [`__setattr__()`](../reference/datamodel.md#object.__setattr__) implementations call `object.__setattr__()` to set
...
entries into [`self.__dict__`](../reference/datamodel.md#object.__dict__) directly.

### How do I call a method defined in a base class from a derived class that extends it?

Use the built-in [`super()`](../library/functions.md#super) function:
...
In the example, [`super()`](../library/functions.md#super) will automatically determine the instance from
which it was called (the `self` value), look up the [method resolution
order](../glossary.md#term-method-resolution-order) (MRO) with `type(self).__mro__`, and return the next in line after
...
### I try to use \_\_spam and I get an error about \_SomeClassName_\_spam.
...
#### SEE ALSO
The [private name mangling specifications](../reference/expressions.md#private-name-mangling)
for details and special cases.

### My class defines \_\_del_\_ but it is not called when I delete the object.

There are several possible reasons for this.

The [`del`](../reference/simple_stmts.md#del) statement does not necessarily call [`__del__()`](../reference/datamodel.md#object.__del__) – it simply
...
methods are executed is arbitrary.  You can run [`gc.collect()`](../library/gc.md#gc.collect) to force a
...
Another way to avoid cyclical references is to use the [`weakref`](../library/weakref.md#module-weakref) module,
...
is printed to [`sys.stderr`](../library/sys.md#sys.stderr).
...
The [`id()`](../library/functions.md#id) builtin returns an integer that is guaranteed to be unique during
...
constants such as [`int`](../library/functions.md#int) and [`str`](../library/stdtypes.md#str) which aren’t guaranteed to be
singletons:
...
2. Detecting optional arguments can be tricky when `None` is a valid input
   ...
   to implement a method that behaves like [`dict.pop()`](../library/stdtypes.md#dict.pop):
   ```python
   _sentinel = object()
...
When subclassing an immutable type, override the [`__new__()`](../reference/datamodel.md#object.__new__) method
instead of the [`__init__()`](../reference/datamodel.md#object.__init__) method.  The latter only runs *after* an
...
<a id="faq-cache-method-calls"></a>

### How do I cache method calls?

The two principal tools for caching methods are
[`functools.cached_property()`](../library/functools.md#functools.cached_property) and [`functools.lru_cache()`](../library/functools.md#functools.lru_cache).  The
...
The `lru_cache` approach works with methods that have [hashable](../glossary.md#term-hashable)
...
the class needs to define the [`__eq__()`](../reference/datamodel.md#object.__eq__) and [`__hash__()`](../reference/datamodel.md#object.__hash__)
...
Unless the [`PYTHONDONTWRITEBYTECODE`](../using/cmdline.md#envvar-PYTHONDONTWRITEBYTECODE) environment variable is set,
...
[`py_compile`](../library/py_compile.md#module-py_compile) and [`compileall`](../library/compileall.md#module-compileall) modules.

The [`py_compile`](../library/py_compile.md#module-py_compile) module can manually compile any module.  One way is to use
...
the [`compileall`](../library/compileall.md#module-compileall) module.  You can do it from the shell prompt by running
...
### \_\_import_\_(‘x.y.z’) returns <module ‘x’>; how do I get z?

Consider using the convenience function [`import_module()`](../library/importlib.md#importlib.import_module) from
[`importlib`](../library/importlib.md#module-importlib) instead:
