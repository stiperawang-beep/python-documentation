...
The [`float`](../library/functions.md#float) type in CPython uses a C `double` for storage.  A
[`float`](../library/functions.md#float) object’s value is stored in binary floating-point with a fixed
...
For a fuller explanation, please see the [floating-point arithmetic](../tutorial/floatingpoint.md#tut-fp-issues) chapter in the Python tutorial.
...
for [`__init__()`](../reference/datamodel.md#object.__init__) methods, and in general in cases where a derived class
...
<a id="why-can-t-i-use-an-assignment-in-an-expression"></a>
...
reason there seems to be much less difficulty with having [`split()`](../library/stdtypes.md#str.split) as
...
[`join()`](../library/stdtypes.md#str.join) is a string method because in using it you are telling the
...
A [`try`](../reference/compound_stmts.md#try)/[`except`](../reference/compound_stmts.md#except) block is extremely efficient if no exceptions
are raised.  Actually
...
[`getattr()`](../library/functions.md#getattr) built-in to retrieve methods with a particular name:
...
standard implementation of Python, [CPython](../glossary.md#term-CPython), uses reference counting to
...
cycles and deletes the objects involved. The [`gc`](../library/gc.md#module-gc) module provides functions
...
you should explicitly close the file or use the [`with`](../reference/compound_stmts.md#with) statement;
...
[`atexit`](../library/atexit.md#module-atexit) module to run a function that will force those deletions.
...
operated on one-by-one.  For example, [`os.listdir('.')`](../library/os.md#os.listdir)
returns a list of
strings representing the files in the current directory.  Functions which
operate on this output would generally not break if you added another file or
two to the directory.

Tuples are [immutable](../glossary.md#term-immutable), meaning that once a tuple has been created, you can’t
replace any of its elements with a new value.  Lists are [mutable](../glossary.md#term-mutable), meaning that
you can always change a list’s elements.  Only [hashable](../glossary.md#term-hashable) objects can
...
using the [`hash()`](../library/functions.md#hash) built-in function.  The hash code varies widely depending
...
- Hash lists by their address (object ID).  This doesn’t work because if you
  ...
  would raise a [`KeyError`](../library/exceptions.md#KeyError) exception because the id of the `[1, 2]` used in the
  second line differs from that in the first line.  In other words, dictionary
  keys should be compared using `==`, not using [`is`](../reference/expressions.md#is).
...
[`__eq__()`](../reference/datamodel.md#object.__eq__) and a [`__hash__()`](../reference/datamodel.md#object.__hash__) method.
...
it would be wasteful. Therefore, [`list.sort()`](../library/stdtypes.md#list.sort) sorts the list in place. In
order to remind you of that fact, it does not return the sorted list.  This way,
you won’t be fooled into accidentally overwriting a list when you need a sorted
copy but also need to keep the unsorted version around.

If you want to return a new list, use the built-in [`sorted()`](../library/functions.md#sorted) function
...
Python 2.6 adds an [`abc`](../library/abc.md#module-abc) module that lets you define Abstract Base Classes
(ABCs).  You can then use [`isinstance()`](../library/functions.md#isinstance) and [`issubclass()`](../library/functions.md#issubclass) to check
whether an instance or a class implements a particular ABC.  The
[`collections.abc`](../library/collections.abc.md#module-collections.abc) module defines a set of useful ABCs such as
[`Iterable`](../library/collections.abc.md#collections.abc.Iterable), [`Container`](../library/collections.abc.md#collections.abc.Container), and
...
“stub” emulations of the external interface.  The [`doctest`](../library/doctest.md#module-doctest) and
[`unittest`](../library/unittest.md#module-unittest) modules or third-party test frameworks can be used to construct
exhaustive test suites that exercise every line of code in a module.

An appropriate testing discipline can help build large complex applications in
Python as well as having interface specifications would.  In fact, it can be
better because an interface specification cannot test certain properties of a
program.  For example, the [`list.append()`](../library/stdtypes.md#list.append) method is expected to add new elements
to the end of some internal list; an interface specification cannot test that
your [`list.append()`](../library/stdtypes.md#list.append) implementation will actually do this correctly, but it’s
...
are ways to branch (in Python, with [`if`](../reference/compound_stmts.md#if) statements and [`or`](../reference/expressions.md#or),
[`and`](../reference/expressions.md#and), and [`if`](../reference/compound_stmts.md#if)/[`else`](../reference/compound_stmts.md#else) expressions) and loop (with [`while`](../reference/compound_stmts.md#while)
and [`for`](../reference/compound_stmts.md#for) statements, possibly containing [`continue`](../reference/simple_stmts.md#continue) and [`break`](../reference/simple_stmts.md#break)).
...
Python has a [`with`](../reference/compound_stmts.md#with) statement that wraps the execution of a block, calling code
...
will it be used inside the [`with`](../reference/compound_stmts.md#with) block?  As you see, the dynamic nature of Python
makes such choices much harder.

The primary benefit of [`with`](../reference/compound_stmts.md#with) and similar language features (reduction of code
...
[https://mail.python.org/pipermail/python-ideas/2016-May/040070.html](https://mail.python.org/pipermail/python-ideas/2016-May/040070.html)).
...
it as [`contextlib.closing(generator)`](../library/contextlib.md#contextlib.closing)
in the [`with`](../reference/compound_stmts.md#with) statement.
