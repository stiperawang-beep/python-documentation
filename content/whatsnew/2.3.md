...
have been added such as [`sum()`](../library/functions.md#sum) and [`enumerate()`](../library/functions.md#enumerate).  The [`in`](../reference/expressions.md#in)
operator can now be used for substring searches (e.g. `"ab" in "abc"` returns
[`True`](../library/constants.md#True)).
...
The union and intersection of sets can be computed with the [`union()`](../library/stdtypes.md#frozenset.union) and
[`intersection()`](../library/stdtypes.md#frozenset.intersection) methods; an alternative notation uses the bitwise operators
`&` and `|`. Mutable sets also have in-place versions of these methods,
`union_update()` and [`intersection_update()`](../library/stdtypes.md#set.intersection_update).
...
in-place version with the ungainly name [`symmetric_difference_update()`](../library/stdtypes.md#set.symmetric_difference_update).
...
[`yield`](../reference/simple_stmts.md#yield) is now always a keyword.  The rest of this section is a copy of
...
created.  When the function reaches a [`return`](../reference/simple_stmts.md#return) statement, the local
...
A new keyword, [`yield`](../reference/simple_stmts.md#yield), was introduced for generators.  Any function
...
the [`yield`](../reference/simple_stmts.md#yield) statement, the generator outputs the value of `i`,
similar to a [`return`](../reference/simple_stmts.md#return) statement.  The big difference between
...
statement isn’t allowed inside the [`try`](../reference/compound_stmts.md#try) block of a
...
Inside a generator function, the [`return`](../reference/simple_stmts.md#return) statement can only be used
...
generator’s results can also be indicated by raising [`StopIteration`](../library/exceptions.md#StopIteration)
...
0, and having the [`next()`](../library/functions.md#next) method increment `self.count` and return it.
...
“or” is found: 3, 23, 33.  In the [`if`](../reference/compound_stmts.md#if) statement, `i` is first
...
[`DeprecationWarning`](../library/exceptions.md#DeprecationWarning) being signalled by Python 2.3; in 2.4 this will be a
syntax error.
...
The new [`zipimport`](../library/zipimport.md#module-zipimport) module adds support for importing modules from a
...
: Written by James C. Ahlstrom,  who also provided an implementation. Python 2.3
  ...
  [PEP 302: New Import Hooks](#section-pep302) for a description of the new import hooks.
...
[`open()`](../library/functions.md#open) built-in function. If a Unicode string is passed to
[`os.listdir()`](../library/os.md#os.listdir), Python now returns a list of Unicode strings.  A new
...
strings before passing them to the system, which can cause a [`UnicodeError`](../library/exceptions.md#UnicodeError)
to be raised. Applications can test whether arbitrary Unicode strings are
supported as file names by checking [`os.path.supports_unicode_filenames`](../library/os.path.md#os.path.supports_unicode_filenames),
a Boolean value.

Under MacOS, [`os.listdir()`](../library/os.md#os.listdir) may now return Unicode filenames.
...
newlines](../glossary.md#term-universal-newlines) mode.  All three line ending conventions will be translated to a
...
<a id="section-enumerate"></a>

## PEP 279: enumerate()

A new built-in function, [`enumerate()`](../library/functions.md#enumerate), will make certain loops a bit
...
This can be rewritten using [`enumerate()`](../library/functions.md#enumerate) as:
...
A standard package for writing logs, [`logging`](../library/logging.md#module-logging), has been added to Python
...
The [`Logger`](../library/logging.md#logging.Logger) class is the primary class. Most application code will deal
with one or more [`Logger`](../library/logging.md#logging.Logger) objects, each one used by a particular
subsystem of the application. Each [`Logger`](../library/logging.md#logging.Logger) is identified by a name, and
names are organized into a hierarchy using `.`  as the component separator.
For example, you might have [`Logger`](../library/logging.md#logging.Logger) instances named `server`,
...
There’s also a root [`Logger`](../library/logging.md#logging.Logger) that’s the parent of all other loggers.

For simple uses, the [`logging`](../library/logging.md#module-logging) package contains some convenience functions
...
of informational and debugging messages by calling the [`setLevel()`](../library/logging.md#logging.Logger.setLevel) method
on the root logger.

Notice the [`warning()`](../library/logging.md#logging.warning) call’s use of string formatting operators; all of the
functions for logging messages take the arguments `(msg, arg1, arg2, ...)` and
log the string resulting from `msg % (arg1, arg2, ...)`.

There’s also an [`exception()`](../library/logging.md#logging.exception) function that records the most recent
...
`server.auth` is also seen by `server` and `root`, but a [`Logger`](../library/logging.md#logging.Logger)
can prevent this by setting its [`propagate`](../library/logging.md#logging.Logger.propagate) attribute to [`False`](../library/constants.md#False).

There are more classes provided by the [`logging`](../library/logging.md#module-logging) package that can be
customized.  When a [`Logger`](../library/logging.md#logging.Logger) instance is told to log a message, it
creates a [`LogRecord`](../library/logging.md#logging.LogRecord) instance that is sent to any number of different
[`Handler`](../library/logging.md#logging.Handler) instances.  Loggers and handlers can also have an attached list
of filters, and each filter can cause the [`LogRecord`](../library/logging.md#logging.LogRecord) to be ignored or
can modify the record before passing it along.  When they’re finally output,
[`LogRecord`](../library/logging.md#logging.LogRecord) instances are converted to text by a [`Formatter`](../library/logging.md#logging.Formatter)
class.  All of these classes can be replaced by your own specially written
classes.

With all of these features the [`logging`](../library/logging.md#module-logging) package should provide enough
...
`__builtin__` module, [`True`](../library/constants.md#True) and [`False`](../library/constants.md#False).  ([`True`](../library/constants.md#True) and
[`False`](../library/constants.md#False) constants were added to the built-ins in Python 2.2.1, but the
2.2.1 versions are simply set to integer values of 1 and 0 and aren’t a
different type.)

The type object for this new type is named [`bool`](../library/functions.md#bool); the constructor for it
takes any Python value and converts it to [`True`](../library/constants.md#True) or [`False`](../library/constants.md#False).
...
Booleans, and would require that the expression in an [`if`](../reference/compound_stmts.md#if) statement
...
some random object.  The Boolean type is a subclass of the [`int`](../library/functions.md#int) class so
...
To sum up [`True`](../library/constants.md#True) and [`False`](../library/constants.md#False) in a sentence: they’re alternative
ways to spell the integer values 1 and 0, with the single difference that
[`str()`](../library/stdtypes.md#str) and [`repr()`](../library/functions.md#repr) return the strings `'True'` and `'False'`
instead of `'1'` and `'0'`.
...
either “strict” (raising [`UnicodeError`](../library/exceptions.md#UnicodeError)), “ignore” (skipping the
...
error handlers can be added with [`codecs.register_error()`](../library/codecs.md#codecs.register_error), and codecs then
can access the error handler with [`codecs.lookup_error()`](../library/codecs.md#codecs.lookup_error). An equivalent C
...
McMillan’s `iu` module.  Three new items  are added to the [`sys`](../library/sys.md#module-sys)
module:
...
* `sys.path_hooks` is a list of callable objects; most  often they’ll be
  ...
  [`ImportError`](../library/exceptions.md#ImportError) exception if it can’t handle this path.
...
A big ugly regular expression can parse this, but using the new  [`csv`](../library/csv.md#module-csv)
...
The [`reader()`](../library/csv.md#csv.reader) function takes a number of different options. The field
...
[`csv.writer`](../library/csv.md#csv.writer) class will generate comma-separated files from a succession
...
<a id="section-pep307"></a>

## PEP 307: Pickle Enhancements

The [`pickle`](../library/pickle.md#module-pickle) and `cPickle` modules received some attention during the
...
The solution was to invent a new pickle protocol.  The [`pickle.dumps()`](../library/pickle.md#pickle.dumps)
...
new constant, [`pickle.HIGHEST_PROTOCOL`](../library/pickle.md#pickle.HIGHEST_PROTOCOL), can be used to select the
fanciest protocol available.

Unpickling is no longer considered a safe operation.  2.2’s [`pickle`](../library/pickle.md#module-pickle)
...
[`__getstate__()`](../library/pickle.md#object.__getstate__), [`__setstate__()`](../library/pickle.md#object.__setstate__), and [`__getnewargs__()`](../library/pickle.md#object.__getnewargs__).  Consult
...
have never supported this feature, raising a [`TypeError`](../library/exceptions.md#TypeError) if you tried it.
...
One can also now pass slice objects to the [`__getitem__()`](../reference/datamodel.md#object.__getitem__) methods of the
built-in sequences:
...
[`range()`](../library/stdtypes.md#range). `indices()` handles omitted and out-of-bounds indices in a
...
From this example you can also see that the built-in [`slice`](../library/functions.md#slice) object is
now the type object for the slice type, and is no longer a function.  This is
consistent with Python 2.2, where [`int`](../library/functions.md#int), [`str`](../library/stdtypes.md#str), etc., underwent
the same change.
...
* The [`yield`](../reference/simple_stmts.md#yield) statement is now always a keyword, as described in
  section [PEP 255: Simple Generators](#section-generators) of this document.
* A new built-in function [`enumerate()`](../library/functions.md#enumerate) was added, as described in section
  [PEP 279: enumerate()](#section-enumerate) of this document.
* Two new constants, [`True`](../library/constants.md#True) and [`False`](../library/constants.md#False) were added along with the
  built-in [`bool`](../library/functions.md#bool) type, as described in section [PEP 285: A Boolean Type](#section-bool) of this
  document.
* The [`int()`](../library/functions.md#int) type constructor will now return a long integer instead of
  raising an [`OverflowError`](../library/exceptions.md#OverflowError) when a string or floating-point number is too
...
* Built-in types now support the extended slicing syntax, as described in
  section [Extended Slices](#section-slices) of this document.
* A new built-in function, `sum(iterable, start=0)`,  adds up the numeric
  items in the iterable object and returns their sum.  [`sum()`](../library/functions.md#sum) only accepts
...
* Dictionaries have a new method, `pop(key[, *default*])`, that returns
  ...
  returned if it’s specified and [`KeyError`](../library/exceptions.md#KeyError) raised if it isn’t.
  ```python3
  >>> d = {1:2}
  >>> d
  {1: 2}
  ...
  Also, the [`dict()`](../library/stdtypes.md#dict) constructor now accepts keyword arguments to simplify
...
* The [`assert`](../reference/simple_stmts.md#assert) statement no longer checks the `__debug__` flag, so
  you can no longer disable assertions by assigning to `__debug__`. Running
  Python with the [`-O`](../using/cmdline.md#cmdoption-O) switch will still generate code that doesn’t
...
* Most type objects are now callable, so you can use them to create new objects
  ...
  objects available in the [`types`](../library/types.md#module-types) module.) For example, you can create a new
  ...
  'docstring'
  ```
* A new warning, [`PendingDeprecationWarning`](../library/exceptions.md#PendingDeprecationWarning) was added to indicate features
  ...
  supply [`-Walways::PendingDeprecationWarning::`](../using/cmdline.md#cmdoption-W) on the command line or
  use [`warnings.filterwarnings()`](../library/warnings.md#warnings.filterwarnings).
...
* Using `None` as a variable name will now result in a [`SyntaxWarning`](../library/exceptions.md#SyntaxWarning)
...
* The method resolution order used by new-style classes has changed, though
  ...
  [The Python 2.3 Method Resolution Order](../howto/mro.md#python-2-3-mro), or
...
* One of the noted incompatibilities between old- and new-style classes has been
  removed: you can now assign to the [`__name__`](../reference/datamodel.md#type.__name__) and [`__bases__`](../reference/datamodel.md#type.__bases__)
  ...
  an instance’s [`__class__`](../reference/datamodel.md#object.__class__) attribute.

<!-- ====================================================================== -->

### String Changes

* The [`in`](../reference/expressions.md#in) operator now works differently for strings. Previously, when
  ...
  will return [`True`](../library/constants.md#True) if *X* is a substring of *Y*.  If *X* is the empty
  string, the result is always [`True`](../library/constants.md#True).
  ```python3
  >>> 'ab' in 'abcd'
  True
  >>> 'ad' in 'abcd'
  ...
  information, use the [`find()`](../library/stdtypes.md#str.find) string method.
* The [`strip()`](../library/stdtypes.md#str.strip), [`lstrip()`](../library/stdtypes.md#str.lstrip), and [`rstrip()`](../library/stdtypes.md#str.rstrip) string methods now have
...
* The [`startswith()`](../library/stdtypes.md#str.startswith) and [`endswith()`](../library/stdtypes.md#str.endswith) string methods now accept negative
  numbers for the *start* and *end* parameters.
* Another new string method is [`zfill()`](../library/stdtypes.md#str.zfill), originally a function in the
  [`string`](../library/string.md#module-string) module.  [`zfill()`](../library/stdtypes.md#str.zfill) pads a numeric string with zeros on the
  left until it’s the specified width. Note that the `%` operator is still more
  flexible and powerful than [`zfill()`](../library/stdtypes.md#str.zfill).
  ```python3
  >>> '45'.zfill(4)
  '0045'
  >>> '12345'.zfill(4)
...
* A new type object, `basestring`, has been added. Both 8-bit strings and
  ...
  return [`True`](../library/constants.md#True) for either kind of string.  It’s a completely abstract
...
* The [`sort()`](../library/stdtypes.md#list.sort) method of list objects has been extensively rewritten by Tim
...
* Multiplication of large long integers is now much faster thanks to an
  ...
  the *O*(*n*<sub>2</sub>) required for the grade-school multiplication algorithm.  (Original
...
* The `SET_LINENO` opcode is now gone.  This may provide a small speed
  ...
  [Other Changes and Fixes](#section-other) for a longer explanation. (Removed by Michael Hudson.)
...
* The [`array`](../library/array.md#module-array) module now supports arrays of Unicode characters using the
...
* The new [`bz2`](../library/bz2.md#module-bz2) module is an interface to the bz2 data compression library.
  bz2-compressed data is usually smaller than  corresponding
  [`zlib`](../library/zlib.md#module-zlib)-compressed data. (Contributed by Gustavo Niemeyer.)
* A set of standard date/time types has been added in the new [`datetime`](../library/datetime.md#module-datetime)
...
* Other minor changes to Distutils: it now checks for the [`CC`](../using/configure.md#envvar-CC),
  [`CFLAGS`](../using/configure.md#envvar-CFLAGS), `CPP`, [`LDFLAGS`](../using/configure.md#envvar-LDFLAGS), and [`CPPFLAGS`](../using/configure.md#envvar-CPPFLAGS)
  environment variables, using them to override the settings in Python’s
  configuration (contributed by Robert Weber).
* Previously the [`doctest`](../library/doctest.md#module-doctest) module would only search the docstrings of
  public methods and functions for test cases, but it now also examines private
  ones as well.  The [`DocTestSuite()`](../library/doctest.md#doctest.DocTestSuite) function creates a
  [`unittest.TestSuite`](../library/unittest.md#unittest.TestSuite) object from a set of [`doctest`](../library/doctest.md#module-doctest) tests.
* The new `gc.get_referents(object)` function returns a list of all the
  objects referenced by *object*.
* The [`getopt`](../library/getopt.md#module-getopt) module gained a new function, [`gnu_getopt()`](../library/getopt.md#getopt.gnu_getopt), that
  supports the same arguments as the existing [`getopt()`](../library/getopt.md#getopt.getopt) function but uses
  GNU-style scanning mode. The existing [`getopt()`](../library/getopt.md#getopt.getopt) stops processing options as
...
* The [`grp`](../library/grp.md#module-grp), [`pwd`](../library/pwd.md#module-pwd), and [`resource`](../library/resource.md#module-resource) modules now return enhanced
  tuples:
  ```python3
  >>> import grp
  >>> g = grp.getgrnam('amk')
  ...
  ('amk', 500)
  ```
* The [`gzip`](../library/gzip.md#module-gzip) module can now handle files exceeding 2 GiB.
* The new [`heapq`](../library/heapq.md#module-heapq) module contains an implementation of a heap queue
  ...
  *O*(log *n*).  (See [https://xlinux.nist.gov/dads//HTML/priorityque.html](https://xlinux.nist.gov/dads//HTML/priorityque.html) for more
  information about the priority queue data structure.)

  The [`heapq`](../library/heapq.md#module-heapq) module provides [`heappush()`](../library/heapq.md#heapq.heappush) and [`heappop()`](../library/heapq.md#heapq.heappop) functions
...
* The IDLE integrated development environment has been updated using the code
  ...
  has been incorporated into the standard library as the [`idlelib`](../library/idle.md#module-idlelib) package.
* The [`imaplib`](../library/imaplib.md#module-imaplib) module now supports IMAP over SSL. (Contributed by Piers
  Lauder and Tino Lange.)
* The [`itertools`](../library/itertools.md#module-itertools) contains a number of useful functions for use with
  ...
  [`True`](../library/constants.md#True), and `itertools.repeat(obj, N)` returns `obj` *N* times.
  There are a number of other functions in the module; see the package’s reference
  documentation for details.
  (Contributed by Raymond Hettinger.)
* Two new functions in the [`math`](../library/math.md#module-math) module, `degrees(rads)` and
  `radians(degs)`, convert between radians and degrees.  Other functions in
  the [`math`](../library/math.md#module-math) module such as [`math.sin()`](../library/math.md#math.sin) and [`math.cos()`](../library/math.md#math.cos) have always
  required input values measured in radians.  Also, an optional *base* argument
  was added to [`math.log()`](../library/math.md#math.log) to make it easier to compute logarithms for bases
...
* Several new POSIX functions (`getpgid()`, `killpg()`, `lchown()`,
  ...
  `mknod()`) were added to the [`posix`](../library/posix.md#module-posix) module that underlies the
  [`os`](../library/os.md#module-os) module. (Contributed by Gustavo Niemeyer, Geert Jansen, and Denis S.
  Otkidach.)
* In the [`os`](../library/os.md#module-os) module, the `*stat()` family of functions can now report
  fractions of a second in a timestamp.  Such time stamps are represented as
  floats, similar to the value returned by [`time.time()`](../library/time.md#time.time).
  ...
  [`stat_result`](../library/os.md#os.stat_result) time stamps will be represented as integers. When using
...
* The [`optparse`](../library/optparse.md#module-optparse) module contains a new parser for command-line arguments
...
* The new [`platform`](../library/platform.md#module-platform) module contains a number of functions that try to
...
* The parser objects provided by the [`pyexpat`](../library/pyexpat.md#module-xml.parsers.expat) module can now optionally
  ...
  [`buffer_text`](../library/pyexpat.md#xml.parsers.expat.xmlparser.buffer_text) attribute to [`True`](../library/constants.md#True) will enable buffering.
* The `sample(population, k)` function was added to the [`random`](../library/random.md#module-random)
  module.  *population* is a sequence or `xrange` object containing the
  elements of a population, and [`sample()`](../library/random.md#random.sample) chooses *k* elements from the
  ...
  The [`random`](../library/random.md#module-random) module now uses a new algorithm, the Mersenne Twister,
  implemented in C.  It’s faster and more extensively studied than the previous
  algorithm.

  (All changes contributed by Raymond Hettinger.)
* The [`readline`](../library/readline.md#module-readline) module also gained a number of new functions:
  [`get_history_item()`](../library/readline.md#readline.get_history_item), [`get_current_history_length()`](../library/readline.md#readline.get_current_history_length), and
  [`redisplay()`](../library/readline.md#readline.redisplay).
* The `rexec` and `Bastion` modules have been declared dead, and
  attempts to import them will fail with a [`RuntimeError`](../library/exceptions.md#RuntimeError).  New-style classes
...
* The [`shutil`](../library/shutil.md#module-shutil) module gained a `move(src, dest)` function that
  recursively moves a file or directory to a new location.
* Support for more advanced POSIX signal handling was added to the [`signal`](../library/signal.md#module-signal)
  but then removed again as it proved impossible to make it work reliably across
  platforms.
* The [`socket`](../library/socket.md#module-socket) module now supports timeouts.  You can call the
  ...
  abort and raise a [`socket.timeout`](../library/socket.md#socket.timeout) exception.

  The original timeout implementation was by Tim O’Malley.  Michael Gilfix
  integrated it into the Python [`socket`](../library/socket.md#module-socket) module and shepherded it through a
  ...
  action.)
* On Windows, the [`socket`](../library/socket.md#module-socket) module now ships with Secure  Sockets Layer
  (SSL) support.
...
* The new [`tarfile`](../library/tarfile.md#module-tarfile) module  allows reading from and writing to
  **tar**-format archive files. (Contributed by Lars Gustäbel.)
* The new [`textwrap`](../library/textwrap.md#module-textwrap) module contains functions for wrapping strings
  ...
  can guess, [`fill()`](../library/textwrap.md#textwrap.fill) is built on top of [`wrap()`](../library/textwrap.md#textwrap.wrap).  For example:
  ```python3
  >>> import textwrap
  ...
  The module also contains a [`TextWrapper`](../library/textwrap.md#textwrap.TextWrapper) class that actually implements
  the text wrapping strategy.   Both the [`TextWrapper`](../library/textwrap.md#textwrap.TextWrapper) class and the
  [`wrap()`](../library/textwrap.md#textwrap.wrap) and [`fill()`](../library/textwrap.md#textwrap.fill) functions support a number of additional keyword
  arguments for fine-tuning the formatting; consult the module’s documentation
  for details. (Contributed by Greg Ward.)
* The `thread` and [`threading`](../library/threading.md#module-threading) modules now have companion modules,
  ...
  that the module being used is not necessarily the actual [`threading`](../library/threading.md#module-threading)
  module. Code can call functions and use classes in `_threading` whether or
  not threads are supported, avoiding an [`if`](../reference/compound_stmts.md#if) statement and making the
...
* The [`time`](../library/time.md#module-time) module’s [`strptime()`](../library/time.md#time.strptime) function has long been an annoyance
  because it uses the platform C library’s [`strptime()`](../library/time.md#time.strptime) implementation, and
...
* The new [`timeit`](../library/timeit.md#module-timeit) module helps measure how long snippets of Python code
  take to execute.  The `timeit.py` file can be run directly from the
  command line, or the module’s [`Timer`](../library/timeit.md#timeit.Timer) class can be imported and used
...
* The `UserDict` module has a new `DictMixin` class which defines
  ...
  substitutable for dictionaries, such as the classes in  the [`shelve`](../library/shelve.md#module-shelve)
  module.

  Adding the mix-in as a superclass provides the full dictionary interface
  whenever the class defines [`__getitem__()`](../reference/datamodel.md#object.__getitem__), [`__setitem__()`](../reference/datamodel.md#object.__setitem__),
  [`__delitem__()`](../reference/datamodel.md#object.__delitem__), and `keys()`. For example:
  ```python3
  >>> import UserDict
  ...
  (Contributed by Raymond Hettinger.)
* The DOM implementation in [`xml.dom.minidom`](../library/xml.dom.minidom.md#module-xml.dom.minidom) can now generate XML output
  in a particular encoding by providing an optional encoding argument to the
  [`toxml()`](../library/xml.dom.minidom.md#xml.dom.minidom.Node.toxml) and [`toprettyxml()`](../library/xml.dom.minidom.md#xml.dom.minidom.Node.toprettyxml) methods of DOM nodes.
...
* Support for internationalized domain names (RFCs 3454, 3490, 3491, and 3492)
  ...
  The [`socket`](../library/socket.md#module-socket) module has also been extended to transparently convert
  Unicode hostnames to the ACE version before passing them to the C library.
  Modules that deal with hostnames such as `httplib` and [`ftplib`](../library/ftplib.md#module-ftplib))
  also support Unicode host names; `httplib` also sends HTTP `Host`
  headers using the ACE version of the domain name.  [`urllib`](../library/urllib.md#module-urllib) supports
  Unicode URLs with non-ASCII host names as long as the `path` part of the URL
  is ASCII only.

  To implement this change, the [`stringprep`](../library/stringprep.md#module-stringprep) module, the  `mkstringprep`
...
[`datetime`](../library/datetime.md#module-datetime) module.  The types don’t support different calendars or many
fancy features, and just stick to the basics of representing time.

The three primary types are: [`date`](../library/datetime.md#datetime.date), representing a day, month, and year;
[`time`](../library/datetime.md#datetime.time), consisting of hour, minute, and second; and [`datetime`](../library/datetime.md#datetime.datetime),
which contains all the attributes of both [`date`](../library/datetime.md#datetime.date) and [`time`](../library/datetime.md#datetime.time).
There’s also a [`timedelta`](../library/datetime.md#datetime.timedelta) class representing differences between two
points in time, and time zone logic is implemented by classes inheriting from
the abstract [`tzinfo`](../library/datetime.md#datetime.tzinfo) class.

You can create instances of [`date`](../library/datetime.md#datetime.date) and [`time`](../library/datetime.md#datetime.time) by either supplying
...
class methods.  For example, the [`today()`](../library/datetime.md#datetime.date.today) class method returns the
current local date.
...
The [`replace()`](../library/datetime.md#datetime.datetime.replace) method allows modifying one or more fields  of a
[`date`](../library/datetime.md#datetime.date) or [`datetime`](../library/datetime.md#datetime.datetime) instance, returning a new instance:
...
same as that of [`isoformat()`](../library/datetime.md#datetime.datetime.isoformat)).  [`date`](../library/datetime.md#datetime.date) and [`datetime`](../library/datetime.md#datetime.datetime)
instances can be subtracted from each other, and added to [`timedelta`](../library/datetime.md#datetime.timedelta)
instances.  The largest missing feature is that there’s no standard library
support for parsing strings and getting back a [`date`](../library/datetime.md#datetime.date) or
[`datetime`](../library/datetime.md#datetime.datetime).
...
The [`getopt`](../library/getopt.md#module-getopt) module provides simple parsing of command-line arguments.  The
new [`optparse`](../library/optparse.md#module-optparse) module (originally named Optik) provides more elaborate
...
You start by creating an instance of [`OptionParser`](../library/optparse.md#optparse.OptionParser) and telling it what
...
Parsing a command line is then done by calling the [`parse_args()`](../library/optparse.md#optparse.OptionParser.parse_args) method.
...
[`PyObject_Malloc()`](../c-api/memory.md#c.PyObject_Malloc), it has to be freed using [`PyObject_Free()`](../c-api/memory.md#c.PyObject_Free), not
...
* To allocate and free an undistinguished chunk of memory use the “raw memory”
  family: [`PyMem_Malloc()`](../c-api/memory.md#c.PyMem_Malloc), [`PyMem_Realloc()`](../c-api/memory.md#c.PyMem_Realloc), and [`PyMem_Free()`](../c-api/memory.md#c.PyMem_Free).
* The “object memory” family is the interface to the pymalloc facility described
  ...
  [`PyObject_Malloc()`](../c-api/memory.md#c.PyObject_Malloc), [`PyObject_Realloc()`](../c-api/memory.md#c.PyObject_Realloc), and [`PyObject_Free()`](../c-api/memory.md#c.PyObject_Free).
* To allocate and free Python objects, use the “object” family
  [`PyObject_New`](../c-api/allocation.md#c.PyObject_New), [`PyObject_NewVar`](../c-api/allocation.md#c.PyObject_NewVar), and [`PyObject_Del()`](../c-api/allocation.md#c.PyObject_Del).
...
* The `DL_EXPORT` and `DL_IMPORT` macros are now deprecated.
  ...
  using the new macro [`PyMODINIT_FUNC`](../c-api/extension-modules.md#c.PyMODINIT_FUNC), while the Python core will
...
* The `PyArg_NoArgs()` macro is now deprecated, and code that uses it
  ...
  specify the [`METH_NOARGS`](../c-api/structures.md#c.METH_NOARGS) flag, signalling that there are no arguments,
  ...
  "")` instead, but this will be slower than using [`METH_NOARGS`](../c-api/structures.md#c.METH_NOARGS).
* [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple) accepts new format characters for various sizes of
  unsigned integers: `B` for , `H` for ,  `I` for ,  and `K` for .
...
* It’s now possible to define class and static methods for a C extension type by
  setting either the [`METH_CLASS`](../c-api/structures.md#c.METH_CLASS) or [`METH_STATIC`](../c-api/structures.md#c.METH_STATIC) flags in a
  method’s [`PyMethodDef`](../c-api/structures.md#c.PyMethodDef) structure.
...
* If you dynamically allocate type objects in your extension, you should be
  aware of a change in the rules relating to the [`__module__`](../reference/datamodel.md#type.__module__) and
  [`__name__`](../reference/datamodel.md#type.__name__) attributes.  In summary, you will want to ensure the type’s
...
[`fcntl()`](../library/fcntl.md#module-fcntl) are restricted by the limitations of the underlying emulation
...
* If the [`PYTHONINSPECT`](../using/cmdline.md#envvar-PYTHONINSPECT) environment variable is set, the Python
  interpreter will enter the interactive prompt after running a Python program, as
  if Python had been invoked with the [`-i`](../using/cmdline.md#cmdoption-i) option. The environment
...
* The `SET_LINENO` opcode has been removed.  Back in the mists of time, this
  ...
  functions (for, e.g., [`pdb`](../library/pdb.md#module-pdb)). Since Python 1.5, the line numbers in
  ...
  from a slight speed up when Python is run without [`-O`](../using/cmdline.md#cmdoption-O).

  C extensions that access the [`f_lineno`](../reference/datamodel.md#frame.f_lineno) field of frame objects should
  ...
  [`f_lineno`](../reference/datamodel.md#frame.f_lineno) attribute of frame objects, changing the line that will be
  executed next.  A `jump` command has been added to the [`pdb`](../library/pdb.md#module-pdb) debugger
...
* [`yield`](../reference/simple_stmts.md#yield) is now always a keyword; if it’s used as a variable name in
...
* If you have Unicode strings that contain 8-bit characters, you must declare
  ...
  of the file.  See section [PEP 263: Source Code Encodings](#section-encodings) for more information.
* Calling Tcl methods through `_tkinter` no longer  returns only strings.
  Instead, if Tcl returns other objects those objects are converted to their
  Python equivalent, if one exists, or wrapped with a `_tkinter.Tcl_Obj`
  object if no Python equivalent exists.
* Large octal and hex literals such as `0xffffffff` now trigger a
  [`FutureWarning`](../library/exceptions.md#FutureWarning). Currently they’re stored as 32-bit numbers and result in a
