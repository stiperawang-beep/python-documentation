...
floating-point numbers and for the [`Decimal`](../library/decimal.md#decimal.Decimal) class.
There are some useful additions to the standard library, such as a
greatly enhanced [`unittest`](../library/unittest.md#module-unittest) module, the [`argparse`](../library/argparse.md#module-argparse) module
for parsing command-line options, convenient [`OrderedDict`](../library/collections.md#collections.OrderedDict)
and [`Counter`](../library/collections.md#collections.Counter) classes in the [`collections`](../library/collections.md#module-collections) module,
...
* While the preferred approach to enhancing Python 2 is now the publication
  ...
  [New Features Added to Python 2.7 Maintenance Releases](#py27-maintenance-enhancements) section below.
...
The recommended starting point is the [How to port Python 2 Code to Python 3](../howto/pyporting.md#pyporting-howto) HOWTO guide.

## Changes to the Handling of Deprecation Warnings

For Python 2.7, a policy decision was made to silence warnings only of
interest to developers by default.  [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning) and its
...
In previous releases, [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning) messages were
...
those applications.  [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning) messages are
irrelevant to such users, making them worry about an application
that’s actually working correctly and burdening application developers
with responding to these concerns.

You can re-enable display of [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning) messages by
running Python with the [`-Wdefault`](../using/cmdline.md#cmdoption-W) (short form:
[`-Wd`](../using/cmdline.md#cmdoption-W)) switch, or by setting the [`PYTHONWARNINGS`](../using/cmdline.md#envvar-PYTHONWARNINGS)
...
* Multiple context managers in a single [`with`](../reference/compound_stmts.md#with) statement.
* A new version of the [`io`](../library/io.md#module-io) library, rewritten in C for performance.
...
* The [`memoryview`](../library/stdtypes.md#memoryview) object.
* A small subset of the [`importlib`](../library/importlib.md#module-importlib) module,
  [described below](#importlib-section).
* The [`repr()`](../library/functions.md#repr) of a float `x` is shorter in many cases: it’s now
...
* Float-to-string and string-to-float conversions are correctly rounded.
  The [`round()`](../library/functions.md#round) function is also now correctly rounded.
* The [`PyCapsule`](../c-api/capsule.md#c.PyCapsule) type, used to provide a C API for extension modules.
* The [`PyLong_AsLongAndOverflow()`](../c-api/long.md#c.PyLong_AsLongAndOverflow) C API function.
...
[`OrderedDict`](../library/collections.md#collections.OrderedDict) class in the [`collections`](../library/collections.md#module-collections) module.

The [`OrderedDict`](../library/collections.md#collections.OrderedDict) API provides the same interface as regular
...
The [`popitem()`](../library/collections.md#collections.OrderedDict.popitem) method has an optional *last*
...
Comparing an [`OrderedDict`](../library/collections.md#collections.OrderedDict) with a regular dictionary
ignores the insertion order and just compares the keys and values.

How does the [`OrderedDict`](../library/collections.md#collections.OrderedDict) work?  It maintains a
...
* The [`ConfigParser`](../library/configparser.md#module-configparser) module uses them by default, meaning that
  configuration files can now be read, modified, and then written back
  in their original order.
* The [`_asdict()`](../library/collections.md#collections.somenamedtuple._asdict) method for
  [`collections.namedtuple()`](../library/collections.md#collections.namedtuple) now returns an ordered dictionary with the
  values appearing in the same order as the underlying tuple indices.
* The [`json`](../library/json.md#module-json) module’s [`JSONDecoder`](../library/json.md#json.JSONDecoder) class
...
The fully general solution for doing this is the [`locale`](../library/locale.md#module-locale) module,
which can use different separators (“,” in North America, “.” in
Europe) and different grouping sizes, but [`locale`](../library/locale.md#module-locale) is complicated
...
mini-language used by the [`str.format()`](../library/stdtypes.md#str.format) method.  When
...
comma-formatting mechanism isn’t as general as the [`locale`](../library/locale.md#module-locale)
...
The [`argparse`](../library/argparse.md#module-argparse) module for parsing command-line arguments was
added as a more powerful replacement for the
[`optparse`](../library/optparse.md#module-optparse) module.

This means Python now supports three different modules for parsing
command-line arguments: [`getopt`](../library/getopt.md#module-getopt), [`optparse`](../library/optparse.md#module-optparse), and
[`argparse`](../library/argparse.md#module-argparse).  The [`getopt`](../library/getopt.md#module-getopt) module closely resembles the C
library’s `getopt()` function, so it remains useful if you’re writing a
Python prototype that will eventually be rewritten in C.
[`optparse`](../library/optparse.md#module-optparse) becomes redundant, but there are no plans to remove it
because there are many scripts still using it, and there’s no
automated way to update these scripts.  (Making the [`argparse`](../library/argparse.md#module-argparse)
API consistent with [`optparse`](../library/optparse.md#module-optparse)’s interface was discussed but
...
[`argparse`](../library/argparse.md#module-argparse) instead of [`optparse`](../library/optparse.md#module-optparse).
...
As with [`optparse`](../library/optparse.md#module-optparse), the command-line switches and arguments
...
[`argparse`](../library/argparse.md#module-argparse) has much fancier validation than [`optparse`](../library/optparse.md#module-optparse); you
...
specify an argument’s type as [`FileType`](../library/argparse.md#argparse.FileType), which will
automatically open files for you and understands that `'-'` means
standard input or output.

#### SEE ALSO
[`argparse`](../library/argparse.md#module-argparse) documentation
: The documentation page of the argparse module.

[Migrating optparse code to argparse](../howto/argparse-optparse.md#upgrading-optparse-code)
: Part of the Python documentation, describing how to convert
  code that uses [`optparse`](../library/optparse.md#module-optparse).
...
The [`logging`](../library/logging.md#module-logging) module is very flexible; applications can define
...
[`logging`](../library/logging.md#module-logging) also supports a [`fileConfig()`](../library/logging.config.md#logging.config.fileConfig)
function that parses a file, but the file format doesn’t support
configuring filters, and it’s messier to generate programmatically.

Python 2.7 adds a [`dictConfig()`](../library/logging.config.md#logging.config.dictConfig) function that
...
installed.  For more information see [Configuration functions](../library/logging.config.md#logging-config-api).
...
Three smaller enhancements to the [`logging`](../library/logging.md#module-logging) module, all
implemented by Vinay Sajip, are:

<!-- rev79293 -->
* The [`SysLogHandler`](../library/logging.handlers.md#logging.handlers.SysLogHandler) class now supports
  syslogging over TCP.  The constructor has a *socktype* parameter
  giving the type of socket to use, either [`socket.SOCK_DGRAM`](../library/socket.md#socket.SOCK_DGRAM)
  for UDP or [`socket.SOCK_STREAM`](../library/socket.md#socket.SOCK_STREAM) for TCP.  The default
  protocol remains UDP.
* [`Logger`](../library/logging.md#logging.Logger) instances gained a [`getChild()`](../library/logging.md#logging.Logger.getChild)
...
* The [`LoggerAdapter`](../library/logging.md#logging.LoggerAdapter) class gained an
  [`isEnabledFor()`](../library/logging.md#logging.Logger.isEnabledFor) method that takes a
...
The dictionary methods [`keys()`](../library/stdtypes.md#dict.keys), [`values()`](../library/stdtypes.md#dict.values), and
[`items()`](../library/stdtypes.md#dict.items) are different in Python 3.x.  They return an object
called a *view* instead of a fully materialized list.

It’s not possible to change the return values of [`keys()`](../library/stdtypes.md#dict.keys),
[`values()`](../library/stdtypes.md#dict.values), and [`items()`](../library/stdtypes.md#dict.items) in Python 2.7 because
...
converter will change them to the standard [`keys()`](../library/stdtypes.md#dict.keys),
[`values()`](../library/stdtypes.md#dict.values), and [`items()`](../library/stdtypes.md#dict.items) methods.
...
The [`memoryview`](../library/stdtypes.md#memoryview) object provides a view of another object’s
memory content that matches the [`bytes`](../library/stdtypes.md#bytes) type’s interface.
...
[`memoryview`](../library/stdtypes.md#memoryview) objects allow modifying the underlying object if
...
* The [`with`](../reference/compound_stmts.md#with) statement can now use multiple context managers
...
* Conversions between floating-point numbers and strings are
  ...
  in many different places: [`str()`](../library/stdtypes.md#str) on
  floats and complex numbers; the [`float`](../library/functions.md#float) and [`complex`](../library/functions.md#complex)
  constructors;
  numeric formatting; serializing and
  deserializing floats and complex numbers using the
  [`marshal`](../library/marshal.md#module-marshal), [`pickle`](../library/pickle.md#module-pickle)
  and [`json`](../library/json.md#module-json) modules;
  parsing of float and imaginary literals in Python code;
  and [`Decimal`](../library/decimal.md#decimal.Decimal)-to-float conversion.

  Related to this, the [`repr()`](../library/functions.md#repr) of a floating-point number *x*
  ...
  by checking [`sys.float_repr_style`](../library/sys.md#sys.float_repr_style),  which will be `short`
...
* The [`str.format()`](../library/stdtypes.md#str.format) method now supports automatic numbering of the replacement
  fields.  This makes using [`str.format()`](../library/stdtypes.md#str.format) more closely resemble using
  `%s` formatting:
  ```python3
  ...
  specifier will use the first argument to [`str.format()`](../library/stdtypes.md#str.format), the next
  ...
  Complex numbers now correctly support usage with [`format()`](../library/functions.md#format),
  ...
  A low-level change: the [`object.__format__()`](../reference/datamodel.md#object.__format__) method now triggers
  a [`PendingDeprecationWarning`](../library/exceptions.md#PendingDeprecationWarning) if it’s passed a format string,
  because the `__format__()` method for [`object`](../library/functions.md#object) converts
...
* The [`int()`](../library/functions.md#int) and `long()` types gained a `bit_length`
...
* The [`import`](../reference/simple_stmts.md#import) statement will no longer try an absolute import
...
* The [`bytearray`](../library/stdtypes.md#bytearray) type’s [`translate()`](../library/stdtypes.md#bytearray.translate) method now accepts
...
* When using [`@classmethod`](../library/functions.md#classmethod) and
  [`@staticmethod`](../library/functions.md#staticmethod) to wrap
  methods as class or static methods, the wrapper object now
  exposes the wrapped function as their [`__func__`](../reference/datamodel.md#method.__func__)
  attribute.
...
* When a restricted set of attributes were set using `__slots__`,
  deleting an unset attribute would not raise [`AttributeError`](../library/exceptions.md#AttributeError)
...
* The `file` object will now set the `filename` attribute
  on the [`IOError`](../library/exceptions.md#IOError) exception when trying to open a directory
...
* The Python tokenizer now translates line endings itself, so the
  [`compile()`](../library/functions.md#compile) built-in function now accepts code using any
...
<a id="new-27-interpreter"></a>

### Interpreter Changes

A new environment variable, [`PYTHONWARNINGS`](../using/cmdline.md#envvar-PYTHONWARNINGS),
allows controlling warnings.  It should be set to a string
containing warning settings, equivalent to those
used with the [`-W`](../using/cmdline.md#cmdoption-W) switch, separated by commas.
...
they occur, but turn warnings from the [`Cookie`](../library/http.cookies.md#module-http.cookies) module into an
...
* A new opcode was added to perform the initial setup for
  [`with`](../reference/compound_stmts.md#with) statements, looking up the [`__enter__()`](../reference/datamodel.md#object.__enter__) and
  [`__exit__()`](../reference/datamodel.md#object.__exit__) methods.  (Contributed by Benjamin Peterson.)
...
* The `split()`, `replace()`, `rindex()`,
  ...
  (strings, Unicode strings, and [`bytearray`](../library/stdtypes.md#bytearray) objects) now use a
...
* The [`pickle`](../library/pickle.md#module-pickle) and `cPickle` modules now automatically
...
* The [`bdb`](../library/bdb.md#module-bdb) module’s base debugging class [`Bdb`](../library/bdb.md#bdb.Bdb)
...
* The [`binascii`](../library/binascii.md#module-binascii) module now supports the buffer API, so it can be
  used with [`memoryview`](../library/stdtypes.md#memoryview) instances and other similar buffer objects.
...
* The [`bz2`](../library/bz2.md#module-bz2) module’s [`BZ2File`](../library/bz2.md#bz2.BZ2File) now supports the context
  management protocol, so you can write `with bz2.BZ2File(...) as f:`.
  (Contributed by Hagen Fürstenau; [bpo-3860](https://bugs.python.org/issue?@action=redirect&bpo=3860).)
* New class: the [`Counter`](../library/collections.md#collections.Counter) class in the [`collections`](../library/collections.md#module-collections)
  module is useful for tallying data.  [`Counter`](../library/collections.md#collections.Counter) instances
  behave mostly like dictionaries but return zero for missing keys instead of
  raising a [`KeyError`](../library/exceptions.md#KeyError):
  ```pycon
  ...
  There are three additional [`Counter`](../library/collections.md#collections.Counter) methods.
  [`most_common()`](../library/collections.md#collections.Counter.most_common) returns the N most common
  elements and their counts.  [`elements()`](../library/collections.md#collections.Counter.elements)
  returns an iterator over the contained elements, repeating each
  element as many times as its count.
  [`subtract()`](../library/collections.md#collections.Counter.subtract) takes an iterable and
  ...
  New class: [`OrderedDict`](../library/collections.md#collections.OrderedDict) is described in the earlier
  section [PEP 372: Adding an Ordered Dictionary to collections](#pep-0372).

  New method: The [`deque`](../library/collections.md#collections.deque) data type now has a
  [`count()`](../library/collections.md#collections.deque.count) method that returns the number of
  contained elements equal to the supplied argument *x*, and a
  [`reverse()`](../library/collections.md#collections.deque.reverse) method that reverses the elements
  of the deque in-place.  [`deque`](../library/collections.md#collections.deque) also exposes its maximum
  length as the read-only [`maxlen`](../library/collections.md#collections.deque.maxlen) attribute.
  (Both features added by Raymond Hettinger.)

  The [`namedtuple`](../library/collections.md#collections.namedtuple) class now has an optional *rename* parameter.
  ...
  Finally, the [`Mapping`](../library/collections.abc.md#collections.abc.Mapping) abstract base class now
  returns [`NotImplemented`](../library/constants.md#NotImplemented) if a mapping is compared to
  another type that isn’t a `Mapping`.
  (Fixed by Daniel Stutzbach; [bpo-8729](https://bugs.python.org/issue?@action=redirect&bpo=8729).)
* Constructors for the parsing classes in the [`ConfigParser`](../library/configparser.md#module-configparser) module now
...
* Deprecated function: `contextlib.nested()`, which allows
  handling more than one context manager with a single [`with`](../reference/compound_stmts.md#with)
  statement, has been deprecated, because the `with` statement
  now supports multiple context managers.
* The [`cookielib`](../library/http.cookiejar.md#module-http.cookiejar) module now ignores cookies that have an invalid
...
* The [`copy`](../library/copy.md#module-copy) module’s [`deepcopy()`](../library/copy.md#copy.deepcopy) function will now
  correctly copy bound instance methods.  (Implemented by
  Robert Collins; [bpo-1515](https://bugs.python.org/issue?@action=redirect&bpo=1515).)
* The [`ctypes`](../library/ctypes.md#module-ctypes) module now always converts `None` to a C `NULL`
...
* New method: the [`datetime`](../library/datetime.md#module-datetime) module’s [`timedelta`](../library/datetime.md#datetime.timedelta) class
  gained a [`total_seconds()`](../library/datetime.md#datetime.timedelta.total_seconds) method that returns the
...
* New method: the [`Decimal`](../library/decimal.md#decimal.Decimal) class gained a
  [`from_float()`](../library/decimal.md#decimal.Decimal.from_float) class method that performs an exact
  ...
  Comparing instances of [`Decimal`](../library/decimal.md#decimal.Decimal) with floating-point
  ...
  The constructor for [`Decimal`](../library/decimal.md#decimal.Decimal) now accepts
  ...
  Most of the methods of the [`Context`](../library/decimal.md#decimal.Context) class now accept integers
  as well as [`Decimal`](../library/decimal.md#decimal.Decimal) instances; the only exceptions are the
  [`canonical()`](../library/decimal.md#decimal.Context.canonical) and [`is_canonical()`](../library/decimal.md#decimal.Context.is_canonical)
  methods.  (Patch by Juan José Conti; [bpo-7633](https://bugs.python.org/issue?@action=redirect&bpo=7633).)

  When using [`Decimal`](../library/decimal.md#decimal.Decimal) instances with a string’s
  [`format()`](../library/stdtypes.md#str.format) method, the default alignment was previously
  ...
  [`InvalidOperation`](../library/decimal.md#decimal.InvalidOperation) instead of silently returning a true or
...
* The [`difflib`](../library/difflib.md#module-difflib) module now produces output that is more
...
* The [`doctest`](../library/doctest.md#module-doctest) module’s [`IGNORE_EXCEPTION_DETAIL`](../library/doctest.md#doctest.IGNORE_EXCEPTION_DETAIL) flag
...
* The [`email`](../library/email.md#module-email) module’s [`Message`](../library/email.compat32-message.md#email.message.Message) class will
...
* The [`Fraction`](../library/fractions.md#fractions.Fraction) class now accepts a single float or
  [`Decimal`](../library/decimal.md#decimal.Decimal) instance, or two rational numbers, as
  ...
  fractions and complex numbers now raise a [`TypeError`](../library/exceptions.md#TypeError).
  This fixes an oversight, making the [`Fraction`](../library/fractions.md#fractions.Fraction)
  match the other numeric types.
  <!-- revision 79455 -->
* New class: [`FTP_TLS`](../library/ftplib.md#ftplib.FTP_TLS) in
  the [`ftplib`](../library/ftplib.md#module-ftplib) module provides secure FTP
  ...
  The [`storbinary()`](../library/ftplib.md#ftplib.FTP.storbinary) method for binary uploads can now restart
  uploads thanks to an added *rest* parameter (patch by Pablo Mouzo;
  [bpo-6845](https://bugs.python.org/issue?@action=redirect&bpo=6845).)
* New class decorator: [`total_ordering()`](../library/functools.md#functools.total_ordering) in the [`functools`](../library/functools.md#module-functools)
  module takes a class that defines an [`__eq__()`](../reference/datamodel.md#object.__eq__) method and one of
  [`__lt__()`](../reference/datamodel.md#object.__lt__), [`__le__()`](../reference/datamodel.md#object.__le__), [`__gt__()`](../reference/datamodel.md#object.__gt__), or [`__ge__()`](../reference/datamodel.md#object.__ge__),
  ...
  New function: [`cmp_to_key()`](../library/functools.md#functools.cmp_to_key) will take an old-style comparison
  function that expects two arguments and return a new callable that
  can be used as the *key* parameter to functions such as
  [`sorted()`](../library/functions.md#sorted), [`min()`](../library/functions.md#min) and [`max()`](../library/functions.md#max), etc.  The primary
  intended use is to help with making code compatible with Python 3.x.
  (Added by Raymond Hettinger.)
* New function: the [`gc`](../library/gc.md#module-gc) module’s [`is_tracked()`](../library/gc.md#gc.is_tracked) returns
  true if a given instance is tracked by the garbage collector, false
  otherwise. (Contributed by Antoine Pitrou; [bpo-4688](https://bugs.python.org/issue?@action=redirect&bpo=4688).)
* The [`gzip`](../library/gzip.md#module-gzip) module’s [`GzipFile`](../library/gzip.md#gzip.GzipFile) now supports the context
  ...
  the [`io.BufferedIOBase`](../library/io.md#io.BufferedIOBase) ABC, so you can wrap it with
  [`io.BufferedReader`](../library/io.md#io.BufferedReader) for faster processing
  ...
  [`gzip`](../library/gzip.md#module-gzip) module will now consume these trailing bytes.  (Fixed by
  Tadek Pietraszek and Brian Curtin; [bpo-2846](https://bugs.python.org/issue?@action=redirect&bpo=2846).)
* New attribute: the [`hashlib`](../library/hashlib.md#module-hashlib) module now has an `algorithms`
...
* The default [`HTTPResponse`](../library/http.client.md#http.client.HTTPResponse) class used by the [`httplib`](../library/http.md#module-http) module now
  supports buffering, resulting in much faster reading of HTTP responses.
  (Contributed by Kristján Valur Jónsson; [bpo-4879](https://bugs.python.org/issue?@action=redirect&bpo=4879).)

  The [`HTTPConnection`](../library/http.client.md#http.client.HTTPConnection) and [`HTTPSConnection`](../library/http.client.md#http.client.HTTPSConnection) classes
...
* The `ihooks` module now supports relative imports.  Note that
  ...
  <!-- revision 75423 -->
* The [`imaplib`](../library/imaplib.md#module-imaplib) module now supports IPv6 addresses.
  (Contributed by Derek Morr; [bpo-1655](https://bugs.python.org/issue?@action=redirect&bpo=1655).)
* New function: the [`inspect`](../library/inspect.md#module-inspect) module’s [`getcallargs()`](../library/inspect.md#inspect.getcallargs)
...
* Updated module: The [`io`](../library/io.md#module-io) library has been upgraded to the version shipped with
  ...
  One minor resulting change: the [`io.TextIOBase`](../library/io.md#io.TextIOBase) class now
  has an [`errors`](../library/io.md#io.TextIOBase.errors) attribute giving the error setting
  used for encoding and decoding errors (one of `'strict'`, `'replace'`,
  `'ignore'`).

  The [`io.FileIO`](../library/io.md#io.FileIO) class now raises an [`OSError`](../library/exceptions.md#OSError) when passed
  an invalid file descriptor.  (Implemented by Benjamin Peterson;
  [bpo-4991](https://bugs.python.org/issue?@action=redirect&bpo=4991).)  The [`truncate()`](../library/io.md#io.IOBase.truncate) method now preserves the
...
* New function: `itertools.compress(data, selectors)` takes two
  ...
  iterable *iter*.  Unlike [`combinations()`](../library/itertools.md#itertools.combinations), individual elements
  ...
  The [`itertools.count()`](../library/itertools.md#itertools.count) function now has a *step* argument that
  allows incrementing by values other than 1.  [`count()`](../library/itertools.md#itertools.count) also
  now allows keyword arguments, and using non-integer values such as
  floats or [`Decimal`](../library/decimal.md#decimal.Decimal) instances.  (Implemented by Raymond
  Hettinger; [bpo-5032](https://bugs.python.org/issue?@action=redirect&bpo=5032).)

  [`itertools.combinations()`](../library/itertools.md#itertools.combinations) and [`itertools.product()`](../library/itertools.md#itertools.product)
  previously raised [`ValueError`](../library/exceptions.md#ValueError) for values of *r* larger than
...
* Updated module: The [`json`](../library/json.md#module-json) module was upgraded to version 2.0.9 of the
  simplejson package, which includes a C extension that makes
  encoding and decoding faster.
  (Contributed by Bob Ippolito; [bpo-4136](https://bugs.python.org/issue?@action=redirect&bpo=4136).)

  To support the new [`collections.OrderedDict`](../library/collections.md#collections.OrderedDict) type, [`json.load()`](../library/json.md#json.load)
...
* The [`mailbox`](../library/mailbox.md#module-mailbox) module’s [`Maildir`](../library/mailbox.md#mailbox.Maildir) class now records the
...
* New functions: the [`math`](../library/math.md#module-math) module gained
  [`erf()`](../library/math.md#math.erf) and [`erfc()`](../library/math.md#math.erfc) for the error function and the complementary error function,
  [`expm1()`](../library/math.md#math.expm1) which computes `e**x - 1` with more precision than
  using [`exp()`](../library/math.md#math.exp) and subtracting 1,
  [`gamma()`](../library/math.md#math.gamma) for the Gamma function, and
  [`lgamma()`](../library/math.md#math.lgamma) for the natural log of the Gamma function.
  (Contributed by Mark Dickinson and nirinA raseliarison; [bpo-3366](https://bugs.python.org/issue?@action=redirect&bpo=3366).)
* The [`multiprocessing`](../library/multiprocessing.md#module-multiprocessing) module’s `Manager*` classes
  can now be passed a callable that will be called whenever
  a subprocess is started, along with a set of arguments that will be
  passed to the callable.
  (Contributed by lekma; [bpo-5585](https://bugs.python.org/issue?@action=redirect&bpo=5585).)

  The [`Pool`](../library/multiprocessing.md#multiprocessing.pool.Pool) class, which controls a pool of worker processes,
...
* New functions: the [`os`](../library/os.md#module-os) module wraps the following POSIX system
  calls: [`getresgid()`](../library/os.md#os.getresgid) and [`getresuid()`](../library/os.md#os.getresuid), which return the
  real, effective, and saved GIDs and UIDs;
  [`setresgid()`](../library/os.md#os.setresgid) and [`setresuid()`](../library/os.md#os.setresuid), which set
  real, effective, and saved GIDs and UIDs to new values;
  [`initgroups()`](../library/os.md#os.initgroups), which initialize the group access list
  ...
  The [`os.fork()`](../library/os.md#os.fork) function now re-initializes the import lock in
  the child process; this fixes problems on Solaris when [`fork()`](../library/os.md#os.fork)
  is called from a thread.  (Fixed by Zsolt Cserna; [bpo-7242](https://bugs.python.org/issue?@action=redirect&bpo=7242).)
* In the [`os.path`](../library/os.path.md#module-os.path) module, the [`normpath()`](../library/os.path.md#os.path.normpath) and
  [`abspath()`](../library/os.path.md#os.path.abspath) functions now preserve Unicode; if their input path
  is a Unicode string, the return value is also a Unicode string.
  ([`normpath()`](../library/os.path.md#os.path.normpath) fixed by Matt Giuca in [bpo-5827](https://bugs.python.org/issue?@action=redirect&bpo=5827);
  [`abspath()`](../library/os.path.md#os.path.abspath) fixed by Ezio Melotti in [bpo-3426](https://bugs.python.org/issue?@action=redirect&bpo=3426).)
* The [`pydoc`](../library/pydoc.md#module-pydoc) module now has help for the various symbols that Python
  uses.  You can now do `help('<<')` or `help('@')`, for example.
  (Contributed by David Laban; [bpo-4739](https://bugs.python.org/issue?@action=redirect&bpo=4739).)
* The [`re`](../library/re.md#module-re) module’s [`split()`](../library/re.md#re.split), [`sub()`](../library/re.md#re.sub), and [`subn()`](../library/re.md#re.subn)
  now accept an optional *flags* argument, for consistency with the
  other functions in the module.  (Added by Gregory P. Smith.)
* New function: [`run_path()`](../library/runpy.md#runpy.run_path) in the [`runpy`](../library/runpy.md#module-runpy) module
  ...
  `sys.path` and the module [`__main__`](../library/__main__.md#module-__main__) will be imported.  It’s
  ...
  of [`runpy`](../library/runpy.md#module-runpy) available to scripts that want to mimic the way
...
* New function: in the [`shutil`](../library/shutil.md#module-shutil) module, [`make_archive()`](../library/shutil.md#shutil.make_archive)
  ...
  [`shutil`](../library/shutil.md#module-shutil)’s [`copyfile()`](../library/shutil.md#shutil.copyfile) and [`copytree()`](../library/shutil.md#shutil.copytree)
  functions now raise a [`SpecialFileError`](../library/shutil.md#shutil.SpecialFileError) exception when
...
* The [`signal`](../library/signal.md#module-signal) module no longer re-installs the signal handler
...
* New functions: in the [`site`](../library/site.md#module-site) module, three new functions
  return various site- and user-specific paths.
  [`getsitepackages()`](../library/site.md#site.getsitepackages) returns a list containing all
  global site-packages directories,
  [`getusersitepackages()`](../library/site.md#site.getusersitepackages) returns the path of the user’s
  site-packages directory, and
  [`getuserbase()`](../library/site.md#site.getuserbase) returns the value of the [`USER_BASE`](../library/site.md#site.USER_BASE)
  environment variable, giving the path to a directory that can be used
  to store data.
  (Contributed by Tarek Ziadé; [bpo-6693](https://bugs.python.org/issue?@action=redirect&bpo=6693).)

  The [`site`](../library/site.md#module-site) module now reports exceptions occurring
  when the [`sitecustomize`](../library/site.md#module-sitecustomize) module is imported, and will no longer
  catch and swallow the [`KeyboardInterrupt`](../library/exceptions.md#KeyboardInterrupt) exception.  (Fixed by
  Victor Stinner; [bpo-3137](https://bugs.python.org/issue?@action=redirect&bpo=3137).)
* The [`create_connection()`](../library/socket.md#socket.create_connection) function
  gained a *source_address* parameter, a `(host, port)` 2-tuple
  giving the source address that will be used for the connection.
  (Contributed by Eldon Ziegler; [bpo-3972](https://bugs.python.org/issue?@action=redirect&bpo=3972).)

  The [`recv_into()`](../library/socket.md#socket.socket.recv_into) and [`recvfrom_into()`](../library/socket.md#socket.socket.recvfrom_into)
  methods will now write into objects that support the buffer API, most usefully
  the [`bytearray`](../library/stdtypes.md#bytearray) and [`memoryview`](../library/stdtypes.md#memoryview) objects.  (Implemented by
  Antoine Pitrou; [bpo-8104](https://bugs.python.org/issue?@action=redirect&bpo=8104).)
* The [`SocketServer`](../library/socketserver.md#module-socketserver) module’s [`TCPServer`](../library/socketserver.md#socketserver.TCPServer) class now
  ...
  The [`timeout`](../library/socketserver.md#socketserver.BaseServer.timeout) class attribute can hold
  a timeout in seconds that will be applied to the request socket; if
  no request is received within that time, [`handle_timeout()`](../library/socketserver.md#socketserver.BaseServer.handle_timeout)
  will be called and [`handle_request()`](../library/socketserver.md#socketserver.BaseServer.handle_request) will return.
...
* Updated module: the [`sqlite3`](../library/sqlite3.md#module-sqlite3) module has been updated to
  ...
  and then call [`load_extension()`](../library/sqlite3.md#sqlite3.Connection.load_extension) to load a particular shared library.
  (Updated by Gerhard Häring.)
* The [`ssl`](../library/ssl.md#module-ssl) module’s [`SSLSocket`](../library/ssl.md#ssl.SSLSocket) objects now support the
  ...
  The [`wrap_socket()`](../library/ssl.md#ssl.SSLContext.wrap_socket) constructor function now takes a
  *ciphers* argument that’s a string listing the encryption algorithms
  ...
  [`ssl.OPENSSL_VERSION_NUMBER`](../library/ssl.md#ssl.OPENSSL_VERSION_NUMBER) (an integer).  (Added by Antoine
  Pitrou; [bpo-8321](https://bugs.python.org/issue?@action=redirect&bpo=8321).)
* The [`struct`](../library/struct.md#module-struct) module will no longer silently ignore overflow
  errors when a value is too large for a particular integer format
  code (one of `bBhHiIlLqQ`); it now always raises a
  [`struct.error`](../library/struct.md#struct.error) exception.  (Changed by Mark Dickinson;
  [bpo-1523](https://bugs.python.org/issue?@action=redirect&bpo=1523).)  The [`pack()`](../library/struct.md#struct.pack) function will also
  attempt to use [`__index__()`](../reference/datamodel.md#object.__index__) to convert and pack non-integers
  before trying the [`__int__()`](../reference/datamodel.md#object.__int__) method or reporting an error.
  (Changed by Mark Dickinson; [bpo-8300](https://bugs.python.org/issue?@action=redirect&bpo=8300).)
* New function: the [`subprocess`](../library/subprocess.md#module-subprocess) module’s
  [`check_output()`](../library/subprocess.md#subprocess.check_output) runs a command with a specified set of arguments
  and returns the command’s output as a string when the command runs without
  error, or raises a [`CalledProcessError`](../library/subprocess.md#subprocess.CalledProcessError) exception otherwise.
  ```python3
  ...
  The [`subprocess`](../library/subprocess.md#module-subprocess) module will now retry its internal system calls
  on receiving an [`EINTR`](../library/errno.md#errno.EINTR) signal.  (Reported by several people; final
  patch by Gregory P. Smith in [bpo-1068268](https://bugs.python.org/issue?@action=redirect&bpo=1068268).)
* New function: [`is_declared_global()`](../library/symtable.md#symtable.Symbol.is_declared_global) in the [`symtable`](../library/symtable.md#module-symtable) module
  returns true for variables that are explicitly declared to be global,
  false for ones that are implicitly global.
  (Contributed by Jeremy Hylton.)
* The [`syslog`](../library/syslog.md#module-syslog) module will now use the value of `sys.argv[0]` as the
  identifier instead of the previous default value of `'python'`.
  (Changed by Sean Reifschneider; [bpo-8451](https://bugs.python.org/issue?@action=redirect&bpo=8451).)
* The [`sys.version_info`](../library/sys.md#sys.version_info) value is now a named tuple, with attributes
  ...
  [`sys.getwindowsversion()`](../library/sys.md#sys.getwindowsversion) also returns a named tuple,
...
* The [`tarfile`](../library/tarfile.md#module-tarfile) module’s default error handling has changed, to
  ...
  [`tarfile`](../library/tarfile.md#module-tarfile) now supports filtering the [`TarInfo`](../library/tarfile.md#tarfile.TarInfo)
  objects being added to a tar file.  When you call [`add()`](../library/tarfile.md#tarfile.TarFile.add),
  you may supply an optional *filter* argument
  that’s a callable.  The *filter* callable will be passed the
  [`TarInfo`](../library/tarfile.md#tarfile.TarInfo) for every file being added, and can modify and return it.
  ...
  *exclude* argument, which has therefore been deprecated.
  ...
  The [`TarFile`](../library/tarfile.md#tarfile.TarFile) class also now supports the context management protocol.
  (Added by Lars Gustäbel; [bpo-7232](https://bugs.python.org/issue?@action=redirect&bpo=7232).)
* The [`wait()`](../library/threading.md#threading.Event.wait) method of the [`threading.Event`](../library/threading.md#threading.Event) class
  now returns the internal flag on exit.  This means the method will usually
  return true because [`wait()`](../library/threading.md#threading.Event.wait) is supposed to block until the
  internal flag becomes true.  The return value will only be false if
  a timeout was provided and the operation timed out.
  (Contributed by Tim Lesher; [bpo-1674032](https://bugs.python.org/issue?@action=redirect&bpo=1674032).)
* The Unicode database provided by the [`unicodedata`](../library/unicodedata.md#module-unicodedata) module is
...
* The [`urlparse`](../library/urllib.parse.md#module-urllib.parse) module’s [`urlsplit()`](../library/urllib.parse.md#urllib.parse.urlsplit) now handles
  ...
  The [`urlparse`](../library/urllib.parse.md#module-urllib.parse) module also supports IPv6 literal addresses as defined by
  ...
  ```
* New class: the [`WeakSet`](../library/weakref.md#weakref.WeakSet) class in the [`weakref`](../library/weakref.md#module-weakref)
...
* The [`xml.etree.ElementTree`](../library/xml.etree.elementtree.md#module-xml.etree.ElementTree) library, no longer escapes
  ampersands and angle brackets when outputting an XML processing
  instruction (which looks like `<?xml-stylesheet href="#style1"?>`)
  or comment (which looks like `<!-- comment -->`).
  (Patch by Neil Muller; [bpo-2746](https://bugs.python.org/issue?@action=redirect&bpo=2746).)
* The XML-RPC client and server, provided by the [`xmlrpclib`](../library/xmlrpc.client.md#module-xmlrpc.client) and
  [`SimpleXMLRPCServer`](../library/xmlrpc.server.md#module-xmlrpc.server) modules, have improved performance by
  ...
  [`SimpleXMLRPCRequestHandler`](../library/xmlrpc.server.md#xmlrpc.server.SimpleXMLRPCRequestHandler), which contains a size in bytes;
...
* The [`zipfile`](../library/zipfile.md#module-zipfile) module’s [`ZipFile`](../library/zipfile.md#zipfile.ZipFile) now supports the context
  management protocol, so you can write `with zipfile.ZipFile(...) as f:`.
  (Contributed by Brian Curtin; [bpo-5511](https://bugs.python.org/issue?@action=redirect&bpo=5511).)

  [`zipfile`](../library/zipfile.md#module-zipfile) now also supports archiving empty directories and
  ...
  [`read()`](../library/io.md#io.BufferedIOBase.read) and
  [`readline()`](../library/io.md#io.IOBase.readline) now works correctly.
  (Contributed by Nir Aides; [bpo-7610](https://bugs.python.org/issue?@action=redirect&bpo=7610).)

  The [`is_zipfile()`](../library/zipfile.md#zipfile.is_zipfile) function now
  accepts a file object, in addition to the path names accepted in earlier
  versions.  (Contributed by Gabriel Genellina; [bpo-4756](https://bugs.python.org/issue?@action=redirect&bpo=4756).)

  The [`writestr()`](../library/zipfile.md#zipfile.ZipFile.writestr) method now has an optional *compress_type* parameter
  that lets you override the default compression method specified in the
  [`ZipFile`](../library/zipfile.md#zipfile.ZipFile) constructor.  (Contributed by Ronald Oussoren;
  [bpo-6003](https://bugs.python.org/issue?@action=redirect&bpo=6003).)

<!-- ====================================================================== -->
<!-- whole new modules get described in subsections here -->

<a id="importlib-section"></a>

### New module: importlib

Python 3.1 includes the [`importlib`](../library/importlib.md#module-importlib) package, a re-implementation
of the logic underlying Python’s [`import`](../reference/simple_stmts.md#import) statement.
[`importlib`](../library/importlib.md#module-importlib) is useful for implementers of Python interpreters and
to users who wish to write new importers that can participate in the
import process.  Python 2.7 doesn’t contain the complete
[`importlib`](../library/importlib.md#module-importlib) package, but instead has a tiny subset that contains
a single function, [`import_module()`](../library/importlib.md#importlib.import_module).
...
the relative import.  [`import_module()`](../library/importlib.md#importlib.import_module) both inserts the imported
...
[`importlib`](../library/importlib.md#module-importlib) was implemented by Brett Cannon and introduced in
Python 3.1.

### New module: sysconfig

The [`sysconfig`](../library/sysconfig.md#module-sysconfig) module has been pulled out of the Distutils
package, becoming a new top-level module in its own right.
[`sysconfig`](../library/sysconfig.md#module-sysconfig) provides functions for getting information about
...
* [`get_config_var()`](../library/sysconfig.md#sysconfig.get_config_var) returns variables from Python’s
  Makefile and the `pyconfig.h` file.
* [`get_config_vars()`](../library/sysconfig.md#sysconfig.get_config_vars) returns a dictionary containing
  all of the configuration variables.
* [`get_path()`](../library/sysconfig.md#sysconfig.get_path) returns the configured path for
  a particular type of module: the standard library,
  site-specific modules, platform-specific modules, etc.
* [`is_python_build()`](../library/sysconfig.md#sysconfig.is_python_build) returns true if you’re running a
  binary from a Python source tree, and false otherwise.

Consult the [`sysconfig`](../library/sysconfig.md#module-sysconfig) documentation for more details and for
a complete list of functions.

The Distutils package and [`sysconfig`](../library/sysconfig.md#module-sysconfig) are now maintained by Tarek
...
To learn more, read the [`ttk`](../library/tkinter.ttk.md#module-tkinter.ttk) module documentation.  You may also
wish to read the Tcl/Tk manual page describing the
Ttk theme engine, available at
[https://www.tcl.tk/man/tcl8.5/TkCmd/ttk_intro.html](https://www.tcl.tk/man/tcl8.5/TkCmd/ttk_intro.html). Some
...
The [`tkinter.ttk`](../library/tkinter.ttk.md#module-tkinter.ttk) module was written by Guilherme Polo and added in
...
<a id="unittest-section"></a>

### Updated module: unittest

The [`unittest`](../library/unittest.md#module-unittest) module was greatly enhanced; many
...
Consult the [`unittest`](../library/unittest.md#module-unittest) module documentation for more details.
(Developed in [bpo-6001](https://bugs.python.org/issue?@action=redirect&bpo=6001).)

The [`main()`](../library/unittest.md#unittest.main) function supports some other new options:

* [`-b`](../library/unittest.md#cmdoption-unittest-b) or `--buffer` will buffer the standard output
  and standard error streams during each test.  If the test passes,
  any resulting output will be discarded; on failure, the buffered
  output will be displayed.
* [`-c`](../library/unittest.md#cmdoption-unittest-c) or `--catch` will cause the control-C interrupt
  ...
  [`removeHandler()`](../library/unittest.md#unittest.removeHandler) decorator that can be used to mark tests that
  should have the control-C handling disabled.
* [`-f`](../library/unittest.md#cmdoption-unittest-f) or `--failfast` makes
...
Test cases can raise the [`SkipTest`](../library/unittest.md#unittest.SkipTest) exception to skip a
test ([bpo-1034053](https://bugs.python.org/issue?@action=redirect&bpo=1034053)).

The error messages for [`assertEqual()`](../library/unittest.md#unittest.TestCase.assertEqual),
[`assertTrue()`](../library/unittest.md#unittest.TestCase.assertTrue), and [`assertFalse()`](../library/unittest.md#unittest.TestCase.assertFalse)
failures now provide more information.  If you set the
[`longMessage`](../library/unittest.md#unittest.TestCase.longMessage) attribute of your [`TestCase`](../library/unittest.md#unittest.TestCase) classes to
...
The [`assertRaises()`](../library/unittest.md#unittest.TestCase.assertRaises) method now
...
Modules can contain [`setUpModule()`](../library/unittest.md#unittest.setUpModule) and [`tearDownModule()`](../library/unittest.md#unittest.tearDownModule)
functions.  Classes can have [`setUpClass()`](../library/unittest.md#unittest.TestCase.setUpClass) and
[`tearDownClass()`](../library/unittest.md#unittest.TestCase.tearDownClass) methods that must be defined as class methods
...
The methods [`addCleanup()`](../library/unittest.md#unittest.TestCase.addCleanup) and
[`doCleanups()`](../library/unittest.md#unittest.TestCase.doCleanups) were added.
[`addCleanup()`](../library/unittest.md#unittest.TestCase.addCleanup) lets you add cleanup functions that
will be called unconditionally (after [`setUp()`](../library/unittest.md#unittest.TestCase.setUp) if
[`setUp()`](../library/unittest.md#unittest.TestCase.setUp) fails, otherwise after [`tearDown()`](../library/unittest.md#unittest.TestCase.tearDown)). This allows
...
GvR worked on merging them into Python’s version of [`unittest`](../library/unittest.md#module-unittest).

* [`assertIsNone()`](../library/unittest.md#unittest.TestCase.assertIsNone) and [`assertIsNotNone()`](../library/unittest.md#unittest.TestCase.assertIsNotNone) take one
  expression and verify that the result is or is not `None`.
* [`assertIs()`](../library/unittest.md#unittest.TestCase.assertIs) and [`assertIsNot()`](../library/unittest.md#unittest.TestCase.assertIsNot)
...
* [`assertIsInstance()`](../library/unittest.md#unittest.TestCase.assertIsInstance) and
  [`assertNotIsInstance()`](../library/unittest.md#unittest.TestCase.assertNotIsInstance) check whether
...
* [`assertGreater()`](../library/unittest.md#unittest.TestCase.assertGreater), [`assertGreaterEqual()`](../library/unittest.md#unittest.TestCase.assertGreaterEqual),
  [`assertLess()`](../library/unittest.md#unittest.TestCase.assertLess), and [`assertLessEqual()`](../library/unittest.md#unittest.TestCase.assertLessEqual) compare
  two quantities.
* [`assertMultiLineEqual()`](../library/unittest.md#unittest.TestCase.assertMultiLineEqual) compares two strings, and if they’re
  not equal, displays a helpful comparison that highlights the
  differences in the two strings.  This comparison is now used by
  default when Unicode strings are compared with [`assertEqual()`](../library/unittest.md#unittest.TestCase.assertEqual).
* [`assertRegexpMatches()`](../library/unittest.md#unittest.TestCase.assertRegex) and
  [`assertNotRegexpMatches()`](../library/unittest.md#unittest.TestCase.assertNotRegex) checks whether the
...
* [`assertRaisesRegexp()`](../library/unittest.md#unittest.TestCase.assertRaisesRegex) checks
  whether a particular exception
  is raised, and then also checks that the string representation of
  the exception matches the provided regular expression.
* [`assertIn()`](../library/unittest.md#unittest.TestCase.assertIn) and [`assertNotIn()`](../library/unittest.md#unittest.TestCase.assertNotIn)
  tests whether *first* is or is not in  *second*.
* [`assertItemsEqual()`](../library/unittest.md#unittest.TestCase.assertCountEqual) tests whether two provided sequences
  contain the same elements.
* [`assertSetEqual()`](../library/unittest.md#unittest.TestCase.assertSetEqual) compares whether two sets are equal, and
  only reports the differences between the sets in case of error.
* Similarly, [`assertListEqual()`](../library/unittest.md#unittest.TestCase.assertListEqual) and [`assertTupleEqual()`](../library/unittest.md#unittest.TestCase.assertTupleEqual)
  ...
  when comparing lists and tuples using [`assertEqual()`](../library/unittest.md#unittest.TestCase.assertEqual).
  More generally, [`assertSequenceEqual()`](../library/unittest.md#unittest.TestCase.assertSequenceEqual) compares two sequences
  and can optionally check whether both sequences are of a
  particular type.
* [`assertDictEqual()`](../library/unittest.md#unittest.TestCase.assertDictEqual) compares two dictionaries and reports the
  differences; it’s now used by default when you compare two dictionaries
  using [`assertEqual()`](../library/unittest.md#unittest.TestCase.assertEqual).  `assertDictContainsSubset()` checks whether
  all of the key/value pairs in *first* are found in *second*.
* [`assertAlmostEqual()`](../library/unittest.md#unittest.TestCase.assertAlmostEqual) and [`assertNotAlmostEqual()`](../library/unittest.md#unittest.TestCase.assertNotAlmostEqual) test
...
* [`loadTestsFromName()`](../library/unittest.md#unittest.TestLoader.loadTestsFromName) properly honors the
  [`suiteClass`](../library/unittest.md#unittest.TestLoader.suiteClass) attribute of
  the [`TestLoader`](../library/unittest.md#unittest.TestLoader). (Fixed by Mark Roddy; [bpo-6866](https://bugs.python.org/issue?@action=redirect&bpo=6866).)
* A new hook lets you extend the [`assertEqual()`](../library/unittest.md#unittest.TestCase.assertEqual) method to handle
  new data types.  The [`addTypeEqualityFunc()`](../library/unittest.md#unittest.TestCase.addTypeEqualityFunc) method takes a type
...
[`unittest.main()`](../library/unittest.md#unittest.main) now takes an optional `exit` argument.  If
false, [`main()`](../library/unittest.md#unittest.main) doesn’t call [`sys.exit()`](../library/sys.md#sys.exit), allowing
[`main()`](../library/unittest.md#unittest.main) to be used from the interactive interpreter.
(Contributed by J. Pablo Fernández; [bpo-3379](https://bugs.python.org/issue?@action=redirect&bpo=3379).)

[`TestResult`](../library/unittest.md#unittest.TestResult) has new [`startTestRun()`](../library/unittest.md#unittest.TestResult.startTestRun) and
[`stopTestRun()`](../library/unittest.md#unittest.TestResult.stopTestRun) methods that are called immediately before
...
* The various parsing functions now take a *parser* keyword argument
  giving an [`XMLParser`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.XMLParser) instance that will
  ...
  Errors in parsing XML now raise a [`ParseError`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.ParseError) exception, whose
...
* ElementTree’s code for converting trees to a string has been
  ...
  cases.  The [`ElementTree.write()`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.ElementTree.write)
  ...
  you set the [`tag`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.Element.tag) attribute of an
  element to `None` but
  ...
  register new prefixes with [`register_namespace()`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.register_namespace).  In XML mode,
  you can use the true/false *xml_declaration* parameter to suppress the
  XML declaration.
* New [`Element`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.Element) method:
  [`extend()`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.Element.extend) appends the items from a
  ...
  ```
* New [`Element`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.Element) method:
  [`iter()`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.Element.iter) yields the children of the
...
* New [`Element`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.Element) method:
  [`itertext()`](../library/xml.etree.elementtree.md#xml.etree.ElementTree.Element.itertext) yields all chunks of
...
* Deprecated: using an element as a Boolean (i.e., `if elem:`) would
  ...
  [`FutureWarning`](../library/exceptions.md#FutureWarning).  In your code, you should be explicit: write
...
* [`Py_AddPendingCall()`](../c-api/threads.md#c.Py_AddPendingCall) is now thread-safe, letting any
...
* New function: [`PyCode_NewEmpty()`](../c-api/code.md#c.PyCode_NewEmpty) creates an empty code object;
...
* New function: [`PyErr_NewExceptionWithDoc()`](../c-api/exceptions.md#c.PyErr_NewExceptionWithDoc) creates a new
  exception class, just as the existing [`PyErr_NewException()`](../c-api/exceptions.md#c.PyErr_NewException) does,
...
* New function: [`PyFrame_GetLineNumber()`](../c-api/frame.md#c.PyFrame_GetLineNumber) takes a frame object
  and returns the line number that the frame is currently executing.
  Previously code would need to get the index of the bytecode
  instruction currently executing, and then look up the line number
  corresponding to that address.  (Added by Jeffrey Yasskin.)
* New functions: [`PyLong_AsLongAndOverflow()`](../c-api/long.md#c.PyLong_AsLongAndOverflow) and
  [`PyLong_AsLongLongAndOverflow()`](../c-api/long.md#c.PyLong_AsLongLongAndOverflow)  approximates a Python long
  integer as a C  or .
...
* New function: stemming from the rewrite of string-to-float conversion,
  a new [`PyOS_string_to_double()`](../c-api/conversion.md#c.PyOS_string_to_double) function was added.  The old
...
* New macros: the Python header files now define the following macros:
  [`Py_ISALNUM`](../c-api/conversion.md#c.Py_ISALNUM),
  [`Py_ISALPHA`](../c-api/conversion.md#c.Py_ISALPHA),
  [`Py_ISDIGIT`](../c-api/conversion.md#c.Py_ISDIGIT),
  [`Py_ISLOWER`](../c-api/conversion.md#c.Py_ISLOWER),
  [`Py_ISSPACE`](../c-api/conversion.md#c.Py_ISSPACE),
  [`Py_ISUPPER`](../c-api/conversion.md#c.Py_ISUPPER),
  [`Py_ISXDIGIT`](../c-api/conversion.md#c.Py_ISXDIGIT),
  [`Py_TOLOWER`](../c-api/conversion.md#c.Py_TOLOWER), and [`Py_TOUPPER`](../c-api/conversion.md#c.Py_TOUPPER).
...
* New format codes: the `PyString_FromFormat()`,
  `PyString_FromFormatV()`, and [`PyErr_Format()`](../c-api/exceptions.md#c.PyErr_Format) functions now
...
* The complicated interaction between threads and process forking has
  ...
  [`os.fork()`](../library/os.md#os.fork) might fail because the child is created with only a
  single thread running, the thread performing the [`os.fork()`](../library/os.md#os.fork).
  ...
  [`os.fork()`](../library/os.md#os.fork), and will also clean up any locks created using the
  [`threading`](../library/threading.md#module-threading) module.  C extension modules that have internal
...
* The [`Py_Finalize()`](../c-api/interp-lifecycle.md#c.Py_Finalize) function now calls the internal
...
* When using the [`PyMemberDef`](../c-api/structures.md#c.PyMemberDef) structure to define attributes
  of a type, Python will no longer let you try to delete or set a
  `T_STRING_INPLACE` attribute.
  <!-- rev 79644 -->
* Global symbols defined by the [`ctypes`](../library/ctypes.md#module-ctypes) module are now prefixed
...
* New configure option: the `--with-system-expat` switch allows
  building the [`pyexpat`](../library/pyexpat.md#module-xml.parsers.expat) module to use the system Expat library.
...
<a id="whatsnew27-capsules"></a>

### Capsules

Python 3.1 adds a new C datatype, [`PyCapsule`](../c-api/capsule.md#c.PyCapsule), for providing a
...
example, the [`socket`](../library/socket.md#module-socket) module’s API is exposed as `socket.CAPI`,
and [`unicodedata`](../library/unicodedata.md#module-unicodedata) exposes `ucnhash_CAPI`.  Other extensions
...
If a different capsule was passed in, [`PyCapsule_IsValid()`](../c-api/capsule.md#c.PyCapsule_IsValid) would
detect the mismatched name and return false.  Refer to
[Providing a C API for an Extension Module](../extending/extending.md#using-capsules) for more information on using these objects.
...
[`PendingDeprecationWarning`](../library/exceptions.md#PendingDeprecationWarning), which is silent by default.
...
* The [`msvcrt`](../library/msvcrt.md#module-msvcrt) module now contains some constants from
  the `crtassem.h` header file:
  [`CRT_ASSEMBLY_VERSION`](../library/msvcrt.md#msvcrt.CRT_ASSEMBLY_VERSION),
  [`VC_ASSEMBLY_PUBLICKEYTOKEN`](../library/msvcrt.md#msvcrt.VC_ASSEMBLY_PUBLICKEYTOKEN),
  and [`LIBRARIES_ASSEMBLY_NAME_PREFIX`](../library/msvcrt.md#msvcrt.LIBRARIES_ASSEMBLY_NAME_PREFIX).
  (Contributed by David Cournapeau; [bpo-4365](https://bugs.python.org/issue?@action=redirect&bpo=4365).)
* The [`_winreg`](../library/winreg.md#module-winreg) module for accessing the registry now implements
  the [`CreateKeyEx()`](../library/winreg.md#winreg.CreateKeyEx) and [`DeleteKeyEx()`](../library/winreg.md#winreg.DeleteKeyEx)
  functions, extended versions of previously supported functions that
  take several extra arguments.  The [`DisableReflectionKey()`](../library/winreg.md#winreg.DisableReflectionKey),
  [`EnableReflectionKey()`](../library/winreg.md#winreg.EnableReflectionKey), and [`QueryReflectionKey()`](../library/winreg.md#winreg.QueryReflectionKey)
...
* The [`os.kill()`](../library/os.md#os.kill) function now works on Windows.  The signal value
  can be the constants [`CTRL_C_EVENT`](../library/signal.md#signal.CTRL_C_EVENT),
  [`CTRL_BREAK_EVENT`](../library/signal.md#signal.CTRL_BREAK_EVENT), or any integer.  The first two constants
...
* The [`os.listdir()`](../library/os.md#os.listdir) function now correctly fails
  for an empty path.  (Fixed by Hirokazu Yamamoto; [bpo-5913](https://bugs.python.org/issue?@action=redirect&bpo=5913).)
* The [`mimetypes`](../library/mimetypes.md#module-mimetypes) module will now read the MIME database from
...
* FreeBSD 7.1’s `SO_SETFIB` constant, used with the [`socket()`](../library/socket.md#socket.socket) methods
  [`getsockopt()`](../library/socket.md#socket.socket.getsockopt)/[`setsockopt()`](../library/socket.md#socket.socket.setsockopt) to select an
  alternate routing table, is now available in the [`socket`](../library/socket.md#module-socket)
...
* Two benchmark scripts, `iobench` and `ccbench`, were
  ...
  speed of the built-in file I/O objects returned by [`open()`](../library/functions.md#open)
...
* When importing a module from a `.pyc` or `.pyo` file
  with an existing `.py` counterpart, the [`co_filename`](../reference/datamodel.md#codeobject.co_filename)
...
* The [`range()`](../library/stdtypes.md#range) function processes its arguments more
  consistently; it will now call [`__int__()`](../reference/datamodel.md#object.__int__) on non-float,
...
* The string [`format()`](../library/functions.md#format) method changed the default precision used
  for floating-point and complex numbers from 6 decimal
  places to 12, which matches the precision used by [`str()`](../library/stdtypes.md#str).
  (Changed by Eric Smith; [bpo-5920](https://bugs.python.org/issue?@action=redirect&bpo=5920).)
* Because of an optimization for the [`with`](../reference/compound_stmts.md#with) statement, the special
  methods [`__enter__()`](../reference/datamodel.md#object.__enter__) and [`__exit__()`](../reference/datamodel.md#object.__exit__) must belong to the object’s
  type, and cannot be directly attached to the object’s instance.  This
  affects new-style classes (derived from [`object`](../library/functions.md#object)) and C extension
  types.  ([bpo-6101](https://bugs.python.org/issue?@action=redirect&bpo=6101).)
* Due to a bug in Python 2.6, the *exc_value* parameter to
  [`__exit__()`](../reference/datamodel.md#object.__exit__) methods was often the string representation of the
...
* Operations with [`datetime`](../library/datetime.md#datetime.datetime) instances that resulted in a year
  falling outside the supported range didn’t always raise
  [`OverflowError`](../library/exceptions.md#OverflowError).  Such errors are now checked more carefully
...
* When using [`Decimal`](../library/decimal.md#decimal.Decimal) instances with a string’s
  [`format()`](../library/functions.md#format) method, the default alignment was previously
...
* The [`xml.etree.ElementTree`](../library/xml.etree.elementtree.md#module-xml.etree.ElementTree) library no longer escapes
  ...
  or comment (which looks like `<!-- comment -->`).
  (Patch by Neil Muller; [bpo-2746](https://bugs.python.org/issue?@action=redirect&bpo=2746).)
* The `readline()` method of [`StringIO`](../library/io.md#io.StringIO) objects now does
...
* C extensions that use integer format codes with the `PyArg_Parse*`
  family of functions will now raise a [`TypeError`](../library/exceptions.md#TypeError) exception
  instead of triggering a [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning) ([bpo-5080](https://bugs.python.org/issue?@action=redirect&bpo=5080)).
* Use the new [`PyOS_string_to_double()`](../c-api/conversion.md#c.PyOS_string_to_double) function instead of the old
  `PyOS_ascii_strtod()` and `PyOS_ascii_atof()` functions,
...
<a id="py27-maintenance-enhancements"></a>
...
* [`hmac.compare_digest()`](../library/hmac.md#hmac.compare_digest) was backported from Python 3 to make a timing
...
* [`hashlib.pbkdf2_hmac()`](../library/hashlib.md#hashlib.pbkdf2_hmac) was backported from Python 3 to make a hashing
...
* Most of Python 3.4’s [`ssl`](../library/ssl.md#module-ssl) module was backported. This means [`ssl`](../library/ssl.md#module-ssl)
  now supports Server Name Indication, TLS1.x settings, access to the platform
  certificate store, the [`SSLContext`](../library/ssl.md#ssl.SSLContext) class, and other
  ...
  specific details.
* [`os.urandom()`](../library/os.md#os.urandom) was changed to cache a file descriptor to `/dev/urandom`
...
* [`hashlib.algorithms_guaranteed`](../library/hashlib.md#hashlib.algorithms_guaranteed) and
  [`hashlib.algorithms_available`](../library/hashlib.md#hashlib.algorithms_available) were backported from Python 3 to make
...
[**PEP 476**](https://peps.python.org/pep-0476/) updated [`httplib`](../library/http.md#module-http) and modules which use it, such as
[`urllib2`](../library/urllib.request.md#module-urllib.request) and [`xmlrpclib`](../library/xmlrpc.client.md#module-xmlrpc.client), to now
