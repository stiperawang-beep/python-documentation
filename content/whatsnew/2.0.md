...
Python’s project page, located at [https://sourceforge.net/projects/python/](https://sourceforge.net/projects/python/).
...
the proposal.  Quoting from the introduction to [**PEP 1**](https://peps.python.org/pep-0001/), “PEP Purpose and
Guidelines”:
...
Read the rest of [**PEP 1**](https://peps.python.org/pep-0001/) for the details of the PEP editorial process, style, and
format.  PEPs are kept in the Python CVS tree on SourceForge, though they’re not
part of the Python 2.0 distribution, and are also available in HTML form from
[https://peps.python.org/](https://peps.python.org/).  As of September 2000, there are 25 PEPs, ranging
from [**PEP 201**](https://peps.python.org/pep-0201/), “Lockstep Iteration”, to PEP 225, “Elementwise/Objectwise
Operators”.
...
Lundh.  A detailed explanation of the interface was written up as [**PEP 100**](https://peps.python.org/pep-0100/),
...
Unicode characters can be written using a new escape sequence, `\u*HHHH*`, where
*HHHH* is a 4-digit hexadecimal number from 0000 to FFFF.  The existing
`\x*HH*` escape sequence can also be used, and octal escapes can be used for
...
A new module, [`unicodedata`](../library/unicodedata.md#module-unicodedata), provides an interface to Unicode character
...
The [`codecs`](../library/codecs.md#module-codecs) module contains functions to look up existing encodings and
...
Unicode-aware regular expressions are available through the [`re`](../library/re.md#module-re) module,
...
The existing [`map()`](../library/functions.md#map) and [`filter()`](../library/functions.md#filter) functions can be used for this
...
anonymous function created by the [`lambda`](../reference/expressions.md#lambda) expression knows what
...
language Haskell ([https://www.haskell.org](https://www.haskell.org)).  Greg Ewing argued most effectively
...
Until now string-manipulation functionality was in the [`string`](../library/string.md#module-string) module,
...
The old [`string`](../library/string.md#module-string) module is still around for backwards compatibility, but it
...
and is equivalent to the `string.join()` function from the old [`string`](../library/string.md#module-string)
...
A new [`gc`](../library/gc.md#module-gc) module provides functions to perform a garbage collection,
...
the [`repr()`](../library/functions.md#repr) of its argument.  This was also added from symmetry
considerations, this time for symmetry with the existing ‘%s’ format style,
which inserts the [`str()`](../library/stdtypes.md#str) of its argument.  For example, `'%r %s' % ('abc',
'abc')` returns a string containing `'abc' abc`.

Previously there was no way to implement a class that overrode Python’s built-in
[`in`](../reference/expressions.md#in) operator and implemented a custom version.  `obj in seq` returns
...
[`IndexError`](../library/exceptions.md#IndexError) is encountered.  Moshe Zadka contributed a patch which adds a
...
Python CE page at [https://pythonce.sourceforge.net/](https://pythonce.sourceforge.net/) for more information.
...
[`NameError`](../library/exceptions.md#NameError) exception when code refers to a local variable before the
...
[`NameError`](../library/exceptions.md#NameError) exception is raised, while 2.0 raises a new
[`UnboundLocalError`](../library/exceptions.md#UnboundLocalError) exception. [`UnboundLocalError`](../library/exceptions.md#UnboundLocalError) is a subclass of
[`NameError`](../library/exceptions.md#NameError), so any existing code that expects [`NameError`](../library/exceptions.md#NameError) to be
...
Two new exceptions, [`TabError`](../library/exceptions.md#TabError) and [`IndentationError`](../library/exceptions.md#IndentationError), have been
introduced.  They’re both subclasses of [`SyntaxError`](../library/exceptions.md#SyntaxError), and are raised when
Python code is found to be improperly indented.

### Changes to Built-in Functions

A new built-in, `zip(seq1, seq2, ...)`, has been added.  [`zip()`](../library/functions.md#zip)
returns a list of tuples where each tuple contains the i-th element from each of
the argument sequences.  The difference between [`zip()`](../library/functions.md#zip) and `map(None,
seq1, seq2)` is that [`map()`](../library/functions.md#map) pads the sequences with `None` if the
sequences aren’t all of the same length, while [`zip()`](../library/functions.md#zip) truncates the
returned list to the length of the shortest argument sequence.

The [`int()`](../library/functions.md#int) and `long()` functions now accept an optional “base”
parameter when the first argument is a string. `int('123', 10)` returns 123,
while `int('123', 16)` returns 291.  `int(123, 16)` raises a
[`TypeError`](../library/exceptions.md#TypeError) exception with the message “can’t convert non-string with
explicit base”.

A new variable holding more detailed version information has been added to the
[`sys`](../library/sys.md#module-sys) module.  `sys.version_info` is a tuple `(major, minor, micro,
...
recursion depth can be read and modified using [`sys.getrecursionlimit()`](../library/sys.md#sys.getrecursionlimit) and
[`sys.setrecursionlimit()`](../library/sys.md#sys.setrecursionlimit). The default value is 1000, and a rough maximum
...
[`append()`](../library/stdtypes.md#list.append) and [`insert()`](../library/stdtypes.md#list.insert).
In earlier versions of Python, if `L` is
a list, `L.append( 1,2 )` appends the tuple `(1,2)` to the list.  In Python
2.0 this causes a [`TypeError`](../library/exceptions.md#TypeError) exception to be raised, with the message:
...
them to use [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple), the current argument parsing function,
...
Some of the functions in the [`socket`](../library/socket.md#module-socket) module are still forgiving in this
...
also works. [`socket.connect_ex`](../library/socket.md#socket.socket.connect_ex) and [`socket.bind`](../library/socket.md#socket.socket.bind)
...
the changes in the face of public reaction, so for the [`socket`](../library/socket.md#module-socket) module, the
...
The [`AttributeError`](../library/exceptions.md#AttributeError) and [`NameError`](../library/exceptions.md#NameError) exceptions have a more friendly
...
or slice a string, but this raised a [`TypeError`](../library/exceptions.md#TypeError).  In 2.0, long integers
...
The subtlest long integer change of all is that the [`str()`](../library/stdtypes.md#str) of a long
integer no longer has a trailing ‘L’ character, though [`repr()`](../library/functions.md#repr) still
...
Taking the [`repr()`](../library/functions.md#repr) of a float now uses a different formatting precision
than [`str()`](../library/stdtypes.md#str).  [`repr()`](../library/functions.md#repr) uses `%.17g` format string for C’s
`sprintf()`, while [`str()`](../library/stdtypes.md#str) uses `%.12g` as before.  The effect is that
[`repr()`](../library/functions.md#repr) may occasionally show more decimal places than  [`str()`](../library/stdtypes.md#str), for
...
[`isinstance()`](../library/functions.md#isinstance) and [`issubclass()`](../library/functions.md#issubclass). This means you no longer have to
...
dictionary at module initialization time were added: [`PyModule_AddObject()`](../c-api/module.md#c.PyModule_AddObject),
[`PyModule_AddIntConstant()`](../c-api/module.md#c.PyModule_AddIntConstant), and [`PyModule_AddStringConstant()`](../c-api/module.md#c.PyModule_AddStringConstant).  Each
...
A wrapper API was added for Unix-style signal handlers. [`PyOS_getsig()`](../c-api/sys.md#c.PyOS_getsig) gets
a signal handler and [`PyOS_setsig()`](../c-api/sys.md#c.PyOS_setsig) will set a new handler.
...
DOM interface as part of the [`xml`](../library/xml.md#module-xml) package. Here we will give a brief
...
XML parser.  For example, the [`startElement()`](../library/xml.sax.handler.md#xml.sax.handler.ContentHandler.startElement) and [`endElement()`](../library/xml.sax.handler.md#xml.sax.handler.ContentHandler.endElement)
methods are called for every starting and end tag encountered by the parser, the
[`characters()`](../library/xml.sax.handler.md#xml.sax.handler.ContentHandler.characters) method is called for every chunk of character data, and so
forth.
...
[https://pyxml.sourceforge.net/topics/howto/xml-howto.html](https://pyxml.sourceforge.net/topics/howto/xml-howto.html).
...
The DOM implementation included with Python lives in the [`xml.dom.minidom`](../library/xml.dom.minidom.md#module-xml.dom.minidom)
...
pages at [https://www.python.org/community/sigs/current/xml-sig](https://www.python.org/community/sigs/current/xml-sig). The PyXML distribution also used
the package name `xml`.  If you’ve written programs that used PyXML, you’re
probably wondering about its compatibility with the 2.0 [`xml`](../library/xml.md#module-xml) package.

The answer is that Python 2.0’s [`xml`](../library/xml.md#module-xml) package isn’t compatible with PyXML,
...
[`xml`](../library/xml.md#module-xml) package shipped with Python, and will be a strict superset of the
...
library; some of the affected modules include [`readline`](../library/readline.md#module-readline),
[`ConfigParser`](../library/configparser.md#module-configparser), `cgi`, [`calendar`](../library/calendar.md#module-calendar), [`posix`](../library/posix.md#module-posix), [`readline`](../library/readline.md#module-readline),
`xmllib`, `aifc`, `chunk`, [`wave`](../library/wave.md#module-wave), [`random`](../library/random.md#module-random), [`shelve`](../library/shelve.md#module-shelve),
and `nntplib`.  Consult the CVS logs for the exact patch-by-patch details.

Brian Gallew contributed OpenSSL support for the [`socket`](../library/socket.md#module-socket) module.  OpenSSL
...
to include SSL support, which adds an additional function to the [`socket`](../library/socket.md#module-socket)
module: `socket.ssl(socket, keyfile, certfile)`, which takes a socket
object and returns an SSL socket.  The [`httplib`](../library/http.md#module-http) and [`urllib`](../library/urllib.md#module-urllib) modules
were also changed to support `https://` URLs, though no one has implemented
FTP or SMTP over SSL.

The [`httplib`](../library/http.md#module-http) module has been rewritten by Greg Stein to support HTTP/1.1.
...
The [`curses`](../library/curses.md#module-curses) module has been greatly extended, starting from Oliver
...
implementation of the regular expressions provided by the [`re`](../library/re.md#module-re) module has
...
* [`atexit`](../library/atexit.md#module-atexit):  For registering functions to be called before the Python
  interpreter exits. Code that currently sets `sys.exitfunc` directly should be
  changed to  use the [`atexit`](../library/atexit.md#module-atexit) module instead, importing [`atexit`](../library/atexit.md#module-atexit) and
  calling [`atexit.register()`](../library/atexit.md#atexit.register) with  the function to be called on exit.
  (Contributed by Skip Montanaro.)
* [`codecs`](../library/codecs.md#module-codecs), `encodings`, [`unicodedata`](../library/unicodedata.md#module-unicodedata):  Added as part of the new
  Unicode support.
* [`filecmp`](../library/filecmp.md#module-filecmp): Supersedes the old `cmp`, `cmpcache` and
  `dircmp` modules, which have now become deprecated. (Contributed by Gordon
  MacMillan and Moshe Zadka.)
* [`gettext`](../library/gettext.md#module-gettext): This module provides internationalization (I18N) and
...
* [`mmap`](../library/mmap.md#module-mmap): An interface to memory-mapped files on both Windows and Unix.  A
  ...
  be passed to functions that expect ordinary strings, such as the [`re`](../library/re.md#module-re)
...
* `pyexpat`: An interface to the Expat XML parser. (Contributed by Paul
  Prescod.)
* [`robotparser`](../library/urllib.robotparser.md#module-urllib.robotparser): Parse a `robots.txt` file, which is used for writing
  ...
  by Skip Montanaro.)
* [`tabnanny`](../library/tabnanny.md#module-tabnanny): A module/script to  check Python source code for ambiguous
  indentation. (Contributed by Tim Peters.)
* `UserString`: A base class useful for deriving objects that behave like
  strings.
* [`webbrowser`](../library/webbrowser.md#module-webbrowser): A module that provides a platform independent way to launch
  ...
  *BROWSER* environment variable.  (Originally inspired by Eric S. Raymond’s patch
  to [`urllib`](../library/urllib.md#module-urllib) which added similar functionality, but the final module comes
  from code originally  implemented by Fred Drake as
  `Tools/idle/BrowserControl.py`, and adapted for the standard library by
  Fred.)
* [`_winreg`](../library/winreg.md#module-winreg): An interface to the Windows registry.  `_winreg` is an
...
* [`zipfile`](../library/zipfile.md#module-zipfile): A module for reading and writing ZIP-format archives.  These
  are archives produced by **PKZIP** on DOS/Windows or **zip** on
  Unix, not to be confused with **gzip**-format files (which are
  supported by the [`gzip`](../library/gzip.md#module-gzip) module) (Contributed by James C. Ahlstrom.)
