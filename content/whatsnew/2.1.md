...
In code which uses [`lambda`](../reference/expressions.md#lambda) you can often find local variables being
...
## PEP 236: \_\_future_\_ Directives
...
[`__future__`](../library/__future__.md#module-__future__).  Nested scopes can be enabled by the following statement:

```python3
from __future__ import nested_scopes
```

While it looks like a normal [`import`](../reference/simple_stmts.md#import) statement, it’s not; there are
...
#### SEE ALSO
[**PEP 236**](https://peps.python.org/pep-0236/) - Back to the [`__future__`](../library/__future__.md#module-__future__)
...
|-------------|-------------------------------------------------------|
| `<`         | [`__lt__()`](../reference/datamodel.md#object.__lt__) |
| `<=`        | [`__le__()`](../reference/datamodel.md#object.__le__) |
| `>`         | [`__gt__()`](../reference/datamodel.md#object.__gt__) |
| `>=`        | [`__ge__()`](../reference/datamodel.md#object.__ge__) |
| `==`        | [`__eq__()`](../reference/datamodel.md#object.__eq__) |
| `!=`        | [`__ne__()`](../reference/datamodel.md#object.__ne__) |
...
[`warnings`](../library/warnings.md#module-warnings) module that provide functions to issue warnings, and to filter
...
Warnings can be issued by calling the [`warnings.warn()`](../library/warnings.md#warnings.warn) function:
...
and not want to spare the time to convert it to use the [`re`](../library/re.md#module-re) module right
...
[`DeprecationWarning`](../library/exceptions.md#DeprecationWarning) triggered in the [`__main__`](../library/__main__.md#module-__main__) module, and applies
...
Weak references, available through the [`weakref`](../library/weakref.md#module-weakref) module, are a minor but
...
The [`weakref`](../library/weakref.md#module-weakref) module also allows creating proxy objects which behave like
...
methods, because the [`__doc__`](../reference/datamodel.md#function.__doc__) attribute was the only way of
attaching any
...
[`__dict__`](../reference/datamodel.md#function.__dict__). Unlike the [`__dict__`](../reference/datamodel.md#type.__dict__) attribute of class instances, in
functions you can actually assign a new dictionary to [`__dict__`](../reference/datamodel.md#function.__dict__), though
...
In Python 2.1, the [`import`](../reference/simple_stmts.md#import) statement will work to simulate case-sensitivity
on case-insensitive platforms.  Python will now search for the first
case-sensitive match by default, raising an [`ImportError`](../library/exceptions.md#ImportError) if no such file
is found, so `import file` will not import a module named `FILE.PY`.
Case-insensitive matching can be requested by setting the [`PYTHONCASEOK`](../using/cmdline.md#envvar-PYTHONCASEOK)
...
displayed using the built-in [`repr()`](../library/functions.md#repr) function. In Python 2.1, the variable
[`sys.displayhook()`](../library/sys.md#sys.displayhook) can be set to a callable object which will be called
instead of [`repr()`](../library/functions.md#repr). For example, you can set it to a special
...
a [`TypeError`](../library/exceptions.md#TypeError) will be raised.  Numeric methods written in Python can also
return `Py_NotImplemented`, causing the interpreter to act as if the method
did not exist (perhaps raising a [`TypeError`](../library/exceptions.md#TypeError), perhaps trying another
...
Internet Archive Wayback Machine](https://web.archive.org/web/20090130140102/http://www.vex.net/parnassus/))
...
version, and author (metadata, in cataloguing terminology).  [**PEP 241**](https://peps.python.org/pep-0241/) contains
...
described in [**PEP 241**](https://peps.python.org/pep-0241/), as well as various bugfixes and enhancements.  It will be
available from the Distutils SIG at [https://www.python.org/community/sigs/current/distutils-sig/](https://www.python.org/community/sigs/current/distutils-sig/).
...
* Two different modules for unit testing were added to the standard library.
  The [`doctest`](../library/doctest.md#module-doctest) module, contributed by Tim Peters, provides a testing
  ...
  Kent Beck’s Smalltalk testing framework.  See [https://pyunit.sourceforge.net/](https://pyunit.sourceforge.net/) for
  more information about PyUnit.
* The [`difflib`](../library/difflib.md#module-difflib) module contains a class, [`SequenceMatcher`](../library/difflib.md#difflib.SequenceMatcher), which
...
* [`curses.panel`](../library/curses.panel.md#module-curses.panel), a wrapper for the panel library, part of ncurses and of
...
* The PyXML package has gone through a few releases since Python 2.0, and Python
  2.1 includes an updated version of the [`xml`](../library/xml.md#module-xml) package.  Some of the
...
* Ping also contributed another hook for handling uncaught exceptions.
  [`sys.excepthook()`](../library/sys.md#sys.excepthook) can be set to a callable object.  When an exception isn’t
  caught by any [`try`](../reference/compound_stmts.md#try)…[`except`](../reference/compound_stmts.md#except) blocks, the exception will be
  passed to [`sys.excepthook()`](../library/sys.md#sys.excepthook), which can then do whatever it likes.  At the
...
* Various functions in the [`time`](../library/time.md#module-time) module, such as [`asctime()`](../library/time.md#time.asctime) and
  [`localtime()`](../library/time.md#time.localtime), require a floating-point argument containing the time in
...
* The [`ftplib`](../library/ftplib.md#module-ftplib) module now defaults to retrieving files in passive mode,
  ...
  use [`ftplib`](../library/ftplib.md#module-ftplib) to retrieve files and then don’t work from behind a firewall.
...
* Support for raw socket access has been added to the [`socket`](../library/socket.md#module-socket) module,
  contributed by Grant Edwards.
* The [`pstats`](../library/pstats.md#module-pstats) module now contains a simple interactive statistics browser
...
* A new implementation-dependent function, `sys._getframe([depth])`, has
  ...
  [`sys._getframe()`](../library/sys.md#sys._getframe) returns the frame at the top of the call stack;  if the
...
* A specialized object allocator is now optionally available, that should be
  ...
  example, if memory was allocated using [`PyMem_New`](../c-api/memory.md#c.PyMem_New), it has to be freed
  using [`PyMem_Del()`](../c-api/memory.md#c.PyMem_Del), not `free()`.  A few modules included with Python
...
* The speed of line-oriented file I/O has been improved because people often
  ...
  benchmark.  The [`readline()`](../library/readline.md#module-readline) method of file objects has therefore been
  ...
  January 1–15, 2001 at [https://mail.python.org/pipermail/python-dev/2001-January/](https://mail.python.org/pipermail/python-dev/2001-January/).
* A new method, [`popitem()`](../library/stdtypes.md#dict.popitem), was added to dictionaries to enable
...
* Modules can now control which names are imported when `from module import *`
  ...
  modules such as [`sys`](../library/sys.md#module-sys) or [`string`](../library/string.md#module-string), `from module import *` will add
  ...
  checked in.
* Applying [`repr()`](../library/functions.md#repr) to strings previously used octal escapes for
...
* C extensions which import other modules have been changed to use
  [`PyImport_ImportModule()`](../c-api/import.md#c.PyImport_ImportModule), which means that they will use any import hooks
