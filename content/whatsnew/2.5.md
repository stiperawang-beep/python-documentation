...
Python 2.5 is scheduled for August 2006; [**PEP 356**](https://peps.python.org/pep-0356/) describes the planned
...
the SQLite database module (`sqlite`), and the [`ctypes`](../library/ctypes.md#module-ctypes)
...
see section [PEP 308: Conditional Expressions](#pep-308).  The new ‘[`with`](../reference/compound_stmts.md#with)’ statement will make
writing cleanup code easier (section [PEP 343: The ‘with’ statement](#pep-343)).  Values can now be passed
into generators (section [PEP 342: New Generator Features](#pep-342)).  Imports are now visible as either
absolute or relative (section [PEP 328: Absolute and Relative Imports](#pep-328)).  Some corner cases of exception
handling are handled better (section [PEP 341: Unified try/except/finally](#pep-341)).  All these improvements
...
[https://docs.python.org](https://docs.python.org). If you want to understand the complete implementation
...
See [**PEP 308**](https://peps.python.org/pep-0308/) for some examples.  If you put parentheses around your
...
The [`functools`](../library/functools.md#module-functools) module is intended to contain tools for functional-style
programming.
...
Another function in the [`functools`](../library/functools.md#module-functools) module is the
...
[https://pypi.org](https://pypi.org) is storing source and binary archives for a
...
The simpler part of [**PEP 328**](https://peps.python.org/pep-0328/) was implemented in Python 2.4: parentheses could now
...
Python’s standard [`string`](../library/string.md#module-string) module?  There’s no clean way to ignore
...
reader may be confused about which module, [`string`](../library/string.md#module-string) or `pkg.string`,
...
In Python 2.5, you can switch [`import`](../reference/simple_stmts.md#import)’s behaviour to  absolute imports
...
This imports the [`string`](../library/string.md#module-string) module relative to the current package, so in
...
[https://pylib.readthedocs.io/](https://pylib.readthedocs.io/)
...
The [`-m`](../using/cmdline.md#cmdoption-m) switch added in Python 2.4 to execute a module as a script
...
[`runpy`](../library/runpy.md#module-runpy).

The [`runpy`](../library/runpy.md#module-runpy) module implements a more sophisticated import mechanism so that
...
[`zipimport`](../library/zipimport.md#module-zipimport) module.  This means you can add a .zip archive’s path to
`sys.path` and then use the [`-m`](../using/cmdline.md#cmdoption-m) switch to execute code from the
archive.
...
Until Python 2.5, the [`try`](../reference/compound_stmts.md#try) statement came in two flavours. You could
use a [`finally`](../reference/compound_stmts.md#finally) block to ensure that code is always executed, or one or
more [`except`](../reference/compound_stmts.md#except) blocks to catch  specific exceptions.  You couldn’t
...
equivalent of combining [`except`](../reference/compound_stmts.md#except) blocks and a [`finally`](../reference/compound_stmts.md#finally) block,
...
[`except`](../reference/compound_stmts.md#except) blocks are tested: if the exception is of class
...
from 0 up to 9.  On encountering the [`yield`](../reference/simple_stmts.md#yield) statement, the iterator
...
[`next()`](../library/functions.md#next) method, picking up after the `yield` statement.

In Python 2.3, [`yield`](../reference/simple_stmts.md#yield) was a statement; it didn’t return any value.  In
...
I recommend that you always put parentheses around a [`yield`](../reference/simple_stmts.md#yield) expression
when you’re doing something with the returned value, as in the above example.
The parentheses aren’t always necessary, but it’s easier to always add them
instead of having to remember when they’re needed.

([**PEP 342**](https://peps.python.org/pep-0342/) explains the exact rules, which are that a
[`yield`](../reference/simple_stmts.md#yield)-expression must always be parenthesized except when it
...
generator’s code is then resumed and the [`yield`](../reference/simple_stmts.md#yield) expression returns the
specified *value*.  If the regular [`next()`](../library/functions.md#next) method is called, the
`yield` returns [`None`](../library/constants.md#None).
...
[`yield`](../reference/simple_stmts.md#yield) will usually return [`None`](../library/constants.md#None), so you should always check
...
* `throw(type, value=None, traceback=None)` is used to raise an exception
  inside the generator; the exception is raised by the [`yield`](../reference/simple_stmts.md#yield) expression
  where the generator’s execution is paused.
* `close()` raises a new [`GeneratorExit`](../library/exceptions.md#GeneratorExit) exception inside the generator
  to terminate the iteration.  On receiving this exception, the generator’s code
  must either raise [`GeneratorExit`](../library/exceptions.md#GeneratorExit) or [`StopIteration`](../library/exceptions.md#StopIteration).  Catching the
  [`GeneratorExit`](../library/exceptions.md#GeneratorExit) exception and returning a value is illegal and will trigger
  a [`RuntimeError`](../library/exceptions.md#RuntimeError); if the function raises some other exception, that
...
function, and a [`return`](../reference/simple_stmts.md#return) statement), but coroutines can be entered,
exited, and resumed at many different points (the [`yield`](../reference/simple_stmts.md#yield) statements).
...
be guaranteed to work; the [`finally`](../reference/compound_stmts.md#finally) clause will now always get a
chance to run.  The syntactic restriction that you couldn’t mix [`yield`](../reference/simple_stmts.md#yield)
...
[`with`](../reference/compound_stmts.md#with) statement described by [**PEP 343**](https://peps.python.org/pep-0343/).  I’ll look at this new statement
...
: PEP written by  Guido van Rossum and Phillip J. Eby; implemented by Phillip J.
  ...
  <br/>
  Earlier versions of these features were proposed in  [**PEP 288**](https://peps.python.org/pep-0288/) by Raymond
  Hettinger and [**PEP 325**](https://peps.python.org/pep-0325/) by Samuele Pedroni.

[https://en.wikipedia.org/wiki/Coroutine](https://en.wikipedia.org/wiki/Coroutine)
: The Wikipedia entry for  coroutines.

[https://web.archive.org/web/20160321211320/http://www.sidhe.org/~dan/blog/archives/000178.html](https://web.archive.org/web/20160321211320/http://www.sidhe.org/~dan/blog/archives/000178.html)
...
The ‘[`with`](../reference/compound_stmts.md#with)’ statement is a new control-flow structure whose basic
structure is:
...
The new `localcontext()` function in the [`decimal`](../library/decimal.md#module-decimal) module makes it easy
...
<a id="new-25-context-managers"></a>
...
* The expression is evaluated and should result in an object called a “context
  manager”.  The context manager must have [`__enter__()`](../reference/datamodel.md#object.__enter__) and [`__exit__()`](../reference/datamodel.md#object.__exit__)
  methods.
* The context manager’s [`__enter__()`](../reference/datamodel.md#object.__enter__) method is called.  The value returned
  is assigned to *VAR*.  If no `'as VAR'` clause is present, the value is simply
  discarded.
* The code in *BLOCK* is executed.
* If *BLOCK* raises an exception, the `__exit__(type, value, traceback)`
  is called with the exception details, the same values returned by
  [`sys.exc_info()`](../library/sys.md#sys.exc_info).  The method’s return value controls whether the exception
...
<a id="contextlibmod"></a>

### The contextlib module

The new [`contextlib`](../library/contextlib.md#module-contextlib) module provides some functions and a decorator that
are useful for writing objects for use with the ‘[`with`](../reference/compound_stmts.md#with)’ statement.

The decorator is called `contextmanager()`, and lets you write a single
generator function instead of defining a new class.  The generator should yield
exactly one value.  The code up to the [`yield`](../reference/simple_stmts.md#yield) will be executed as the
[`__enter__()`](../reference/datamodel.md#object.__enter__) method, and the value yielded will be the method’s return
value that will get bound to the variable in the ‘[`with`](../reference/compound_stmts.md#with)’ statement’s
`as` clause, if any.  The code after the [`yield`](../reference/simple_stmts.md#yield) will be
executed in the [`__exit__()`](../reference/datamodel.md#object.__exit__) method.  Any exception raised in the block will
...
the built-in [`Exception`](../library/exceptions.md#Exception) class and all the standard built-in exceptions
([`NameError`](../library/exceptions.md#NameError), [`ValueError`](../library/exceptions.md#ValueError), etc.) are now new-style classes.
...
that indicate program errors.  [`KeyboardInterrupt`](../library/exceptions.md#KeyboardInterrupt) and [`SystemExit`](../library/exceptions.md#SystemExit)
aren’t errors, though, and usually represent an explicit action such as the user
hitting `Control`-`C` or code calling [`sys.exit()`](../library/sys.md#sys.exit).  A bare `except:` will
catch all exceptions, so you commonly need to list [`KeyboardInterrupt`](../library/exceptions.md#KeyboardInterrupt) and
[`SystemExit`](../library/exceptions.md#SystemExit) in order to re-raise them.  The usual pattern is:
...
[`KeyboardInterrupt`](../library/exceptions.md#KeyboardInterrupt) and [`SystemExit`](../library/exceptions.md#SystemExit) alone.  As in previous versions,
...
from [`BaseException`](../library/exceptions.md#BaseException) or some descendant of [`BaseException`](../library/exceptions.md#BaseException), and future
...
[`Exception`](../library/exceptions.md#Exception) now.  It’s been suggested that the bare `except:` form should
...
A wide-ranging change to Python’s C API, using a new  [`Py_ssize_t`](../c-api/intro.md#c.Py_ssize_t) type
definition instead of ,  will permit the interpreter to handle more
data on 64-bit platforms. This change doesn’t affect Python’s capacity on 32-bit
platforms.

Various pieces of the Python interpreter used C’s  type to store
sizes or counts; for example, the number of items in a list or tuple were stored
in an .  The C compilers for most 64-bit platforms still define
...
[https://unix.org/version2/whatsnew/lp64_wp.html](https://unix.org/version2/whatsnew/lp64_wp.html) for a discussion – but the
most commonly available model leaves  as 32 bits.)
...
[`PyObject`](../c-api/structures.md#c.PyObject) representing the item.  2147483647\*4 is already more bytes
...
[`Py_ssize_t`](../c-api/intro.md#c.Py_ssize_t) to store their size.   Functions such as
[`PyList_Size()`](../c-api/list.md#c.PyList_Size)  now return [`Py_ssize_t`](../c-api/intro.md#c.Py_ssize_t).  Code in extension modules
may therefore need to have some variables changed to [`Py_ssize_t`](../c-api/intro.md#c.Py_ssize_t).

The [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple) and [`Py_BuildValue()`](../c-api/arg.md#c.Py_BuildValue) functions have a new
conversion code, `n`, for [`Py_ssize_t`](../c-api/intro.md#c.Py_ssize_t).   [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple)’s
`s#` and `t#` still output  by default, but you can define the
macro  `PY_SSIZE_T_CLEAN` before including `Python.h`  to make
them return [`Py_ssize_t`](../c-api/intro.md#c.Py_ssize_t).

[**PEP 353**](https://peps.python.org/pep-0353/) has a section on conversion guidelines that  extension authors should
...
## PEP 357: The ‘_\_index_\_’ method
...
[`TypeError`](../library/exceptions.md#TypeError) if this requirement isn’t met.

A corresponding [`nb_index`](../c-api/typeobj.md#c.PyNumberMethods.nb_index) slot was added to the C-level
[`PyNumberMethods`](../c-api/typeobj.md#c.PyNumberMethods) structure to let C extensions implement this protocol.
...
* The [`dict`](../library/stdtypes.md#dict) type has a new hook for letting subclasses provide a default
  ...
  to implement the new `defaultdict` class in the [`collections`](../library/collections.md#module-collections)
...
* The `startswith()` and `endswith()` methods of string types now accept
  ...
  <!-- RFE #1491485 -->
* The [`min()`](../library/functions.md#min) and [`max()`](../library/functions.md#max) built-in functions gained a `key` keyword
  ...
  in the list; [`min()`](../library/functions.md#min)/[`max()`](../library/functions.md#max) will return the element with the
...
* Two new built-in functions, [`any()`](../library/functions.md#any) and [`all()`](../library/functions.md#all), evaluate whether an
  iterator contains any true or false values.  [`any()`](../library/functions.md#any) returns [`True`](../library/constants.md#True)
  if any value returned by the iterator is true; otherwise it will return
  [`False`](../library/constants.md#False).  [`all()`](../library/functions.md#all) returns [`True`](../library/constants.md#True) only if all of the values
...
* The result of a class’s `__hash__()` method can now be either a long
  ...
  regular integer, but in 2.5 the [`id()`](../library/functions.md#id) built-in was changed to always
...
* ASCII is now the default encoding for modules.  It’s now  a syntax error if a
  ...
  error.  See [**PEP 263**](https://peps.python.org/pep-0263/)  for how to declare a module’s encoding; for example, you
  might add  a line like this near the top of the source file:
  ```python3
  # -*- coding: latin1 -*-
  ```
* A new warning, [`UnicodeWarning`](../library/exceptions.md#UnicodeWarning), is triggered when  you attempt to
  ...
  Previously this would raise a [`UnicodeDecodeError`](../library/exceptions.md#UnicodeDecodeError) exception, but in 2.5
  ...
  [`UnicodeDecodeError`](../library/exceptions.md#UnicodeDecodeError) exception.  Other changes in 2.5 resulted in this
  ...
  might have broken code, so instead  [`UnicodeWarning`](../library/exceptions.md#UnicodeWarning) was introduced.
...
* One error that Python programmers sometimes make is forgetting to include an
  ...
  confusing, and usually requires running Python with the [`-v`](../using/cmdline.md#cmdoption-v) switch to
  log all the paths searched. In Python 2.5, a new [`ImportWarning`](../library/exceptions.md#ImportWarning) warning is
  ...
  provide the [`-Wd`](../using/cmdline.md#cmdoption-W) option when running the Python executable to display
...
The Python executable now accepts the standard long options  [`--help`](../using/cmdline.md#cmdoption-help)
and [`--version`](../using/cmdline.md#cmdoption-version); on Windows,  it also accepts the [`/?`](../using/cmdline.md#cmdoption-0) option
...
* When they were introduced  in Python 2.4, the built-in [`set`](../library/stdtypes.md#set) and
  [`frozenset`](../library/stdtypes.md#frozenset) types were built on top of Python’s dictionary type.   In 2.5
...
* It’s now illegal to mix iterating over a file  with `for line in file` and
  calling  the file object’s `read()`/[`readline()`](../library/readline.md#module-readline)/`readlines()`
  ...
  methods will now trigger a [`ValueError`](../library/exceptions.md#ValueError) from the `read*()` method.
  (Implemented by Thomas Wouters.)
  <!-- Patch 1397960 -->
* The [`struct`](../library/struct.md#module-struct) module now compiles structure format  strings into an
...
* The [`re`](../library/re.md#module-re) module got a 1 or 2% speedup by switching to  Python’s allocator
...
* The [`codecs`](../library/codecs.md#module-codecs) module gained support for incremental codecs.  The
  ...
  entire input was fed to the non-incremental codec. See the [`codecs`](../library/codecs.md#module-codecs) module
  documentation for details. (Designed and implemented by Walter Dörwald.)
  <!-- Patch  1436130 -->
* The [`collections`](../library/collections.md#module-collections) module gained a new type, `defaultdict`, that
  subclasses the standard [`dict`](../library/stdtypes.md#dict) type.  The new type mostly behaves like a
  ...
  as [`list()`](../library/stdtypes.md#list) or [`int()`](../library/functions.md#int).  For example,  you can make an index of words
...
* The `deque` double-ended queue type supplied by the [`collections`](../library/collections.md#module-collections)
  module now has a `remove(value)` method that removes the first occurrence
  of *value* in the queue, raising [`ValueError`](../library/exceptions.md#ValueError) if the value isn’t found.
  (Contributed by Raymond Hettinger.)
* New module: The [`contextlib`](../library/contextlib.md#module-contextlib) module contains helper functions for use
  with the new ‘[`with`](../reference/compound_stmts.md#with)’ statement.  See section [The contextlib module](#contextlibmod)
...
* New module: The `cProfile` module is a C implementation of  the existing
  ...
  Also, the [`pstats`](../library/pstats.md#module-pstats) module for analyzing the data measured by the profiler
...
* The [`csv`](../library/csv.md#module-csv) module, which parses files in comma-separated value format,
...
* The [`datetime`](../library/datetime.md#datetime.datetime) class in the [`datetime`](../library/datetime.md#module-datetime)  module now has a
  `strptime(string, format)`  method for parsing date strings, contributed
  by Josh Spoerri. It uses the same format characters as [`time.strptime()`](../library/time.md#time.strptime) and
  ...
  ts = dt.datetime.strptime('10:13:15 2006-03-07',
                            '%H:%M:%S %Y-%m-%d')
  ```
* The `SequenceMatcher.get_matching_blocks()` method in the [`difflib`](../library/difflib.md#module-difflib)
...
* The [`doctest`](../library/doctest.md#module-doctest) module gained a `SKIP` option that keeps an example from
  ...
  <!-- Patch 1080727 -->
* The [`email`](../library/email.md#module-email) package has been updated to version 4.0. (Contributed by
  Barry Warsaw.)
  <!-- XXX need to provide some more detail here -->

  <a id="index-20"></a>
* The [`fileinput`](../library/fileinput.md#module-fileinput) module was made more flexible. Unicode filenames are now
  supported, and a *mode* parameter that defaults to `"r"` was added to the
  [`input()`](../library/functions.md#input) function to allow opening files in binary or [universal
  newlines](../glossary.md#term-universal-newlines) mode.  Another new parameter, *openhook*, lets you use a function
  other than [`open()`](../library/functions.md#open)  to open the input files.  Once you’re iterating over
  the set of files, the `FileInput` object’s new [`fileno()`](../library/fileinput.md#fileinput.fileno) returns
  the file descriptor for the currently opened file. (Contributed by Georg
  Brandl.)
* In the [`gc`](../library/gc.md#module-gc) module, the new `get_count()` function returns a 3-tuple
  ...
  [`gc.collect()`](../library/gc.md#gc.collect) function now takes an optional *generation* argument of 0, 1,
  or 2 to specify which generation to collect. (Contributed by Barry Warsaw.)
* The `nsmallest()` and  `nlargest()` functions in the [`heapq`](../library/heapq.md#module-heapq)
  module  now support a `key` keyword parameter similar to the one provided by
  the [`min()`](../library/functions.md#min)/[`max()`](../library/functions.md#max) functions and the `sort()` methods.  For
  example:
  ```python3
  >>> import heapq
  ...
  (Contributed by Raymond Hettinger.)
* The [`itertools.islice()`](../library/itertools.md#itertools.islice) function now accepts `None` for the start and
  ...
  (Contributed by Raymond Hettinger.)
* The [`format()`](../library/functions.md#format) function in the [`locale`](../library/locale.md#module-locale) module has been modified and
  two new functions were added, `format_string()` and `currency()`.

  The [`format()`](../library/functions.md#format) function’s *val* parameter could previously be a string as
  ...
  `format_string()` function that works like [`format()`](../library/functions.md#format) but also supports
  ...
  <!-- Patch 1180296 -->
* The [`mailbox`](../library/mailbox.md#module-mailbox) module underwent a massive rewrite to add the capability to
...
* The [`operator`](../library/operator.md#module-operator) module’s `itemgetter()`  and `attrgetter()`
...
* The [`optparse`](../library/optparse.md#module-optparse) module was updated to version 1.5.1 of the Optik library.
...
* The [`os`](../library/os.md#module-os) module underwent several changes.  The `stat_float_times`
  variable now defaults to true, meaning that [`os.stat()`](../library/os.md#os.stat) will now return time
  values as floats.  (This doesn’t necessarily mean that [`os.stat()`](../library/os.md#os.stat) will
  return times that are precise to fractions of a second; not all systems support
  such precision.)

  Constants named [`os.SEEK_SET`](../library/os.md#os.SEEK_SET), [`os.SEEK_CUR`](../library/os.md#os.SEEK_CUR), and
  [`os.SEEK_END`](../library/os.md#os.SEEK_END) have been added; these are the parameters to the
  [`os.lseek()`](../library/os.md#os.lseek) function.  Two new constants for locking are
  [`os.O_SHLOCK`](../library/os.md#os.O_SHLOCK) and [`os.O_EXLOCK`](../library/os.md#os.O_EXLOCK).
  ...
  [`resource.getrusage()`](../library/resource.md#resource.getrusage) function. `wait4(pid)` does take a process ID.
  (Contributed by Chad J. Schroeder.)

  On FreeBSD, the [`os.stat()`](../library/os.md#os.stat) function now returns  times with nanosecond
...
* The Python debugger provided by the [`pdb`](../library/pdb.md#module-pdb) module can now store lists of
  ...
  <!-- Patch 790710 -->
* The [`pickle`](../library/pickle.md#module-pickle) and `cPickle` modules no longer accept a return value
  of `None` from the [`__reduce__()`](../library/pickle.md#object.__reduce__) method; the method must return a tuple
  of arguments instead.  The ability to return `None` was deprecated in Python
  2.4, so this completes the removal of the feature.
* The [`pkgutil`](../library/pkgutil.md#module-pkgutil) module, containing various utility functions for finding
  packages, was enhanced to support [**PEP 302**](https://peps.python.org/pep-0302/)’s import hooks and now also works for
...
* The [`Queue`](../library/queue.md#queue.Queue) class provided by the `Queue` module gained two new
...
* The [`rlcompleter`](../library/rlcompleter.md#module-rlcompleter) module is no longer  dependent on importing the
  [`readline`](../library/readline.md#module-readline) module and therefore now works on non-Unix platforms. (Patch
  from Robert Kiendl.)
  <!-- Patch #1472854 -->
* The [`SimpleXMLRPCServer`](../library/xmlrpc.server.md#module-xmlrpc.server) and [`DocXMLRPCServer`](../library/xmlrpc.server.md#module-xmlrpc.server)  classes now have a
  ...
  <!-- Bug #1473048 -->
* The [`socket`](../library/socket.md#module-socket) module now supports `AF_NETLINK` sockets on Linux,
  ...
  introductory  article about them is at [https://www.linuxjournal.com/article/7356](https://www.linuxjournal.com/article/7356).
...
* The [`struct`](../library/struct.md#module-struct) is now faster because it  compiles format strings into
  `Struct` objects with `pack()` and `unpack()` methods.  This is
  similar to how the [`re`](../library/re.md#module-re) module lets you create compiled regular expression
...
* The Python developers switched from CVS to Subversion during the 2.5
  ...
  [`Py_GetBuildInfo()`](../c-api/interp-lifecycle.md#c.Py_GetBuildInfo) function that returns a  string of build information
  like this: `"trunk:45355:45356M, Apr 13 2006, 07:42:19"`.   (Contributed by
  Barry Warsaw.)
* Another new function, [`sys._current_frames()`](../library/sys.md#sys._current_frames), returns the current stack
...
* The `TarFile` class in the [`tarfile`](../library/tarfile.md#module-tarfile) module now has an
  ...
  <!-- patch 918101 -->
* The [`threading`](../library/threading.md#module-threading) module now lets you set the stack size used when new
  ...
  <!-- Patch 1454481 -->
* The [`unicodedata`](../library/unicodedata.md#module-unicodedata) module has been updated to use version 4.1.0 of the
  Unicode character database.  Version 3.2.0 is required  by some specifications,
  so it’s still available as  [`unicodedata.ucd_3_2_0`](../library/unicodedata.md#unicodedata.ucd_3_2_0).
* New module: the  [`uuid`](../library/uuid.md#module-uuid) module generates  universally unique identifiers
  (UUIDs) according to [**RFC 4122**](https://datatracker.ietf.org/doc/html/rfc4122.html).  The RFC defines several different UUID
  ...
  [**RFC 4122**](https://datatracker.ietf.org/doc/html/rfc4122.html) and are not supported by this module.)
  ```python3
  >>> import uuid
...
* The [`weakref`](../library/weakref.md#module-weakref) module’s `WeakKeyDictionary` and
...
* The [`webbrowser`](../library/webbrowser.md#module-webbrowser) module received a number of enhancements. It’s now
  ...
  module’s [`open()`](../library/functions.md#open) function supports an additional feature, an *autoraise*
  ...
  <!-- Patch #754022 -->
* The [`xmlrpclib`](../library/xmlrpc.client.md#module-xmlrpc.client) module now supports returning  [`datetime`](../library/datetime.md#datetime.datetime) objects
  for the XML-RPC date type.  Supply  `use_datetime=True` to the [`loads()`](../library/xmlrpc.client.md#xmlrpc.client.loads)
  function or the `Unmarshaller` class to enable this feature. (Contributed
  by Skip Montanaro.)
  <!-- Patch 1120353 -->
* The [`zipfile`](../library/zipfile.md#module-zipfile) module now supports the ZIP64 version of the  format,
  ...
  <!-- Patch 1446489 -->
* The [`zlib`](../library/zlib.md#module-zlib) module’s `Compress` and `Decompress` objects now
  support a [`copy()`](../library/copy.md#module-copy) method that makes a copy of the  object’s internal state
...
<a id="module-ctypes"></a>

### The ctypes package

The [`ctypes`](../library/ctypes.md#module-ctypes) package, written by Thomas Heller, has been added  to the
standard library.  [`ctypes`](../library/ctypes.md#module-ctypes) lets you call arbitrary functions  in shared
...
The [`ctypes`](../library/ctypes.md#module-ctypes) package is much fancier.
...
`c_float()`, `c_double()`, `c_char_p()` (equivalent to ), and so forth.  Unlike Python’s types, the C versions are all mutable; you
...
[`ctypes`](../library/ctypes.md#module-ctypes) also provides a wrapper for Python’s C API  as the
`ctypes.pythonapi` object.  This object does *not*  release the global
interpreter lock before calling a function, because the lock must be held when
calling into the interpreter’s code.   There’s a [`py_object`](../library/ctypes.md#ctypes.py_object) type
constructor that will create a   pointer.  A simple usage:
...
Don’t forget to use [`py_object()`](../library/ctypes.md#ctypes.py_object); if it’s omitted you end  up with a
segmentation fault.

[`ctypes`](../library/ctypes.md#module-ctypes) has been around for a while, but people still write  and
distribution hand-coded extension modules because you can’t rely on
[`ctypes`](../library/ctypes.md#module-ctypes) being present. Perhaps developers will begin to write  Python
wrappers atop a library accessed through [`ctypes`](../library/ctypes.md#module-ctypes) instead of extension
modules, now that [`ctypes`](../library/ctypes.md#module-ctypes) is included with core Python.

#### SEE ALSO
[https://web.archive.org/web/20180410025338/http://starship.python.net/crew/theller/ctypes/](https://web.archive.org/web/20180410025338/http://starship.python.net/crew/theller/ctypes/)
: The pre-stdlib ctypes web page, with a tutorial, reference, and FAQ.

The documentation  for the [`ctypes`](../library/ctypes.md#module-ctypes) module.
...
[https://web.archive.org/web/20201124024954/http://effbot.org/zone/element-index.htm](https://web.archive.org/web/20201124024954/http://effbot.org/zone/element-index.htm).
...
| `elem[m:n]`                 | Returns list of m’th through n’th child<br/>elements. |
...
| `elem.attrib`               | Retrieves the dictionary containing<br/>attributes.   |
...
<a id="module-hashlib"></a>

### The hashlib package

A new [`hashlib`](../library/hashlib.md#module-hashlib) module, written by Gregory P. Smith,  has been added to
replace the `md5` and `sha` modules.  [`hashlib`](../library/hashlib.md#module-hashlib) adds support for
...
string or a string of hex digits, and [`copy()`](../library/copy.md#module-copy) returns a new hashing object
with the same digest state.

#### SEE ALSO
The documentation  for the [`hashlib`](../library/hashlib.md#module-hashlib) module.

<!-- ====================================================================== -->

<a id="module-sqlite"></a>

### The sqlite3 package

The pysqlite module ([https://www.pysqlite.org](https://www.pysqlite.org)), a wrapper for the SQLite embedded
...
with the DB-API 2.0 specification described by [**PEP 249**](https://peps.python.org/pep-0249/).
...
#### SEE ALSO
[https://www.pysqlite.org](https://www.pysqlite.org)
...
The documentation  for the [`sqlite3`](../library/sqlite3.md#module-sqlite3) module.
...
<a id="module-wsgiref"></a>

### The wsgiref package

The Web Server Gateway Interface (WSGI) v1.0 defines a standard interface
between web servers and Python web applications and is described in [**PEP 333**](https://peps.python.org/pep-0333/).
The [`wsgiref`](../library/wsgiref.md#module-wsgiref) package is a reference implementation of the WSGI
specification.
...
#### SEE ALSO
[https://web.archive.org/web/20160331090247/http://wsgi.readthedocs.org/en/latest/](https://web.archive.org/web/20160331090247/http://wsgi.readthedocs.org/en/latest/)
...
* The Python source tree was converted from CVS to Subversion,  in a complex
  ...
  Löwis.  The procedure was developed as [**PEP 347**](https://peps.python.org/pep-0347/).
* Coverity, a company that markets a source code analysis tool called Prevent,
  ...
  [https://scan.coverity.com](https://scan.coverity.com) for the statistics.
* The largest change to the C API came from [**PEP 353**](https://peps.python.org/pep-0353/), which modifies the
  interpreter to use a [`Py_ssize_t`](../c-api/intro.md#c.Py_ssize_t) type definition instead of
  .  See the earlier section [PEP 353: Using ssize_t as the index type](#pep-353) for a discussion of this
  change.
* The design of the bytecode compiler has changed a great deal,  no longer
  ...
  [`compile()`](../library/functions.md#compile) built-in and specifying `_ast.PyCF_ONLY_AST` as the value of
  the  *flags* parameter:
  ```python3
  ...
  No official documentation has been written for the AST code yet, but [**PEP 339**](https://peps.python.org/pep-0339/)
...
* Evan Jones’s patch to obmalloc, first described in a talk at PyCon DC 2005,
  ...
  memory that are grouped into families.  For example, [`PyMem_Malloc()`](../c-api/memory.md#c.PyMem_Malloc),
  [`PyMem_Realloc()`](../c-api/memory.md#c.PyMem_Realloc), and [`PyMem_Free()`](../c-api/memory.md#c.PyMem_Free) are one family that allocates
  raw memory, while [`PyObject_Malloc()`](../c-api/memory.md#c.PyObject_Malloc), [`PyObject_Realloc()`](../c-api/memory.md#c.PyObject_Realloc), and
  [`PyObject_Free()`](../c-api/memory.md#c.PyObject_Free) are another family that’s supposed to be used for
...
* The built-in set types now have an official C API.  Call [`PySet_New()`](../c-api/set.md#c.PySet_New)
  and [`PyFrozenSet_New()`](../c-api/set.md#c.PyFrozenSet_New) to create a new set, [`PySet_Add()`](../c-api/set.md#c.PySet_Add) and
  [`PySet_Discard()`](../c-api/set.md#c.PySet_Discard) to add and remove elements, and [`PySet_Contains()`](../c-api/set.md#c.PySet_Contains)
  and [`PySet_Size()`](../c-api/set.md#c.PySet_Size) to examine the set’s state. (Contributed by Raymond
  Hettinger.)
* C code can now obtain information about the exact revision of the Python
  interpreter by calling the  [`Py_GetBuildInfo()`](../c-api/interp-lifecycle.md#c.Py_GetBuildInfo) function that returns a
...
* Two new macros can be used to indicate C functions that are local to the
  ...
  [`Py_LOCAL`](../c-api/intro.md#c.Py_LOCAL) declares the function as returning a value of the
  specified *type* and uses a fast-calling qualifier.
  [`Py_LOCAL_INLINE`](../c-api/intro.md#c.Py_LOCAL_INLINE) does the same thing and also requests the
  function be inlined.  If macro `PY_LOCAL_AGGRESSIVE` is defined before
...
* The `PyErr_Warn()` function for issuing warnings is now deprecated in
  ...
  *stacklevel* of 1 is the function calling [`PyErr_WarnEx()`](../c-api/exceptions.md#c.PyErr_WarnEx), 2 is the
...
* MacOS X: an `--enable-universalsdk` switch was added to the
  **configure** script that compiles the interpreter as a universal binary
  ...
  Oussoren; [bpo-2573](https://bugs.python.org/issue?@action=redirect&bpo=2573).)
...
* Previously, the `gi_frame` attribute of a generator was always a frame
  object.  Because of the [**PEP 342**](https://peps.python.org/pep-0342/) changes described in section [PEP 342: New Generator Features](#pep-342),
  it’s now possible for `gi_frame` to be `None`.
* A new warning, [`UnicodeWarning`](../library/exceptions.md#UnicodeWarning), is triggered when  you attempt to
  compare a Unicode string and an 8-bit string that can’t be converted to Unicode
  using the default ASCII encoding.  Previously such comparisons would raise a
  [`UnicodeDecodeError`](../library/exceptions.md#UnicodeDecodeError) exception.
* Library: the [`csv`](../library/csv.md#module-csv) module is now stricter about multi-line quoted fields.
...
* Library: the [`locale`](../library/locale.md#module-locale) module’s  [`format()`](../library/functions.md#format) function’s would
  ...
  no surrounding text.
* Library: The [`pickle`](../library/pickle.md#module-pickle) and `cPickle` modules no longer accept a
  return value of `None` from the [`__reduce__()`](../library/pickle.md#object.__reduce__) method; the method must
  return a tuple of arguments instead.  The modules also no longer accept the
  deprecated *bin* keyword parameter.
* Library: The [`SimpleXMLRPCServer`](../library/xmlrpc.server.md#module-xmlrpc.server) and [`DocXMLRPCServer`](../library/xmlrpc.server.md#module-xmlrpc.server)  classes now
  ...
  checking.
* C API: Many functions now use [`Py_ssize_t`](../c-api/intro.md#c.Py_ssize_t)  instead of  to
  ...
  earlier section [PEP 353: Using ssize_t as the index type](#pep-353) for a discussion of this change.
