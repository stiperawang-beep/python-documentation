...
that expects a regular Python list and won’t accept a [`UserList`](../library/collections.md#collections.UserList)
instance.
...
* It’s also possible to automatically call methods on accessing or setting an
  ...
  of [`__getattr__()`](../reference/datamodel.md#object.__getattr__) can be rewritten to use properties instead, making the
...
only the broad strokes.  See section [Related Links](#sect-rellinks), “Related Links”, for
...
named [`object`](../library/functions.md#object), the base class for all built-in types, has also been
...
This means that [`class`](../reference/compound_stmts.md#class) statements that don’t have any base classes are
always classic classes in Python 2.2.  (Actually you can also change this by
setting a module-level variable named `__metaclass__` — see [**PEP 253**](https://peps.python.org/pep-0253/)
for the details — but it’s easier to just subclass [`object`](../library/functions.md#object).)

The type objects for the built-in types are available as built-ins, named using
a clever trick.  Python has always had built-in functions named [`int()`](../library/functions.md#int),
[`float()`](../library/functions.md#float), and [`str()`](../library/stdtypes.md#str).  In 2.2, they aren’t functions any more, but
...
To make the set of types complete, new type objects such as [`dict()`](../library/stdtypes.md#dict) and
...
the [`__dict__`](../reference/datamodel.md#object.__dict__) of an object, but when class inheritance or an arbitrary
...
* [`__name__`](../library/stdtypes.md#definition.__name__) is the attribute’s name.
* [`__doc__`](../library/stdtypes.md#definition.__doc__) is the attribute’s docstring.
...
For methods, [`descriptor.__get__`](../reference/datamodel.md#object.__get__) returns a temporary
object that’s
...
The [`staticmethod()`](../library/functions.md#staticmethod) function takes the function `f()`, and returns it
...
from [**PEP 253**](https://peps.python.org/pep-0253/) by Guido van Rossum):
...
followed by Common Lisp.  A new built-in function, [`super()`](../library/functions.md#super), provides a way
...
[`super()`](../library/functions.md#super) can also return unbound superclass objects when called as
...
using [`__getattr__()`](../reference/datamodel.md#object.__getattr__); most commonly this is done for convenience, to make
...
that [`__getattribute__()`](../reference/datamodel.md#object.__getattribute__) is *always* called whenever any attribute is
accessed, while the old [`__getattr__()`](../reference/datamodel.md#object.__getattr__) is only called if `foo` isn’t
...
[`__dict__`](../reference/datamodel.md#object.__dict__). [`__getattr__()`](../reference/datamodel.md#object.__getattr__) methods also end up being called by Python
when it checks for other methods such as [`__repr__()`](../reference/datamodel.md#object.__repr__) or `__coerce__()`,
...
[`property`](../library/functions.md#property) is a new built-in type that packages up three functions that
...
instance’s [`__dict__`](../reference/datamodel.md#object.__dict__).  Accesses to `size` are also the only ones
...
referenced on an object using the new [`__slots__`](../reference/datamodel.md#object.__slots__) class attribute. Python
...
can define a class attribute named [`__slots__`](../reference/datamodel.md#object.__slots__) to limit the legal
...
Note how you get an [`AttributeError`](../library/exceptions.md#AttributeError) on the attempt to assign to an
attribute not listed in [`__slots__`](../reference/datamodel.md#object.__slots__).

<a id="sect-rellinks"></a>
...
The [Descriptor Guide](../howto/descriptor.md#descriptorhowto) is a lengthy tutorial introduction to
the descriptor features, written by Guido van Rossum. If my description has
whetted your appetite, go read this tutorial next, because it goes into much
more detail about the new features while still remaining quite easy to read.

Next, there are two relevant PEPs, [**PEP 252**](https://peps.python.org/pep-0252/) and [**PEP 253**](https://peps.python.org/pep-0253/).  [**PEP 252**](https://peps.python.org/pep-0252/) is
titled “Making Types Look More Like Classes”, and covers the descriptor API.
[**PEP 253**](https://peps.python.org/pep-0253/) is titled “Subtyping Built-in Types”, and describes the changes to
type objects that make it possible to subtype built-in objects.  [**PEP 253**](https://peps.python.org/pep-0253/) is
...
to define a [`__getitem__()`](../reference/datamodel.md#object.__getitem__) method that looks something like this:
...
[`__getitem__()`](../reference/datamodel.md#object.__getitem__) is more properly used to define an indexing operation on an
object so that you can write `obj[5]` to retrieve the sixth element.  It’s a
bit misleading when you’re using this only to support [`for`](../reference/compound_stmts.md#for) loops.
...
series of [`__getitem__()`](../reference/datamodel.md#object.__getitem__) calls will be made with *index* incrementing by
one each time.  In other words, the presence of the [`__getitem__()`](../reference/datamodel.md#object.__getitem__) method
doesn’t mean that using `file[5]`  to randomly access the sixth element will
work, though it really should.

In Python 2.2, iteration can be implemented separately, and [`__getitem__()`](../reference/datamodel.md#object.__getitem__)
...
own iterators.  Extension types implemented in C can implement a [`tp_iter`](../c-api/typeobj.md#c.PyTypeObject.tp_iter)
function in order to return an iterator, and extension types that want to behave
as iterators can define a [`tp_iternext`](../c-api/typeobj.md#c.PyTypeObject.tp_iternext) function.

So, after all this, what do iterators actually do?  They have one required
method, [`next()`](../library/functions.md#next), which takes no arguments and returns the next value.  When
there are no more values to be returned, calling [`next()`](../library/functions.md#next) should raise the
[`StopIteration`](../library/exceptions.md#StopIteration) exception.
...
In 2.2, Python’s [`for`](../reference/compound_stmts.md#for) statement no longer expects a sequence; it
expects something for which [`iter()`](../library/functions.md#iter) will return an iterator. For backward
compatibility and convenience, an iterator is automatically constructed for
sequences that don’t implement `__iter__()` or a [`tp_iter`](../c-api/typeobj.md#c.PyTypeObject.tp_iter) slot, so
...
[`iter()`](../library/functions.md#iter) on a dictionary will return an iterator which loops over its keys:
...
In a minor related change, the [`in`](../reference/expressions.md#in) operator now works on dictionaries,
so `key in dict` is now equivalent to `dict.has_key(key)`.

Files also provide an iterator, which calls the [`readline()`](../library/readline.md#module-readline) method until
...
requires a [`next()`](../library/functions.md#next) method.
...
[`try`](../reference/compound_stmts.md#try)…[`finally`](../reference/compound_stmts.md#finally) statement; read [**PEP 255**](https://peps.python.org/pep-0255/) for a full
...
`'abc'[1L:]` would raise a [`TypeError`](../library/exceptions.md#TypeError) exception with the message ‘slice
index must be int’.
...
[`OverflowError`](../library/exceptions.md#OverflowError) will now return a long integer as their result.  For
example:
...
can still distinguish them with the [`type()`](../library/functions.md#type) built-in function, but that’s
rarely needed.
...
and will stick to describing what’s  implemented in 2.2.  Read [**PEP 238**](https://peps.python.org/pep-0238/) for a
...
First, I’ll borrow some terminology from [**PEP 238**](https://peps.python.org/pep-0238/).  “True division” is the
...
* Classes can define methods called [`__truediv__()`](../reference/datamodel.md#object.__truediv__) and [`__floordiv__()`](../reference/datamodel.md#object.__floordiv__)
  to overload the two division operators.  At the C level, there are also slots in
  the [`PyNumberMethods`](../c-api/typeobj.md#c.PyNumberMethods) structure so extension types can define the two
  operators.
...
cause `unichr()` to raise a [`ValueError`](../library/exceptions.md#ValueError) exception. This is all
described in [**PEP 261**](https://peps.python.org/pep-0261/), “Support for ‘wide’ Unicode characters”; consult it for
further details.
...
encoding, and compression with the [`zlib`](../library/zlib.md#module-zlib) module:
...
The function `g()` will always raise a [`NameError`](../library/exceptions.md#NameError) exception, because
the binding of the name `g` isn’t in either its local namespace or in the
module-level namespace.  This isn’t much of a problem in practice (how often do
you recursively define interior functions like this?), but this also made using
the [`lambda`](../reference/expressions.md#lambda) expression clumsier, and this was a problem in practice.
...
or the [`def`](../reference/compound_stmts.md#def), [`class`](../reference/compound_stmts.md#class), or [`import`](../reference/simple_stmts.md#import) statements),
...
definitions or [`lambda`](../reference/expressions.md#lambda) expressions with free variables, the compiler
will flag this by raising a [`SyntaxError`](../library/exceptions.md#SyntaxError) exception.
...
* The [`xmlrpclib`](../library/xmlrpc.client.md#module-xmlrpc.client) module was contributed to the standard library by Fredrik
  ...
  The [`SimpleXMLRPCServer`](../library/xmlrpc.server.md#module-xmlrpc.server) module makes it easy to create straightforward
  XML-RPC servers.  See [http://xmlrpc.scripting.com/](http://xmlrpc.scripting.com/) for more information about XML-RPC.
* The new [`hmac`](../library/hmac.md#module-hmac) module implements the HMAC algorithm described by
  [**RFC 2104**](https://datatracker.ietf.org/doc/html/rfc2104.html). (Contributed by Gerhard Häring.)
* Several functions that originally returned lengthy tuples now return
  ...
  as `memberst_mtime` or [`tm_year`](../library/time.md#time.struct_time.tm_year). The enhanced functions include
  [`stat()`](../library/os.md#os.stat), [`fstat()`](../library/os.md#os.fstat), [`statvfs()`](../library/os.md#os.statvfs), and [`fstatvfs()`](../library/os.md#os.fstatvfs) in the
  [`os`](../library/os.md#module-os) module, and [`localtime()`](../library/time.md#time.localtime), [`gmtime()`](../library/time.md#time.gmtime), and [`strptime()`](../library/time.md#time.strptime) in
  the [`time`](../library/time.md#module-time) module.
...
* The [`socket`](../library/socket.md#module-socket) module can be compiled to support IPv6; specify the
  `--enable-ipv6` option to Python’s configure script.  (Contributed by
  Jun-ichiro “itojun” Hagino.)
* Two new format characters were added to the [`struct`](../library/struct.md#module-struct) module for 64-bit
  integers on platforms that support the C  type.  `q` is for
...
* In the interpreter’s interactive mode, there’s a new built-in function
  [`help()`](../library/functions.md#help) that uses the [`pydoc`](../library/pydoc.md#module-pydoc) module introduced in Python 2.1 to
  provide interactive help. `help(object)` displays any available help text
  about *object*.  [`help()`](../library/functions.md#help) with no argument puts you in an online help
  ...
  [`pydoc`](../library/pydoc.md#module-pydoc) module.)
* Various bugfixes and performance improvements have been made to the SRE engine
  underlying the [`re`](../library/re.md#module-re) module.  For example, the [`re.sub()`](../library/re.md#re.sub) and
  [`re.split()`](../library/re.md#re.split) functions have been rewritten in C.  Another contributed patch
  speeds up certain Unicode character ranges by a factor of two, and a new
  [`finditer()`](../library/re.md#re.finditer)  method that returns an iterator over all the non-overlapping
  matches in  a given string.  (SRE is maintained by Fredrik Lundh.  The
  BIGCHARSET patch was contributed by Martin von Löwis.)
* The [`smtplib`](../library/smtplib.md#module-smtplib) module now supports [**RFC 2487**](https://datatracker.ietf.org/doc/html/rfc2487.html), “Secure SMTP over TLS”, so
  it’s now possible to encrypt the SMTP traffic between a Python program and the
  mail transport agent being handed a message.  [`smtplib`](../library/smtplib.md#module-smtplib) also supports SMTP
  authentication.  (Contributed by Gerhard Häring.)
* The [`imaplib`](../library/imaplib.md#module-imaplib) module, maintained by Piers Lauder, has support for several
  new extensions: the NAMESPACE extension defined in [**RFC 2342**](https://datatracker.ietf.org/doc/html/rfc2342.html), SORT, GETACL and
...
* The `rfc822` module’s parsing of email addresses is now compliant with
  [**RFC 2822**](https://datatracker.ietf.org/doc/html/rfc2822.html), an update to [**RFC 822**](https://datatracker.ietf.org/doc/html/rfc822.html).  (The module’s name is *not* going to be
  changed to `rfc2822`.)  A new package, [`email`](../library/email.md#module-email), has also been added for
  parsing and generating e-mail messages.  (Contributed by Barry Warsaw, and
  arising out of his work on Mailman.)
* The [`difflib`](../library/difflib.md#module-difflib) module now contains a new `Differ` class for
...
* New constants `ascii_letters`, `ascii_lowercase`, and
  `ascii_uppercase` were added to the [`string`](../library/string.md#module-string) module.  There were
  ...
  Fred L. Drake, Jr.)
* The [`mimetypes`](../library/mimetypes.md#module-mimetypes) module now makes it easier to use alternative MIME-type
  databases by the addition of a [`MimeTypes`](../library/mimetypes.md#mimetypes.MimeTypes) class, which takes a list of
  filenames to be parsed.  (Contributed by Fred L. Drake, Jr.)
* A [`Timer`](../library/threading.md#threading.Timer) class was added to the [`threading`](../library/threading.md#module-threading) module that allows
...
* Profiling and tracing functions can now be implemented in C, which can operate
  ...
  [`PyEval_SetProfile()`](../c-api/profiling.md#c.PyEval_SetProfile) and [`PyEval_SetTrace()`](../c-api/profiling.md#c.PyEval_SetTrace). The existing
  [`sys.setprofile()`](../library/sys.md#sys.setprofile) and [`sys.settrace()`](../library/sys.md#sys.settrace) functions still exist, and have
...
* Another low-level API, primarily of interest to implementers of Python
  debuggers and development tools, was added. [`PyInterpreterState_Head()`](../c-api/subinterpreters.md#c.PyInterpreterState_Head) and
  [`PyInterpreterState_Next()`](../c-api/subinterpreters.md#c.PyInterpreterState_Next) let a caller walk through all the existing
  interpreter objects; [`PyInterpreterState_ThreadHead()`](../c-api/subinterpreters.md#c.PyInterpreterState_ThreadHead) and
  [`PyThreadState_Next()`](../c-api/subinterpreters.md#c.PyThreadState_Next) allow looping over all the thread states for a given
...
* Rename `Py_TPFLAGS_GC` to [`Py_TPFLAGS_HAVE_GC`](../c-api/typeobj.md#c.Py_TPFLAGS_HAVE_GC).
* Use [`PyObject_GC_New()`](../c-api/gcsupport.md#c.PyObject_GC_New) or [`PyObject_GC_NewVar()`](../c-api/gcsupport.md#c.PyObject_GC_NewVar) to allocate
  : objects, and [`PyObject_GC_Del()`](../c-api/gcsupport.md#c.PyObject_GC_Del) to deallocate them.
* Rename `PyObject_GC_Init()` to [`PyObject_GC_Track()`](../c-api/gcsupport.md#c.PyObject_GC_Track) and
  `PyObject_GC_Fini()` to [`PyObject_GC_UnTrack()`](../c-api/gcsupport.md#c.PyObject_GC_UnTrack).
* Remove `PyGC_HEAD_SIZE` from object size calculations.
* Remove calls to `PyObject_AS_GC()` and `PyObject_FROM_GC()`.
* A new `et` format sequence was added to [`PyArg_ParseTuple()`](../c-api/arg.md#c.PyArg_ParseTuple); `et`
...
* A different argument parsing function, [`PyArg_UnpackTuple()`](../c-api/arg.md#c.PyArg_UnpackTuple), has been
  added that’s simpler and presumably faster.  Instead of specifying a format
  string, the caller simply gives the minimum and maximum number of arguments
  expected, and a set of pointers to  variables that will be
  filled in with argument values.
* Two new flags [`METH_NOARGS`](../c-api/structures.md#c.METH_NOARGS) and [`METH_O`](../c-api/structures.md#c.METH_O) are available in method
  ...
  corresponding method that uses [`METH_VARARGS`](../c-api/structures.md#c.METH_VARARGS).  Also, the old
  `METH_OLDARGS` style of writing C methods is  now officially deprecated.
* Two new wrapper functions, [`PyOS_snprintf()`](../c-api/conversion.md#c.PyOS_snprintf) and [`PyOS_vsnprintf()`](../c-api/conversion.md#c.PyOS_vsnprintf)
  ...
  M.-A. Lemburg.)
* The [`_PyTuple_Resize()`](../c-api/tuple.md#c._PyTuple_Resize) function has lost an unused parameter, so now it
...
* Keyword arguments passed to built-in functions that don’t take them now cause a
  [`TypeError`](../library/exceptions.md#TypeError) exception to be raised, with the message “*function* takes no
  keyword arguments”.
* Weak references, added in Python 2.1 as an extension module, are now part of
  ...
  [`ReferenceError`](../library/exceptions.md#ReferenceError) exception has therefore moved from the [`weakref`](../library/weakref.md#module-weakref)
...
* An additional *flags* argument has been added to the built-in function
  [`compile()`](../library/functions.md#compile), so the behaviour of `__future__` statements can now be
  correctly observed in simulated shells, such as those presented by IDLE and
  other development environments.  This is described in [**PEP 264**](https://peps.python.org/pep-0264/). (Contributed
  by Michael Hudson.)
...
* Some features of the object returned by the `xrange()` function are now
  ...
  types by supporting slicing, sequence multiplication, and the [`in`](../reference/expressions.md#in)
...
* On platforms where Python uses the C `dlopen()` function  to load
  ...
  using the [`sys.getdlopenflags()`](../library/sys.md#sys.getdlopenflags) and [`sys.setdlopenflags()`](../library/sys.md#sys.setdlopenflags) functions.
  (Contributed by Bram Stolk.)
* The [`pow()`](../library/functions.md#pow) built-in function no longer supports 3 arguments when
  ...
  will now raise a [`TypeError`](../library/exceptions.md#TypeError) exception.
