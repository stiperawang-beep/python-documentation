...
The [`frozenset()`](../library/stdtypes.md#frozenset) type is an immutable version of [`set()`](../library/stdtypes.md#set). Since it is
...
behave differently after int/long unification triggered [`FutureWarning`](../library/exceptions.md#FutureWarning)
...
The iterator feature introduced in Python 2.2 and the [`itertools`](../library/itertools.md#module-itertools) module
...
[`ValueError`](../library/exceptions.md#ValueError), and fix the problem.  However, consider an application such
...
PEP 292 adds a `Template` class to the [`string`](../library/string.md#module-string) module that uses
...
raise a [`KeyError`](../library/exceptions.md#KeyError).  There’s also a `safe_substitute()` method that
...
static or class methods.  Instead, you had to write a [`def`](../reference/compound_stmts.md#def) statement
in the usual way, and pass the resulting method to a [`staticmethod()`](../library/functions.md#staticmethod) or
[`classmethod()`](../library/functions.md#classmethod) function that would wrap up the function as a method of the
...
[`classmethod()`](../library/functions.md#classmethod) invocation after the function body.
...
that [`classmethod()`](../library/functions.md#classmethod), [`staticmethod()`](../library/functions.md#staticmethod), and friends are storing
...
An example in [**PEP 318**](https://peps.python.org/pep-0318/) contains a fancier version of this idea that lets you
...
A small related change makes the [`func_name`](../reference/datamodel.md#function.__name__)
...
Compared to extended slicing, such as `range(1,4)[::-1]`, [`reversed()`](../library/functions.md#reversed) is
easier to read, runs faster, and uses substantially less memory.

Note that [`reversed()`](../library/functions.md#reversed) only accepts sequences, not arbitrary iterators.  If
you want to reverse an iterator, first convert it to  a list with [`list()`](../library/stdtypes.md#list).
...
is confusing.  The [`subprocess`](../library/subprocess.md#module-subprocess) module cleans  this up, providing a unified
interface that offers all the features you might need.

Instead of `popen2`’s collection of classes, [`subprocess`](../library/subprocess.md#module-subprocess) contains a
single class called [`subprocess.Popen`](../library/subprocess.md#subprocess.Popen)  whose constructor supports a number of
...
just as [`os.system()`](../library/os.md#os.system) does.)
...
* *universal_newlines* opens the child’s input and output using Python’s
  [universal newlines](../glossary.md#term-universal-newlines) feature.
...
the subprocess.  It can serve as a safer analog to [`os.system()`](../library/os.md#os.system):
...
translated into Python code that uses [`subprocess`](../library/subprocess.md#module-subprocess).  Reading this section
...
C  type, as a data type.  However, while most programming
...
standard called IEEE 754.  C’s  type is usually implemented as a
...
A new module, [`decimal`](../library/decimal.md#module-decimal), was added to Python’s standard library.  It
...
`Decimal` numbers can be used with the [`math`](../library/math.md#module-math) and [`cmath`](../library/cmath.md#module-cmath)
...
* `rounding` specifies the rounding mode.  The [`decimal`](../library/decimal.md#module-decimal) module has
...
For more information, see the documentation for the [`decimal`](../library/decimal.md#module-decimal) module, which
...
The PEP also proposes that all [`import`](../reference/simple_stmts.md#import) statements be absolute imports,
...
The [`locale`](../library/locale.md#module-locale) modules lets Python software select various conversions and
...
Foundation.  The [`locale`](../library/locale.md#module-locale) module  can now change the numeric locale,
...
* Built-in [`set()`](../library/stdtypes.md#set) and [`frozenset()`](../library/stdtypes.md#frozenset) types were  added ([**PEP 218**](https://peps.python.org/pep-0218/)).
  Other new built-ins include the `reversed(seq)` function ([**PEP 322**](https://peps.python.org/pep-0322/)).
...
* Certain numeric expressions no longer return values restricted to 32 or 64
  bits ([**PEP 237**](https://peps.python.org/pep-0237/)).
* You can now put parentheses around the list of names in a `from module import
  names` statement ([**PEP 328**](https://peps.python.org/pep-0328/)).
* The [`dict.update()`](../library/stdtypes.md#dict.update) method now accepts the same argument forms as the
  [`dict`](../library/stdtypes.md#dict) constructor.  This includes any mapping, any iterable of key/value
...
* Three keyword parameters, *cmp*, *key*, and *reverse*, were added to the
  ...
  *key* should be a single-parameter function that takes a list element and
  ...
  a [`lambda`](../reference/expressions.md#lambda) expression by using an unbound method instead.  For example,
...
* There is a new built-in function `sorted(iterable)` that works like the
  in-place [`list.sort()`](../library/stdtypes.md#list.sort) method but can be used in expressions.  The
  differences are:
...
* The interpreter gained a new switch, [`-m`](../using/cmdline.md#cmdoption-m), that takes a name, searches
...
* The [`zip()`](../library/functions.md#zip) built-in function and `itertools.izip()` now return an
  empty list if called with no arguments. Previously they raised a
  [`TypeError`](../library/exceptions.md#TypeError) exception.  This makes them more suitable for use with variable
...
* [`None`](../library/constants.md#None) is now a constant; code that binds a new value to  the name
...
* The machinery for growing and shrinking lists was optimized for speed and for
  ...
  `realloc()`.  List comprehensions also benefit.   [`list.extend()`](../library/stdtypes.md#list.extend) was
...
* [`list()`](../library/stdtypes.md#list), [`tuple()`](../library/stdtypes.md#tuple), [`map()`](../library/functions.md#map), [`filter()`](../library/functions.md#filter), and [`zip()`](../library/functions.md#zip) now
...
* The [`base64`](../library/base64.md#module-base64) module now has more complete [**RFC 3548**](https://datatracker.ietf.org/doc/html/rfc3548.html) support for Base64,
...
* The [`bisect`](../library/bisect.md#module-bisect) module now has an underlying C implementation for improved
...
* There is a new [`collections`](../library/collections.md#module-collections) module for  various specialized collection
  ...
  Several modules, such as the `Queue` and [`threading`](../library/threading.md#module-threading) modules, now take
  advantage of [`collections.deque`](../library/collections.md#collections.deque) for improved performance.  (Contributed
  by Raymond Hettinger.)
* The [`ConfigParser`](../library/configparser.md#module-configparser) classes have been enhanced slightly. The [`read()`](../library/configparser.md#configparser.ConfigParser.read)
  method now returns a list of the files that were successfully parsed, and the
  [`set()`](../library/configparser.md#configparser.ConfigParser.set) method raises [`TypeError`](../library/exceptions.md#TypeError) if passed a *value* argument that
  isn’t a string.   (Contributed by John Belmonte and David Goodger.)
* The [`curses`](../library/curses.md#module-curses) module now supports the ncurses extension
...
* The [`difflib`](../library/difflib.md#module-difflib) module now includes an `HtmlDiff` class that creates
  an HTML table showing a side by side comparison of two versions of a text.
  (Contributed by Dan Gass.)
* The [`email`](../library/email.md#module-email) package was updated to version 3.0,  which dropped various
  ...
  Wouters, and others.)
* The [`heapq`](../library/heapq.md#module-heapq) module has been converted to C.  The resulting tenfold
...
* The [`httplib`](../library/http.md#module-http) module now contains constants for HTTP status codes defined
  ...
  Andrew Eland.)
* The [`imaplib`](../library/imaplib.md#module-imaplib) module now supports IMAP’s THREAD command (contributed by
  Yves Dionne) and new `deleteacl()` and `myrights()` methods (contributed
  by Arnaud Mazin).
* The [`itertools`](../library/itertools.md#module-itertools) module gained a `groupby(iterable[, *func*])`
...
* [`itertools`](../library/itertools.md#module-itertools) also gained a function named `tee(iterator, N)` that
  ...
  you might as well use  [`list()`](../library/stdtypes.md#list) instead.  When the iterators track closely
  ...
  Hettinger.)
* A number of functions were added to the [`locale`](../library/locale.md#module-locale)  module, such as
...
* Some keyword arguments were added to the [`logging`](../library/logging.md#module-logging) package’s
  ...
  Other additions to the [`logging`](../library/logging.md#module-logging) package include a `log(level, msg)`
...
* The [`marshal`](../library/marshal.md#module-marshal) module now shares interned strings on unpacking a  data
...
* Two new functions were added to the [`operator`](../library/operator.md#module-operator) module,
  ...
  item; these callables make excellent data extractors when used with [`map()`](../library/functions.md#map)
  or [`sorted()`](../library/functions.md#sorted).  For example:
  ```python3
  >>> L = [('c', 2), ('d', 1), ('a', 4), ('b', 3)]
  ...
  (Contributed by Raymond Hettinger.)
* The [`optparse`](../library/optparse.md#module-optparse) module was updated in various ways.  The module now passes
  its messages through [`gettext.gettext()`](../library/gettext.md#gettext.gettext), making it possible to
...
* The long-term plan is to deprecate the `rfc822` module in some future
  Python release in favor of the [`email`](../library/email.md#module-email) package. To this end, the
  [`email.Utils.formatdate`](../library/email.utils.md#email.utils.formatdate) function has been changed to make it usable as a
...
* A new `urandom(n)` function was added to the [`os`](../library/os.md#module-os) module, returning
...
* A new `getsid()` function was added to the [`posix`](../library/posix.md#module-posix) module that
  underlies the [`os`](../library/os.md#module-os) module. (Contributed by J. Raynor.)
* The [`poplib`](../library/poplib.md#module-poplib) module now supports POP over SSL.  (Contributed by Hector
  Urtubia.)
* The [`profile`](../library/profile.md#module-profile) module can now profile C extension functions. (Contributed
  by Nick Bastin.)
* The [`random`](../library/random.md#module-random) module has a new method called `getrandbits(N)` that
  ...
  Hettinger.)
* The regular expression language accepted by the [`re`](../library/re.md#module-re) module was extended
...
* The [`re`](../library/re.md#module-re) module is also no longer recursive, thanks to a massive amount
  ...
  a [`RuntimeError`](../library/exceptions.md#RuntimeError) exception, but certain patterns could sidestep the
...
* The [`signal`](../library/signal.md#module-signal) module now performs tighter error-checking on the parameters
  to the [`signal.signal()`](../library/signal.md#signal.signal) function.  For example, you can’t set a handler on
  the `SIGKILL` signal; previous versions of Python would quietly accept
  this, but 2.4 will raise a [`RuntimeError`](../library/exceptions.md#RuntimeError) exception.
* Two new functions were added to the [`socket`](../library/socket.md#module-socket) module. `socketpair()`
...
* The `sys.exitfunc()` function has been deprecated.  Code should be using
  the existing [`atexit`](../library/atexit.md#module-atexit) module, which correctly handles calling multiple exit
  functions.  Eventually `sys.exitfunc()` will become a purely internal
  interface, accessed only by [`atexit`](../library/atexit.md#module-atexit).
* The [`tarfile`](../library/tarfile.md#module-tarfile) module now generates GNU-format tar files by default.
  (Contributed by Lars Gustäbel.)
* The [`threading`](../library/threading.md#module-threading) module now has an elegantly simple way to support
...
* The [`timeit`](../library/timeit.md#module-timeit) module now automatically disables periodic garbage
  collection during the timing loop.  This change makes consecutive timings more
  comparable.  (Contributed by Raymond Hettinger.)
* The [`weakref`](../library/weakref.md#module-weakref) module now supports a wider variety of objects including
  ...
  Hettinger.)
* The [`xmlrpclib`](../library/xmlrpc.client.md#module-xmlrpc.client) module now supports a multi-call extension for
...
The [`cookielib`](../library/http.cookiejar.md#module-http.cookiejar) library supports client-side handling for HTTP cookies,
mirroring the [`Cookie`](../library/http.cookies.md#module-http.cookies) module’s server-side cookie support. Cookies are
...
[`urllib2`](../library/urllib.request.md#module-urllib.request) has been changed to interact with [`cookielib`](../library/http.cookiejar.md#module-http.cookiejar):
...
The [`doctest`](../library/doctest.md#module-doctest) module underwent considerable refactoring thanks to Edward
Loper and Tim Peters.  Testing can still be as simple as running
[`doctest.testmod()`](../library/doctest.md#doctest.testmod), but the refactorings allow customizing the module’s
...
with the [`doctest.ELLIPSIS`](../library/doctest.md#doctest.ELLIPSIS) option flag, an ellipsis (`...`) in the
...
specifying the [`doctest.REPORT_UDIFF`](../library/doctest.md#doctest.REPORT_UDIFF) (unified diffs),
[`doctest.REPORT_CDIFF`](../library/doctest.md#doctest.REPORT_CDIFF) (context diffs), or [`doctest.REPORT_NDIFF`](../library/doctest.md#doctest.REPORT_NDIFF)
...
Running the above function’s tests with [`doctest.REPORT_UDIFF`](../library/doctest.md#doctest.REPORT_UDIFF) specified,
...
* Three new convenience macros were added for common return values from
  extension functions: [`Py_RETURN_NONE`](../c-api/none.md#c.Py_RETURN_NONE), [`Py_RETURN_TRUE`](../c-api/bool.md#c.Py_RETURN_TRUE), and
  [`Py_RETURN_FALSE`](../c-api/bool.md#c.Py_RETURN_FALSE). (Contributed by Brett Cannon.)
* Another new macro, [`Py_CLEAR`](../c-api/refcounting.md#c.Py_CLEAR),  decreases the reference count of
...
* A new function, `PyDict_Contains(d, k)`, implements fast dictionary
  ...
  (Contributed by Raymond Hettinger.)
* The  macro returns 1 if  its float or double argument
...
* A new function, [`PyArg_VaParseTupleAndKeywords()`](../c-api/arg.md#c.PyArg_VaParseTupleAndKeywords), is the same as
  [`PyArg_ParseTupleAndKeywords()`](../c-api/arg.md#c.PyArg_ParseTupleAndKeywords) but takes a  `va_list` instead of a
  number of arguments. (Contributed by Greg Chapman.)
* A new method flag, [`METH_COEXIST`](../c-api/structures.md#c.METH_COEXIST), allows a function defined in slots
  to co-exist with a [`PyCFunction`](../c-api/structures.md#c.PyCFunction) having the same name.  This can halve
...
* Left shifts and hexadecimal/octal constants that are too  large no longer
  trigger a [`FutureWarning`](../library/exceptions.md#FutureWarning) and return  a value limited to 32 or 64 bits;
  instead they return a long integer.
* Integer operations will no longer trigger an `OverflowWarning`. The
  `OverflowWarning` warning will disappear in Python 2.5.
* The [`zip()`](../library/functions.md#zip) built-in function and `itertools.izip()` now return  an
  empty list instead of raising a [`TypeError`](../library/exceptions.md#TypeError) exception if called with no
  arguments.
* You can no longer compare the `date` and [`datetime`](../library/datetime.md#datetime.datetime) instances
  provided by the [`datetime`](../library/datetime.md#module-datetime) module.  Two  instances of different classes
...
* [`fcntl.ioctl()`](../library/fcntl.md#fcntl.ioctl) now warns if the *mutate*  argument is omitted and
  relevant.
* The [`tarfile`](../library/tarfile.md#module-tarfile) module now generates GNU-format tar files by default.
...
* The `signals.signal()` function now raises a [`RuntimeError`](../library/exceptions.md#RuntimeError) exception
