...
The `print` statement has been replaced with a [`print()`](../library/functions.md#print)
...
* The [`print()`](../library/functions.md#print) function doesn’t support the “softspace” feature of
...
* When using the `2to3` source-to-source conversion tool, all
  ...
  [`print()`](../library/functions.md#print) function calls, so this is mostly a non-issue for
  larger projects.
...
* [`dict`](../library/stdtypes.md#dict) methods [`dict.keys()`](../library/stdtypes.md#dict.keys), [`dict.items()`](../library/stdtypes.md#dict.items) and
  [`dict.values()`](../library/stdtypes.md#dict.values) return “views” instead of lists.  For example,
...
* [`map()`](../library/functions.md#map) and [`filter()`](../library/functions.md#filter) return iterators.  If you really need
  a list and the input sequences are all of equal length, a quick
  fix is to wrap [`map()`](../library/functions.md#map) in [`list()`](../library/stdtypes.md#list), e.g. `list(map(...))`,
  but a better fix is
  often to use a list comprehension (especially when the original code
  uses [`lambda`](../reference/expressions.md#lambda)), or rewriting the code so it doesn’t need a
  list at all.  Particularly tricky is [`map()`](../library/functions.md#map) invoked for the
  side effects of the function; the correct transformation is to use a
  regular [`for`](../reference/compound_stmts.md#for) loop (since creating a list would just be
  wasteful).

  If the input sequences are not of equal length, [`map()`](../library/functions.md#map) will
  stop at the termination of the shortest of the sequences. For full
  compatibility with [`map()`](../library/functions.md#map) from Python 2.x, also wrap the sequences in
  [`itertools.zip_longest()`](../library/itertools.md#itertools.zip_longest), e.g. `map(func, *sequences)` becomes
  `list(map(func, itertools.zip_longest(*sequences)))`.
* [`range()`](../library/stdtypes.md#range) now behaves like `xrange()` used to behave, except
  it works with values of arbitrary size.  The latter no longer
  exists.
* [`zip()`](../library/functions.md#zip) now returns an iterator.
...
* The ordering comparison operators (`<`, `<=`, `>=`, `>`)
  ...
  None` raises [`TypeError`](../library/exceptions.md#TypeError) instead of returning
...
* [`sorted()`](../library/functions.md#sorted) and [`list.sort()`](../library/stdtypes.md#list.sort) no longer accept the
...
* The `cmp()` function should be treated as gone, and the `__cmp__()`
  special method is no longer supported.  Use [`__lt__()`](../reference/datamodel.md#object.__lt__) for sorting,
  [`__eq__()`](../reference/datamodel.md#object.__eq__) with [`__hash__()`](../reference/datamodel.md#object.__hash__), and other rich comparisons as needed.
  (If you really need the `cmp()` functionality, you could use the
  expression `(a > b) - (a < b)` as the equivalent for `cmp(a, b)`.)

### Integers

* [**PEP 237**](https://peps.python.org/pep-0237/): Essentially, `long` renamed to [`int`](../library/functions.md#int).
  That is, there is only one built-in integral type, named
  [`int`](../library/functions.md#int); but it behaves mostly like the old `long` type.
* [**PEP 238**](https://peps.python.org/pep-0238/): An expression like `1/2` returns a float.  Use
...
* The `sys.maxint` constant was removed, since there is no
  longer a limit to the value of integers.  However, [`sys.maxsize`](../library/sys.md#sys.maxsize)
...
* The [`repr()`](../library/functions.md#repr) of a long integer doesn’t include the trailing `L`
  anymore, so code that unconditionally strips that character will
  chop off the last digit instead.  (Use [`str()`](../library/stdtypes.md#str) instead.)
...
* Python 3.0 uses the concepts of *text* and (binary) *data* instead
  ...
  *encoded* Unicode is represented as binary data.  The type used to
  hold text is [`str`](../library/stdtypes.md#str), the type used to hold data is
  [`bytes`](../library/stdtypes.md#bytes).  The biggest difference with the 2.x situation is
  that any attempt to mix text and data in Python 3.0 raises
  [`TypeError`](../library/exceptions.md#TypeError), whereas if you were to mix Unicode and 8-bit
  strings in Python 2.x, it would work if the 8-bit string happened to
  contain only 7-bit (ASCII) bytes, but you would get
  [`UnicodeDecodeError`](../library/exceptions.md#UnicodeDecodeError) if it contained non-ASCII values.  This
...
* As a consequence of this change in philosophy, pretty much all code
  ...
  for all unencoded text, and [`str`](../library/stdtypes.md#str) for binary or encoded data
...
* As the [`str`](../library/stdtypes.md#str) and [`bytes`](../library/stdtypes.md#bytes) types cannot be mixed, you
  must always explicitly convert between them.  Use [`str.encode()`](../library/stdtypes.md#str.encode)
  to go from [`str`](../library/stdtypes.md#str) to [`bytes`](../library/stdtypes.md#bytes), and [`bytes.decode()`](../library/stdtypes.md#bytes.decode)
  to go from [`bytes`](../library/stdtypes.md#bytes) to [`str`](../library/stdtypes.md#str).  You can also use
  `bytes(s, encoding=...)` and `str(b, encoding=...)`,
  respectively.
* Like [`str`](../library/stdtypes.md#str), the [`bytes`](../library/stdtypes.md#bytes) type is immutable.  There is a
  separate *mutable* type to hold buffered binary data,
  [`bytearray`](../library/stdtypes.md#bytearray).  Nearly all APIs that accept [`bytes`](../library/stdtypes.md#bytes) also
  accept [`bytearray`](../library/stdtypes.md#bytearray).  The mutable API is based on
  [`collections.MutableSequence`](../library/collections.abc.md#collections.abc.MutableSequence).
...
* The built-in `basestring` abstract type was removed.  Use
  [`str`](../library/stdtypes.md#str) instead.  The [`str`](../library/stdtypes.md#str) and [`bytes`](../library/stdtypes.md#bytes) types
  don’t have functionality enough in common to warrant a shared base
  class.  The `2to3` tool (see below) replaces every occurrence of
  `basestring` with [`str`](../library/stdtypes.md#str).
* Files opened as text files (still the default mode for [`open()`](../library/functions.md#open))
  ...
  in the [`codecs`](../library/codecs.md#module-codecs) module.
* The initial values of [`sys.stdin`](../library/sys.md#sys.stdin), [`sys.stdout`](../library/sys.md#sys.stdout) and
  [`sys.stderr`](../library/sys.md#sys.stderr) are now unicode-only text files (i.e., they are
  instances of [`io.TextIOBase`](../library/io.md#io.TextIOBase)).  To read and write bytes data
  with these streams, you need to use their [`io.TextIOBase.buffer`](../library/io.md#io.TextIOBase.buffer)
  attribute.
* Filenames are passed to and returned from APIs as (Unicode) strings.
  ...
  work-around, most APIs (e.g. [`open()`](../library/functions.md#open) and many functions in the
  [`os`](../library/os.md#module-os) module) that take filenames accept [`bytes`](../library/stdtypes.md#bytes) objects
  as well as strings, and a few APIs have a way to ask for a
  [`bytes`](../library/stdtypes.md#bytes) return value.  Thus, [`os.listdir()`](../library/os.md#os.listdir) returns a
  list of [`bytes`](../library/stdtypes.md#bytes) instances if the argument is a [`bytes`](../library/stdtypes.md#bytes)
  instance, and [`os.getcwdb()`](../library/os.md#os.getcwdb) returns the current working
  directory as a [`bytes`](../library/stdtypes.md#bytes) instance.  Note that when
  [`os.listdir()`](../library/os.md#os.listdir) returns a list of strings, filenames that
  cannot be decoded properly are omitted rather than raising
  [`UnicodeError`](../library/exceptions.md#UnicodeError).
* Some system APIs like [`os.environ`](../library/os.md#os.environ) and [`sys.argv`](../library/sys.md#sys.argv) can
...
* [**PEP 3138**](https://peps.python.org/pep-3138/): The [`repr()`](../library/functions.md#repr) of a string no longer escapes
...
* [**PEP 3131**](https://peps.python.org/pep-3131/): Non-ASCII letters are now allowed in identifiers.
...
* The `StringIO` and `cStringIO` modules are gone.  Instead,
  import the [`io`](../library/io.md#module-io) module and use [`io.StringIO`](../library/io.md#io.StringIO) or
  [`io.BytesIO`](../library/io.md#io.BytesIO) for text and data respectively.
* See also the [Unicode HOWTO](../howto/unicode.md#unicode-howto), which was updated for Python 3.0.
...
* [**PEP 3107**](https://peps.python.org/pep-3107/): Function argument and return value annotations.  This
  provides a standardized way of annotating a function’s parameters
  and return value.  There are no semantics attached to such
  annotations except that they can be introspected at runtime using
  the [`__annotations__`](../reference/datamodel.md#object.__annotations__) attribute.  The intent is to
  encourage experimentation through metaclasses, decorators or frameworks.
* [**PEP 3102**](https://peps.python.org/pep-3102/): Keyword-only arguments.  Named parameters occurring
...
* [**PEP 3104**](https://peps.python.org/pep-3104/): [`nonlocal`](../reference/simple_stmts.md#nonlocal) statement.  Using `nonlocal x`
  you can now assign directly to a variable in an outer (but
  non-global) scope.  `nonlocal` is a new reserved word.
* [**PEP 3132**](https://peps.python.org/pep-3132/): Extended Iterable Unpacking.  You can now write things
...
* Dictionary comprehensions: `{k: v for k, v in stuff}` means the
  ...
  [**PEP 274**](https://peps.python.org/pep-0274/) vindicated. :-)
...
* New binary literals, e.g. `0b1010` (already in 2.6), and
  there is a new corresponding built-in function, [`bin()`](../library/functions.md#bin).
* Bytes literals are introduced with a leading `b` or `B`, and
  there is a new corresponding built-in function, [`bytes()`](../library/stdtypes.md#bytes).

### Changed Syntax

* [**PEP 3109**](https://peps.python.org/pep-3109/) and [**PEP 3134**](https://peps.python.org/pep-3134/): new [`raise`](../reference/simple_stmts.md#raise) statement syntax:
  `raise [*expr* [from *expr*]]`.  See below.
* `as` and [`with`](../reference/compound_stmts.md#with) are now reserved words.  (Since
  2.6, actually.)
* `True`, `False`, and `None` are reserved words.  (2.6 partially enforced
  the restrictions on `None` already.)
* Change from [`except`](../reference/compound_stmts.md#except) *exc*, *var* to
  `except` *exc* `as` *var*.  See [**PEP 3110**](https://peps.python.org/pep-3110/).
* [**PEP 3115**](https://peps.python.org/pep-3115/): New Metaclass Syntax.  Instead of:
  ...
  [`object`](../library/functions.md#object).)
* List comprehensions no longer support the syntactic form
  ...
  [`list()`](../library/stdtypes.md#list) constructor, and in particular the loop control
...
* [**PEP 3113**](https://peps.python.org/pep-3113/): Tuple parameter unpacking removed.  You can no longer
  write `def foo(a, (b, c)): ...`.
  Use `def foo(a, b_c): b, c = b_c` instead.
* Removed backticks (use [`repr()`](../library/functions.md#repr) instead).
* Removed `<>` (use `!=` instead).
* Removed keyword: [`exec()`](../library/functions.md#exec) is no longer a keyword; it remains as
  a function.  (Fortunately the function syntax was also accepted in
  2.x.)  Also note that [`exec()`](../library/functions.md#exec) no longer takes a stream argument;
  instead of `exec(f)` you can use `exec(f.read())`.
* Integer literals no longer support a trailing `l` or `L`.
* String literals no longer support a leading `u` or `U`.
* The [`from`](../reference/simple_stmts.md#from) *module* [`import`](../reference/simple_stmts.md#import) `*` syntax is only
  allowed at the module level, no longer inside functions.
* The only acceptable syntax for relative imports is `from .[*module*]
  import *name*`.  All [`import`](../reference/simple_stmts.md#import) forms not starting with `.` are
...
2.6.  The corresponding sections in [What’s New in Python 2.6](2.6.md#whats-new-in-2-6) should be
consulted for longer descriptions.

* [PEP 343: The ‘with’ statement](2.6.md#pep-0343).  The [`with`](../reference/compound_stmts.md#with) statement is now a standard
  feature and no longer needs to be imported from the [`__future__`](../library/__future__.md#module-__future__).
  Also check out [Writing Context Managers](2.6.md#new-26-context-managers) and
  [The contextlib module](2.6.md#new-module-contextlib).
* [PEP 366: Explicit Relative Imports From a Main Module](2.6.md#pep-0366).  This enhances the usefulness of the [`-m`](../using/cmdline.md#cmdoption-m)
  option when the referenced module lives in a package.
* [PEP 370: Per-user site-packages Directory](2.6.md#pep-0370).
* [PEP 371: The multiprocessing Package](2.6.md#pep-0371).
* [PEP 3101: Advanced String Formatting](2.6.md#pep-3101).  Note: the 2.6 description mentions the
  [`format()`](../library/functions.md#format) method for both 8-bit and Unicode strings.  In 3.0,
  only the [`str`](../library/stdtypes.md#str) type (text strings with Unicode support)
  supports this method; the [`bytes`](../library/stdtypes.md#bytes) type does not.  The plan is
  to eventually make this the only API for string formatting, and to
  start deprecating the `%` operator in Python 3.1.
* [PEP 3105: print As a Function](2.6.md#pep-3105).  This is now a standard feature and no longer needs
  to be imported from [`__future__`](../library/__future__.md#module-__future__).  More details were given above.
* [PEP 3110: Exception-Handling Changes](2.6.md#pep-3110).  The [`except`](../reference/compound_stmts.md#except) *exc* `as` *var*
  syntax is now standard and `except` *exc*, *var* is no
  longer supported.  (Of course, the `as` *var* part is still
  optional.)
* [PEP 3112: Byte Literals](2.6.md#pep-3112).  The `b"..."` string literal notation (and its
  variants like `b'...'`, `b"""..."""`, and `br"..."`) now
  produces a literal of type [`bytes`](../library/stdtypes.md#bytes).
* [PEP 3116: New I/O Library](2.6.md#pep-3116).  The [`io`](../library/io.md#module-io) module is now the standard way of
  doing file I/O.  The built-in [`open()`](../library/functions.md#open) function is now an
  alias for [`io.open()`](../library/io.md#io.open) and has additional keyword arguments
  *encoding*, *errors*, *newline* and *closefd*.  Also note that an
  invalid *mode* argument now raises [`ValueError`](../library/exceptions.md#ValueError), not
  [`IOError`](../library/exceptions.md#IOError).  The binary file object underlying a text file
...
* [PEP 3118: Revised Buffer Protocol](2.6.md#pep-3118).  The old builtin `buffer()` is now really gone;
  the new builtin [`memoryview()`](../library/stdtypes.md#memoryview) provides (mostly) similar
  functionality.
* [PEP 3119: Abstract Base Classes](2.6.md#pep-3119).  The [`abc`](../library/abc.md#module-abc) module and the ABCs defined in the
  [`collections`](../library/collections.md#module-collections) module plays a somewhat more prominent role in
  the language now, and built-in collection types like [`dict`](../library/stdtypes.md#dict)
  and [`list`](../library/stdtypes.md#list) conform to the [`collections.MutableMapping`](../library/collections.abc.md#collections.abc.MutableMapping)
  and [`collections.MutableSequence`](../library/collections.abc.md#collections.abc.MutableSequence) ABCs, respectively.
* [PEP 3127: Integer Literal Support and Syntax](2.6.md#pep-3127).  As mentioned above, the new octal literal
  notation is the only one supported, and binary literals have been
  added.
* [PEP 3129: Class Decorators](2.6.md#pep-3129).
* [PEP 3141: A Type Hierarchy for Numbers](2.6.md#pep-3141).  The [`numbers`](../library/numbers.md#module-numbers) module is another new use of
  ABCs, defining Python’s “numeric tower”.  Also note the new
  [`fractions`](../library/fractions.md#module-fractions) module which implements [`numbers.Rational`](../library/numbers.md#numbers.Rational).
...
* Many old modules were removed.  Some, like `gopherlib` (no
  longer used) and `md5` (replaced by [`hashlib`](../library/hashlib.md#module-hashlib)), were
...
* The `bsddb3` package was removed because its presence in the
  ...
  externally maintained at [https://www.jcea.es/programacion/pybsddb.htm](https://www.jcea.es/programacion/pybsddb.htm).
...
* A common pattern in Python 2.x is to have one version of a module
  ...
  implemented as a C extension; for example, [`pickle`](../library/pickle.md#module-pickle) and
  ...
  version.  The [`pickle`](../library/pickle.md#module-pickle) / `cPickle` pair received this
  treatment.  The [`profile`](../library/profile.md#module-profile) module is on the list for 3.1.  The
  `StringIO` module has been turned into a class in the [`io`](../library/io.md#module-io)
  module.
* Some related modules have been grouped into packages, and usually
  ...
  packages are:
  * [`dbm`](../library/dbm.md#module-dbm) (`anydbm`, `dbhash`, `dbm`,
    `dumbdbm`, `gdbm`, `whichdb`).
  * [`html`](../library/html.md#module-html) (`HTMLParser`, `htmlentitydefs`).
  * [`http`](../library/http.md#module-http) (`httplib`, `BaseHTTPServer`,
    `CGIHTTPServer`, `SimpleHTTPServer`, `Cookie`,
    `cookielib`).
  * [`tkinter`](../library/tkinter.md#module-tkinter) (all `Tkinter`-related modules except
    [`turtle`](../library/turtle.md#module-turtle)).  The target audience of [`turtle`](../library/turtle.md#module-turtle) doesn’t
    really care about [`tkinter`](../library/tkinter.md#module-tkinter).  Also note that as of Python
    2.6, the functionality of [`turtle`](../library/turtle.md#module-turtle) has been greatly enhanced.
  * [`urllib`](../library/urllib.md#module-urllib) (`urllib`, `urllib2`, `urlparse`,
    `robotparse`).
  * [`xmlrpc`](../library/xmlrpc.md#module-xmlrpc) (`xmlrpclib`, `DocXMLRPCServer`,
    `SimpleXMLRPCServer`).

Some other changes to standard library modules, not covered by
[**PEP 3108**](https://peps.python.org/pep-3108/):

* Killed `sets`.  Use the built-in [`set()`](../library/stdtypes.md#set) class.
* Cleanup of the [`sys`](../library/sys.md#module-sys) module: removed `sys.exitfunc()`,
  `sys.exc_clear()`, `sys.exc_type`, `sys.exc_value`,
  `sys.exc_traceback`.  (Note that [`sys.last_type`](../library/sys.md#sys.last_type)
  etc. remain.)
* Cleanup of the [`array.array`](../library/array.md#array.array) type: the `read()` and
  `write()` methods are gone; use [`fromfile()`](../library/array.md#array.array.fromfile) and
  [`tofile()`](../library/array.md#array.array.tofile) instead.  Also, the `'c'` typecode for array is
  gone – use either `'b'` for bytes or `'u'` for Unicode
  characters.
* Cleanup of the [`operator`](../library/operator.md#module-operator) module: removed
  `sequenceIncludes()` and `isCallable()`.
* Cleanup of the `thread` module: `acquire_lock()` and
  `release_lock()` are gone; use [`acquire()`](../library/threading.md#threading.Lock.acquire) and
  [`release()`](../library/threading.md#threading.Lock.release) instead.
* Cleanup of the [`random`](../library/random.md#module-random) module: removed the `jumpahead()` API.
* The `new` module is gone.
* The functions `os.tmpnam()`, `os.tempnam()` and
  `os.tmpfile()` have been removed in favor of the [`tempfile`](../library/tempfile.md#module-tempfile)
  module.
* The [`tokenize`](../library/tokenize.md#module-tokenize) module has been changed to work with bytes.  The
  main entry point is now [`tokenize.tokenize()`](../library/tokenize.md#tokenize.tokenize), instead of
  generate_tokens.
* `string.letters` and its friends (`string.lowercase` and
  `string.uppercase`) are gone.  Use
  [`string.ascii_letters`](../library/string.md#string.ascii_letters) etc. instead.  (The reason for the
  removal is that `string.letters` and friends had
  locale-specific behavior, which is a bad idea for such
  attractively named global “constants”.)
* Renamed module `__builtin__` to [`builtins`](../library/builtins.md#module-builtins) (removing the
  underscores, adding an ‘s’).  The `__builtins__` variable
  found in most global namespaces is unchanged.  To modify a builtin,
  you should use [`builtins`](../library/builtins.md#module-builtins), not `__builtins__`!
...
* [**PEP 352**](https://peps.python.org/pep-0352/): All exceptions must be derived (directly or indirectly)
  from [`BaseException`](../library/exceptions.md#BaseException).  This is the root of the exception
  hierarchy.  This is not new as a recommendation, but the
  *requirement* to inherit from [`BaseException`](../library/exceptions.md#BaseException) is new.  (Python
...
* Almost all exceptions should actually derive from [`Exception`](../library/exceptions.md#Exception);
  [`BaseException`](../library/exceptions.md#BaseException) should only be used as a base class for
  exceptions that should only be handled at the top level, such as
  [`SystemExit`](../library/exceptions.md#SystemExit) or [`KeyboardInterrupt`](../library/exceptions.md#KeyboardInterrupt).  The recommended
  idiom for handling all exceptions except for this latter category is
  to use [`except`](../reference/compound_stmts.md#except) [`Exception`](../library/exceptions.md#Exception).
* `StandardError` was removed.
* Exceptions no longer behave as sequences.  Use the [`args`](../library/exceptions.md#BaseException.args)
  attribute instead.
* [**PEP 3109**](https://peps.python.org/pep-3109/): Raising exceptions.  You must now use `raise
  *Exception*(*args*)` instead of `raise *Exception*, *args*`.
  ...
  [`__traceback__`](../library/exceptions.md#BaseException.__traceback__) attribute (see below).
* [**PEP 3110**](https://peps.python.org/pep-3110/): Catching exceptions.  You must now use
  ...
  *variable* is explicitly deleted when the [`except`](../reference/compound_stmts.md#except) block
  is left.
* [**PEP 3134**](https://peps.python.org/pep-3134/): Exception chaining.  There are two cases: implicit
  ...
  exception is raised in an [`except`](../reference/compound_stmts.md#except) or [`finally`](../reference/compound_stmts.md#finally)
  ...
  [`__context__`](../library/exceptions.md#BaseException.__context__) attribute of the secondary exception.
  ...
  [`__cause__`](../library/exceptions.md#BaseException.__cause__) attribute of the secondary exception.  The
  traceback printed when an unhandled exception occurs walks the chain
  of `__cause__` and [`__context__`](../library/exceptions.md#BaseException.__context__) attributes and
  prints a
...
* [**PEP 3134**](https://peps.python.org/pep-3134/): Exception objects now store their traceback as the
  [`__traceback__`](../library/exceptions.md#BaseException.__traceback__) attribute.  This means that an exception
  object now contains all the information pertaining to an exception,
  and there are fewer reasons to use [`sys.exc_info()`](../library/sys.md#sys.exc_info) (though the
...
* `__getslice__()`, `__setslice__()` and `__delslice__()`
  ...
  `a.__getitem__(slice(i, j))` (or [`__setitem__()`](../reference/datamodel.md#object.__setitem__) or
  [`__delitem__()`](../reference/datamodel.md#object.__delitem__), when used as an assignment or deletion target,
  respectively).
* [**PEP 3114**](https://peps.python.org/pep-3114/): the standard [`next()`](../library/functions.md#next) method has been renamed to
  [`__next__()`](../library/stdtypes.md#iterator.__next__).
* The `__oct__()` and `__hex__()` special methods are removed
  – [`oct()`](../library/functions.md#oct) and [`hex()`](../library/functions.md#hex) use [`__index__()`](../reference/datamodel.md#object.__index__) now to convert
...
* The function attributes named `func_X` have been renamed to
  ...
  `func_name` were renamed to [`__closure__`](../reference/datamodel.md#function.__closure__),
  [`__code__`](../reference/datamodel.md#function.__code__), [`__defaults__`](../reference/datamodel.md#function.__defaults__),
  [`__dict__`](../reference/datamodel.md#function.__dict__), [`__doc__`](../reference/datamodel.md#function.__doc__),
  [`__globals__`](../reference/datamodel.md#function.__globals__), [`__name__`](../reference/datamodel.md#function.__name__),
  respectively.
* `__nonzero__()` is now [`__bool__()`](../reference/datamodel.md#object.__bool__).

### Builtins

* [**PEP 3135**](https://peps.python.org/pep-3135/): New [`super()`](../library/functions.md#super).  You can now invoke [`super()`](../library/functions.md#super)
  without arguments and (assuming this is in a regular instance method
  defined inside a [`class`](../reference/compound_stmts.md#class) statement) the right class and
  instance will automatically be chosen.  With arguments, the behavior
  of [`super()`](../library/functions.md#super) is unchanged.
* [**PEP 3111**](https://peps.python.org/pep-3111/): `raw_input()` was renamed to [`input()`](../library/functions.md#input).  That
  is, the new [`input()`](../library/functions.md#input) function reads a line from
  [`sys.stdin`](../library/sys.md#sys.stdin) and returns it with the trailing newline stripped.
  It raises [`EOFError`](../library/exceptions.md#EOFError) if the input is terminated prematurely.
  To get the old behavior of [`input()`](../library/functions.md#input), use `eval(input())`.
* A new built-in function [`next()`](../library/functions.md#next) was added to call the
  [`__next__()`](../library/stdtypes.md#iterator.__next__) method on an object.
* The [`round()`](../library/functions.md#round) function rounding strategy and return type have
  ...
  arguments.
* Moved `intern()` to [`sys.intern()`](../library/sys.md#sys.intern).
* Removed: `apply()`.  Instead of `apply(f, args)` use
  `f(*args)`.
* Removed [`callable()`](../library/functions.md#callable).  Instead of `callable(f)` you can use
...
* Removed `execfile()`.  Instead of `execfile(fn)` use
  `exec(open(fn).read())`.
* Removed the `file` type.  Use [`open()`](../library/functions.md#open).  There are now several
  different kinds of streams that open can return in the [`io`](../library/io.md#module-io) module.
* Removed `reduce()`.  Use [`functools.reduce()`](../library/functools.md#functools.reduce) if you really
  need it; however, 99 percent of the time an explicit [`for`](../reference/compound_stmts.md#for)
  loop is more readable.
* Removed `reload()`.  Use `imp.reload()`.
* Removed. `dict.has_key()` – use the [`in`](../reference/expressions.md#in) operator
  instead.
...
* [**PEP 3123**](https://peps.python.org/pep-3123/): Making [`PyObject_HEAD`](../c-api/structures.md#c.PyObject_HEAD) conform to standard C.
...
* New C API `PyImport_ImportModuleNoBlock()`, works like
  [`PyImport_ImportModule()`](../c-api/import.md#c.PyImport_ImportModule) but won’t block on the import lock
...
For porting C extensions to Python 3.0, please see [Porting Extension Modules to Python 3](../howto/cporting.md#cporting-howto).
