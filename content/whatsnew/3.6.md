...
* [`secrets`](../library/secrets.md#module-secrets): [PEP 506 – Adding A Secrets Module To The Standard Library](#whatsnew36-pep506).

CPython implementation improvements:

* The [dict](../library/stdtypes.md#typesmapping) type has been reimplemented to use
...
* A new [file system path protocol](#whatsnew36-pep519) has been
  implemented to support [path-like objects](../glossary.md#term-path-like-object).
  All standard library functions operating on paths have been updated to
  work with the new protocol.
* The [`datetime`](../library/datetime.md#module-datetime) module has gained support for
  [Local Time Disambiguation](#whatsnew36-pep495).
* The [`typing`](../library/typing.md#module-typing) module received a number of
  [improvements](#whatsnew36-typing).
* The [`tracemalloc`](../library/tracemalloc.md#module-tracemalloc) module has been significantly reworked
  and is now used to provide better output for [`ResourceWarning`](../library/exceptions.md#ResourceWarning)
...
* The new [`secrets`](../library/secrets.md#module-secrets) module has been added to simplify the generation of
  cryptographically strong pseudo-random numbers suitable for
  managing secrets such as account authentication, tokens, and similar.
* On Linux, [`os.urandom()`](../library/os.md#os.urandom) now blocks until the system urandom entropy
  pool is initialized to increase the security. See the [**PEP 524**](https://peps.python.org/pep-0524/) for the
  rationale.
* The [`hashlib`](../library/hashlib.md#module-hashlib) and [`ssl`](../library/ssl.md#module-ssl) modules now support OpenSSL 1.1.0.
* The default settings and feature set of the [`ssl`](../library/ssl.md#module-ssl) module have been
  improved.
* The [`hashlib`](../library/hashlib.md#module-hashlib) module received support for the BLAKE2, SHA-3 and SHAKE
  hash algorithms and the [`scrypt()`](../library/hashlib.md#hashlib.scrypt) key derivation function.
...
* `python.exe` and `pythonw.exe` have been marked as long-path aware,
  ...
  See [removing the MAX_PATH limitation](../using/windows.md#max-path) for details.
* A `._pth` file can be added to force isolated mode and fully specify
  ...
  [the documentation](../using/windows.md#windows-finding-modules) for more information.
* A `python36.zip` file now works as a landmark to infer
  [`PYTHONHOME`](../using/cmdline.md#envvar-PYTHONHOME). See [the documentation](../using/windows.md#windows-finding-modules) for
  more information.

## New Features

<a id="whatsnew36-pep498"></a>
...
the format strings accepted by [`str.format()`](../library/stdtypes.md#str.format).  They contain replacement
...
[`format()`](../library/functions.md#format) protocol:
...
[Feature documentation](../reference/lexical_analysis.md#f-strings).

<a id="whatsnew36-pep526"></a>
...
The [string formatting](../library/string.md#formatspec) language also now has support
...
In order to allow zero-argument [`super()`](../library/functions.md#super) calls to work correctly from
[`__init_subclass__()`](../reference/datamodel.md#object.__init_subclass__) implementations, custom metaclasses must
ensure that the new `__classcell__` namespace entry is propagated to
`type.__new__` (as described in [Creating the class object](../reference/datamodel.md#class-object-creation)).
...
[Feature documentation](../reference/datamodel.md#class-customization)

<a id="whatsnew36-pep487-descriptors"></a>

### PEP 487: Descriptor Protocol Enhancements

[**PEP 487**](https://peps.python.org/pep-0487/) extends the descriptor protocol to include the new optional
[`__set_name__()`](../reference/datamodel.md#object.__set_name__) method.  Whenever a new class is defined, the new
...
[Feature documentation](../reference/datamodel.md#descriptors)

<a id="whatsnew36-pep519"></a>

### PEP 519: Adding a file system path protocol

File system paths have historically been represented as [`str`](../library/stdtypes.md#str)
or [`bytes`](../library/stdtypes.md#bytes) objects. This has led to people who write code which
operate on file system paths to assume that such objects are only one
of those two types (an [`int`](../library/functions.md#int) representing a file descriptor
...
paths like [`pathlib`](../library/pathlib.md#module-pathlib) from working with pre-existing code,
including Python’s standard library.

To fix this situation, a new interface represented by
[`os.PathLike`](../library/os.md#os.PathLike) has been defined. By implementing the
[`__fspath__()`](../library/os.md#os.PathLike.__fspath__) method, an object signals that it
represents a path. An object can then provide a low-level
representation of a file system path as a [`str`](../library/stdtypes.md#str) or
[`bytes`](../library/stdtypes.md#bytes) object. This means an object is considered
[path-like](../glossary.md#term-path-like-object) if it implements
[`os.PathLike`](../library/os.md#os.PathLike) or is a [`str`](../library/stdtypes.md#str) or [`bytes`](../library/stdtypes.md#bytes) object
which represents a file system path. Code can use [`os.fspath()`](../library/os.md#os.fspath),
[`os.fsdecode()`](../library/os.md#os.fsdecode), or [`os.fsencode()`](../library/os.md#os.fsencode) to explicitly get a
[`str`](../library/stdtypes.md#str) and/or [`bytes`](../library/stdtypes.md#bytes) representation of a path-like
object.

The built-in [`open()`](../library/functions.md#open) function has been updated to accept
[`os.PathLike`](../library/os.md#os.PathLike) objects, as have all relevant functions in the
[`os`](../library/os.md#module-os) and [`os.path`](../library/os.path.md#module-os.path) modules, and most other functions and
classes in the standard library.  The [`os.DirEntry`](../library/os.md#os.DirEntry) class
and relevant classes in [`pathlib`](../library/pathlib.md#module-pathlib) have also been updated to
implement [`os.PathLike`](../library/os.md#os.PathLike).
...
support all [path-like objects](../glossary.md#term-path-like-object) without any
code changes, or at least very minimal ones (e.g. calling
[`os.fspath()`](../library/os.md#os.fspath) at the beginning of code before operating on a
path-like object).

Here are some examples of how the new interface allows for
[`pathlib.Path`](../library/pathlib.md#pathlib.Path) to be used more easily and transparently with
pre-existing code:
...
[`datetime.datetime`](../library/datetime.md#datetime.datetime) and [`datetime.time`](../library/datetime.md#datetime.time) classes to differentiate
...
The values of the [`fold`](../library/datetime.md#datetime.datetime.fold) attribute have the
...
[`sys.getfilesystemencoding()`](../library/sys.md#sys.getfilesystemencoding), which now defaults to `'utf-8'`.

Applications that do not use str to represent paths should use
[`os.fsencode()`](../library/os.md#os.fsencode) and [`os.fsdecode()`](../library/os.md#os.fsdecode) to ensure their bytes are
correctly encoded. To revert to the previous behaviour, set
[`PYTHONLEGACYWINDOWSFSENCODING`](../using/cmdline.md#envvar-PYTHONLEGACYWINDOWSFSENCODING) or call
...
console use, set [`PYTHONLEGACYWINDOWSSTDIO`](../using/cmdline.md#envvar-PYTHONLEGACYWINDOWSSTDIO).
...
<a id="whatsnew36-pep520"></a>

### PEP 520: Preserving Class Attribute Definition Order

Attributes in a class definition body have a natural ordering: the same
order in which the names appear in the source.  This order is now
preserved in the new class’s [`__dict__`](../reference/datamodel.md#type.__dict__) attribute.

Also, the effective default class *execution* namespace (returned from
[type._\_prepare_\_()](../reference/datamodel.md#prepare)) is now an insertion-order-preserving
mapping.
...
#### SEE ALSO
[**PEP 468**](https://peps.python.org/pep-0468/) – Preserving Keyword Argument Order
: PEP written and implemented by Eric Snow.

<a id="whatsnew36-compactdict"></a>

### New [dict](../library/stdtypes.md#typesmapping) implementation

The [dict](../library/stdtypes.md#typesmapping) type now uses a “compact” representation
based on [a proposal by Raymond Hettinger](https://mail.python.org/pipermail/python-dev/2012-December/123028.html)
which was [first implemented by PyPy](https://morepypy.blogspot.com/2015/01/faster-more-memory-efficient-and-more.html).
The memory usage of the new [`dict()`](../library/stdtypes.md#dict) is between 20% and 25% smaller
...
<a id="whatsnew36-pythonmalloc"></a>

### PYTHONMALLOC environment variable

The new [`PYTHONMALLOC`](../using/cmdline.md#envvar-PYTHONMALLOC) environment variable allows setting the Python
...
* Detect violations of the Python memory allocator API. For example,
  [`PyObject_Free()`](../c-api/memory.md#c.PyObject_Free) called on a memory block allocated by
  [`PyMem_Malloc()`](../c-api/memory.md#c.PyMem_Malloc).
* Detect writes before the start of a buffer (buffer underflows)
* Detect writes after the end of a buffer (buffer overflows)
* Check that the [GIL](../glossary.md#term-global-interpreter-lock) is held when allocator
  functions of [`PYMEM_DOMAIN_OBJ`](../c-api/memory.md#c.PYMEM_DOMAIN_OBJ) (ex: [`PyObject_Malloc()`](../c-api/memory.md#c.PyObject_Malloc)) and
  [`PYMEM_DOMAIN_MEM`](../c-api/memory.md#c.PYMEM_DOMAIN_MEM) (ex: [`PyMem_Malloc()`](../c-api/memory.md#c.PyMem_Malloc)) domains are called.

Checking if the GIL is held is also a new feature of Python 3.6.

See the [`PyMem_SetupDebugHooks()`](../c-api/memory.md#c.PyMem_SetupDebugHooks) function for debug hooks on Python
memory allocators.
...
[`tracemalloc`](../library/tracemalloc.md#module-tracemalloc) module to get the traceback where a memory block was
allocated.
...
without the need to recompile specific [debug builds](../using/configure.md#debug-build) or
providing application-specific profiling/debugging code.

More details in [Instrumenting CPython with DTrace and SystemTap](../howto/instrumentation.md#instrumentation).
...
* A `global` or `nonlocal` statement must now textually appear
  ...
  Previously this was a [`SyntaxWarning`](../library/exceptions.md#SyntaxWarning).
* It is now possible to set a [special method](../reference/datamodel.md#specialnames) to
  `None` to indicate that the corresponding operation is not available.
  For example, if a class sets [`__iter__()`](../reference/datamodel.md#object.__iter__) to `None`, the class
  is not iterable.
...
* Import now raises the new exception [`ModuleNotFoundError`](../library/exceptions.md#ModuleNotFoundError)
  (subclass of [`ImportError`](../library/exceptions.md#ImportError)) when it cannot find a module.  Code
...
<a id="whatsnew36-pep506"></a>

### secrets

The main purpose of the new [`secrets`](../library/secrets.md#module-secrets) module is to provide an obvious way
...
#### WARNING
Note that the pseudo-random generators in the [`random`](../library/random.md#module-random) module
should *NOT* be used for security purposes.  Use [`secrets`](../library/secrets.md#module-secrets)
on Python 3.6+ and [`os.urandom()`](../library/os.md#os.urandom) on Python 3.5 and earlier.
...
Exhausted iterators of [`array.array`](../library/array.md#array.array) will now stay exhausted even
...
The new [`ast.Constant`](../library/ast.md#ast.Constant) AST node has been added.  It can be used
...
Notable changes in the [`asyncio`](../library/asyncio.md#module-asyncio) module since Python 3.5.0
(all backported to 3.5.x due to the provisional status):

* The [`get_event_loop()`](../library/asyncio-eventloop.md#asyncio.get_event_loop) function has been changed to
...
* The performance of [`StreamReader.readexactly()`](../library/asyncio-stream.md#asyncio.StreamReader.readexactly)
  has been improved.
  (Contributed by Mark Korenberg in [bpo-28370](https://bugs.python.org/issue?@action=redirect&bpo=28370).)
* The [`loop.getaddrinfo()`](../library/asyncio-eventloop.md#asyncio.loop.getaddrinfo)
  method is optimized to avoid calling the system `getaddrinfo`
  function if the address is already resolved.
  (Contributed by A. Jesse Jiryu Davis.)
* The [`loop.stop()`](../library/asyncio-eventloop.md#asyncio.loop.stop)
...
* [`Future.set_exception`](../library/asyncio-future.md#asyncio.Future.set_exception)
  will now raise [`TypeError`](../library/exceptions.md#TypeError) when passed an instance of
  the [`StopIteration`](../library/exceptions.md#StopIteration) exception.
  (Contributed by Chris Angelico in [bpo-26221](https://bugs.python.org/issue?@action=redirect&bpo=26221).)
* New [`loop.connect_accepted_socket()`](../library/asyncio-eventloop.md#asyncio.loop.connect_accepted_socket)
...
* New [`loop.shutdown_asyncgens()`](../library/asyncio-eventloop.md#asyncio.loop.shutdown_asyncgens)
...
* [`Future`](../library/asyncio-future.md#asyncio.Future) and [`Task`](../library/asyncio-task.md#asyncio.Task)
...
The [`b2a_base64()`](../library/binascii.md#binascii.b2a_base64) function now accepts an optional *newline*
...
The new [`cmath.tau`](../library/cmath.md#cmath.tau) ( *τ*) constant has been added.
...
New constants: [`cmath.inf`](../library/cmath.md#cmath.inf) and [`cmath.nan`](../library/cmath.md#cmath.nan) to
match [`math.inf`](../library/math.md#math.inf) and [`math.nan`](../library/math.md#math.nan), and also [`cmath.infj`](../library/cmath.md#cmath.infj)
and [`cmath.nanj`](../library/cmath.md#cmath.nanj) to match the format used by complex repr.
(Contributed by Mark Dickinson in [bpo-23229](https://bugs.python.org/issue?@action=redirect&bpo=23229).)

### collections

The new [`Collection`](../library/collections.abc.md#collections.abc.Collection) abstract base class has been
...
The new [`Reversible`](../library/collections.abc.md#collections.abc.Reversible) abstract base class represents
iterable classes that also provide the [`__reversed__()`](../reference/datamodel.md#object.__reversed__) method.
(Contributed by Ivan Levkivskyi in [bpo-25987](https://bugs.python.org/issue?@action=redirect&bpo=25987).)

The new [`AsyncGenerator`](../library/collections.abc.md#collections.abc.AsyncGenerator) abstract base class represents
asynchronous generators.
(Contributed by Yury Selivanov in [bpo-28720](https://bugs.python.org/issue?@action=redirect&bpo=28720).)

The [`namedtuple()`](../library/collections.md#collections.namedtuple) function now accepts an optional
keyword argument *module*, which, when specified, is used for
the [`__module__`](../reference/datamodel.md#type.__module__) attribute of the returned named tuple class.
...
[`namedtuple()`](../library/collections.md#collections.namedtuple) are now keyword-only.
(Contributed by Raymond Hettinger in [bpo-25628](https://bugs.python.org/issue?@action=redirect&bpo=25628).)

Recursive [`collections.deque`](../library/collections.md#collections.deque) instances can now be pickled.
(Contributed by Serhiy Storchaka in [bpo-26482](https://bugs.python.org/issue?@action=redirect&bpo=26482).)

### concurrent.futures

The [`ThreadPoolExecutor`](../library/concurrent.futures.md#concurrent.futures.ThreadPoolExecutor)
...
The [`contextlib.AbstractContextManager`](../library/contextlib.md#contextlib.AbstractContextManager) class has been added to
...
class has been added to the [`typing`](../library/typing.md#module-typing) module as
[`typing.ContextManager`](../library/typing.md#typing.ContextManager).
(Contributed by Brett Cannon in [bpo-25609](https://bugs.python.org/issue?@action=redirect&bpo=25609).)

### datetime

The [`datetime`](../library/datetime.md#datetime.datetime) and [`time`](../library/datetime.md#datetime.time) classes have
the new [`fold`](../library/datetime.md#datetime.time.fold) attribute used to disambiguate local time
when necessary.  Many functions in the [`datetime`](../library/datetime.md#module-datetime) have been
...
The [`datetime.strftime()`](../library/datetime.md#datetime.datetime.strftime) and
[`date.strftime()`](../library/datetime.md#datetime.date.strftime) methods now support
...
The [`datetime.isoformat()`](../library/datetime.md#datetime.datetime.isoformat) function
...
The [`datetime.combine()`](../library/datetime.md#datetime.datetime.combine) now
...
New [`Decimal.as_integer_ratio()`](../library/decimal.md#decimal.Decimal.as_integer_ratio)
method that returns a pair `(n, d)` of integers that represent the given
[`Decimal`](../library/decimal.md#decimal.Decimal) instance as a fraction, in lowest terms and
...
no longer provisional.  The [`email`](../library/email.md#module-email) documentation has been reorganized and
...
The [`email.mime`](../library/email.mime.md#module-email.mime) classes now all accept an optional *policy* keyword.
(Contributed by Berker Peksag in [bpo-27331](https://bugs.python.org/issue?@action=redirect&bpo=27331).)

The [`DecodedGenerator`](../library/email.generator.md#email.generator.DecodedGenerator) now supports the *policy*
keyword.

There is a new [`policy`](../library/email.policy.md#module-email.policy) attribute,
[`message_factory`](../library/email.policy.md#email.policy.Policy.message_factory), that controls what class is used
by default when the parser creates new message objects.  For the
[`email.policy.compat32`](../library/email.policy.md#email.policy.compat32) policy this is [`Message`](../library/email.compat32-message.md#email.message.Message),
for the new policies it is [`EmailMessage`](../library/email.message.md#email.message.EmailMessage).
...
Two new enumeration base classes have been added to the [`enum`](../library/enum.md#module-enum) module:
[`Flag`](../library/enum.md#enum.Flag) and [`IntFlag`](../library/enum.md#enum.IntFlag).  Both are used to define
...
[`IntFlag`](../library/enum.md#enum.IntFlag) class for their constants.

The new [`enum.auto`](../library/enum.md#enum.auto) value can be used to assign values to enum
...
On Windows, the [`faulthandler`](../library/faulthandler.md#module-faulthandler) module now installs a handler for Windows
exceptions: see [`faulthandler.enable()`](../library/faulthandler.md#faulthandler.enable). (Contributed by Victor Stinner in
[bpo-23848](https://bugs.python.org/issue?@action=redirect&bpo=23848).)

### fileinput

[`hook_encoded()`](../library/fileinput.md#fileinput.hook_encoded) now supports the *errors* argument.
(Contributed by Joseph Hackman in [bpo-25788](https://bugs.python.org/issue?@action=redirect&bpo=25788).)

### hashlib

[`hashlib`](../library/hashlib.md#module-hashlib) supports OpenSSL 1.1.0.  The minimum recommend version is 1.0.2.
(Contributed by Christian Heimes in [bpo-26470](https://bugs.python.org/issue?@action=redirect&bpo=26470).)

BLAKE2 hash functions were added to the module. [`blake2b()`](../library/hashlib.md#hashlib.blake2b)
and [`blake2s()`](../library/hashlib.md#hashlib.blake2s) are always available and support the full
...
The SHA-3 hash functions [`sha3_224()`](../library/hashlib.md#hashlib.sha3_224), [`sha3_256()`](../library/hashlib.md#hashlib.sha3_256),
[`sha3_384()`](../library/hashlib.md#hashlib.sha3_384), [`sha3_512()`](../library/hashlib.md#hashlib.sha3_512), and SHAKE hash functions
[`shake_128()`](../library/hashlib.md#hashlib.shake_128) and [`shake_256()`](../library/hashlib.md#hashlib.shake_256) were added.
...
The password-based key derivation function [`scrypt()`](../library/hashlib.md#hashlib.scrypt) is now
...
[`HTTPConnection.request()`](../library/http.client.md#http.client.HTTPConnection.request) and
[`endheaders()`](../library/http.client.md#http.client.HTTPConnection.endheaders) both now support
...
[`importlib.util.LazyLoader`](../library/importlib.md#importlib.util.LazyLoader) now calls
[`create_module()`](../library/importlib.md#importlib.abc.Loader.create_module) on the wrapped loader, removing the
restriction that [`importlib.machinery.BuiltinImporter`](../library/importlib.md#importlib.machinery.BuiltinImporter) and
[`importlib.machinery.ExtensionFileLoader`](../library/importlib.md#importlib.machinery.ExtensionFileLoader) couldn’t be used with
[`importlib.util.LazyLoader`](../library/importlib.md#importlib.util.LazyLoader).

[`importlib.util.cache_from_source()`](../library/importlib.md#importlib.util.cache_from_source),
[`importlib.util.source_from_cache()`](../library/importlib.md#importlib.util.source_from_cache), and
[`importlib.util.spec_from_file_location()`](../library/importlib.md#importlib.util.spec_from_file_location) now accept a
[path-like object](../glossary.md#term-path-like-object).

### inspect

The [`inspect.signature()`](../library/inspect.md#inspect.signature) function now reports the
...
[`inspect.getfullargspec()`](../library/inspect.md#inspect.getfullargspec) has been reversed. While this function is
convenient for single/source Python 2/3 code bases, the richer
[`inspect.signature()`](../library/inspect.md#inspect.signature) interface remains the recommended approach for new
code. (Contributed by Nick Coghlan in [bpo-27172](https://bugs.python.org/issue?@action=redirect&bpo=27172))

### json

[`json.load()`](../library/json.md#json.load) and [`json.loads()`](../library/json.md#json.loads) now support binary input.  Encoded
...
The new [`WatchedFileHandler.reopenIfNeeded()`](../library/logging.handlers.md#logging.handlers.WatchedFileHandler.reopenIfNeeded)
...
The tau ( *τ*) constant has been added to the [`math`](../library/math.md#module-math) and [`cmath`](../library/cmath.md#module-cmath)
modules.
(Contributed by Lisa Roach in [bpo-12345](https://bugs.python.org/issue?@action=redirect&bpo=12345), see [**PEP 628**](https://peps.python.org/pep-0628/) for details.)

### multiprocessing

[Proxy Objects](../library/multiprocessing.md#multiprocessing-proxy-objects) returned by
[`multiprocessing.Manager()`](../library/multiprocessing.md#multiprocessing.Manager) can now be nested.
...
[`os`](../library/os.md#module-os) and [`os.path`](../library/os.path.md#module-os.path) modules now support
[path-like objects](../glossary.md#term-path-like-object).

[`scandir()`](../library/os.md#os.scandir) now supports [`bytes`](../library/stdtypes.md#bytes) paths on Windows.

A new [`close()`](../library/os.md#os.scandir.close) method allows explicitly closing a
[`scandir()`](../library/os.md#os.scandir) iterator.  The [`scandir()`](../library/os.md#os.scandir) iterator now
supports the [context manager](../glossary.md#term-context-manager) protocol.  If a `scandir()`
iterator is neither exhausted nor explicitly closed a [`ResourceWarning`](../library/exceptions.md#ResourceWarning)
will be emitted in its destructor.
(Contributed by Serhiy Storchaka in [bpo-25994](https://bugs.python.org/issue?@action=redirect&bpo=25994).)

On Linux, [`os.urandom()`](../library/os.md#os.urandom) now blocks until the system urandom entropy pool
...
[`os.getrandom()`](../library/os.md#os.getrandom) function.
(Contributed by Victor Stinner, part of the [**PEP 524**](https://peps.python.org/pep-0524/))

### pathlib

[`pathlib`](../library/pathlib.md#module-pathlib) now supports [path-like objects](../glossary.md#term-path-like-object).
...
The [`Pdb`](../library/pdb.md#pdb.Pdb) class constructor has a new optional *readrc* argument
...
[`pickletools.dis()`](../library/pickletools.md#pickletools.dis) now outputs the implicit memo index for the
`MEMOIZE` opcode.
(Contributed by Serhiy Storchaka in [bpo-25382](https://bugs.python.org/issue?@action=redirect&bpo=25382).)

### pydoc

The [`pydoc`](../library/pydoc.md#module-pydoc) module has learned to respect the `MANPAGER`
environment variable.
(Contributed by Matthias Klose in [bpo-8637](https://bugs.python.org/issue?@action=redirect&bpo=8637).)

[`help()`](../library/functions.md#help) and [`pydoc`](../library/pydoc.md#module-pydoc) can now list named tuple fields in the
...
The new [`choices()`](../library/random.md#random.choices) function returns a list of elements of
...
[`Match`](../library/re.md#re.Match) objects now support
[`index-like objects`](../reference/datamodel.md#object.__index__) as group
indices.
(Contributed by Jeroen Demeyer and Xiang Zhang in [bpo-27177](https://bugs.python.org/issue?@action=redirect&bpo=27177).)

### readline

Added [`set_auto_history()`](../library/readline.md#readline.set_auto_history) to enable or disable
...
The [`shlex`](../library/shlex.md#shlex.shlex) has much
[improved shell compatibility](../library/shlex.md#improved-shell-compatibility)
...
When specifying paths to add to [`sys.path`](../library/sys.md#sys.path) in a `.pth` file,
you may now specify file paths on top of directories (e.g. zip files).
(Contributed by Wolfgang Langner in [bpo-26587](https://bugs.python.org/issue?@action=redirect&bpo=26587)).

### sqlite3

[`sqlite3.Cursor.lastrowid`](../library/sqlite3.md#sqlite3.Cursor.lastrowid) now supports the `REPLACE` statement.
(Contributed by Alex LordThorsen in [bpo-16864](https://bugs.python.org/issue?@action=redirect&bpo=16864).)

### socket

The [`ioctl()`](../library/socket.md#socket.socket.ioctl) function now supports the
[`SIO_LOOPBACK_FAST_PATH`](../library/socket.md#socket.SIO_LOOPBACK_FAST_PATH) control code.
(Contributed by Daniel Stokes in [bpo-26536](https://bugs.python.org/issue?@action=redirect&bpo=26536).)

The [`getsockopt()`](../library/socket.md#socket.socket.getsockopt) constants `SO_DOMAIN`,
...
The [`setsockopt()`](../library/socket.md#socket.socket.setsockopt) now supports the
...
[`AF_ALG`](../library/socket.md#socket.AF_ALG) to interface with Linux Kernel crypto API. `ALG_*`,
`SOL_ALG` and [`sendmsg_afalg()`](../library/socket.md#socket.socket.sendmsg_afalg) were added.
...
Servers based on the [`socketserver`](../library/socketserver.md#module-socketserver) module, including those
defined in [`http.server`](../library/http.server.md#module-http.server), [`xmlrpc.server`](../library/xmlrpc.server.md#module-xmlrpc.server) and
[`wsgiref.simple_server`](../library/wsgiref.md#module-wsgiref.simple_server), now support the [context manager](../glossary.md#term-context-manager)
protocol.
(Contributed by Aviv Palivoda in [bpo-26404](https://bugs.python.org/issue?@action=redirect&bpo=26404).)

The [`wfile`](../library/socketserver.md#socketserver.DatagramRequestHandler.wfile) attribute of
[`StreamRequestHandler`](../library/socketserver.md#socketserver.StreamRequestHandler) classes now implements
the [`io.BufferedIOBase`](../library/io.md#io.BufferedIOBase) writable interface.  In particular,
calling [`write()`](../library/io.md#io.BufferedIOBase.write) is now guaranteed to send the
data in full.  (Contributed by Martin Panter in [bpo-26721](https://bugs.python.org/issue?@action=redirect&bpo=26721).)

### ssl

[`ssl`](../library/ssl.md#module-ssl) supports OpenSSL 1.1.0.  The minimum recommend version is 1.0.2.
(Contributed by Christian Heimes in [bpo-26470](https://bugs.python.org/issue?@action=redirect&bpo=26470).)
...
[`SSLContext`](../library/ssl.md#ssl.SSLContext) has better default configuration for options
and ciphers.
...
with the new [`SSLSession`](../library/ssl.md#ssl.SSLSession) class.  TLS session resumption can
...
The new [`get_ciphers()`](../library/ssl.md#ssl.SSLContext.get_ciphers) method can be used to
get a list of enabled ciphers in order of cipher priority.

All constants and flags have been converted to [`IntEnum`](../library/enum.md#enum.IntEnum) and
[`IntFlag`](../library/enum.md#enum.IntFlag).
(Contributed by Christian Heimes in [bpo-28025](https://bugs.python.org/issue?@action=redirect&bpo=28025).)

Server and client-side specific TLS protocols for [`SSLContext`](../library/ssl.md#ssl.SSLContext)
were added.
...
A new [`harmonic_mean()`](../library/statistics.md#statistics.harmonic_mean) function has been added.
(Contributed by Steven D’Aprano in [bpo-27181](https://bugs.python.org/issue?@action=redirect&bpo=27181).)

### struct

[`struct`](../library/struct.md#module-struct) now supports IEEE 754 half-precision floats via the `'e'`
format specifier.
(Contributed by Eli Stevens, Mark Dickinson in [bpo-11734](https://bugs.python.org/issue?@action=redirect&bpo=11734).)

### subprocess

[`subprocess.Popen`](../library/subprocess.md#subprocess.Popen) destructor now emits a [`ResourceWarning`](../library/exceptions.md#ResourceWarning) warning
if the child process is still running. Use the context manager protocol (`with
proc: ...`) or explicitly call the [`wait()`](../library/subprocess.md#subprocess.Popen.wait) method to
...
The [`subprocess.Popen`](../library/subprocess.md#subprocess.Popen) constructor and all functions that pass arguments
...
The new [`getfilesystemencodeerrors()`](../library/sys.md#sys.getfilesystemencodeerrors) function returns the name of
...
On Windows the return value of the [`getwindowsversion()`](../library/sys.md#sys.getwindowsversion) function
...
The [`struct_time`](../library/time.md#time.struct_time) attributes `tm_gmtoff` and
`tm_zone` are now available on all platforms.

### timeit

The new [`Timer.autorange()`](../library/timeit.md#timeit.Timer.autorange) convenience
method has been added to call [`Timer.timeit()`](../library/timeit.md#timeit.Timer.timeit)
...
[`timeit`](../library/timeit.md#module-timeit) now warns when there is substantial (4x) variance
...
`Variable.trace_remove()` and `trace_info()`
...
The [`tracemalloc`](../library/tracemalloc.md#module-tracemalloc) module now supports tracing memory allocations in
multiple different address spaces.

The new [`DomainFilter`](../library/tracemalloc.md#tracemalloc.DomainFilter) filter class has been added
...
<a id="whatsnew36-typing"></a>

### typing

Since the [`typing`](../library/typing.md#module-typing) module is [provisional](../glossary.md#term-provisional-API),
all changes introduced in Python 3.6 have also been
backported to Python 3.5.x.

The [`typing`](../library/typing.md#module-typing) module has a much improved support for generic type
...
The [`typing.ContextManager`](../library/typing.md#typing.ContextManager) class has been added for
representing [`contextlib.AbstractContextManager`](../library/contextlib.md#contextlib.AbstractContextManager).
(Contributed by Brett Cannon in [bpo-25609](https://bugs.python.org/issue?@action=redirect&bpo=25609).)

The [`typing.Collection`](../library/typing.md#typing.Collection) class has been added for
representing [`collections.abc.Collection`](../library/collections.abc.md#collections.abc.Collection).
(Contributed by Ivan Levkivskyi in [bpo-27598](https://bugs.python.org/issue?@action=redirect&bpo=27598).)

The [`typing.ClassVar`](../library/typing.md#typing.ClassVar) type construct has been added to
...
A new [`TYPE_CHECKING`](../library/typing.md#typing.TYPE_CHECKING) constant that is assumed to be
...
A new [`NewType()`](../library/typing.md#typing.NewType) helper function has been added to create
...
The [`unicodedata`](../library/unicodedata.md#module-unicodedata) module now uses data from [Unicode 9.0.0](https://unicode.org/versions/Unicode9.0.0/).
...
* Two new methods, [`Mock.assert_called()`](../library/unittest.mock.md#unittest.mock.Mock.assert_called) and [`Mock.assert_called_once()`](../library/unittest.mock.md#unittest.mock.Mock.assert_called_once) to check if the mock object
  was called.
  (Contributed by Amit Saha in [bpo-26323](https://bugs.python.org/issue?@action=redirect&bpo=26323).)
* The [`Mock.reset_mock()`](../library/unittest.mock.md#unittest.mock.Mock.reset_mock) method
...
throwing an error, [`AbstractHTTPHandler`](../library/urllib.request.md#urllib.request.HTTPHandler)
now falls back to use chunked transfer encoding.
(Contributed by Demian Brecht and Rolf Krahl in [bpo-12319](https://bugs.python.org/issue?@action=redirect&bpo=12319).)

### urllib.robotparser

[`RobotFileParser`](../library/urllib.robotparser.md#urllib.robotparser.RobotFileParser) now supports the `Crawl-delay` and
...
[`venv`](../library/venv.md#module-venv) accepts a new parameter `--prompt`. This parameter provides an
...
[`warnings.warn_explicit()`](../library/warnings.md#warnings.warn_explicit) function: the destroyed object which emitted a
[`ResourceWarning`](../library/exceptions.md#ResourceWarning). A *source* attribute has also been added to
...
When a [`ResourceWarning`](../library/exceptions.md#ResourceWarning) warning is logged, the [`tracemalloc`](../library/tracemalloc.md#module-tracemalloc) module is now
...
[`tracemalloc`](../library/tracemalloc.md#module-tracemalloc) is tracing Python memory allocations and if the
[`warnings`](../library/warnings.md#module-warnings) module was already imported.

### winreg

Added the 64-bit integer type [`REG_QWORD`](../library/winreg.md#winreg.REG_QWORD).
(Contributed by Clement Rouault in [bpo-23026](https://bugs.python.org/issue?@action=redirect&bpo=23026).)

### winsound

Allowed keyword arguments to be passed to [`Beep`](../library/winsound.md#winsound.Beep),
[`MessageBeep`](../library/winsound.md#winsound.MessageBeep), and [`PlaySound`](../library/winsound.md#winsound.PlaySound) ([bpo-27982](https://bugs.python.org/issue?@action=redirect&bpo=27982)).

### xmlrpc.client

The [`xmlrpc.client`](../library/xmlrpc.client.md#module-xmlrpc.client) module now supports unmarshalling
...
A new [`ZipInfo.from_file()`](../library/zipfile.md#zipfile.ZipInfo.from_file) class method
allows making a [`ZipInfo`](../library/zipfile.md#zipfile.ZipInfo) instance from a filesystem file.
A new [`ZipInfo.is_dir()`](../library/zipfile.md#zipfile.ZipInfo.is_dir) method can be used
to check if the [`ZipInfo`](../library/zipfile.md#zipfile.ZipInfo) instance represents a directory.
(Contributed by Thomas Kluyver in [bpo-26039](https://bugs.python.org/issue?@action=redirect&bpo=26039).)

The [`ZipFile.open()`](../library/zipfile.md#zipfile.ZipFile.open) method can now be used to
write data into a ZIP file, as well as for extracting data.
(Contributed by Thomas Kluyver in [bpo-26039](https://bugs.python.org/issue?@action=redirect&bpo=26039).)

### zlib

The [`compress()`](../library/zlib.md#zlib.compress) and [`decompress()`](../library/zlib.md#zlib.decompress) functions now accept
keyword arguments.
...
* The [`asyncio.Future`](../library/asyncio-future.md#asyncio.Future) class now has an optimized C implementation.
  (Contributed by Yury Selivanov and INADA Naoki in [bpo-26081](https://bugs.python.org/issue?@action=redirect&bpo=26081).)
* The [`asyncio.Task`](../library/asyncio-task.md#asyncio.Task) class now has an optimized
  C implementation. (Contributed by Yury Selivanov in [bpo-28544](https://bugs.python.org/issue?@action=redirect&bpo=28544).)
* Various implementation improvements in the [`typing`](../library/typing.md#module-typing) module
...
* Optimize [`bytes.fromhex()`](../library/stdtypes.md#bytes.fromhex) and [`bytearray.fromhex()`](../library/stdtypes.md#bytearray.fromhex): they are now
...
* Allocator functions of the [`PyMem_Malloc()`](../c-api/memory.md#c.PyMem_Malloc) domain
  ([`PYMEM_DOMAIN_MEM`](../c-api/memory.md#c.PYMEM_DOMAIN_MEM)) now use the [pymalloc memory allocator](../c-api/memory.md#pymalloc) instead of `malloc()` function of the C library. The
...
* [`pickle.load()`](../library/pickle.md#pickle.load) and [`pickle.loads()`](../library/pickle.md#pickle.loads) are now up to 10% faster when
...
* Passing [keyword arguments](../glossary.md#term-keyword-argument) to a function has an
  overhead in comparison with passing [positional arguments](../glossary.md#term-positional-argument).  Now in extension functions implemented with using
...
* Optimized [`glob()`](../library/glob.md#glob.glob) and [`iglob()`](../library/glob.md#glob.iglob) functions in the
  [`glob`](../library/glob.md#module-glob) module; they are now about 3–6 times faster.
  (Contributed by Serhiy Storchaka in [bpo-25596](https://bugs.python.org/issue?@action=redirect&bpo=25596)).
* Optimized globbing in [`pathlib`](../library/pathlib.md#module-pathlib) by using [`os.scandir()`](../library/os.md#os.scandir);
...
* [`xml.etree.ElementTree`](../library/xml.etree.elementtree.md#module-xml.etree.ElementTree) parsing, iteration and deepcopy performance
...
* Creation of [`fractions.Fraction`](../library/fractions.md#fractions.Fraction) instances from floats and
...
* The [GIL](../glossary.md#term-global-interpreter-lock) must now be held when allocator
  functions of [`PYMEM_DOMAIN_OBJ`](../c-api/memory.md#c.PYMEM_DOMAIN_OBJ) (ex: [`PyObject_Malloc()`](../c-api/memory.md#c.PyObject_Malloc)) and
  [`PYMEM_DOMAIN_MEM`](../c-api/memory.md#c.PYMEM_DOMAIN_MEM) (ex: [`PyMem_Malloc()`](../c-api/memory.md#c.PyMem_Malloc)) domains are called.
* New [`Py_FinalizeEx()`](../c-api/interp-lifecycle.md#c.Py_FinalizeEx) API which indicates if flushing buffered data
  failed.
  (Contributed by Martin Panter in [bpo-5319](https://bugs.python.org/issue?@action=redirect&bpo=5319).)
* [`PyArg_ParseTupleAndKeywords()`](../c-api/arg.md#c.PyArg_ParseTupleAndKeywords) now supports [positional-only
  parameters](../glossary.md#positional-only-parameter).  Positional-only parameters are
...
* `PyTraceback_Print` method now abbreviates long sequences of repeated lines
  ...
  (Contributed by Emanuel Barry in [bpo-26823](https://bugs.python.org/issue?@action=redirect&bpo=26823).)
* The new [`PyErr_SetImportErrorSubclass()`](../c-api/exceptions.md#c.PyErr_SetImportErrorSubclass) function allows for
  specifying a subclass of [`ImportError`](../library/exceptions.md#ImportError) to raise.
  (Contributed by Eric Snow in [bpo-15767](https://bugs.python.org/issue?@action=redirect&bpo=15767).)
* The new [`PyErr_ResourceWarning()`](../c-api/exceptions.md#c.PyErr_ResourceWarning) function can be used to generate
  a [`ResourceWarning`](../library/exceptions.md#ResourceWarning) providing the source of the resource allocation.
  (Contributed by Victor Stinner in [bpo-26567](https://bugs.python.org/issue?@action=redirect&bpo=26567).)
* The new [`PyOS_FSPath()`](../c-api/sys.md#c.PyOS_FSPath) function returns the file system
  representation of a [path-like object](../glossary.md#term-path-like-object).
  (Contributed by Brett Cannon in [bpo-27186](https://bugs.python.org/issue?@action=redirect&bpo=27186).)
* The [`PyUnicode_FSConverter()`](../c-api/unicode.md#c.PyUnicode_FSConverter) and [`PyUnicode_FSDecoder()`](../c-api/unicode.md#c.PyUnicode_FSDecoder)
  functions will now accept [path-like objects](../glossary.md#term-path-like-object).

## Other Improvements

* When [`--version`](../using/cmdline.md#cmdoption-version) (short form: [`-V`](../using/cmdline.md#cmdoption-V)) is supplied twice,
  Python prints [`sys.version`](../library/sys.md#sys.version) for detailed information.
  ```shell-session
  $ ./python -VV
...
`async` or `await` as names will generate a [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning).

### Deprecated Python behavior

Raising the [`StopIteration`](../library/exceptions.md#StopIteration) exception inside a generator will now
generate a [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning), and will trigger a [`RuntimeError`](../library/exceptions.md#RuntimeError)
in Python 3.7.  See [PEP 479: Change StopIteration handling inside generators](3.5.md#whatsnew-pep-479) for details.

The [`__aiter__()`](../reference/datamodel.md#object.__aiter__) method is now expected to return an asynchronous
iterator directly instead of returning an awaitable as previously.
Doing the former will trigger a [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning).  Backward
...
a [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning).  Although this will eventually become a
[`SyntaxError`](../library/exceptions.md#SyntaxError), that will not be for several Python releases.
...
`__package__` are not defined now raises an [`ImportWarning`](../library/exceptions.md#ImportWarning).
...
The `asynchat` has been deprecated in favor of [`asyncio`](../library/asyncio.md#module-asyncio).
(Contributed by Mariatta in [bpo-25002](https://bugs.python.org/issue?@action=redirect&bpo=25002).)

#### asyncore

The `asyncore` has been deprecated in favor of [`asyncio`](../library/asyncio.md#module-asyncio).
(Contributed by Mariatta in [bpo-25002](https://bugs.python.org/issue?@action=redirect&bpo=25002).)

#### dbm

Unlike other [`dbm`](../library/dbm.md#module-dbm) implementations, the [`dbm.dumb`](../library/dbm.md#module-dbm.dumb) module
...
The support of non-integer arguments in [`getgrgid()`](../library/grp.md#grp.getgrgid) has been
deprecated.
...
`importlib.abc.Loader.load_module` in [`importlib`](../library/importlib.md#module-importlib) that had not
been deprecated in previous versions of Python in favour of
[`importlib.abc.Loader.exec_module()`](../library/importlib.md#importlib.abc.Loader.exec_module).

The [`importlib.machinery.WindowsRegistryFinder`](../library/importlib.md#importlib.machinery.WindowsRegistryFinder) class is now
deprecated. As of 3.6.0, it is still added to [`sys.meta_path`](../library/sys.md#sys.meta_path) by
default (on Windows), but this may change in future releases.

#### os

Undocumented support of general [bytes-like objects](../glossary.md#term-bytes-like-object)
as paths in [`os`](../library/os.md#module-os) functions, [`compile()`](../library/functions.md#compile) and similar functions is
now deprecated.
...
In the future the [`ssl`](../library/ssl.md#module-ssl) module will require at least OpenSSL 1.0.2 or
1.1.0.

SSL-related arguments like `certfile`, `keyfile` and `check_hostname`
in [`ftplib`](../library/ftplib.md#module-ftplib), [`http.client`](../library/http.client.md#module-http.client), [`imaplib`](../library/imaplib.md#module-imaplib), [`poplib`](../library/poplib.md#module-poplib),
and [`smtplib`](../library/smtplib.md#module-smtplib) have been deprecated in favor of `context`.
(Contributed by Christian Heimes in [bpo-28022](https://bugs.python.org/issue?@action=redirect&bpo=28022).)

A couple of protocols and functions of the [`ssl`](../library/ssl.md#module-ssl) module are now
...
The `tkinter.tix` module is now deprecated.  [`tkinter`](../library/tkinter.md#module-tkinter) users
should use [`tkinter.ttk`](../library/tkinter.ttk.md#module-tkinter.ttk) instead.

<a id="whatsnew36-venv"></a>
...
Use the [generic codec based API](../c-api/codec.md#codec-registry) instead.
...
* Unknown escapes consisting of `'\'` and an ASCII letter in
  ...
  [`re.sub()`](../library/re.md#re.sub) they are still allowed, but deprecated.
  The [`re.LOCALE`](../library/re.md#re.LOCALE) flag can now only be used with binary patterns.
* `inspect.getmoduleinfo()` was removed (was deprecated since CPython 3.3).
  [`inspect.getmodulename()`](../library/inspect.md#inspect.getmodulename) should be used for obtaining the module
...
* `traceback.Ignore` class and `traceback.usage`, `traceback.modname`,
  ...
  [`traceback`](../library/traceback.md#module-traceback) module. They were undocumented methods deprecated since
...
* The `tk_menuBar()` and `tk_bindForTraversal()` dummy methods in
  [`tkinter`](../library/tkinter.md#module-tkinter) widget classes were removed (corresponding Tk commands
  were obsolete since Tk 4.0).
* The [`open()`](../library/zipfile.md#zipfile.ZipFile.open) method of the [`zipfile.ZipFile`](../library/zipfile.md#zipfile.ZipFile)
  class no longer supports the `'U'` mode (was deprecated since Python 3.4).
  Use [`io.TextIOWrapper`](../library/io.md#io.TextIOWrapper) for reading compressed text files in
  [universal newlines](../glossary.md#term-universal-newlines) mode.
...
* [`open()`](../library/functions.md#open) will no longer allow combining the `'U'` mode flag
  with `'+'`.
  (Contributed by Jeff Balogh and John O’Connor in [bpo-2091](https://bugs.python.org/issue?@action=redirect&bpo=2091).)
* [`sqlite3`](../library/sqlite3.md#module-sqlite3) no longer implicitly commits an open transaction before DDL
  statements.
* On Linux, [`os.urandom()`](../library/os.md#os.urandom) now blocks until the system urandom entropy pool
  is initialized to increase the security.
* When [`importlib.abc.Loader.exec_module()`](../library/importlib.md#importlib.abc.Loader.exec_module) is defined,
  [`importlib.abc.Loader.create_module()`](../library/importlib.md#importlib.abc.Loader.create_module) must also be defined.
...
* The format of the [`co_lnotab`](../reference/datamodel.md#codeobject.co_lnotab) attribute of code objects
  changed to support
  a negative line number delta. By default, Python does not emit bytecode with
  a negative line number delta. Functions using [`frame.f_lineno`](../reference/datamodel.md#frame.f_lineno),
...
* The functions in the [`compileall`](../library/compileall.md#module-compileall) module now return booleans instead
...
* Reading the `port` attribute of
  [`urllib.parse.urlsplit()`](../library/urllib.parse.md#urllib.parse.urlsplit) and [`urlparse()`](../library/urllib.parse.md#urllib.parse.urlparse) results
  now raises [`ValueError`](../library/exceptions.md#ValueError) for out-of-range values, rather than
  returning [`None`](../library/constants.md#None).  See [bpo-20059](https://bugs.python.org/issue?@action=redirect&bpo=20059).
* The `imp` module now raises a [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning) instead of
...
* The following modules have had missing APIs added to their
  [`__all__`](../reference/simple_stmts.md#module.__all__) attributes to match the documented APIs:
  [`calendar`](../library/calendar.md#module-calendar), `cgi`, [`csv`](../library/csv.md#module-csv),
  [`ElementTree`](../library/xml.etree.elementtree.md#module-xml.etree.ElementTree), [`enum`](../library/enum.md#module-enum),
  [`fileinput`](../library/fileinput.md#module-fileinput), [`ftplib`](../library/ftplib.md#module-ftplib), [`logging`](../library/logging.md#module-logging), [`mailbox`](../library/mailbox.md#module-mailbox),
  [`mimetypes`](../library/mimetypes.md#module-mimetypes), [`optparse`](../library/optparse.md#module-optparse), [`plistlib`](../library/plistlib.md#module-plistlib), `smtpd`,
  [`subprocess`](../library/subprocess.md#module-subprocess), [`tarfile`](../library/tarfile.md#module-tarfile), [`threading`](../library/threading.md#module-threading) and
  [`wave`](../library/wave.md#module-wave).  This means they will export new symbols when `import *`
  is used.
...
* When performing a relative import, if `__package__` does not compare equal
  to `__spec__.parent` then [`ImportWarning`](../library/exceptions.md#ImportWarning) is raised.
...
* When a relative import is performed and no parent package is known, then
  [`ImportError`](../library/exceptions.md#ImportError) will be raised. Previously, [`SystemError`](../library/exceptions.md#SystemError) could be
  raised. (Contributed by Brett Cannon in [bpo-18018](https://bugs.python.org/issue?@action=redirect&bpo=18018).)
* Servers based on the [`socketserver`](../library/socketserver.md#module-socketserver) module, including those
  defined in [`http.server`](../library/http.server.md#module-http.server), [`xmlrpc.server`](../library/xmlrpc.server.md#module-xmlrpc.server) and
  [`wsgiref.simple_server`](../library/wsgiref.md#module-wsgiref.simple_server), now only catch exceptions derived
  from [`Exception`](../library/exceptions.md#Exception). Therefore if a request handler raises
  an exception like [`SystemExit`](../library/exceptions.md#SystemExit) or [`KeyboardInterrupt`](../library/exceptions.md#KeyboardInterrupt),
  [`handle_error()`](../library/socketserver.md#socketserver.BaseServer.handle_error) is no longer called, and
...
* `spwd.getspnam()` now raises a [`PermissionError`](../library/exceptions.md#PermissionError) instead of
  [`KeyError`](../library/exceptions.md#KeyError) if the user doesn’t have privileges.
* The [`socket.socket.close()`](../library/socket.md#socket.socket.close) method now raises an exception if
...
* All optional arguments of the [`dump()`](../library/json.md#json.dump), [`dumps()`](../library/json.md#json.dumps),
  [`load()`](../library/json.md#json.load) and [`loads()`](../library/json.md#json.loads) functions and
  [`JSONEncoder`](../library/json.md#json.JSONEncoder) and [`JSONDecoder`](../library/json.md#json.JSONDecoder) class
  constructors in the [`json`](../library/json.md#module-json) module are now [keyword-only](../glossary.md#keyword-only-parameter).
  (Contributed by Serhiy Storchaka in [bpo-18726](https://bugs.python.org/issue?@action=redirect&bpo=18726).)
* Subclasses of [`type`](../library/functions.md#type) which don’t override `type.__new__` may no
  longer use the one-argument form to get the type of an object.
* As part of [**PEP 487**](https://peps.python.org/pep-0487/), the handling of keyword arguments passed to
  [`type`](../library/functions.md#type) (other than the metaclass hint, `metaclass`) is now
  consistently delegated to [`object.__init_subclass__()`](../reference/datamodel.md#object.__init_subclass__). This means that
  [`type.__new__`](../reference/datamodel.md#object.__new__) and [`type.__init__`](../reference/datamodel.md#object.__init__) both now accept arbitrary keyword arguments,
  but [`object.__init_subclass__()`](../reference/datamodel.md#object.__init_subclass__) (which is called from
  [`type.__new__`](../reference/datamodel.md#object.__new__)) will reject them by default.
  Custom metaclasses accepting additional keyword arguments will need to adjust
  their calls to [`type.__new__`](../reference/datamodel.md#object.__new__)
  (whether direct or via [`super`](../library/functions.md#super)) accordingly.
...
* In the [`urllib.request`](../library/urllib.request.md#module-urllib.request) module and the
  [`http.client.HTTPConnection.request()`](../library/http.client.md#http.client.HTTPConnection.request) method, if no Content-Length
...
* The [`DictReader`](../library/csv.md#csv.DictReader) now returns rows of type
  [`OrderedDict`](../library/collections.md#collections.OrderedDict).
...
* The *verbose* and *rename* arguments for
  [`namedtuple()`](../library/collections.md#collections.namedtuple) are now keyword-only.
  (Contributed by Raymond Hettinger in [bpo-25628](https://bugs.python.org/issue?@action=redirect&bpo=25628).)
* On Linux, [`ctypes.util.find_library()`](../library/ctypes.md#ctypes.util.find_library) now looks in
...
* The [`imaplib.IMAP4`](../library/imaplib.md#imaplib.IMAP4) class now handles flags containing the
...
* The [`mmap.mmap.write()`](../library/mmap.md#mmap.mmap.write) function now returns the number
...
* The [`pkgutil.iter_modules()`](../library/pkgutil.md#pkgutil.iter_modules) and [`pkgutil.walk_packages()`](../library/pkgutil.md#pkgutil.walk_packages)
  functions now return [`ModuleInfo`](../library/pkgutil.md#pkgutil.ModuleInfo) named tuples.
  (Contributed by Ramchandra Apte in [bpo-17211](https://bugs.python.org/issue?@action=redirect&bpo=17211).)
* [`re.sub()`](../library/re.md#re.sub) now raises an error for invalid numerical group
...
* [`zipfile.ZipFile`](../library/zipfile.md#zipfile.ZipFile) will now raise [`NotImplementedError`](../library/exceptions.md#NotImplementedError) for
  unrecognized compression values.  Previously a plain [`RuntimeError`](../library/exceptions.md#RuntimeError)
  was raised.  Additionally, calling [`ZipFile`](../library/zipfile.md#zipfile.ZipFile) methods
  on a closed ZipFile or calling the [`write()`](../library/zipfile.md#zipfile.ZipFile.write) method
  on a ZipFile created with mode `'r'` will raise a [`ValueError`](../library/exceptions.md#ValueError).
  Previously, a [`RuntimeError`](../library/exceptions.md#RuntimeError) was raised in those scenarios.
* when custom metaclasses are combined with zero-argument [`super()`](../library/functions.md#super) or
  ...
  a [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning) in Python 3.6 and a [`RuntimeError`](../library/exceptions.md#RuntimeError) in
  Python 3.8.
* With the introduction of [`ModuleNotFoundError`](../library/exceptions.md#ModuleNotFoundError), import system consumers
  may start expecting import system replacements to raise that more specific
  exception when appropriate, rather than the less-specific [`ImportError`](../library/exceptions.md#ImportError).
  To provide future compatibility with such consumers, implementers of
  alternative import systems that completely replace [`__import__()`](../library/functions.md#import__) will
...
* The [`PyMem_Malloc()`](../c-api/memory.md#c.PyMem_Malloc) allocator family now uses the [pymalloc allocator](../c-api/memory.md#pymalloc) rather than the system `malloc()`. Applications calling
  [`PyMem_Malloc()`](../c-api/memory.md#c.PyMem_Malloc) without holding the GIL can now crash. Set the
  [`PYTHONMALLOC`](../using/cmdline.md#envvar-PYTHONMALLOC) environment variable to `debug` to validate the
  usage of memory allocators in your application. See [bpo-26249](https://bugs.python.org/issue?@action=redirect&bpo=26249).
* [`Py_Exit()`](../c-api/sys.md#c.Py_Exit) (and the main interpreter) now override the exit status
...
* The Python interpreter now uses a 16-bit wordcode instead of bytecode.
  (Contributed by Demur Rumed with input and reviews from
  Serhiy Storchaka and Victor Stinner in [bpo-26647](https://bugs.python.org/issue?@action=redirect&bpo=26647) and [bpo-28050](https://bugs.python.org/issue?@action=redirect&bpo=28050).)
* The new `FORMAT_VALUE` and [`BUILD_STRING`](../library/dis.md#opcode-BUILD_STRING) opcodes as part
...
* The function call opcodes have been heavily reworked for better performance
  ...
  The [`MAKE_FUNCTION`](../library/dis.md#opcode-MAKE_FUNCTION), `CALL_FUNCTION`,
  `CALL_FUNCTION_KW` and `BUILD_MAP_UNPACK_WITH_CALL` opcodes
  have been modified, the new [`CALL_FUNCTION_EX`](../library/dis.md#opcode-CALL_FUNCTION_EX) and
...
* The new [`SETUP_ANNOTATIONS`](../library/dis.md#opcode-SETUP_ANNOTATIONS) and `STORE_ANNOTATION` opcodes
  have been added to support the new [variable annotation](../glossary.md#term-variable-annotation) syntax.
...
The [`locale.localeconv()`](../library/locale.md#locale.localeconv) function now sets temporarily the `LC_CTYPE`
...
In 3.6.7 the [`tokenize`](../library/tokenize.md#module-tokenize) module now implicitly emits a `NEWLINE` token
