...
modules.  These are documented in [The Python Standard Library](../library/index.md#library-index).  A few built-in modules
...
* `name`: A name refers to another rule.
  ...
  * `TOKEN`: An uppercase name refers to a [token](../glossary.md#term-token).
...
* `"text"`, `'text'`: Text in single or double quotes must match literally
  ...
  of `text`:
  * `'if'`: A name in single quotes denotes a [keyword](lexical_analysis.md#keywords).
  * `"case"`: A name in double quotes denotes a
    [soft-keyword](lexical_analysis.md#soft-keywords).
  * `'@'`: A non-letter symbol in single quotes denotes an
    [`OP`](../library/token.md#token.OP) token, that is, a [delimiter](lexical_analysis.md#delimiters) or
    [operator](lexical_analysis.md#operators).
...
* `e1 | e2`: A vertical bar is used to separate alternatives.
  ...
  a vertical bar.
  See [**PEP 617**](https://peps.python.org/pep-0617/) for more background and details.
...
[lexical definitions](#notation-lexical-vs-syntactic).
...
<a id="notation-lexical-vs-syntactic"></a>

### Lexical and Syntactic definitions

There is some difference between *lexical* and *syntactic* analysis:
the [lexical analyzer](../glossary.md#term-lexical-analyzer) operates on the individual characters of the
input source, while the *parser* (syntactic analyzer) operates on the stream
of [tokens](../glossary.md#term-token) generated by the lexical analysis.
...
The lexical analyzer [discards](lexical_analysis.md#whitespace) all whitespace that is not
converted to tokens like [`token.INDENT`](../library/token.md#token.INDENT) or [`NEWLINE`](../library/token.md#token.NEWLINE).
...
All uses of BNF in the next chapter ([Lexical analysis](lexical_analysis.md#lexical)) are lexical definitions;
