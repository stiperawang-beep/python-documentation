...
`Python.h`.  See [Parsing arguments and building values](arg.md#arg-parsing) for a description of this macro.
...
`*exec_prefix*/include/pythonversion/`, where [`prefix`](../using/configure.md#cmdoption-prefix) and
[`exec_prefix`](../using/configure.md#cmdoption-exec-prefix) are defined by the corresponding parameters to Python’s
...
[`prefix`](../using/configure.md#cmdoption-prefix) include the platform specific headers from
[`exec_prefix`](../using/configure.md#cmdoption-exec-prefix).

C++ users should note that although the API is defined entirely using C, the
header files properly declare the entry points to be `extern "C"`. As a result,
there is no need to do anything special to use the API from C++.

<a id="capi-system-includes"></a>
...
> [Limited API](stable.md#limited-c-api) 3.13 or newer:

> * `<ctype.h>`
> * `<unistd.h>` (on POSIX)

> The following are included for backwards compatibility, unless using
> [Limited API](stable.md#limited-c-api) 3.11 or newer:
...
defined closer to where they are useful (for example, [`Py_RETURN_NONE`](none.md#c.Py_RETURN_NONE),
[`PyMODINIT_FUNC`](extension-modules.md#c.PyMODINIT_FUNC)).
...
Currently, all systems supported by CPython (per [**PEP 11**](https://peps.python.org/pep-0011/)), with the
...
Like `getenv(*s*)`, but returns `NULL` if [`-E`](../using/cmdline.md#cmdoption-E) was passed
on the command line (see [`PyConfig.use_environment`](init_config.md#c.PyConfig.use_environment)).
...
If Python is built without docstrings ([`--without-doc-strings`](../using/configure.md#cmdoption-without-doc-strings)),
...
if docstrings are disabled ([`--without-doc-strings`](../using/configure.md#cmdoption-without-doc-strings)).
...
This expands to `__attribute__((*name)*)` on a GCC compiler,
...
`INT_MIN` value for  type), the behavior is
undefined.
...
#### Versionchanged
Changed in version 3.1: This macro is now valid for all signed integer types, not just those for
...
be reported to caller, [`Py_FatalError()`](sys.md#c.Py_FatalError) can be used.
...
`((*smaller*) *value*)`
(in C++, `static_cast<*smaller*>(*value*)` will be used instead).

On debug builds (implying that [`Py_DEBUG`](#c.Py_DEBUG) is defined), this asserts
...
If Python is [built in debug mode](../using/configure.md#debug-build) (if the [`Py_DEBUG`](#c.Py_DEBUG)
macro is defined), the [`Py_ALWAYS_INLINE`](#c.Py_ALWAYS_INLINE) macro does nothing.
...
Equivalent to [`Py_LOCAL`](#c.Py_LOCAL) but additionally requests the function
be inlined.
...
The following [soft deprecated](../glossary.md#term-soft-deprecated) macros have been used to features that
...
The `printf()` formatting modifier for `size_t`.
...
Aliases for the types `long long`, `int32_t`,
...
Aliases for the values `LLONG_MIN`, `LLONG_MAX`,
`ULLONG_MAX`, and `SIZE_MAX`, respectively.
Use these standard names instead.

The required header, `<limits.h>`,
[is included](#capi-system-includes) in `Python.h`.
...
#### Soft-deprecated
[Soft deprecated](../glossary.md#term-soft-deprecated) since version 3.14: Use `memcpy()` directly instead.
...
#### Versionchanged
Changed in version 3.6: This is now an alias to `va_copy`.
...
of type .  This type is a pointer to an opaque data type
representing an arbitrary Python object.  Since all Python object types are
treated the same way by the Python language in most situations (e.g.,
assignments, scope rules, and argument passing), it is only fitting that they
should be represented by a single C type.  Almost all Python objects live on the
heap: you never declare an automatic or static variable of type
[`PyObject`](structures.md#c.PyObject), only pointer variables of type  can  be
declared.  The sole exception are the type objects; since these must never be
deallocated, they are typically static [`PyTypeObject`](type.md#c.PyTypeObject) objects.
...
explained in [The standard type hierarchy](../reference/datamodel.md#types)).  For each of the well-known types there is a macro
...
<a id="api-refcounts"></a>

### Reference Counts

The reference count is important because today’s computers have a  finite
(and often severely limited) memory size; it counts how many different
places there are that have a [strong reference](../glossary.md#term-strong-reference) to an object.
Such a place could be another object, or a global (or static) C variable,
or a local variable in some C function.
When the last [strong reference](../glossary.md#term-strong-reference) to an object is released
...
to use the macro [`Py_INCREF()`](refcounting.md#c.Py_INCREF) to take a new reference to an
object (i.e. increment its reference count by one),
and [`Py_DECREF()`](refcounting.md#c.Py_DECREF) to release that reference (i.e. decrement the
reference count by one).  The [`Py_DECREF()`](refcounting.md#c.Py_DECREF) macro
...
It is not necessary to hold a [strong reference](../glossary.md#term-strong-reference) (i.e. increment
...
our variable, there is no need to take a new [strong reference](../glossary.md#term-strong-reference)
...
path which allows control to flow back to the user from a [`Py_DECREF()`](refcounting.md#c.Py_DECREF), so
almost any operation is potentially dangerous.

A safe approach is to always use the generic operations (functions  whose name
begins with `PyObject_`, `PyNumber_`, `PySequence_` or `PyMapping_`).
These operations always create a new [strong reference](../glossary.md#term-strong-reference)
(i.e. increment the reference count) of the object they return.
This leaves the caller with the responsibility to call [`Py_DECREF()`](refcounting.md#c.Py_DECREF) when
they are done with the result; this soon becomes second nature.

<a id="api-refcountdetails"></a>
...
eventually releasing it by calling [`Py_DECREF()`](refcounting.md#c.Py_DECREF) or [`Py_XDECREF()`](refcounting.md#c.Py_XDECREF)
...
[`PyList_SetItem()`](list.md#c.PyList_SetItem) and [`PyTuple_SetItem()`](tuple.md#c.PyTuple_SetItem), which  steal a reference
...
Here, [`PyLong_FromLong()`](long.md#c.PyLong_FromLong) returns a new reference which is immediately
stolen by [`PyTuple_SetItem()`](tuple.md#c.PyTuple_SetItem).  When you want to keep using an object
although the reference to it will be stolen, use [`Py_INCREF()`](refcounting.md#c.Py_INCREF) to grab
another reference before calling the reference-stealing function.

Incidentally, [`PyTuple_SetItem()`](tuple.md#c.PyTuple_SetItem) is the *only* way to set tuple items;
[`PySequence_SetItem()`](sequence.md#c.PySequence_SetItem) and [`PyObject_SetItem()`](object.md#c.PyObject_SetItem) refuse to do this
since tuples are an immutable data type.  You should only use
[`PyTuple_SetItem()`](tuple.md#c.PyTuple_SetItem) for tuples that you are creating yourself.

Equivalent code for populating a list can be written using [`PyList_New()`](list.md#c.PyList_New)
and [`PyList_SetItem()`](list.md#c.PyList_SetItem).

However, in practice, you will rarely use these ways of creating and populating
a tuple or list.  There’s a generic function, [`Py_BuildValue()`](arg.md#c.Py_BuildValue), that can
...
It is much more common to use [`PyObject_SetItem()`](object.md#c.PyObject_SetItem) and friends with items
...
references, like [`PyObject_GetItem()`](object.md#c.PyObject_GetItem) and  [`PySequence_GetItem()`](sequence.md#c.PySequence_GetItem),
...
Thus, if you  extract an item from a list using [`PyList_GetItem()`](list.md#c.PyList_GetItem), you
don’t own the reference — but if you obtain the same item from the same list
using [`PySequence_GetItem()`](sequence.md#c.PySequence_GetItem) (which happens to take exactly the same
arguments), you do own a reference to the returned object.

<a id="index-6"></a>

Here is an example of how you could write a function that computes the sum of
the items in a list of integers; once using  [`PyList_GetItem()`](list.md#c.PyList_GetItem), and once
using [`PySequence_GetItem()`](sequence.md#c.PySequence_GetItem).
...
API; most are simple C types such as ,  ,
 and .  A few structure types  are used to
...
of type [`Py_ssize_t`](#c.Py_ssize_t).

<a id="api-exceptions"></a>
...
[`PyErr_Occurred()`](exceptions.md#c.PyErr_Occurred).  These exceptions are always explicitly documented.
...
[`PyErr_Occurred()`](exceptions.md#c.PyErr_Occurred) can be used to check for this: it returns a borrowed
reference to the exception type object when an exception has occurred, and
`NULL` otherwise.  There are a number of functions to set the exception state:
[`PyErr_SetString()`](exceptions.md#c.PyErr_SetString) is the most common (though not the most general)
function to set the exception state, and [`PyErr_Clear()`](exceptions.md#c.PyErr_Clear) clears the
exception state.
...
the last exception being handled by a Python  [`try`](../reference/compound_stmts.md#try) …
[`except`](../reference/compound_stmts.md#except) statement, while the C level exception state only exists while
...
exception state from Python code is to call the function [`sys.exc_info()`](../library/sys.md#sys.exc_info),
...
It illustrates the use of [`PyErr_ExceptionMatches()`](exceptions.md#c.PyErr_ExceptionMatches) and
[`PyErr_Clear()`](exceptions.md#c.PyErr_Clear) to handle specific exceptions, and the use of
[`Py_XDECREF()`](refcounting.md#c.Py_XDECREF) to dispose of owned references that may be `NULL` (note the
`'X'` in the name; [`Py_DECREF()`](refcounting.md#c.Py_DECREF) would crash when confronted with a
`NULL` reference).  It is important that the variables used to hold owned
references are initialized to `NULL` for this to work; likewise, the proposed
return value is initialized to `-1` (failure) and only set to success after
the final call made is successful.

<a id="api-embedding"></a>
...
The basic initialization function is [`Py_Initialize()`](interp-lifecycle.md#c.Py_Initialize). This initializes
the table of loaded modules, and creates the fundamental modules
[`builtins`](../library/builtins.md#module-builtins), [`__main__`](../library/__main__.md#module-__main__), and [`sys`](../library/sys.md#module-sys).  It also
initializes the module search path (`sys.path`).

[`Py_Initialize()`](interp-lifecycle.md#c.Py_Initialize) does not set the “script argument list”  (`sys.argv`).
If this variable is needed by Python code that will be executed later, setting
[`PyConfig.argv`](init_config.md#c.PyConfig.argv) and [`PyConfig.parse_argv`](init_config.md#c.PyConfig.parse_argv) must be set: see
[Python Initialization Configuration](init_config.md#init-config).

On most systems (in particular, on Unix and Windows, although the details are
slightly different), [`Py_Initialize()`](interp-lifecycle.md#c.Py_Initialize) calculates the module search path
based upon its best guess for the location of the standard Python interpreter
executable, assuming that the Python library is found in a fixed location
relative to the Python interpreter executable.  In particular, it looks for a
directory named `lib/python*X.Y*` relative to the parent directory
...
environment variable [`PYTHONHOME`](../using/cmdline.md#envvar-PYTHONHOME), or insert additional directories in
front of the standard path by setting [`PYTHONPATH`](../using/cmdline.md#envvar-PYTHONPATH).

The embedding application can steer the search by setting
[`PyConfig.program_name`](init_config.md#c.PyConfig.program_name) *before* calling
[`Py_InitializeFromConfig()`](interp-lifecycle.md#c.Py_InitializeFromConfig). Note that
[`PYTHONHOME`](../using/cmdline.md#envvar-PYTHONHOME) still overrides this and [`PYTHONPATH`](../using/cmdline.md#envvar-PYTHONPATH) is still
...
[`Py_Initialize()`](interp-lifecycle.md#c.Py_Initialize)) or the application is simply done with its  use of
Python and wants to free memory allocated by Python.  This can be accomplished
by calling [`Py_FinalizeEx()`](interp-lifecycle.md#c.Py_FinalizeEx).  The function [`Py_IsInitialized()`](interp-lifecycle.md#c.Py_IsInitialized) returns
true if Python is currently in the initialized state.  More information about
these functions is given in a later chapter. Notice that [`Py_FinalizeEx()`](interp-lifecycle.md#c.Py_FinalizeEx)
does *not* free all memory allocated by the Python interpreter, e.g. memory
allocated by extension modules currently cannot be released.

<a id="api-debugging"></a>
...
what is generally meant by [a debug build of Python](../using/configure.md#debug-build).

On Unix, `Py_DEBUG` can be enabled by adding [`--with-pydebug`](../using/configure.md#cmdoption-with-pydebug)
to the `./configure` command. This will also disable compiler optimization.

On Windows, selecting a debug build (e.g., by passing the [`-d`](../using/cmdline.md#cmdoption-d) option to
`PCbuild/build.bat`) automatically enables `Py_DEBUG`.
Additionally, the presence of the not-Python-specific `_DEBUG` macro,
when defined by the compiler, will also implicitly enable `Py_DEBUG`.

In addition to the reference count debugging described below, extra checks are
performed. See [Python Debug Build](../using/configure.md#debug-build) for more details.
...
fields to every [`PyObject`](structures.md#c.PyObject).  Total allocations are tracked as well.  Upon
...
* [pybind11](https://pybind11.readthedocs.io/) (C++)
* [PyO3](https://pyo3.rs/) (Rust)
